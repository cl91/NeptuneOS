...
public:
	...
	// stupid utf8 bom
	if ((fill > 2) && (buffer[0] == 0xef) && (buffer[1] == 0xbb) &&
	    (buffer[2] == 0xbf)) {
	    index += 3;
	    fill -= 3;
	...
	if ((fill > 1) && (buffer[0] == 0xfe) && (buffer[1] == 0xff)) {
	    index += 2;
	    fill -= 2;
#ifdef DISPLAY_DETECTED_UNICODE
	...
	if ((fill > 1) && (buffer[0] == 0xff) && (buffer[1] == 0xfe)) {
	    if ((fill == 4) && (buffer[2] == 0x00) && (buffer[3] == 0x00)) {
		cerr << "UTF Error: ambiguous BOM UTF-16 or UTF-32; assume UTF-32"
		     << endl;
		fill = 0;
		index = 0;
		return utf32le;
	...
	if ((fill == 4) && (buffer[0] == 0x00) && (buffer[1] == 0x00) &&
