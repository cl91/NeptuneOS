...
#define DbgPrint(...)						\
    ...
    (!gbDebug || fprintf(stderr, __VA_ARGS__));			\
...
int OutputLine_stub(FILE *file, EXPORT *pexp)
{
	...
	if ((giArch == ARCH_X86) && pexp->nCallingConvention == CC_STDCALL) {
	...
	if (pexp->strName.buf[0] == '?') {
		...
		fprintf(file, "%.*s(", pexp->strName.len,
			pexp->strName.buf);
	    else
	...
	case ARG_INT64:
	    ...
	    break;
	case ARG_INT128:
	    fprintf(file, "a%d.lower, a%d.upper", i, i);
	    break;
	case ARG_FLOAT:
	    fprintf(file, "a%d", i);
	    break;
...
void OutputLine_def_GCC(FILE *fileDest, EXPORT *pexp)
{
	...
	if (((pexp->uFlags & FL_STUB)
	    ...
	    STRING strTarget;
	    fprintf(fileDest, "=");
	    sprintf(buf, "$relaytrace$%.*s", pexp->strName.len,
		    pexp->strName.buf);
	    strTarget.buf = buf;
...
EXPORT *ParseFile(char *pcStart, unsigned *cExports)
{
    EXPORT *pexports;
    ...
    *cExports = 0;
    ...
    /* Count the lines */
    for (cLines = 1, pcLine = pcStart; *pcLine;
	 pcLine = NextLine(pcLine), cLines++) {
	/* Nothing */
    }
    ...
    if (pexports == NULL) {
	fprintf(stderr,
		"ERROR: %s: failed to allocate EXPORT array of %u elements\n",
	...
	if (*pc == '@') {
	    ...
	    if ((*end != ' ') && (*end != '\t')) {
		Fatal(pszSourceFileName, nLine, pcLine, end, 0,
		      "Unexpected character(s) after ordinal");
	    }

	    if ((number < 0) || (number > 0xFFFE)) {
		Fatal(pszSourceFileName, nLine, pcLine, pc, 0,
		      "Invalid value for ordinal");
	    }
	...
	if (!(pc = NextToken(pc))) {
	    Fatal(pszSourceFileName, nLine, pcLine, pc, 1,
	...
	while (*pc == '-') {
		...
		fprintf(stdout,
			...
			pszSourceFileName, nLine, TokenLength(pc), pc);
	    }
	...
	if (pc && (exp.nCallingConvention != CC_EXTERN)) {
	    /* Verify syntax */
	    if (*pc++ != '(') {
		Fatal(pszSourceFileName, nLine, pcLine, pc - 1, 0,
		      "Expected '('");
	    }
	...
	if (exp.nCallingConvention == CC_STUB) {
		...
		if (p && (p - exp.strName.buf < exp.strName.len)) {
			...
			Fatal(pszSourceFileName, nLine, pcLine, p, 1,
			      "Unexpected @");
		    }
	...
	if (pc) {
	    ...
	    if (NextToken(pc)) {
		Fatal(pszSourceFileName, nLine, pcLine, NextToken(pc), 0,
		      "Excess token(s) at end of definition");
	    }
	...
	if ((exp.uFlags & FL_ORDINAL) && (exp.nOrdinal == -1)) {
	    Fatal(pszSourceFileName, nLine, pcLine, pc, 0,
		  "Ordinal export without ordinal");
	}
	...
	if (!gbMSComp) {
	    ...
	    int bIsNotPrivate = (!gbNotPrivateNoWarn
				 && /*gbImportLib && */
				 !(exp.uFlags & FL_PRIVATE));
		...
		for (i = 0; i < ARRAYSIZE(astrOlePrivateExports); ++i) {
			...
			&& strncmp(exp.strName.buf,
				   astrOlePrivateExports[i],
				   exp.strName.len) == 0) {
...
{
    size_t nFileSize;
    char *pszSource, *pszDefFileName = NULL, *pszStubFileName =
	NULL, *pszLibStubName = NULL;
	...
	if ((strcasecmp(argv[i], "--help") == 0) ||
	    ...
	    if (strncasecmp
		(argv[i], pszVersionOption,
		 strlen(pszVersionOption)) == 0) {
		guOsVersion =
		    strtoul(argv[i] + strlen(pszVersionOption), NULL, 16);
	...
	giArch = ARCH_ARM;
    else if (strcasecmp(pszArchString, "ppc") == 0)
	giArch = ARCH_PPC;
    ...
    pexports = ParseFile(pszSource, &cExports);
    if (pexports == NULL) {
