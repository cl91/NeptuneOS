...
{
    DbgPrintFunc();
    PLIST_ENTRY RootUntypedList = &MmNtosVaddrSpace.RootUntypedList;
    ...
    PLIST_ENTRY SmallUntypedList = &MmNtosVaddrSpace.SmallUntypedList;
    DbgPrint("  Small free untyped:\n");
    LoopOverList(Untyped, SmallUntypedList, MM_UNTYPED, FreeListEntry) {
	DbgPrint("    cap = %d  log2(size) = %d\n", Untyped->TreeNode.Cap, Untyped->Log2Size);
    }
    PLIST_ENTRY MediumUntypedList = &MmNtosVaddrSpace.MediumUntypedList;
    DbgPrint("  Medium free untyped:\n");
    LoopOverList(Untyped, MediumUntypedList, MM_UNTYPED, FreeListEntry) {
	DbgPrint("    cap = %d  log2(size) = %d\n", Untyped->TreeNode.Cap, Untyped->Log2Size);
    }
    PLIST_ENTRY LargeUntypedList = &MmNtosVaddrSpace.LargeUntypedList;
