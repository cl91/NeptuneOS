...
VOID IopDriverObjectDeleteProc(IN POBJECT Self)
{
    ...
    /* Delete the CloseDevice request that we queued in IopGrantDeviceHandleToDriver.
     * Note at this point the device objects in the CloseDevice request list are all
     * foreign device objects (those that are not created by this driver). */
    LoopOverList(Req, &Driver->CloseDeviceReqList, CLOSE_DEVICE_REQUEST, DriverLink) {
	assert(Req->DeviceObject->DriverObject != Driver);
	RemoveEntryList(&Req->DeviceLink);
	RemoveEntryList(&Req->DriverLink);
	ObDereferenceObject(&Req->DeviceObject);
	IopFreePool(Req->Req);
	IopFreePool(Req);
    }
