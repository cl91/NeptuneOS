                         ...
                         OUT GLOBAL_HANDLE *DeviceHandle)
{
    ...
    CHAR Buffer[64] = {};
    if (!DeviceName && DeviceInfo->Flags & FILE_AUTOGENERATED_DEVICE_NAME) {
	DeviceName = Buffer;
	PCSTR Format = sizeof(PVOID) == 4 ? "\\Device\\%08x" : "\\Device\\%016x";
	snprintf(Buffer, sizeof(Buffer), Format,
		 OBJECT_TO_GLOBAL_HANDLE(DeviceObject));
    }
    if (DeviceName) {
...
NTSTATUS WdmGetTopDeviceObject(IN ASYNC_STATE AsyncState,
			       IN PTHREAD Thread,
...
NTSTATUS WdmGetDevice(IN ASYNC_STATE AsyncState,
		      IN PTHREAD Thread,
		      IN GLOBAL_HANDLE DeviceHandle,
		      OUT IO_DEVICE_INFO *DeviceInfo,
		      OUT ULONG *StackSize)
{
    assert(IopThreadIsAtPassiveLevel(Thread));
    if (!DeviceHandle) {
    ...
    GLOBAL_HANDLE GrantedDeviceHandle;
    RET_ERR(IopGrantDeviceHandleToDriver(Device, DriverObject,
					 &GrantedDeviceHandle));
    assert(GrantedDeviceHandle == DeviceHandle);
