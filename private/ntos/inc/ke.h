 ...
 *
 * A client thread can block on a number of kernel objects known as
 * the dispatcher objects. When a client thread blocks on a dispatcher
 * object the system service handler returns STATUS_ASYNC_PENDING and
 * the system executive dispatcher saves the reply capability (generated
 * by the seL4 microkernel) into the THREAD object, and simply moves
 * on to the next system service message. The thread is effectively
 * suspended until a later time when the NTOS server replies to the
 * saved reply capability.
 *
 ...
 * Description:
 *
 * The main event loop of the NT Executive runs in a single thread so there is no
 * need to synchronize access to data that are only accessed within the event loop
 * thread (you do need to save the local context when crossing a WAIT statement).
 * However, we DO run our interrupt handlers in their separate thread, and since
 * interrupts can arrive at any time, we must protect the access of shared data
 * shared by the interrupt handlers and the main event loop thread.
 *
