...
#define IS_VALID_DISK_INFO(Disk)                                              \
    (Disk) && (Disk->DeviceObject) && (Disk->SectorSize) && (Disk->Buffer) && \
	(Disk->SectorCount)

static VOID FstubDbgPrintPartitionEx(IN PPARTITION_INFORMATION_EX PartitionEntry,
				    IN ULONG PartitionNumber);

static NTSTATUS FstubDetectPartitionStyle(IN PDISK_INFORMATION Disk,
					  IN PARTITION_STYLE *PartitionStyle);

static VOID FstubFreeDiskInformation(IN PDISK_INFORMATION DiskBuffer);

static NTSTATUS FstubGetDiskGeometry(IN PDEVICE_OBJECT DeviceObject,
				    OUT PDISK_GEOMETRY_EX Geometry);

static NTSTATUS FstubReadSector(IN PDEVICE_OBJECT DeviceObject,
				IN ULONG SectorSize,
				IN OPTIONAL ULONGLONG StartingSector,
				OUT PVOID Buffer);

static NTSTATUS FstubWriteBootSectorEFI(IN PDISK_INFORMATION Disk);

static NTSTATUS FstubWriteHeaderEFI(IN PDISK_INFORMATION Disk,
...
static NTSTATUS FstubWritePartitionTableEFI(IN PDISK_INFORMATION Disk,
					    IN GUID DiskGUID,
					    ...
					    IN OPTIONAL PPARTITION_INFORMATION_EX Entries);

static NTSTATUS FstubWriteSector(IN PDEVICE_OBJECT DeviceObject,
				 IN ULONG SectorSize,
...
NTAPI NTSTATUS FstubAllocateDiskInformation(IN PDEVICE_OBJECT DeviceObject,
			     ...
			     IN PDISK_GEOMETRY_EX DiskGeometry OPTIONAL)
{
    ...
    DiskInformation->SectorCount = DiskInformation->DiskGeometry.DiskSize.QuadPart /
...
VOID NTAPI FstubCopyEntryEFI(OUT PEFI_PARTITION_ENTRY Entry,
			     IN PPARTITION_INFORMATION_EX Partition, ULONG SectorSize)
{
    PAGED_CODE();
    ...
    Entry->EndingLBA = (Partition->StartingOffset.QuadPart +
			Partition->PartitionLength.QuadPart - 1) /
		       SectorSize;
    Entry->Attributes = Partition->Gpt.Attributes;
    RtlCopyMemory(Entry->Name, Partition->Gpt.Name, sizeof(Entry->Name));
}

NTAPI NTSTATUS FstubCreateDiskMBR(IN PDEVICE_OBJECT DeviceObject, IN PCREATE_DISK_MBR DiskInfo)
{
    NTSTATUS Status;
...
NTAPI NTSTATUS FstubCreateDiskEFI(IN PDEVICE_OBJECT DeviceObject, IN PCREATE_DISK_GPT DiskInfo)
{
    NTSTATUS Status;
    PDISK_INFORMATION Disk = NULL;
    ...
    Status = FstubWritePartitionTableEFI(Disk, DiskInfo->DiskId, MaxPartitionCount,
					 FirstUsableLBA, LastUsableLBA, FALSE, 0, NULL);
    /* Then, write backup table */
    if (NT_SUCCESS(Status)) {
	Status = FstubWritePartitionTableEFI(Disk, DiskInfo->DiskId, MaxPartitionCount,
					     FirstUsableLBA, LastUsableLBA, TRUE, 0,
					     NULL);
    }
...
NTAPI NTSTATUS FstubCreateDiskRaw(IN PDEVICE_OBJECT DeviceObject)
{
    NTSTATUS Status;
    PDISK_INFORMATION Disk = NULL;
    ...
    if (NT_SUCCESS(Status)) {
	Status = FstubWriteSector(Disk->DeviceObject, Disk->SectorSize,
				  Disk->SectorCount - 1ULL, Disk->Buffer);
    }
...
static PCHAR FstubDbgGuidToString(IN PGUID Guid,
				  OUT PCHAR String,
				  IN ULONG BufferSize)
{
    snprintf(String, BufferSize,
...
static VOID FstubDbgPrintDriveLayoutEx(IN PDRIVE_LAYOUT_INFORMATION_EX DriveLayout)
{
    ULONG i;
	...
	if (DriveLayout->PartitionCount % 4 != 0) {
	    DPRINT("Warning: Partition count isn't a 4-factor: %u!\n",
	...
	break;
    default:
	DPRINT("Unsupported partition style: %u\n", DriveLayout->PartitionStyle);
    }
}
				     ...
				     IN ULONG PartitionNumber)
{
	...
	DPRINT("  RewritePartition: %u\n",
	       PartitionEntry[PartitionNumber].RewritePartition);
	FstubDbgGuidToString(&(PartitionEntry[PartitionNumber].Gpt.PartitionType),
			     Guid, sizeof(Guid));
	...
	break;
    default:
	DPRINT("  Unsupported partition style: %d\n",
				       ...
				       IN ULONG PartitionNumber)
{
    ...
    DPRINT("Modifying partition %u\n", PartitionNumber);
    switch (PartitionEntry->PartitionStyle) {
    case PARTITION_STYLE_MBR:
	DPRINT("  PartitionType: %02x\n", PartitionEntry->Mbr.PartitionType);
	break;
    case PARTITION_STYLE_GPT:
	FstubDbgGuidToString(&(PartitionEntry->Gpt.PartitionType), Guid, sizeof(Guid));
	DPRINT("  PartitionType: %s\n", Guid);
					  ...
					  IN PARTITION_STYLE *PartitionStyle)
{
    ...
    if (*(PUSHORT)&Disk->Buffer[BOOT_SIGNATURE_OFFSET] != BOOT_RECORD_SIGNATURE) {
				     ...
				     OUT PDISK_GEOMETRY_EX Geometry)
{
    ...
    Irp = IoBuildDeviceIoControlRequest(IOCTL_DISK_GET_DRIVE_GEOMETRY_EX, DeviceObject, 0,
					0, DiskGeometry, sizeof(DISK_GEOMETRY_EX), FALSE,
					Event, IoStatusBlock);
    if (!Irp) {
	Status = STATUS_INSUFFICIENT_RESOURCES;
	goto Cleanup;
...
NTAPI NTSTATUS FstubReadHeaderEFI(IN PDISK_INFORMATION Disk,
				  ...
				  PEFI_PARTITION_HEADER *HeaderBuffer)
{
    ...
    Status = FstubReadSector(Disk->DeviceObject, Disk->SectorSize, StartingSector,
			     Disk->Buffer);
    if (!NT_SUCCESS(Status)) {
    ...
    SectoredPartitionEntriesSize = (EFIHeader->NumberOfEntries * PARTITION_ENTRY_SIZE) /
				   Disk->SectorSize;
    /* Compute partition table checksum */
    for (i = 0, PreviousCRC32 = 0; i < SectoredPartitionEntriesSize; i++) {
	Status = FstubReadSector(Disk->DeviceObject, Disk->SectorSize,
				 EFIHeader->PartitionEntryLBA + i, Sector);
	if (!NT_SUCCESS(Status)) {
    ...
    LonelyPartitions = (EFIHeader->NumberOfEntries * PARTITION_ENTRY_SIZE) %
		       Disk->SectorSize;
...
NTAPI NTSTATUS FstubReadPartitionTableEFI(IN PDISK_INFORMATION Disk,
					  IN BOOLEAN ReadBackupTable,
					  OUT PDRIVE_LAYOUT_INFORMATION_EX *DriveLayout)
{
    ...
    *DriveLayout = NULL;
	...
	DriveLayoutEx->PartitionEntry[PartitionCount].PartitionLength.QuadPart =
	    (PartitionEntry.EndingLBA - PartitionEntry.StartingLBA + 1) *
	    Disk->SectorSize;
	/* This number starts from 1 */
	DriveLayoutEx->PartitionEntry[PartitionCount].PartitionNumber = PartitionCount +
									1;
	...
	DriveLayoutEx->PartitionEntry[PartitionCount].Gpt.PartitionType =
	    PartitionEntry.PartitionType;
	DriveLayoutEx->PartitionEntry[PartitionCount].Gpt.PartitionId =
	    PartitionEntry.UniquePartition;
	DriveLayoutEx->PartitionEntry[PartitionCount].Gpt.Attributes =
...
NTAPI NTSTATUS FstubReadPartitionTableMBR(IN PDISK_INFORMATION Disk,
					  IN BOOLEAN ReturnRecognizedPartitions,
					  OUT PDRIVE_LAYOUT_INFORMATION_EX *OutputBuffer)
{
    NTSTATUS Status;
    ULONG i;
    ...
    *OutputBuffer = NULL;
    ...
    Status = IoReadPartitionTable(Disk->DeviceObject, Disk->SectorSize,
				  ReturnRecognizedPartitions, &DriveLayout);
    if (!NT_SUCCESS(Status)) {
	return Status;
    }
...
NTAPI NTSTATUS FstubSetPartitionInformationEFI(IN PDISK_INFORMATION Disk,
					       IN ULONG PartitionNumber,
					       IN SET_PARTITION_INFORMATION_GPT *Info)
{
    NTSTATUS Status;
    ...
    Layout->PartitionEntry[PartitionNumber].Gpt.PartitionType =
	Info->PartitionType;
    Layout->PartitionEntry[PartitionNumber].Gpt.PartitionId = Info->PartitionId;
    Layout->PartitionEntry[PartitionNumber].Gpt.Attributes = Info->Attributes;
    RtlCopyMemory(Layout->PartitionEntry[PartitionNumber].Gpt.Name, Info->Name,
		  sizeof(Info->Name));
...
NTAPI NTSTATUS FstubVerifyPartitionTableEFI(IN PDISK_INFORMATION Disk,
					    IN BOOLEAN FixErrors)
{
	...
	Status = FstubWriteHeaderEFI(Disk, SectorsForPartitions, EFIHeader->DiskGUID,
				     EFIHeader->NumberOfEntries,
				     EFIHeader->FirstUsableLBA, EFIHeader->LastUsableLBA,
				     EFIHeader->PartitionEntryCRC32, WriteBackup);
	...
	Status = FstubReadSector(Disk->DeviceObject, Disk->SectorSize,
				 ReadPosition + PartitionIndex, Disk->Buffer);
	if (!NT_SUCCESS(Status)) {
	    goto Cleanup;
	}
	...
	Status = FstubWriteSector(Disk->DeviceObject, Disk->SectorSize,
				  WritePosition + PartitionIndex, Disk->Buffer);
	if (!NT_SUCCESS(Status)) {
	    goto Cleanup;
	}
    }
    ...
    Status = FstubWriteHeaderEFI(Disk, SectorsForPartitions, EFIHeader->DiskGUID,
				 EFIHeader->NumberOfEntries, EFIHeader->FirstUsableLBA,
				 EFIHeader->LastUsableLBA, EFIHeader->PartitionEntryCRC32,
				 WriteBackup);
...
NTAPI NTSTATUS FstubWriteEntryEFI(IN PDISK_INFORMATION Disk,
				  IN ULONG PartitionsSizeSector,
				  ...
				  OUT PULONG PartitionEntryCRC32 OPTIONAL)
{
     ...
     */
    RtlCopyMemory((PVOID)((ULONG_PTR)Disk->Buffer +
			  ((PartitionEntryNumber * PARTITION_ENTRY_SIZE) %
			   Disk->SectorSize)),
		  PartitionEntry, sizeof(EFI_PARTITION_ENTRY));
    /* Compute size of buffer */
    Offset = (PartitionEntryNumber * PARTITION_ENTRY_SIZE) % Disk->SectorSize +
	     PARTITION_ENTRY_SIZE;
	...
	*PartitionEntryCRC32 = RtlComputeCrc32(*PartitionEntryCRC32,
					       (PUCHAR)PartitionEntry,
					       PARTITION_ENTRY_SIZE);
    }
				    ...
				    IN BOOLEAN WriteBackupTable)
{
    ...
    EFIHeader->HeaderCRC32 = RtlComputeCrc32(0, (PUCHAR)EFIHeader,
					     sizeof(EFI_PARTITION_HEADER));
    ...
    return FstubWriteSector(Disk->DeviceObject, Disk->SectorSize, EFIHeader->MyLBA,
			    Disk->Buffer);
}
					    ...
					    IN OPTIONAL PPARTITION_INFORMATION_EX Entries)
{
    ...
    /* Count how much sectors we'll have to read to read the whole partition table */
    SectoredPartitionEntriesSize = (MaxPartitionCount * PARTITION_ENTRY_SIZE) /
				   Disk->SectorSize;

    for (i = 0, WrittenPartitions = 0; i < PartitionCount; i++) {
	/* If partition GUID is 00000000-0000-0000-0000-000000000000, then it's unused, skip it */
	if (Entries[i].Gpt.PartitionType.Data1 == 0 &&
	    Entries[i].Gpt.PartitionType.Data2 == 0 &&
	    Entries[i].Gpt.PartitionType.Data3 == 0 &&
	    ((PULONGLONG)Entries[i].Gpt.PartitionType.Data4)[0] == 0) {
	    continue;
	}

	/* Copy the entry in the partition entry format */
	FstubCopyEntryEFI(&Entry, &Entries[i], Disk->SectorSize);
	/* Then write the entry to the disk */
	Status = FstubWriteEntryEFI(Disk, SectoredPartitionEntriesSize, WrittenPartitions,
				    &Entry, WriteBackupTable, FALSE,
				    &PartitionEntryCRC32);
	if (!NT_SUCCESS(Status)) {
	    return Status;
	}
	...
	Status = FstubWriteEntryEFI(Disk, SectoredPartitionEntriesSize, i, &Entry,
				    WriteBackupTable, FALSE, &PartitionEntryCRC32);
	if (!NT_SUCCESS(Status)) {
	    return Status;
	}
    }
    ...
    return FstubWriteHeaderEFI(Disk, SectoredPartitionEntriesSize, DiskGUID,
			       MaxPartitionCount, FirstUsableLBA, LastUsableLBA,
			       PartitionEntryCRC32, WriteBackupTable);
}

NTAPI NTSTATUS FstubWritePartitionTableMBR(IN PDISK_INFORMATION Disk,
					   IN PDRIVE_LAYOUT_INFORMATION_EX LayoutEx)
{
    ...
    Status = IoWritePartitionTable(Disk->DeviceObject, Disk->SectorSize,
 ...
 * @implemented
 */
NTAPI NTSTATUS IoCreateDisk(IN PDEVICE_OBJECT DeviceObject, IN PCREATE_DISK Disk)
{
    PARTITION_STYLE PartitionStyle;
				   ...
				   OUT PDISK_SIGNATURE Signature)
{
	...
	for (i = 0, CheckSum = 0; i < 512; i += sizeof(ULONG)) {
	...
	Signature->Mbr.Signature = *(PULONG)&Buffer[DISK_SIGNATURE_OFFSET];
 ...
 * @implemented
 */
NTAPI NTSTATUS IoSetPartitionInformationEx(IN PDEVICE_OBJECT DeviceObject,
					   IN ULONG PartitionNumber,
					   IN PSET_PARTITION_INFORMATION_EX PartitionInfo)
{
    ...
    switch (PartitionStyle) {
    case PARTITION_STYLE_MBR:
	Status = IoSetPartitionInformation(DeviceObject, Disk->SectorSize,
					   PartitionNumber,
					   PartitionInfo->Mbr.PartitionType);
	break;
    case PARTITION_STYLE_GPT:
	Status = FstubSetPartitionInformationEFI(Disk, PartitionNumber,
						 &(PartitionInfo->Gpt));
	break;
    default:
 ...
 * @implemented
 */
NTAPI NTSTATUS IoVerifyPartitionTable(IN PDEVICE_OBJECT DeviceObject,
				      IN BOOLEAN FixErrors)
{
    NTSTATUS Status;
    PDISK_INFORMATION Disk;
				       ...
				       IN PDRIVE_LAYOUT_INFORMATION_EX DriveLayout)
{
	...
	if (NT_SUCCESS(Status)) {
		...
		/* Count number of sectors needed to store partitions */
		SectorsForPartitions = ((ULONGLONG)NumberOfEntries *
					PARTITION_ENTRY_SIZE) /
				       Disk->SectorSize;
		/* Set first usable LBA: Legacy MBR + GPT header + Partitions entries */
		FirstUsableLBA = SectorsForPartitions + 2;
		...
		LastUsableLBA = Disk->SectorCount - SectorsForPartitions - 1;
		/* Write primary table */
		Status = FstubWritePartitionTableEFI(Disk, DiskGuid, NumberOfEntries,
						     FirstUsableLBA, LastUsableLBA, FALSE,
						     DriveLayout->PartitionCount,
