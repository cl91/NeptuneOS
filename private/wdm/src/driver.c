...
NTAPI NTSTATUS HalRegisterFrameBuffer(IN PVOID VirtBase,
				      IN SIZE_T Size,
				      IN ULONG Offset,
				      IN ULONG Width,
				      IN ULONG Height,
...
{
    return WdmHalRegisterFrameBuffer(VirtBase, Size, Offset, Width, Height, Pitch,
				     BitsPerPixel, BlueIndex, GreenIndex, RedIndex, NeedFlush);
}

NTAPI NTSTATUS HalUnregisterFrameBuffer(IN PVOID VirtBase)
{
    return WdmHalUnregisterFrameBuffer(VirtBase);
}
				...
				IN ULONG EndHeight)
{
	...
	HalpFrameBufferDamageHandler(VirtBase, StartWidth, StartHeight, EndWidth, EndHeight);
    }
}
