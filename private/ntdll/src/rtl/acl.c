...
NTAPI BOOLEAN RtlFirstFreeAce(IN PACL Acl, OUT PACE *FirstFreeAce)
{
    PACE Current;
    ULONG_PTR AclEnd;
    ULONG i;
    ...
    *FirstFreeAce = NULL;
    ...
    if ((ULONG_PTR)Current <= AclEnd)
	*FirstFreeAce = Current;
    return TRUE;
}

static VOID RtlpAddData(IN PVOID AceList, IN ULONG AceListLength, IN PVOID Ace,
			IN ULONG Offset)
{
    /* Shift the buffer down */
    if (Offset > 0) {
	RtlCopyMemory((PVOID)((ULONG_PTR)Ace + AceListLength), Ace, Offset);
    }
...
static VOID RtlpDeleteData(IN PVOID Ace, IN ULONG AceSize, IN ULONG Offset)
{
    /* Move the data up */
    if (AceSize < Offset) {
	RtlMoveMemory(Ace, (PVOID)((ULONG_PTR)Ace + AceSize), Offset - AceSize);
    }
...
static NTSTATUS RtlpAddKnownAce(IN PACL Acl,
				IN ULONG Revision,
				IN ULONG Flags,
				...
				IN UCHAR Type)
{
	...
	InvalidFlags = Flags & ~(VALID_INHERIT_FLAGS | SUCCESSFUL_ACCESS_ACE_FLAG |
...
static NTSTATUS RtlpAddKnownObjectAce(IN PACL Acl,
				      IN ULONG Revision,
				      IN ULONG Flags,
				      IN ACCESS_MASK AccessMask,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlAddAccessAllowedAce(IN OUT PACL Acl,
				      IN ULONG Revision,
				      IN ACCESS_MASK AccessMask,
				      IN PSID Sid)
{
    /* Call the worker function */
    return RtlpAddKnownAce(Acl, Revision, 0, AccessMask, Sid, ACCESS_ALLOWED_ACE_TYPE);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlAddAccessAllowedAceEx(IN OUT PACL Acl,
					IN ULONG Revision,
					IN ULONG Flags,
					IN ACCESS_MASK AccessMask,
					IN PSID Sid)
{
    /* Call the worker function */
    return RtlpAddKnownAce(Acl, Revision, Flags, AccessMask, Sid,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlAddAccessAllowedObjectAce(IN OUT PACL Acl,
					    IN ULONG Revision,
					    IN ULONG Flags,
					    IN ACCESS_MASK AccessMask,
					    IN OPTIONAL GUID *ObjectTypeGuid,
					    ...
					    IN PSID Sid)
{
    ...
    return RtlpAddKnownObjectAce(Acl, Revision, Flags, AccessMask, ObjectTypeGuid,
				 InheritedObjectTypeGuid, Sid,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlAddAccessDeniedAce(IN PACL Acl,
				     IN ULONG Revision,
				     IN ACCESS_MASK AccessMask,
				     IN PSID Sid)
{
    /* Call the worker function */
    return RtlpAddKnownAce(Acl, Revision, 0, AccessMask, Sid, ACCESS_DENIED_ACE_TYPE);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlAddAccessDeniedAceEx(IN OUT PACL Acl,
				       IN ULONG Revision,
				       IN ULONG Flags,
				       IN ACCESS_MASK AccessMask,
				       IN PSID Sid)
{
    /* Call the worker function */
    return RtlpAddKnownAce(Acl, Revision, Flags, AccessMask, Sid, ACCESS_DENIED_ACE_TYPE);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlAddAccessDeniedObjectAce(IN OUT PACL Acl,
					   IN ULONG Revision,
					   IN ULONG Flags,
					   IN ACCESS_MASK AccessMask,
					   IN OPTIONAL GUID *ObjectTypeGuid,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlAddAuditAccessAce(IN PACL Acl,
				    IN ULONG Revision,
				    IN ACCESS_MASK AccessMask,
				    IN PSID Sid,
				    IN BOOLEAN Success,
				    IN BOOLEAN Failure)
{
    ...
    return RtlpAddKnownAce(Acl, Revision, Flags, AccessMask, Sid, SYSTEM_AUDIT_ACE_TYPE);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlAddAuditAccessAceEx(IN PACL Acl,
				      IN ULONG Revision,
				      IN ULONG Flags,
				      IN ACCESS_MASK AccessMask,
				      IN PSID Sid,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlAddAuditAccessObjectAce(IN PACL Acl,
					  IN ULONG Revision,
					  IN ULONG Flags,
					  IN ACCESS_MASK AccessMask,
					  IN OPTIONAL GUID *ObjectTypeGuid,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetAce(IN PACL Acl, IN ULONG AceIndex, OUT PVOID *Ace)
{
    ULONG i;
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlAddAce(IN PACL Acl,
			 ...
			 IN ULONG AceListLength)
{
    ...
    if (!RtlFirstFreeAce(Acl, &FreeAce))
	return STATUS_INVALID_PARAMETER;

    /* Loop over all the ACEs, keeping track of new ACEs as we go along */
    for (Ace = AceList, NewAceCount = 0; Ace < (PACE)((ULONG_PTR)AceList + AceListLength);
	 NewAceCount++) {
    ...
    RtlpAddData(AceList, AceListLength, Ace, (ULONG_PTR)FreeAce - (ULONG_PTR)Ace);
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDeleteAce(IN PACL Acl, IN ULONG AceIndex)
{
    PACE FreeAce, Ace;
    ...
    /* Delete this ACE */
    RtlpDeleteData(Ace, Ace->Header.AceSize,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlCreateAcl(IN PACL Acl, IN ULONG AclSize, IN ULONG AclRevision)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlQueryInformationAcl(IN PACL Acl,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetInformationAcl(IN PACL Acl,
				    IN PVOID Information,
				    IN ULONG InformationLength,
				    IN ACL_INFORMATION_CLASS InformationClass)
{
 ...
 * @implemented
 */
NTAPI BOOLEAN RtlValidAcl(IN PACL Acl)
{
    PACE_HEADER Ace;
    PISID Sid;
    ULONG i;
	...
	for (i = 0; i < Acl->AceCount; i++) {
	    ...
	    if (((ULONG_PTR)Ace + sizeof(ACE_HEADER)) >=
		...
		/* Validate the length of this ACE */
		if (ROUND_DOWN(Ace->AceSize, sizeof(ULONG)) != Ace->AceSize) {
		    DPRINT1("Misaligned Object ACE size: 0x%x\n", Ace->AceSize);
		    return FALSE;
		}

		/* The ACE size should at least have enough space for the known object ACE
		 * header */
		if (Ace->AceSize < sizeof(KNOWN_OBJECT_ACE)) {
		    DPRINT1("Too small Object ACE size to hold KNOWN_OBJECT_ACE header: "
			    "0x%x\n", Ace->AceSize);
		    return FALSE;
		}
		...
		/* Check if the SID revision is valid */
		Sid = (PISID)((ULONG_PTR) &
			      ((PKNOWN_OBJECT_ACE)Ace)->SidStart + GuidSize);
		if (Sid->Revision != SID_REVISION) {
