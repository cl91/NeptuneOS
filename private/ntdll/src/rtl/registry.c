...
static NTSTATUS RtlpQueryRegistryDirect(IN ULONG ValueType,
					IN PVOID ValueData,
					...
					IN PVOID Buffer)
{
    ...
    if ((ValueType == REG_SZ) ||
	(ValueType == REG_EXPAND_SZ) || (ValueType == REG_MULTI_SZ)) {
...
static NTSTATUS RtlpCallQueryRegistryRoutine(IN PRTL_QUERY_REGISTRY_TABLE QueryTable,
					     ...
					     IN PVOID Environment)
{
    ...
    *InfoSize = 0;
	...
	(!(KeyValueInfo->DataLength) && (KeyValueInfo->Type == QueryTable->DefaultType))) {
	...
	if (!(QueryTable->Flags & RTL_QUERY_REGISTRY_DIRECT)) {
		...
		SpareData += KeyValueInfo->DataOffset + KeyValueInfo->DataLength;
	    } else {
		/* Otherwise, the spare data only has the name data */
		SpareData += FIELD_OFFSET(KEY_VALUE_FULL_INFORMATION,
					  Name) + KeyValueInfo->NameLength;
	    }
	...
	if (Type == REG_MULTI_SZ) {
		...
		if (QueryTable->Flags & RTL_QUERY_REGISTRY_DIRECT) {
		    ...
		    Status = RtlpQueryRegistryDirect(REG_SZ,
						     ...
						     QueryTable->
						     EntryContext);
		    QueryTable->EntryContext = (PVOID)((ULONG_PTR)QueryTable->EntryContext +
		...
		if (NT_SUCCESS(Status)) {
			...
			*InfoSize = (ULONG)(SpareData - (PCHAR)KeyValueInfo) + RequiredLength;
			...
			DPRINT1("RTL: Expand variables for %wZ failed - "
				...
				Status,
				*InfoSize,
				InfoLength, Destination.MaximumLength);
		    } else {
			/* Notify debugger */
			DPRINT1("RTL: Expand variables for %wZ failed - "
				"Status == %lx\n", &Source, Status);
...
static PVOID RtlpAllocDeallocQueryBuffer(IN OPTIONAL PSIZE_T BufferSize,
					 IN OPTIONAL PVOID OldBuffer,
					 ...
					 OUT OPTIONAL PNTSTATUS Status)
{
	...
	*Status = STATUS_SUCCESS;
    ...
    /* Free the old buffer */
    if (OldBuffer)
	RtlpFreeMemory(OldBuffer, TAG_RTLREGISTRY);
...
static NTSTATUS RtlpGetRegistryHandle(IN ULONG RelativeTo,
				      IN PCWSTR Path,
				      IN BOOLEAN Create,
				      IN PHANDLE KeyHandle)
{
	...
	*KeyHandle = (HANDLE) Path;
	...
	if (RelativeTo == RTL_REGISTRY_USER) {
	    ...
	    Status = RtlAppendUnicodeToString(&KeyName, RtlpRegPaths[RelativeTo]);
	}
...
FORCEINLINE VOID RtlpCloseRegistryHandle(IN ULONG RelativeTo,
					 IN HANDLE KeyHandle)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlCheckRegistryKey(IN ULONG RelativeTo,
				   IN PWSTR Path)
{
    HANDLE KeyHandle;
    NTSTATUS Status;
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlCreateRegistryKey(IN ULONG RelativeTo,
				    IN PWSTR Path)
{
    HANDLE KeyHandle;
    NTSTATUS Status;
    ...
    /* Call the helper */
    Status = RtlpGetRegistryHandle(RelativeTo, Path, TRUE, &KeyHandle);
    if (!NT_SUCCESS(Status))
	return Status;
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDeleteRegistryValue(IN ULONG RelativeTo,
				      IN PCWSTR Path,
				      IN PCWSTR ValueName)
{
    HANDLE KeyHandle;
    ...
    Status = NtDeleteValueKey(KeyHandle, &Name);
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlWriteRegistryValue(IN ULONG RelativeTo,
				     IN PCWSTR Path,
				     IN PCWSTR ValueName,
				     IN ULONG ValueType,
				     IN PVOID ValueData,
				     IN ULONG ValueLength)
{
    ...
    RtlInitUnicodeString(&Name, ValueName);
    Status = NtSetValueKey(KeyHandle, &Name, 0, ValueType, ValueData, ValueLength);
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlOpenCurrentUser(IN ACCESS_MASK DesiredAccess,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlFormatCurrentUserKeyPath(OUT PUNICODE_STRING KeyPath)
{
    HANDLE TokenHandle;
    UCHAR Buffer[256];
    ...
    Status = NtOpenThreadTokenEx(NtCurrentThread(),
				 TOKEN_QUERY,
				 TRUE, OBJ_KERNEL_HANDLE, &TokenHandle);
    if (!NT_SUCCESS(Status)) {
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlpNtCreateKey(OUT HANDLE KeyHandle,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlpNtEnumerateSubKey(IN HANDLE KeyHandle,
				     ...
				     IN ULONG Unused)
{
	...
	BufferLength = SubKeyName->MaximumLength + sizeof(KEY_BASIC_INFORMATION);
    ...
    /* Enumerate the key */
    Status = NtEnumerateKey(KeyHandle, Index, KeyBasicInformation,
			    KeyInfo, BufferLength, &ReturnedLength);
	...
	if (KeyInfo->NameLength <= SubKeyName->MaximumLength) {
	    ...
	    RtlMoveMemory(SubKeyName->Buffer,
			  KeyInfo->Name, SubKeyName->Length);
	} else {
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlpNtMakeTemporaryKey(IN HANDLE KeyHandle)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlpNtOpenKey(OUT HANDLE KeyHandle,
			     IN ACCESS_MASK DesiredAccess,
			     IN POBJECT_ATTRIBUTES ObjectAttributes,
			     IN ULONG Unused)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlpNtQueryValueKey(IN HANDLE KeyHandle,
				   ...
				   IN ULONG Unused)
{
    ...
    Status = NtQueryValueKey(KeyHandle, &ValueName, KeyValuePartialInformation,
			     ValueInfo, BufferLength, &BufferLength);
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlpNtSetValueKey(IN HANDLE KeyHandle,
				 IN ULONG Type,
				 IN PVOID Data,
				 IN ULONG DataLength)
{
    ...
    /* Set the value */
    RtlInitEmptyUnicodeString(&ValueName, NULL, 0);
    return NtSetValueKey(KeyHandle, &ValueName, 0, Type, Data, DataLength);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlQueryRegistryValues(IN ULONG RelativeTo,
				      IN PCWSTR Path,
				      ...
				      IN PVOID Environment OPTIONAL)
{
	...
	if (QueryTable->Flags & RTL_QUERY_REGISTRY_SUBKEY) {
		...
		Status = NtQueryValueKey(CurrentKey,
					 &KeyValueName,
					 KeyValueFullInformation,
					 KeyValueInfo,
					 (ULONG) InfoSize, &ResultLength);
		...
		if (!NT_SUCCESS(Status)) {
			...
			BufferSize = ResultLength + sizeof(ULONG_PTR) + sizeof(UNICODE_NULL);
			...
			/* Delete it */
			NtDeleteValueKey(CurrentKey, &KeyValueName);
		    }
		}
	...
	ProcessValues:
	    ...
	    while (TRUE) {
		/* Enumerate the keys */
		Status = NtEnumerateValueKey(CurrentKey,
					     Value,
					     KeyValueFullInformation,
					     KeyValueInfo,
					     (ULONG) InfoSize,
		...
		if (Status == STATUS_NO_MORE_ENTRIES) {
		    ...
		    if (!Value && (QueryTable->Flags & RTL_QUERY_REGISTRY_REQUIRED)) {
			/* Fail */
		...
		if (Status == STATUS_BUFFER_TOO_SMALL) {
			...
			sizeof(ULONG_PTR) + sizeof(UNICODE_NULL);
		    KeyValueInfo = RtlpAllocDeallocQueryBuffer(&BufferSize,
							       KeyValueInfo,
							       BufferSize,
							       &Status);
		...
		if (QueryTable->Flags & RTL_QUERY_REGISTRY_DELETE) {
		    ...
		    /* Delete the key */
		    Status = NtDeleteValueKey(CurrentKey, &KeyValueName);
		    if (NT_SUCCESS(Status))
			Value--;
		}
    ...
    if (KeyHandle)
	RtlpCloseRegistryHandle(RelativeTo, KeyHandle);
    if ((CurrentKey) && (CurrentKey != KeyHandle))
	NtClose(CurrentKey);
