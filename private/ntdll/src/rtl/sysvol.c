...
static NTSTATUS RtlpSysVolCreateSecurityDescriptor(
    OUT PISECURITY_DESCRIPTOR *SecurityDescriptor, OUT PSID *SystemSid)
{
    ...
    Status = RtlAllocateAndInitializeSid(&LocalSystemAuthority, 1,
    ...
    AbsSD = RtlpAllocateMemory(sizeof(SECURITY_DESCRIPTOR), 'dSeS');
    if (AbsSD == NULL) {
    ...
    DaclSize = sizeof(ACL) + sizeof(ACE) + RtlLengthSid(LocalSystemSid);
    Dacl = RtlpAllocateMemory(DaclSize, 'cAeS');
    if (Dacl == NULL) {
	Status = STATUS_NO_MEMORY;
	goto Cleanup;
    }
    ...
    Status = RtlAddAccessAllowedAceEx(Dacl, ACL_REVISION,
    ...
    Status = RtlSetDaclSecurityDescriptor(AbsSD, TRUE, Dacl, FALSE);
	...
	*SystemSid = LocalSystemSid;
	...
	if (Dacl != NULL) {
	    RtlpFreeMemory(Dacl, 'cAeS');
	}

	if (AbsSD != NULL) {
	    RtlpFreeMemory(AbsSD, 'dSeS');
	}
    }
...
static NTSTATUS RtlpSysVolCheckOwnerAndSecurity(IN HANDLE DirectoryHandle,
						IN PISECURITY_DESCRIPTOR SecurityDescriptor)
{
    ...
    RelSD = RtlpAllocateMemory(DescriptorSize, 'dSeS');
    if (RelSD == NULL) {
	Status = STATUS_NO_MEMORY;
	goto Cleanup;
    }
    ...
    Status = NtQuerySecurityObject(DirectoryHandle,
				   OWNER_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION,
				   RelSD, DescriptorSize, &DescriptorSize);
    if (!NT_SUCCESS(Status)) {
    ...
    Status = RtlGetOwnerSecurityDescriptor(RelSD, &OwnerSid, &OwnerDefaulted);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }

    Status = RtlGetDaclSecurityDescriptor(RelSD, &DaclPresent, &Dacl, &DaclDefaulted);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }

    /* create the Administrators SID */
    Status = RtlAllocateAndInitializeSid(&LocalSystemAuthority, 2,
    ...
    /* check if the Administrators are the owner and at least a not-NULL DACL
       is present */
    if (OwnerSid != NULL && RtlEqualSid(OwnerSid, AdminSid) && DaclPresent &&
	Dacl != NULL) {
	...
	do {
		...
		if (RtlEqualSid((PSID)(Ace + 1), LocalSystemSid)) {
			...
			Status = NtSetSecurityObject(DirectoryHandle,
						     DACL_SECURITY_INFORMATION, RelSD);
		    } else {
    ...
    Status = RtlSelfRelativeToAbsoluteSD2(RelSD, &AbsSDSize);
#ifdef _WIN64
    if (Status == STATUS_BUFFER_TOO_SMALL) {
	...
	AbsSD = RtlpAllocateMemory(DescriptorSize, 'dSeS');
	if (AbsSD == NULL) {
	    Status = STATUS_NO_MEMORY;
	    goto Cleanup;
	}
	...
	RtlCopyMemory(AbsSD, RelSD, DescriptorSize);

	/* finally convert it */
	Status = RtlSelfRelativeToAbsoluteSD2(AbsSD, &AbsSDSize);
    } else
#endif
    {
	AbsSD = RelSD;
    ...
    /* set the owner SID */
    Status = RtlSetOwnerSecurityDescriptor(AbsSD, AdminSid, FALSE);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }

    /* set the DACL in the security descriptor */
    Status = RtlSetDaclSecurityDescriptor(AbsSD, TRUE, SecurityDescriptor->Dacl, FALSE);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }
    ...
    /* convert it back to a self-relative descriptor, find out how much
       memory we need */
    Status = RtlAbsoluteToSelfRelativeSD(AbsSD, NULL, &RelSDSize);
    if (Status != STATUS_BUFFER_TOO_SMALL) {
	goto Cleanup;
    }

    /* allocate enough memory for the new self-relative descriptor */
    NewRelSD = RtlpAllocateMemory(RelSDSize, 'dSeS');
    if (NewRelSD == NULL) {
	Status = STATUS_NO_MEMORY;
	goto Cleanup;
    }
    ...
    Status = RtlAbsoluteToSelfRelativeSD(AbsSD, NewRelSD, &RelSDSize);
    if (Status == STATUS_BUFFER_TOO_SMALL) {
	goto Cleanup;
    }
...
Cleanup:
    ...
    if (RelSD != NULL) {
	RtlpFreeMemory(RelSD, 'dSeS');
    }

    if (NewRelSD != NULL) {
	RtlpFreeMemory(NewRelSD, 'dSeS');
    }
...
static NTSTATUS RtlpSysVolTakeOwnership(IN PUNICODE_STRING DirectoryPath,
					IN PSECURITY_DESCRIPTOR SecurityDescriptor)
{
    ...
    Status = NtOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES,
				&hToken);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }
    ...
    Status = NtAdjustPrivilegesToken(hToken, FALSE, &TokenPrivileges,
				     sizeof(TokenPrivileges), &TokenPrivileges,
				     &ReturnLength);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }
    ...
    /* open the directory */
    InitializeObjectAttributes(&ObjectAttributes, DirectoryPath, 0, NULL,
			       SecurityDescriptor);

    Status = NtOpenFile(&hDirectory, SYNCHRONIZE | WRITE_OWNER, &ObjectAttributes,
			&IoStatusBlock,
    ...
    Status = NtSetSecurityObject(hDirectory, OWNER_SECURITY_INFORMATION, &AbsSD);

Cleanup:
    if (TokenEnabled) {
	/* Disable privileges that we had to enable, whetever the result was. */
	NTSTATUS Status2 = NtAdjustPrivilegesToken(hToken, FALSE, &TokenPrivileges, 0,
						   NULL, NULL);
	/* This must succeed */
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlCreateSystemVolumeInformationFolder(IN PUNICODE_STRING VolumeRootPath)
{
    OBJECT_ATTRIBUTES ObjectAttributes;
    IO_STATUS_BLOCK IoStatusBlock;
    HANDLE hDirectory;
    ...
    RtlInitUnicodeString(&DirectoryName, L"System Volume Information");
    ...
    NewPath.Buffer = RtlpAllocateStringMemory(NewPath.MaximumLength, TAG_USTR);
    ...
    RtlCopyMemory(NewPath.Buffer, VolumeRootPath->Buffer, NewPath.Length);
    if (AddSep) {
	NewPath.Buffer[NewPath.Length / sizeof(WCHAR)] = L'\\';
	NewPath.Length += sizeof(WCHAR);
    }
    RtlCopyMemory(NewPath.Buffer + (NewPath.Length / sizeof(WCHAR)), DirectoryName.Buffer,
    ...
    Status = RtlpSysVolCreateSecurityDescriptor(&SecurityDescriptor, &SystemSid);
    if (NT_SUCCESS(Status)) {
	/* create or open the directory */
	InitializeObjectAttributes(&ObjectAttributes, &NewPath, 0, NULL,
				   SecurityDescriptor);
	...
	Status = NtCreateFile(&hDirectory,
			      ...
			      FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
			      FILE_OPEN_IF,
			      FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL,
			      0);
	if (!NT_SUCCESS(Status)) {
	    Status = RtlpSysVolTakeOwnership(&NewPath, SecurityDescriptor);

	    if (NT_SUCCESS(Status)) {
		/* successfully took ownership, attempt to open it */
		Status = NtCreateFile(
		    &hDirectory, SYNCHRONIZE | WRITE_OWNER | WRITE_DAC | READ_CONTROL,
		    &ObjectAttributes, &IoStatusBlock, NULL,
		    FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_HIDDEN,
	...
	if (NT_SUCCESS(Status)) {
	    ...
	    Status = RtlpSysVolCheckOwnerAndSecurity(hDirectory, SecurityDescriptor);
	    NtClose(hDirectory);
	}
	...
	RtlpFreeMemory(SecurityDescriptor->Dacl, 'cAeS');
	RtlpFreeMemory(SecurityDescriptor, 'dSeS');

	RtlFreeSid(SystemSid);
    }

    RtlpFreeStringMemory(NewPath.Buffer, TAG_USTR);
    return Status;
}
