...
FORCEINLINE TABLE_SEARCH_RESULT
...
FORCEINLINE VOID RtlpPromoteAvlTreeNode(IN PRTL_BALANCED_LINKS Node)
{
    ...
    SwapNode1 =
	RtlIsLeftChildAvl(Node) ? &ParentNode->LeftChild : &ParentNode->
	RightChild;
    SwapNode2 =
	RtlIsLeftChildAvl(Node) ? &Node->RightChild : &Node->LeftChild;

    /* Do the rotate, and update the parent and super-parent as needed */
    *SwapNode1 = *SwapNode2;
    if (*SwapNode1)
	RtlSetParent(*SwapNode1, ParentNode);
				       ...
				       IN OUT TABLE_SEARCH_RESULT SearchResult)
{
	...
	Balance =
	    RtlIsLeftChildAvl(NewNode) ? RtlLeftHeavyAvlTree :
	    RtlRightHeavyAvlTree;
				       ...
				       IN PRTL_BALANCED_LINKS Node)
{
    ...
    if (!(RtlLeftChildAvl(Node)) || !(RtlRightChildAvl(Node)))
	DeleteNode = Node;
	...
	while (RtlLeftChildAvl(DeleteNode))
	...
	while (RtlRightChildAvl(DeleteNode))
	    DeleteNode = RtlRightChildAvl(DeleteNode);
    }
    ...
    Node1 =
	RtlLeftChildAvl(DeleteNode) ? &DeleteNode->
	LeftChild : &DeleteNode->RightChild;
    DPRINT("Node 1: %p %p\n", Node1, *Node1);

    /* Pick which node to swap based on if we're already a left child or not */
    Node2 =
	RtlIsLeftChildAvl(DeleteNode) ? &ParentNode->
	LeftChild : &ParentNode->RightChild;
    DPRINT("Node 2: %p %p\n", Node2, *Node2);

    /* Pick the correct balance depending on which side will get heavier */
    Balance = RtlIsLeftChildAvl(DeleteNode) ?
	RtlLeftHeavyAvlTree : RtlRightHeavyAvlTree;
    DPRINT("Balance: %x\n", Balance);
    ...
    *Node2 = *Node1;
	...
	if (RtlBalance(ParentNode) == Balance) {
	    ...
	    if (RtlpRebalanceAvlTreeNode(ParentNode))
		break;
	...
	LeftChild : &(RtlParentAvl(DeleteNode))->RightChild;
    *Node1 = DeleteNode;

    /* Reparent as appropriate */
    if (RtlLeftChildAvl(DeleteNode))
	RtlSetParent(RtlLeftChildAvl(DeleteNode), DeleteNode);
    if (RtlRightChildAvl(DeleteNode))
	RtlSetParent(RtlRightChildAvl(DeleteNode), DeleteNode);
}
