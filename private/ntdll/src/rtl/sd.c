...
static BOOLEAN RtlpValidateSDOffsetAndSize(IN ULONG Offset,
					   IN ULONG Length,
					   IN ULONG MinLength,
 ...
 */
FORCEINLINE PSID SepGetGroupFromDescriptor(IN OUT PSECURITY_DESCRIPTOR Desc)
{
    PISECURITY_DESCRIPTOR Descriptor = (PISECURITY_DESCRIPTOR)Desc;
...
FORCEINLINE PSID SepGetOwnerFromDescriptor(IN OUT PSECURITY_DESCRIPTOR Desc)
{
    PISECURITY_DESCRIPTOR Descriptor = (PISECURITY_DESCRIPTOR)Desc;
    PISECURITY_DESCRIPTOR_RELATIVE SdRel;
...
FORCEINLINE PACL SepGetDaclFromDescriptor(IN OUT PSECURITY_DESCRIPTOR Desc)
{
    PISECURITY_DESCRIPTOR Descriptor = (PISECURITY_DESCRIPTOR)Desc;
    PISECURITY_DESCRIPTOR_RELATIVE SdRel;

    if (!(Descriptor->Control & SE_DACL_PRESENT))
        return NULL;
...
FORCEINLINE PACL SepGetSaclFromDescriptor(IN OUT PSECURITY_DESCRIPTOR Desc)
{
    PISECURITY_DESCRIPTOR Descriptor = (PISECURITY_DESCRIPTOR)Desc;
    PISECURITY_DESCRIPTOR_RELATIVE SdRel;
...
static VOID RtlpQuerySecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
					OUT PSID *Owner,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlCreateSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
					   IN ULONG Revision)
{
 ...
 * @implemented
 */
NTAPI ULONG RtlLengthSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor)
{
    ...
    if (Owner)
	Length += ROUND_UP(RtlLengthSid(Owner), sizeof(ULONG));
    Group = SepGetGroupFromDescriptor(Sd);
    if (Group)
	Length += ROUND_UP(RtlLengthSid(Group), sizeof(ULONG));
    Dacl = SepGetDaclFromDescriptor(Sd);
    if (Dacl)
	Length += ROUND_UP(Dacl->AclSize, sizeof(ULONG));
    Sacl = SepGetSaclFromDescriptor(Sd);
    if (Sacl)
	Length += ROUND_UP(Sacl->AclSize, sizeof(ULONG));
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetDaclSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetSaclSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetOwnerSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
					     OUT PSID *Owner,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetGroupSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
					     OUT PSID *Group,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetDaclSecurityDescriptor(IN OUT PSECURITY_DESCRIPTOR SecurityDescriptor,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetSaclSecurityDescriptor(IN OUT PSECURITY_DESCRIPTOR SecurityDescriptor,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetOwnerSecurityDescriptor(IN OUT PSECURITY_DESCRIPTOR SecurityDescriptor,
					     IN PSID Owner,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetGroupSecurityDescriptor(IN OUT PSECURITY_DESCRIPTOR SecurityDescriptor,
					     IN PSID Group,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetControlSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
				...
				IN SECURITY_DESCRIPTOR_CONTROL ControlBitsToSet)
{
    ...
    if ((ControlBitsOfInterest &
	 ~(SE_DACL_UNTRUSTED | SE_SERVER_SECURITY | SE_DACL_AUTO_INHERIT_REQ |
 ...
 * @implemented
 */
NTAPI BOOLEAN RtlGetSecurityDescriptorRMControl(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
 ...
 * @implemented
 */
NTAPI VOID RtlSetSecurityDescriptorRMControl(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
					     IN PUCHAR RMControl)
{
    PISECURITY_DESCRIPTOR Sd = (PISECURITY_DESCRIPTOR)SecurityDescriptor;
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetAttributesSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
						  ...
						  OUT PULONG Revision)
{
    ...
    *Revision = Sd->Revision;
    ...
    Control &= SE_DACL_UNTRUSTED | SE_SERVER_SECURITY | SE_DACL_AUTO_INHERIT_REQ |
	SE_SACL_AUTO_INHERIT_REQ | SE_DACL_AUTO_INHERITED |
	SE_SACL_AUTO_INHERITED | SE_DACL_PROTECTED | SE_SACL_PROTECTED;
			  ...
			  OUT PSECURITY_DESCRIPTOR *pDestinationSecurityDescriptor)
{
    ...
    RtlpQuerySecurityDescriptor(Sd, &Owner, &OwnerLength, &Group, &GroupLength, &Dacl,
				&DaclLength, &Sacl, &SaclLength);
    ...
    /* Allocate a copy */
    *pDestinationSecurityDescriptor = RtlAllocateHeap(RtlGetProcessHeap(), 0,
						      TotalLength);
    if (*pDestinationSecurityDescriptor == NULL)
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlMakeSelfRelativeSD(IN PSECURITY_DESCRIPTOR SecurityDescriptor,
				     ...
				     IN OUT PULONG BufferLength)
{
    ...
    RtlpQuerySecurityDescriptor(SecurityDescriptor, &Owner, &OwnerLength, &Group,
				&GroupLength, &Dacl, &DaclLength, &Sacl, &SaclLength);
	...
	*BufferLength = TotalLength;
    ...
    RtlCopyMemory(RelSd, SecurityDescriptor,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSelfRelativeToAbsoluteSD(IN PSECURITY_DESCRIPTOR SelfRelativeSD,
					   ...
					   IN PULONG PrimaryGroupSize)
{
    ...
    /* Get all the components */
    RtlpQuerySecurityDescriptor(RelSd, &pOwner, &OwnerLength, &pGroup, &GroupLength,
				&pDacl, &DaclLength, &pSacl, &SaclLength);

    /* Fail if there's not enough space */
    if (!(Sd) || (sizeof(SECURITY_DESCRIPTOR) > *AbsoluteSDSize) ||
	(OwnerLength > *OwnerSize) || (GroupLength > *PrimaryGroupSize) ||
	(DaclLength > *DaclSize) || (SaclLength > *SaclSize)) {
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSelfRelativeToAbsoluteSD2(IN OUT PSECURITY_DESCRIPTOR SelfRelativeSD,
					    OUT PULONG BufferSize)
{
    ...
    RtlpQuerySecurityDescriptor(Sd, &pOwner, &OwnerLength, &pGroup, &GroupLength, &pDacl,
				&DaclLength, &pSacl, &SaclLength);
	...
	*BufferSize = sizeof(SECURITY_DESCRIPTOR) + DataSize;
	...
	RtlMoveMemory((PVOID)(Sd + 1), DataStart, DataSize);
    }
 ...
 * @implemented
 */
NTAPI BOOLEAN RtlValidSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor)
{
    PISECURITY_DESCRIPTOR Sd = (PISECURITY_DESCRIPTOR)SecurityDescriptor;
    PSID Owner, Group;
    PACL Sacl, Dacl;
	...
	Group = SepGetGroupFromDescriptor(Sd);
	if ((Group) && (!RtlValidSid(Group)))
	    return FALSE;

	/* DACL must be valid if present */
	Dacl = SepGetDaclFromDescriptor(Sd);
	if ((Dacl) && (!RtlValidAcl(Dacl)))
	    return FALSE;

	/* SACL must be valid if present */
	Sacl = SepGetSaclFromDescriptor(Sd);
	if ((Sacl) && (!RtlValidAcl(Sacl)))
	    return FALSE;
...
{
    PISECURITY_DESCRIPTOR_RELATIVE Sd = (PISECURITY_DESCRIPTOR_RELATIVE)
    ...
    if (Sd->Owner) {
	/* Try to access it */
	if (!RtlpValidateSDOffsetAndSize(Sd->Owner, SecurityDescriptorLength, sizeof(SID),
					 &Length)) {
    ...
    if (Sd->Group) {
	/* Try to access it */
	if (!RtlpValidateSDOffsetAndSize(Sd->Group, SecurityDescriptorLength, sizeof(SID),
					 &Length)) {
	    /* It's beyond the buffer, fail */
	    return FALSE;
	}
	...
	/* Try to access it */
	if (!RtlpValidateSDOffsetAndSize(Sd->Dacl, SecurityDescriptorLength, sizeof(ACL),
					 &Length)) {
	    /* It's beyond the buffer, fail */
	    return FALSE;
	}
	...
	/* Try to access it */
	if (!RtlpValidateSDOffsetAndSize(Sd->Sacl, SecurityDescriptorLength, sizeof(ACL),
					 &Length)) {
	    /* It's beyond the buffer, fail */
	    return FALSE;
	}
