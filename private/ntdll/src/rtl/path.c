...
NTAPI RTL_PATH_TYPE RtlDetermineDosPathNameType_Ustr(IN PCUNICODE_STRING PathString)
{
	...
	if ((Chars < 2) || !(IS_PATH_SEPARATOR(Path[1])))
	    return RtlPathTypeRooted;	/* \x             */
	if ((Chars < 3) || ((Path[2] != L'.') && (Path[2] != L'?')))
	    return RtlPathTypeUncAbsolute;	/* \\x            */
	if ((Chars >= 4) && (IS_PATH_SEPARATOR(Path[3])))
	    return RtlPathTypeLocalDevice;	/* \\.\x or \\?\x */
	if (Chars != 3)
	    return RtlPathTypeUncAbsolute;	/* \\.x or \\?x   */
	return RtlPathTypeRootLocalDevice;	/* \\. or \\?     */
    } else {
	if ((Chars < 2) || (Path[1] != L':'))
	    return RtlPathTypeRelative;	/* x              */
	if ((Chars < 3) || !(IS_PATH_SEPARATOR(Path[2])))
	    return RtlPathTypeDriveRelative;	/* x:             */
	return RtlPathTypeDriveAbsolute;	/* x:\            */
    }
}

NTAPI ULONG RtlIsDosDeviceName_Ustr(IN PCUNICODE_STRING PathString)
{
	...
	if (RtlEqualUnicodeString
	    (PathString, &RtlpDosSlashCONDevice, TRUE)) {
	    /* This should return 0x80006 */
	    return MAKELONG(RtlpDosCONDevice.Length,
	...
	for (End = &PathCopy.Buffer[PathChars - 1];
		...
		if ((End <
		     &PathCopy.Buffer[OriginalLength / sizeof(WCHAR)])
		    && ((c == L'l') || (c == L'c') || (c == L'p')
			|| (c == L'a') || (c == L'n'))) {
		    /* Calculate the offset */
		    ReturnOffset =
			(USHORT) ((PCHAR) End - (PCHAR) PathCopy.Buffer);

		    /* Build the final string */
		    PathCopy.Length =
			OriginalLength - ReturnOffset -
    ...
    while ((Start > PathCopy.Buffer) && (Start[-1] == L' '))
	--Start;
	...
	if ((RtlEqualUnicodeString(&PathCopy, &RtlpDosLPTDevice, TRUE)) ||
		...
		(RtlEqualUnicodeString
		 (&PathCopy, &RtlpDosCONDevice, TRUE)))) {
...
NTAPI NTSTATUS RtlpCheckDeviceName(IN PUNICODE_STRING FileName,
				   IN ULONG Length,
 ...
 */
static ULONG RtlpCollapsePath(PWSTR Path, /* ULONG PathBufferSize, ULONG PathLength, */
			      ULONG mark,
    ...
    *next = UNICODE_NULL;
	...
	if (*p == L'.') {
		...
		*p = UNICODE_NULL;
		...
		RtlMoveMemory(p, next,
			      (EndPath - next + 1) * sizeof(WCHAR));
		EndPath -= (next - p);
		continue;
	...
	while (p > Path + mark && IS_PATH_SEPARATOR(p[-1]))
	    p--;
    }
...
static SIZE_T RtlpSkipUNCPrefix(PCWSTR FileNameBuffer)
{
	...
	UncPath++;		/* share name */
    while (IS_PATH_SEPARATOR(*UncPath))
	UncPath++;
    while (*UncPath && !IS_PATH_SEPARATOR(*UncPath))
	UncPath++;		/* dir name */
...
NTAPI NTSTATUS RtlGetLengthWithoutLastFullDosOrNtPathElement(IN ULONG Flags,
							     ...
							     OUT PULONG LengthOut)
{
	...
	*LengthOut = 0;
	...
	(RtlFindCharInUnicodeString
	 (RTL_FIND_CHAR_IN_UNICODE_STRING_START_AT_END, Path,
	  &PathDividers, &Position)))
	Position = 0;
	...
	while (Tmp.Length > sizeof(WCHAR)
	       && IS_PATH_SEPARATOR(Tmp.
				    Buffer[Tmp.Length / sizeof(WCHAR)]))
	    Tmp.Length -= sizeof(WCHAR);

	/* Find the previous occurence */
	if (!NT_SUCCESS
	    (RtlFindCharInUnicodeString
	     (RTL_FIND_CHAR_IN_UNICODE_STRING_START_AT_END, &Tmp,
	      &PathDividers, &Position)))
	    Position = 0;
    }
	...
	while (Position > 1
	       && IS_PATH_SEPARATOR(Path->Buffer[Position - 1]))
	    Position--;
    }
...
NTAPI NTSTATUS RtlComputePrivatizedDllName_U(IN PUNICODE_STRING DllName,
					     ...
					     IN OUT PUNICODE_STRING LocalName)
{
    ...
    if (!(RtlGetCurrentPeb()->ProcessParameters->Flags
	  & RTL_USER_PROCESS_PARAMETERS_NORMALIZED)) {
	/* Normalize it */
	ImagePathName.Buffer = (PWSTR)((ULONG_PTR)ImagePathName.Buffer +
				       (ULONG_PTR)RtlGetCurrentPeb()->ProcessParameters);
    }


    if (!NT_SUCCESS
	(RtlFindCharInUnicodeString
	 (RTL_FIND_CHAR_IN_UNICODE_STRING_START_AT_END, DllName,
	  &RtlpPathDividers, &Position))) {
	DllNameOnly = *DllName;
    } else {
	...
	Status = RtlAppendUnicodeStringToString(&CopyLocalName,
	...
	if (RequiredSize >= UNICODE_STRING_MAX_BYTES) {
	    ...
	    if (NT_SUCCESS(Status)) {
		Status = RtlAppendUnicodeStringToString(&CopyRealName,
							&ImagePathName);
		ASSERT(NT_SUCCESS(Status));
	    }
	    if (NT_SUCCESS(Status)) {
...
NTAPI ULONG RtlGetFullPathName_Ustr(IN PUNICODE_STRING FileName,
				    IN ULONG Size,
				    OUT PWSTR Buffer,
				    ...
				    OUT RTL_PATH_TYPE * PathType)
{
    ...
    *PathType = RtlPathTypeUnknown;
	...
	if (FileNameLength != 0)
	    c = FileNameBuffer[FileNameLength / sizeof(WCHAR) - 1];
    }
    ...
    DPRINT("DOS length for filename: %x %wZ\n", DosLength, FileName);
    if (DosLength != 0) {
	...
	if (InvalidName && (DosLengthOffset != 0)) {
	    /* Do the check */
	    Status =
		RtlpCheckDeviceName(FileName, DosLengthOffset,
				    InvalidName);
    ...
    *PathType = RtlDetermineDosPathNameType_U(FileNameBuffer);
	...
	if ((NewDrive != CurDrive) || CurDirName->Buffer[1] != L':') {
	    ...
	    EnvVarName.MaximumLength =
		EnvVarName.Length + sizeof(WCHAR);
	    ...
	    Status =
		RtlQueryEnvironmentVariable_U(NULL, &EnvVarName,
					      &EnvVarValue);
	    switch (Status) {
	    case STATUS_SUCCESS:
		/*
		 ...
		 */
		EnvVarValue.Buffer[EnvVarValue.Length /
				   sizeof(WCHAR)] = L'\\';
		...
		reqsize =
		    EnvVarValue.Length + SourceLength +
		    sizeof(UNICODE_NULL);
		goto Quit;

	    default:
		DPRINT1
		    ("RtlQueryEnvironmentVariable_U(\"%wZ\") returned 0x%08x\n",
		     &EnvVarName, Status);
    ...
    if (PrefixLength)
	RtlMoveMemory(Buffer, Prefix, PrefixLength);
    /* Copy the remaining part of the path */
    RtlMoveMemory(Buffer + PrefixLength / sizeof(WCHAR), Source,
     ...
     */
    reqsize =
	RtlpCollapsePath(Buffer, /* Size, reqsize, */ PrefixCut,
			 SkipTrailingPathSeparators);
...
static NTSTATUS RtlpWin32NTNameToNtPathName_U(IN PUNICODE_STRING DosPath,
...
static NTSTATUS RtlpDosPathNameToRelativeNtPathName_Ustr(IN BOOLEAN HaveRelative,
							 ...
							 OUT PRTL_RELATIVE_NAME_U RelativeName)
{
    ...
    ULONG MaxLength, PathLength, PrefixLength, PrefixCut, LengthChars,
	Length;
    ...
    DPRINT("Relative: %x DosName: %wZ NtName: %p, PartName: %p, RelativeName: %p\n",
    ...
    PathLength = RtlGetFullPathName_Ustr(&CapturedDosName,
					 ...
					 Buffer,
					 PartName,
					 &NameInvalid, &InputPathType);
    if ((NameInvalid) || !(PathLength)
	|| (PathLength > (MAX_PATH * sizeof(WCHAR)))) {
	/* Invalid name, fail */
	DPRINT("Invalid name: %x Path Length: %x\n", NameInvalid,
	       PathLength);
...
NTAPI BOOLEAN RtlDosPathNameToRelativeNtPathName_Ustr(IN PCUNICODE_STRING DosName,
						      OUT PUNICODE_STRING NtName,
						      OUT PCWSTR *PartName,
						      OUT PRTL_RELATIVE_NAME_U RelativeName)
{
...
NTAPI BOOLEAN RtlDoesFileExists_UstrEx(IN PCUNICODE_STRING FileName,
				       IN BOOLEAN SucceedIfBusy)
{
    ...
    Status = NtQueryAttributesFile(&ObjectAttributes, &BasicInformation);
...
NTAPI BOOLEAN RtlDoesFileExists_UStr(IN PUNICODE_STRING FileName)
{
    /* Call the updated API */
    return RtlDoesFileExists_UstrEx(FileName, TRUE);
}

NTAPI BOOLEAN RtlDoesFileExists_UEx(IN PCWSTR FileName,
				    IN BOOLEAN SucceedIfBusy)
{
    UNICODE_STRING NameString;
 ...
 * @implemented
 */
NTAPI VOID RtlReleaseRelativeName(IN PRTL_RELATIVE_NAME_U RelativeName)
{
 ...
 * @implemented
 */
NTAPI ULONG RtlGetLongestNtPathLength(VOID)
{
    /*
     ...
     */
    return MAX_PATH + RtlpDosDevicesUncPrefix.Length / sizeof(WCHAR) +
	sizeof(ANSI_NULL);
}
 ...
 */
NTAPI NTSTATUS RtlGetLengthWithoutTrailingPathSeparators(IN ULONG Flags,
							 ...
							 OUT PULONG Length)
{
    ...
    *Length = 0;
    ...
    while (NumChars > 0
	   && IS_PATH_SEPARATOR(PathString->Buffer[NumChars - 1])) {
	--NumChars;
    }
 ...
 * @implemented
 */
NTAPI ULONG RtlIsDosDeviceName_U(IN PCWSTR Path)
{
 ...
 * @implemented
 */
NTAPI ULONG RtlGetCurrentDirectory_U(IN ULONG MaximumLength,
				     OUT PWSTR Buffer)
{
    ULONG Length, Bytes;
    PCURDIR CurDir;
    PWSTR CurDirName;
    DPRINT("RtlGetCurrentDirectory %u %p\n", MaximumLength, Buffer);
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetCurrentDirectory_U(IN PUNICODE_STRING Path)
{
    PCURDIR CurDir;
    NTSTATUS Status;
    RTL_PATH_TYPE PathType;
    ...
    if (((ULONG_PTR) (CurDir->Handle) & RTL_CURDIR_ALL_FLAGS) ==
    ...
    FullPathLength =
	RtlGetFullPathName_Ustr(Path, FullPath.MaximumLength,
				FullPath.Buffer, NULL, NULL, &PathType);
    if (!FullPathLength) {
	...
	(FullPath.Buffer, &NtName, NULL, NULL)) {
	Status = STATUS_OBJECT_NAME_INVALID;
	goto Leave;
    }
	...
	CurDirHandle =
	    (HANDLE) ((ULONG_PTR) (CurDir->Handle) &
		      ~RTL_CURDIR_ALL_FLAGS);
	CurDir->Handle = NULL;
	...
	CurDirHandle =
	    (HANDLE) ((ULONG_PTR) CurDirHandle | RTL_CURDIR_IS_REMOVABLE);
    }
    ...
    /* Copy full path */
    RtlCopyMemory(CurDir->DosPath.Buffer, FullPath.Buffer,
		  FullPath.Length + sizeof(WCHAR));
 ...
 * @implemented
 */
NTAPI ULONG RtlGetFullPathName_UEx(IN PWSTR FileName,
 ...
 * @implemented
 */
NTAPI ULONG RtlGetFullPathName_U(IN PCWSTR FileName,
				 IN ULONG Size,
				 OUT PWSTR Buffer,
				 OUT OPTIONAL PWSTR *ShortName)
{
    ...
    return RtlGetFullPathName_UEx((PWSTR) FileName,
				  Size, Buffer, ShortName, &PathType);
}

/*
 * @implemented
 */
NTAPI BOOLEAN RtlDosPathNameToNtPathName_U(IN PCWSTR DosName,
					   OUT PUNICODE_STRING NtName,
					   OUT PCWSTR *PartName,
					   OUT PRTL_RELATIVE_NAME_U RelativeName)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDosPathNameToNtPathName_U_WithStatus(IN PCWSTR DosName,
						       OUT PUNICODE_STRING NtName,
						       OUT PCWSTR *PartName,
						       OUT PRTL_RELATIVE_NAME_U	RelativeName)
{
 ...
 * @implemented
 */
NTAPI BOOLEAN RtlDosPathNameToRelativeNtPathName_U(IN PCWSTR DosName,
						   OUT PUNICODE_STRING NtName,
						   OUT PCWSTR *PartName,
						   OUT PRTL_RELATIVE_NAME_U RelativeName)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDosPathNameToRelativeNtPathName_U_WithStatus(IN PCWSTR DosName,
							       OUT PUNICODE_STRING NtName,
							       OUT PCWSTR *PartName,
							       OUT PRTL_RELATIVE_NAME_U RelativeName)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlNtPathNameToDosPathName(IN ULONG Flags,
					  ...
					  PULONG Unknown)
{
	...
	*PathType = 0;
	...
	(&RtlpDosDevicesUncPrefix, &Path->String, TRUE)) {
	...
	if (RtlPrefixUnicodeString
	    (&RtlpDosDevicesPrefix, &Path->String, FALSE)) {
	...
	if (Len + sizeof(UNICODE_NULL) <= Path->ByteBuffer.Size) {
	    ...
	    if (AlternatePrefix) {
		memcpy(Path->ByteBuffer.Buffer, AlternatePrefix->Buffer,
		       AlternatePrefix->Length);
		memmove(Path->ByteBuffer.Buffer + AlternatePrefix->Length,
			Path->ByteBuffer.Buffer + UsePrefix->Length,
			Len - AlternatePrefix->Length);
	    } else {
		memmove(Path->ByteBuffer.Buffer,
			Path->ByteBuffer.Buffer + UsePrefix->Length, Len);
	    }
 ...
 * @implemented
 */
NTAPI ULONG RtlDosSearchPath_U(IN PCWSTR Path,
			       IN PCWSTR FileName,
			       IN PCWSTR Extension,
			       IN ULONG Size,
			       IN PWSTR Buffer,
			       OUT PWSTR *PartName)
{
	...
	if ((BufferStart != NewBuffer)
	    && (BufferStart[-1] != OBJ_NAME_PATH_SEPARATOR)) {
	...
	if (ExtensionLength) {
	    ...
	    RtlCopyMemory((PCHAR) BufferStart + FileNameLength,
			  Extension, ExtensionLength + sizeof(WCHAR));
	} else {
	    /* Just NULL-terminate */
	    *(PWCHAR) ((PCHAR) BufferStart + FileNameLength) =
		UNICODE_NULL;
	}
	...
	if (RtlDoesFileExists_UEx(NewBuffer, FALSE)) {
	    ...
	    Length =
		RtlGetFullPathName_U(NewBuffer, Size, Buffer, PartName);
	    break;
	}
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlGetFullPathName_UstrEx(IN PUNICODE_STRING FileName,
					 ...
					 OUT PSIZE_T LengthNeeded)
{
	...
	*FilePartSize = 0;
	...
	if (FilePartSize)
	    *FilePartSize =
		ShortName ? (ShortName - StaticString->Buffer) : 0;
	...
	/* Set the part size */
	if (FilePartSize)
	    *FilePartSize = ShortName ? (ShortName - StaticBuffer) : 0;
	...
	RtlpAllocateStringMemory(Length + sizeof(WCHAR), TAG_USTR);
    ...
    TempDynamicString.MaximumLength =
	(USHORT) Length + sizeof(UNICODE_NULL);
    Length =
	RtlGetFullPathName_Ustr(FileName, Length, TempDynamicString.Buffer,
				&ShortName, NameInvalid, PathType);
    if (!Length) {
    ...
    if (FilePartSize)
	*FilePartSize =
	    ShortName ? (ShortName - TempDynamicString.Buffer) : 0;
...
Quickie:
	...
	&& (Status != STATUS_BUFFER_TOO_SMALL)) {
	DbgPrint("RTL: %s - failing on filename %wZ with status %08x\n",
 ...
 * @unimplemented
 */
NTAPI NTSTATUS RtlDosApplyFileIsolationRedirection_Ustr(IN ULONG Flags,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDosSearchPath_Ustr(IN ULONG Flags,
				     ...
				     OUT PSIZE_T LengthNeeded OPTIONAL)
{
    ...
    DPRINT("DOS Path Search: %x %wZ %wZ %wZ %wZ %wZ\n",
	   Flags, PathString, FileNameString, ExtensionString,
	   CallerBuffer, DynamicString);
	...
	*LengthNeeded = 0;
	...
	if (ExtensionString) {
		...
		End =
		    &FileNameString->Buffer[FileNameString->Length /
					    sizeof(WCHAR)];
	...
	WorstCaseLength = ExtensionLength +
	    ...
	    (USHORT) MaxPathSize + sizeof(UNICODE_NULL);
	if (WorstCaseLength > UNICODE_STRING_MAX_BYTES) {
	    ...
	    DbgPrint("%s returning STATUS_NAME_TOO_LONG because the computed "
		     "worst case file name length is %u bytes\n",
		     __FUNCTION__, WorstCaseLength);
	    Status = STATUS_NAME_TOO_LONG;
	    goto Quickie;
	}
	...
	while (p < End) {
		...
		ASSERT(StaticCandidateString.Buffer ==
		       StaticCandidateBuffer);
		...
		ASSERT((SegmentSize + FileNameString->Length +
			ExtensionLength) < UNICODE_STRING_MAX_BYTES);
		if ((SegmentSize + ExtensionLength +
		     FileNameString->Length) >
		    (UNICODE_STRING_MAX_BYTES - sizeof(WCHAR))) {
		    /* For some reason it's not fitting anymore. Something messed up */
		    DbgPrint("%s: internal error #2; SegmentSize = %zu, "
			     "FileName->Length = %u, DefaultExtensionLength = %u\n",
			     __FUNCTION__, SegmentSize,
		...
		StaticCandidateString.MaximumLength =
		    FileNameString->Length + WorstCaseLength;
		StaticCandidateString.Buffer =
		    RtlpAllocateStringMemory(WorstCaseLength, TAG_USTR);
	    ...
	    CandidateEnd =
		&StaticCandidateString.Buffer[ByteCount / sizeof(WCHAR)];
	...
	if (!RtlDoesFileExists_UstrEx(FileNameString, TRUE)) {
		...
		ExtensionString->Length + sizeof(UNICODE_NULL);
	    if (NamePlusExtLength > UNICODE_STRING_MAX_BYTES) {
		/* It won't fit in any kind of valid string, so fail */
		DbgPrint("%s: Failing because filename plus extension (%u bytes) is too big\n",
			 __FUNCTION__, NamePlusExtLength);
		Status = STATUS_NAME_TOO_LONG;
		goto Quickie;
	    }
		...
		StaticCandidateString.Buffer =
		    RtlpAllocateStringMemory(NamePlusExtLength, TAG_USTR);
		if (!StaticCandidateString.Buffer) {
 ...
 * @implemented
 */
NTAPI BOOLEAN RtlDoesFileExists_U(IN PCWSTR FileName)
{
