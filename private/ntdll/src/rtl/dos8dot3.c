 ...
 * @implemented
 */
NTAPI VOID RtlGenerate8dot3Name(IN PUNICODE_STRING Name,
				...
				OUT PUNICODE_STRING Name8dot3)
{
    ...
    RtlCopyMemory(Name8dot3->Buffer + (Name8dot3->Length / sizeof(WCHAR)),
		  IndexBuffer + (8 - IndexLength), Length);
    Name8dot3->Length += Length;
	...
	RtlCopyMemory(Name8dot3->Buffer + (Name8dot3->Length / sizeof(WCHAR)),
		      Context->ExtensionBuffer, Length);
	Name8dot3->Length += Length;
    }
}
 ...
 */
NTAPI BOOLEAN RtlIsNameLegalDOS8Dot3(IN PCUNICODE_STRING Name,
				     ...
				     OUT OPTIONAL PBOOLEAN NameContainsSpaces)
{
    ...
    if ((OemName->Length > 12) || (OemName->Buffer == NULL))
	return FALSE;

    /* a starting . is invalid, except for . and .. */
    if (OemName->Buffer[0] == '.') {
	if (OemName->Length != 1 && (OemName->Length != 2 || OemName->Buffer[1] != '.'))
	    return FALSE;
	...
	default:
	    if (strchr(Illegal, OemName->Buffer[i]))
		return FALSE;
	    break;
	}
    }
	...
	if (Dot > 8 || (OemName->Length - Dot > 4) || Dot == OemName->Length - 1)
	    return FALSE;
    }

    if (NameContainsSpaces)
	*NameContainsSpaces = GotSpace;
