			   ...
			   PRTL_HEAP_PARAMETERS Parameters)
{
	...
	DPRINT1("HEAP: Incorrect ReserveSize %zx\n", ReserveSize);
	return NULL;
    }

    if (ReserveSize < CommitSize) {
	DPRINT1("HEAP: Incorrect CommitSize %zx\n", CommitSize);
	return NULL;
    }

    if (Flags & HEAP_NO_SERIALIZE && Lock) {
	DPRINT1
	    ("HEAP: Can't specify Lock routine and have HEAP_NO_SERIALIZE flag set\n");
	return NULL;
    }
...
BOOLEAN RtlpDebugDestroyHeap(HANDLE HeapPtr)
{
	...
	NtFreeVirtualMemory(NtCurrentProcess(),
			    &Heap->HeaderValidateCopy, &Size, MEM_RELEASE);
    }
...
PVOID RtlpDebugAllocateHeap(PVOID HeapPtr, ULONG Flags, SIZE_T Size)
{
	...
	AllocSize = Size;
    AllocSize =
	((AllocSize + Heap->AlignRound) & Heap->AlignMask) +
	sizeof(HEAP_ENTRY_EXTRA);

    /* Check if size didn't exceed max one */
    if (AllocSize < Size || AllocSize > Heap->MaximumAllocationSize) {
	DPRINT1("HEAP: Too big allocation size %zx (max allowed %zx)\n",
				 ...
				 PULONG UserFlags)
{
    ...
    if (Heap->ForceFlags & HEAP_FLAG_PAGE_ALLOCS)
	return RtlpPageHeapGetUserInfo(HeapHandle, Flags, BaseAddress,
				       UserValue, UserFlags);
	...
	/* If it succeeded - call the main routine */
	Result =
	    RtlGetUserInfoHeap(HeapHandle, Flags, BaseAddress, UserValue,
			       UserFlags);
    }
				  ...
				  PVOID UserValue)
{
    ...
    if (Heap->ForceFlags & HEAP_FLAG_PAGE_ALLOCS)
	return RtlpPageHeapSetUserValue(HeapHandle, Flags, BaseAddress,
					UserValue);
	...
	/* If it succeeded - call the main routine */
	Result =
	    RtlSetUserValueHeap(HeapHandle, Flags, BaseAddress, UserValue);
				  ...
				  ULONG UserFlagsSet)
{
    ...
    if (Heap->ForceFlags & HEAP_FLAG_PAGE_ALLOCS)
	return RtlpPageHeapSetUserFlags(HeapHandle, Flags, BaseAddress,
	...
	/* If it succeeded - call the main routine */
	Result =
	    RtlSetUserFlagsHeap(HeapHandle, Flags, BaseAddress,
				UserFlagsReset, UserFlagsSet);
