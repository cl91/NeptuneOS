...
NTAPI NTSTATUS RtlpSetSecurityObject(IN OPTIONAL PVOID Object,
				     ...
				     IN OPTIONAL HANDLE Token)
{
	...
	Status = RtlGetOwnerSecurityDescriptor(ModificationDescriptor, &pOwnerSid,
					       &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
    } else {
	Status = RtlGetOwnerSecurityDescriptor(*ObjectsSecurityDescriptor, &pOwnerSid,
					       &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
    }
	...
	Status = RtlGetGroupSecurityDescriptor(ModificationDescriptor, &pGroupSid,
					       &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
    } else {
	Status = RtlGetGroupSecurityDescriptor(*ObjectsSecurityDescriptor, &pGroupSid,
					       &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
    }
	...
	Status = RtlGetDaclSecurityDescriptor(ModificationDescriptor, &Present, &pDacl,
					      &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
	...
	Status = RtlGetDaclSecurityDescriptor(*ObjectsSecurityDescriptor, &Present,
					      &pDacl, &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
	...
	Status = RtlGetSaclSecurityDescriptor(ModificationDescriptor, &Present, &pSacl,
					      &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
	...
	Status = RtlGetSaclSecurityDescriptor(*ObjectsSecurityDescriptor, &Present,
					      &pSacl, &Defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
	...
	DPRINT1("New security descriptor allocation failed (Status 0x%08x)\n", Status);
	goto done;
    }
    ...
    if (!NT_SUCCESS(Status)) {
	DPRINT1("New security descriptor creation failed (Status 0x%08x)\n", Status);
	goto done;
    }
...
NTAPI NTSTATUS RtlpNewSecurityObject(IN PSECURITY_DESCRIPTOR ParentDescriptor,
...
NTAPI NTSTATUS RtlpConvertToAutoInheritSecurityObject(IN PSECURITY_DESCRIPTOR ParentDescriptor,
						      IN PSECURITY_DESCRIPTOR CreatorDescriptor,
						      OUT PSECURITY_DESCRIPTOR *NewDescriptor,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDefaultNpAcl(OUT PACL *pAcl)
{
    NTSTATUS Status;
    HANDLE TokenHandle;
    ...
    UCHAR SidBuffer[FIELD_OFFSET(SID, SubAuthority) +
		    2 * RTL_FIELD_SIZE(SID, SubAuthority[0])];
    ...
    *pAcl = NULL;
    ...
    Status = NtQueryInformationToken(TokenHandle, TokenOwner, NULL, 0, &ReturnLength);
    ...
    Status = NtQueryInformationToken(TokenHandle, TokenOwner, OwnerSid, ReturnLength,
				     &ReturnLength);
    if (!NT_SUCCESS(Status))
	goto Cleanup2;
 ...
 * @unimplemented
 */
NTAPI NTSTATUS RtlCreateAndSetSD(IN PRTL_ACE_DATA AceData,
				 IN ULONG AceCount,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlDeleteSecurityObject(IN PSECURITY_DESCRIPTOR *ObjectDescriptor)
{
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlNewSecurityObject(IN PSECURITY_DESCRIPTOR ParentDescriptor,
				    IN PSECURITY_DESCRIPTOR CreatorDescriptor,
				    OUT PSECURITY_DESCRIPTOR *NewDescriptor,
				    IN BOOLEAN IsDirectoryObject,
				    IN HANDLE Token,
				    IN PGENERIC_MAPPING GenericMapping)
{
    ...
    return RtlpNewSecurityObject(ParentDescriptor, CreatorDescriptor, NewDescriptor, NULL,
				 0, IsDirectoryObject, 0, Token, GenericMapping);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlNewSecurityObjectEx(IN PSECURITY_DESCRIPTOR ParentDescriptor,
				      IN PSECURITY_DESCRIPTOR CreatorDescriptor,
				      OUT PSECURITY_DESCRIPTOR *NewDescriptor,
				      IN LPGUID ObjectType,
				      IN BOOLEAN IsDirectoryObject,
				      ...
				      IN PGENERIC_MAPPING GenericMapping)
{
    ...
    /* Call the internal API */
    return RtlpNewSecurityObject(ParentDescriptor, CreatorDescriptor, NewDescriptor,
				 ObjectType ? &ObjectType : NULL, ObjectType ? 1 : 0,
				 IsDirectoryObject, AutoInheritFlags, Token,
				 GenericMapping);
}
					    ...
					    IN PGENERIC_MAPPING GenericMapping)
{
    ...
    /* Call the internal API */
    return RtlpNewSecurityObject(ParentDescriptor, CreatorDescriptor, NewDescriptor,
				 ObjectTypes, GuidCount, IsDirectoryObject,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlNewInstanceSecurityObject(IN BOOLEAN ParentDescriptorChanged,
					    ...
					    IN PGENERIC_MAPPING GenericMapping)
{
    ...
    Status = NtQueryInformationToken(Token, TokenStatistics, &TokenStats,
				     sizeof(TokenStats), &Size);
    if (!NT_SUCCESS(Status))
	return Status;
    ...
    *NewClientTokenModifiedId = TokenStats.ModifiedId;
    ...
    return RtlNewSecurityObject(ParentDescriptor, CreatorDescriptor, NewDescriptor,
				IsDirectoryObject, Token, GenericMapping);
}

/*
 * @implemented
 */
NTAPI NTSTATUS RtlCreateUserSecurityObject(IN PRTL_ACE_DATA AceData,
					   IN ULONG AceCount,
					   IN PSID OwnerSid,
					   IN PSID GroupSid,
					   IN BOOLEAN IsDirectoryObject,
					   ...
					   OUT PSECURITY_DESCRIPTOR *NewDescriptor)
{
    ...
    Status = RtlCreateAndSetSD(AceData, AceCount, OwnerSid, GroupSid, &Sd);
    if (!NT_SUCCESS(Status))
	return Status;
    ...
    Status = RtlNewSecurityObject(NULL, Sd, NewDescriptor, IsDirectoryObject, TokenHandle,
				  GenericMapping);
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlNewSecurityGrantedAccess(IN ACCESS_MASK DesiredAccess,
					   ...
					   OUT PACCESS_MASK RemainingDesiredAccess)
{
    ...
    TOKEN_STATISTICS TokenStats;
    ULONG Size;
    DPRINT1("RtlNewSecurityGrantedAccess(%x)\n", DesiredAccess);
 ...
 * @unimplemented
 */
NTAPI NTSTATUS RtlQuerySecurityObject(IN PSECURITY_DESCRIPTOR ObjectDescriptor,
				      IN SECURITY_INFORMATION SecurityInformation,
				      ...
				      OUT PULONG ReturnLength)
{
    ...
    if (SecurityInformation & DACL_SECURITY_INFORMATION) {
	Status = RtlGetDaclSecurityDescriptor(ObjectDescriptor, &present, &pacl,
					      &defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
    ...
    if (SecurityInformation & SACL_SECURITY_INFORMATION) {
	Status = RtlGetSaclSecurityDescriptor(ObjectDescriptor, &present, &pacl,
					      &defaulted);
	if (!NT_SUCCESS(Status))
	    return Status;
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetSecurityObject(IN SECURITY_INFORMATION SecurityInformation,
				    IN PSECURITY_DESCRIPTOR ModificationDescriptor,
				    IN OUT PSECURITY_DESCRIPTOR *ObjectsSecurityDescriptor,
				    IN PGENERIC_MAPPING GenericMapping,
				    IN OPTIONAL HANDLE Token)
{
    /* Call the internal API */
    return RtlpSetSecurityObject(NULL, SecurityInformation, ModificationDescriptor,
				 ObjectsSecurityDescriptor, 0, GenericMapping,
				 Token);
}
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlSetSecurityObjectEx(IN SECURITY_INFORMATION SecurityInformation,
				      IN PSECURITY_DESCRIPTOR ModificationDescriptor,
				      IN OUT PSECURITY_DESCRIPTOR *ObjectsSecurityDescriptor,
				      IN ULONG AutoInheritFlags,
				      IN PGENERIC_MAPPING GenericMapping,
				      IN OPTIONAL HANDLE Token)
{
    /* Call the internal API */
    return RtlpSetSecurityObject(NULL, SecurityInformation, ModificationDescriptor,
				 ObjectsSecurityDescriptor, AutoInheritFlags,
 ...
 * @implemented
 */
NTAPI NTSTATUS RtlConvertToAutoInheritSecurityObject(IN PSECURITY_DESCRIPTOR ParentDescriptor,
						     IN PSECURITY_DESCRIPTOR CreatorDescriptor,
						     OUT PSECURITY_DESCRIPTOR *NewDescriptor,
						     IN LPGUID ObjectType,
						     IN BOOLEAN IsDirectoryObject,
...
{
    /* Call the internal API */
    return RtlpConvertToAutoInheritSecurityObject(ParentDescriptor, CreatorDescriptor,
						  NewDescriptor, ObjectType,
						  IsDirectoryObject, GenericMapping);
}
 ...
 * @unimplemented
 */
NTAPI BOOLEAN RtlFlushSecureMemoryCache(IN PVOID MemoryCache,
