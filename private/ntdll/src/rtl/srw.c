...
static VOID RtlpReleaseWaitBlockLockExclusive(IN OUT PRTL_SRWLOCK SRWLock,
...
static VOID RtlpReleaseWaitBlockLockLastShared(IN OUT PRTL_SRWLOCK SRWLock,
					       IN PRTLP_SRWLOCK_WAITBLOCK FirstWaitBlock)
{
    PRTLP_SRWLOCK_WAITBLOCK Next;
    LONG_PTR NewValue;
...
static VOID RtlpReleaseWaitBlockLock(IN OUT PRTL_SRWLOCK SRWLock)
{
...
static PRTLP_SRWLOCK_WAITBLOCK NTAPI RtlpAcquireWaitBlockLock(IN OUT PRTL_SRWLOCK SRWLock)
{
    ...
    while (1) {
	PrevValue = InterlockedOrPointer(&SRWLock->Ptr, RTL_SRWLOCK_CONTENTION_LOCK);
...
static VOID RtlpAcquireSRWLockExclusiveWait(IN OUT PRTL_SRWLOCK SRWLock,
...
static VOID RtlpAcquireSRWLockSharedWait(IN OUT PRTL_SRWLOCK SRWLock,
...
NTAPI VOID RtlInitializeSRWLock(OUT PRTL_SRWLOCK SRWLock)
{
    SRWLock->Ptr = NULL;
}

NTAPI VOID RtlAcquireSRWLockShared(IN OUT PRTL_SRWLOCK SRWLock)
{
	...
	if (CurrentValue & RTL_SRWLOCK_SHARED) {
		...
		if (CurrentValue & RTL_SRWLOCK_CONTENDED) {
			...
			CurrentValue) {
			RtlpAcquireSRWLockSharedWait(SRWLock, &StackWaitBlock,
						     &SharedWake);
...
NTAPI VOID RtlReleaseSRWLockShared(IN OUT PRTL_SRWLOCK SRWLock)
{
    LONG_PTR CurrentValue, NewValue;
    PRTLP_SRWLOCK_WAITBLOCK WaitBlock;
    BOOLEAN LastShared;
...
NTAPI VOID RtlAcquireSRWLockExclusive(IN OUT PRTL_SRWLOCK SRWLock)
{
    __ALIGNED(16) RTLP_SRWLOCK_WAITBLOCK StackWaitBlock;
...
NTAPI VOID RtlReleaseSRWLockExclusive(IN OUT PRTL_SRWLOCK SRWLock)
{
    LONG_PTR CurrentValue, NewValue;
    PRTLP_SRWLOCK_WAITBLOCK WaitBlock;
...
NTAPI BOOLEAN RtlTryAcquireSRWLockShared(PRTL_SRWLOCK SRWLock)
{
	...
	((GotValue = (LONG_PTR)InterlockedCompareExchangePointer(
	      &SRWLock->Ptr, (LONG_PTR *)NewValue, (LONG_PTR *)CompareValue)) !=
	 CompareValue) &&
...
NTAPI BOOLEAN RtlTryAcquireSRWLockExclusive(PRTL_SRWLOCK SRWLock)
{
