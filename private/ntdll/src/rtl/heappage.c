...
BOOLEAN RtlpDphGrowVirtual(PDPH_HEAP_ROOT DphRoot, SIZE_T Size);

BOOLEAN RtlpDphIsNormalFreeHeapBlock(PVOID Block,
...
PVOID RtlpDphPointerFromHandle(PVOID Handle)
{
    PHEAP NormalHeap = (PHEAP) Handle;
    PDPH_HEAP_ROOT DphHeap =
	(PDPH_HEAP_ROOT) ((PUCHAR) Handle + PAGE_SIZE);
...
VOID RtlpDphEnterCriticalSection(PDPH_HEAP_ROOT DphRoot, ULONG Flags)
{
    if (Flags & HEAP_NO_SERIALIZE) {
	/* More complex scenario */
	if (!RtlpTryEnterHeapLock(DphRoot->HeapCritSect, TRUE)) {
	    ...
	    RtlpEnterHeapLock(DphRoot->HeapCritSect, TRUE);
	}
    } else {
...
VOID RtlpDphLeaveCriticalSection(PDPH_HEAP_ROOT DphRoot)
{
...
VOID RtlpDphPreProcessing(PDPH_HEAP_ROOT DphRoot, ULONG Flags)
{
...
VOID RtlpDphPostProcessing(PDPH_HEAP_ROOT DphRoot)
{
    if (!DphRoot)
	return;
...
NTSTATUS RtlpSecMemFreeVirtualMemory(HANDLE Process,
				     PVOID *Base,
				     PSIZE_T Size,
				     ULONG Type)
{
	...
	Status =
	    NtFreeVirtualMemory(NtCurrentProcess(), SavedBase, SavedSize,
				Type);
    }
#endif
...
NTSTATUS RtlpDphAllocateVm(PVOID *Base,
			   SIZE_T Size,
			   ULONG Type,
			   ULONG Protection)
{
    ...
    Status = NtAllocateVirtualMemory(NtCurrentProcess(),
				     Base, 0, &Size, Type, Protection);
    DPRINT("Page heap: AllocVm (%p, %Ix, %x) status %x \n", Base, Size,
	   Type, Status);
    /* Check for failures */
    if (!NT_SUCCESS(Status)) {
	if (Type == MEM_RESERVE) {
	    InterlockedIncrement(&RtlpDphCounter);
	    if (RtlpDphBreakOptions & DPH_BREAK_ON_RESERVE_FAIL) {
		DPRINT1
		    ("Page heap: AllocVm (%p, %Ix, %x) failed with %x \n",
		     Base, Size, Type, Status);
		DbgBreakPoint();
		return Status;
	    }
	} else {
	    InterlockedIncrement(&RtlpDphAllocFails);
...
NTSTATUS RtlpDphFreeVm(PVOID Base, SIZE_T Size, ULONG Type)
{
    NTSTATUS Status;

    /* Free the memory */
    Status =
	RtlpSecMemFreeVirtualMemory(NtCurrentProcess(), &Base, &Size,
				    Type);
    ...
    if (!NT_SUCCESS(Status)) {
	if (Type == MEM_RELEASE) {
	    InterlockedIncrement(&RtlpDphReleaseFails);
	    if (RtlpDphBreakOptions & DPH_BREAK_ON_RELEASE_FAIL) {
		DPRINT1
		    ("Page heap: FreeVm (%p, %Ix, %x) failed with %x \n",
		     Base, Size, Type, Status);
		DbgBreakPoint();
		return Status;
	    }
	} else {
	    InterlockedIncrement(&RtlpDphFreeFails);
...
NTSTATUS RtlpDphProtectVm(PVOID Base, SIZE_T Size, ULONG Protection)
{
    NTSTATUS Status;
    ULONG OldProtection;
    ...
    /* Log/report failures */
    if (!NT_SUCCESS(Status)) {
	InterlockedIncrement(&RtlpDphProtectFails);
...
BOOLEAN RtlpDphWritePageHeapBlockInformation(PDPH_HEAP_ROOT DphRoot,
					     ...
					     SIZE_T UserSize)
{
    ...
    RtlFillMemory(FillPtr,
		  ROUND_UP(FillPtr, PAGE_SIZE) - (ULONG_PTR) FillPtr,
		  DPH_FILL_SUFFIX);
...
VOID RtlpDphPlaceOnFreeList(PDPH_HEAP_ROOT DphRoot,
...
VOID RtlpDphPlaceOnPoolList(PDPH_HEAP_ROOT DphRoot,
			    PDPH_HEAP_BLOCK Node)
{
...
VOID RtlpDphPlaceOnVirtualList(PDPH_HEAP_ROOT DphRoot,
			       PDPH_HEAP_BLOCK Node)
{
...
PDPH_HEAP_BLOCK RtlpDphTakeNodeFromUnusedList(PDPH_HEAP_ROOT DphRoot)
{
	...
	DphRoot->pUnusedNodeListHead = Next;
    if (DphRoot->pUnusedNodeListTail == Node)
	DphRoot->pUnusedNodeListTail = NULL;
...
VOID RtlpDphReturnNodeToUnusedList(PDPH_HEAP_ROOT DphRoot,
				   PDPH_HEAP_BLOCK Node)
{
...
VOID RtlpDphRemoveFromAvailableList(PDPH_HEAP_ROOT DphRoot,
				    PDPH_HEAP_BLOCK Node)
{
	...
	while (CurEntry != &DphRoot->AvailableAllocationHead) {
	    NodeEntry =
		CONTAINING_RECORD(CurEntry, DPH_HEAP_BLOCK,
				  AvailableEntry);
    ...
    Node->AdjacencyEntry.Flink =
	(PLIST_ENTRY) POINTER_REMOVE_BIAS(Node->AdjacencyEntry.Flink);
    Node->AdjacencyEntry.Blink =
	(PLIST_ENTRY) POINTER_REMOVE_BIAS(Node->AdjacencyEntry.Blink);
}
...
VOID RtlpDphRemoveFromFreeList(PDPH_HEAP_ROOT DphRoot,
			       PDPH_HEAP_BLOCK Node,
...
VOID RtlpDphCoalesceNodeIntoAvailable(PDPH_HEAP_ROOT DphRoot,
				      PDPH_HEAP_BLOCK Node)
{
	...
	NodeEntry =
	    CONTAINING_RECORD(CurEntry, DPH_HEAP_BLOCK, AvailableEntry);
	...
	if (Node->AvailableEntry.Flink != AvailListHead) {
	    NextNode =
		CONTAINING_RECORD(Node->AvailableEntry.Flink,
...
VOID RtlpDphCoalesceFreeIntoAvailable(PDPH_HEAP_ROOT DphRoot,
				      ULONG LeaveOnFreeList)
{
    ...
    DPRINT("RtlpDphCoalesceFreeIntoAvailable(%p %u)\n", DphRoot,
	   LeaveOnFreeList);

    while (Node) {
	FreeAllocations--;
	if (FreeAllocations < LeaveOnFreeList)
	    break;
...
VOID RtlpDphAddNewPool(PDPH_HEAP_ROOT DphRoot,
...
PDPH_HEAP_BLOCK RtlpDphFindAvailableMemory(PDPH_HEAP_ROOT DphRoot,
					   SIZE_T Size,
					   BOOLEAN Grow)
{
    PDPH_HEAP_BLOCK Node;
	...
	if (DphRoot->nFreeAllocations <= DPH_FREE_LIST_MINIMUM)
	    break;

	/* Calculate a new free list size */
	NewSize = DphRoot->nFreeAllocations >> 2;
	if (NewSize < DPH_FREE_LIST_MINIMUM)
	...
	if (!RtlpDphGrowVirtual(DphRoot, Size))
	    return NULL;
				      ...
				      PVOID pUserMem)
{
    ...
    Ptr =
	RtlLookupElementGenericTableAvl(&DphRoot->BusyNodesTable,
					&pUserMem);
    if (!Ptr)
	return NULL;
...
NTSTATUS RtlpDphSetProtectionBeforeUse(PDPH_HEAP_ROOT DphRoot,
				       PUCHAR VirtualBlock,
				       ULONG UserSize)
{
    ULONG Protection;
...
NTSTATUS RtlpDphSetProtectionAfterUse(PDPH_HEAP_ROOT DphRoot,
				      /*PUCHAR VirtualBlock */
				      PDPH_HEAP_BLOCK Node)
{
    ASSERT((Node->nVirtualAccessSize + PAGE_SIZE) <=
...
PDPH_HEAP_BLOCK RtlpDphAllocateNode(PDPH_HEAP_ROOT DphRoot)
{
    PDPH_HEAP_BLOCK Node;
    NTSTATUS Status;
	...
	if (Node) {
	    ...
	    Status =
		RtlpDphAllocateVm(&Ptr, SizeVirtual, MEM_COMMIT,
				  PAGE_NOACCESS);
...
BOOLEAN RtlpDphGrowVirtual(PDPH_HEAP_ROOT DphRoot, SIZE_T Size)
{
    ...
    Status =
	RtlpDphAllocateVm(&Base, VirtualSize, MEM_COMMIT, PAGE_NOACCESS);
    if (!NT_SUCCESS(Status)) {
	/* Retry again with a smaller size */
	VirtualSize = Size;
	Status =
	    RtlpDphAllocateVm(&Base, VirtualSize, MEM_COMMIT,
			      PAGE_NOACCESS);
	if (!NT_SUCCESS(Status)) {
	    /* Free the allocated node and return failure */
	    RtlpDphReturnNodeToUnusedList(DphRoot, Node);
...
RTL_GENERIC_COMPARE_RESULTS RtlpDphCompareNodeForTable(IN PRTL_AVL_TABLE Table,
...
PVOID RtlpDphAllocateNodeForTable(IN PRTL_AVL_TABLE Table,
				  IN CLONG ByteSize)
{
...
VOID RtlpDphFreeNodeForTable(IN PRTL_AVL_TABLE Table,
			     IN PVOID Buffer)
{
    /* Nothing */
}

NTSTATUS RtlpDphInitializeDelayedFreeQueue(VOID)
{
    NTSTATUS Status;
...
VOID RtlpDphFreeDelayedBlocksFromHeap(PDPH_HEAP_ROOT DphRoot,
				      PHEAP NormalHeap)
{
	...
	if (BlockInfo->Heap == DphRoot) {
	    ...
	    if (!RtlpDphIsNormalFreeHeapBlock
		(BlockInfo + 1, &ValidationInfo, TRUE)) {
		RtlpDphReportCorruptedBlock(DphRoot, 10, BlockInfo + 1,
					    ValidationInfo);
	    }
...
VOID RtlpDphInternalValidatePageHeap(PDPH_HEAP_ROOT DphRoot,
				     PVOID Address,
				     ULONG Value)
{
    UNIMPLEMENTED;
...
VOID RtlpDphVerifyIntegrity(PDPH_HEAP_ROOT DphRoot)
{
    UNIMPLEMENTED;
}
...
NTSTATUS RtlpDphProcessStartupInitialization(VOID)
{
    ...
    Status = RtlpInitializeHeapLock(&RtlpDphPageHeapListLock);
    if (!NT_SUCCESS(Status)) {
	ASSERT(FALSE);
	return Status;
    }
...
BOOLEAN RtlpDphShouldAllocateInPageHeap(PDPH_HEAP_ROOT DphRoot,
					SIZE_T Size)
{
    //UNIMPLEMENTED;
			  ...
			  PRTL_HEAP_PARAMETERS Parameters)
{
    ...
    /* Set protection */
    Status = RtlpDphProtectVm(Base, 2 * PAGE_SIZE + DPH_POOL_SIZE,
			      PAGE_READWRITE);
    if (!NT_SUCCESS(Status)) {
    ...
    DphRoot->NormalHeap =
	RtlCreateHeap(Flags, NULL, TotalSize, CommitSize, NULL,
    ...
    RtlpDphAddNewPool(DphRoot, NULL, DphRoot + 1,
		      DPH_POOL_SIZE - sizeof(DPH_HEAP_ROOT), FALSE);
	...
	DPRINT1
	    ("Page heap: process %p created heap @ %p (%p, flags 0x%X)\n",
...
PVOID RtlpPageHeapDestroy(HANDLE HeapPtr)
{
	...
	Node = CONTAINING_RECORD(Ptr, DPH_HEAP_BLOCK, pUserAllocation);
	if (!(DphRoot->ExtraFlags & DPH_EXTRA_CHECK_UNDERRUN)) {
	    if (!RtlpDphIsPageHeapBlock
		(DphRoot, Node->pUserAllocation, &Value, TRUE)) {
		RtlpDphReportCorruptedBlock(DphRoot, 3,
					    Node->pUserAllocation, Value);
	    }
	}
    ...
    RtlpLeaveHeapLock(DphRoot->HeapCritSect);
    RtlpDeleteHeapLock(DphRoot->HeapCritSect);
    ...
    /* Report success */
    if (RtlpDphDebugOptions & DPH_DEBUG_VERBOSE)
	DPRINT1("Page heap: process %p destroyed heap @ %p (%p)\n",
			   ...
			   IN SIZE_T Size)
{
    ...
    AvailableNode =
	RtlpDphFindAvailableMemory(DphRoot, AllocateSize, TRUE);
    if (!AvailableNode) {
	...
	BusyNode->pUserAllocation =
	    BusyNode->pVirtualBlock + BusyNode->nVirtualAccessSize -
	    UserActualSize;
...
PVOID RtlpPageHeapReAllocate(IN HANDLE HeapPtr,
			     IN ULONG Flags,
			     IN PVOID Ptr,
			     IN SIZE_T Size)
{
    ...
    if (!(DphRoot->ExtraFlags & DPH_EXTRA_CHECK_UNDERRUN)) {
	if (!RtlpDphIsPageHeapBlock
	    (DphRoot, AllocatedNode->pUserAllocation, &ValidationInfo,
	     TRUE)) {
	    RtlpDphReportCorruptedBlock(DphRoot, 3,
	...
	NewAlloc =
	    RtlpPageHeapAllocate((PVOID) POINTER_ADD_BIAS(HeapPtr), Flags,
				 Size);
    }
    ...
    if (DataSize != 0)
	RtlCopyMemory(NewAlloc, Ptr, DataSize);
...
SIZE_T RtlpPageHeapSize(HANDLE HeapHandle, ULONG Flags, PVOID BaseAddress)
{
    PDPH_HEAP_ROOT DphRoot;
    PDPH_HEAP_BLOCK Node;
    SIZE_T Size;
