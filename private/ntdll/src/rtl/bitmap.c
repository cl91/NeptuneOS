			...
			IN BITMAP_INDEX MaxLength)
{
    ...
    Length =
	(BITMAP_INDEX) (Buffer - BitMapHeader->Buffer) * _BITCOUNT -
	StartingIndex;
		      ...
		      IN BITMAP_INDEX MaxLength)
{
    ...
    /* Calculate length up to where we read */
    Length =
	(ULONG) (Buffer - BitMapHeader->Buffer) * _BITCOUNT -
	StartingIndex;
    Length += BitPos - _BITCOUNT;
...
VOID NTAPI RtlClearAllBits(IN PRTL_BITMAP BitMapHeader)
{
    ...
    LengthInUlongs =
	(BitMapHeader->SizeOfBitMap + _BITCOUNT - 1) / _BITCOUNT;
		...
		IN BITMAP_INDEX StartingIndex, IN BITMAP_INDEX Length)
{
    ...
    return RtlpGetLengthOfRunClear(BitMapHeader, StartingIndex,
				   Length) >= Length;
}
	      ...
	      IN BITMAP_INDEX StartingIndex, IN BITMAP_INDEX Length)
{
    ...
    return RtlpGetLengthOfRunSet(BitMapHeader, StartingIndex,
				 Length) >= Length;
}
			   ...
			   OUT PBITMAP_INDEX StartingRunIndex)
{
    ...
    *StartingRunIndex = FromIndex + Length;
    ...
    return RtlpGetLengthOfRunClear(BitMapHeader, FromIndex + Length,
				   MAXINDEX);
}
			 ...
			 OUT PBITMAP_INDEX StartingRunIndex)
{
    ...
    *StartingRunIndex = FromIndex + Length;
    ...
    /* Now return the length of the run */
    return RtlpGetLengthOfRunSet(BitMapHeader, FromIndex + Length,
				 MAXINDEX);
}
			    ...
			    OUT PBITMAP_INDEX StartingRunIndex)
{
    ...
    FromIndex =
	(BITMAP_INDEX) ((Buffer + 1 - BitMapHeader->Buffer) * _BITCOUNT +
			BitPos);

    Value =
	~InvValue << ((_BITCOUNT - 1) - BitPos) >> ((_BITCOUNT - 1) -
						    BitPos);
	...
	*StartingRunIndex =
	    (BITMAP_INDEX) ((Buffer + 1 -
			     BitMapHeader->Buffer) * _BITCOUNT + BitPos +
			    1);
    } else {
