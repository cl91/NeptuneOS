...
typedef NTSTATUS (NTAPI BCLASS_QUERY_INFORMATION_CALLBACK)(IN PVOID Context,
							   IN ULONG BatteryTag,
...
typedef NTSTATUS (NTAPI BCLASS_QUERY_STATUS_CALLBACK)(IN PVOID Context,
						      IN ULONG BatteryTag,
...
typedef NTSTATUS (NTAPI BCLASS_SET_STATUS_NOTIFY_CALLBACK)(IN PVOID Context,
							   IN ULONG BatteryTag,
							   IN PBATTERY_NOTIFY BatteryNotify);
typedef BCLASS_SET_STATUS_NOTIFY_CALLBACK *PBCLASS_SET_STATUS_NOTIFY_CALLBACK;

typedef NTSTATUS (NTAPI BCLASS_SET_INFORMATION_CALLBACK)(IN PVOID Context,
							 IN ULONG BatteryTag,
							 IN BATTERY_SET_INFORMATION_LEVEL Level,
							 IN OPTIONAL PVOID Buffer);
typedef BCLASS_SET_INFORMATION_CALLBACK *PBCLASS_SET_INFORMATION_CALLBACK;

typedef NTSTATUS (NTAPI BCLASS_DISABLE_STATUS_NOTIFY_CALLBACK)(IN PVOID Context);
...
NTAPI BCLASSAPI NTSTATUS BatteryClassInitializeDevice(IN PBATTERY_MINIPORT_INFO MiniportInfo,
						      OUT PVOID *ClassData);

NTAPI BCLASSAPI NTSTATUS BatteryClassIoctl(IN PVOID ClassData,
					   IN OUT PIRP Irp);

NTAPI BCLASSAPI NTSTATUS BatteryClassStatusNotify(IN PVOID ClassData);

NTAPI BCLASSAPI NTSTATUS BatteryClassQueryWmiDataBlock(IN PVOID ClassData,
...
NTAPI BCLASSAPI NTSTATUS BatteryClassSystemControl(IN PVOID ClassData,
						   IN PVOID WmiLibContext, /* PWMILIB_CONTEXT */
						   IN PDEVICE_OBJECT DeviceObject,
						   IN OUT PIRP Irp,
						   OUT PVOID Disposition); /* PSYSCTL_IRP_DISPOSITION */

NTAPI BCLASSAPI NTSTATUS BatteryClassUnload(IN PVOID ClassData);
