...
#define SCSI_FAILURE_PREDICTION_THRESHOLD_EXCEEDED \
...
typedef struct STOR_ADDRESS_ALIGN _STOR_ADDR_BTL8 {
    ...
    USHORT Port;
    _Field_range_(STOR_ADDR_BTL8_ADDRESS_LENGTH,
		  STOR_ADDR_BTL8_ADDRESS_LENGTH) ULONG AddressLength;
    UCHAR Path;
    UCHAR Target;
    UCHAR Lun;
    UCHAR Reserved;
...
#define ScsiGetSenseDescriptorLength(DescriptorBuffer) \
    (sizeof(SCSI_SENSE_DESCRIPTOR_HEADER) +            \
     ((PSCSI_SENSE_DESCRIPTOR_HEADER)(DescriptorBuffer))->AdditionalLength)
...
#define IsDescriptorSenseDataFormat(SenseInfoBuffer) \
    ((ScsiGetSenseErrorCode(SenseInfoBuffer)) ==     \
...
FORCEINLINE BOOLEAN ScsiGetTotalSenseByteCountIndicated(IN PVOID SenseInfoBuffer,
							IN UCHAR SenseInfoBufferLength,
							OUT UCHAR *TotalByteCountIndicated)
{
    ...
    if (RTL_CONTAINS_FIELD(senseInfoBuffer, SenseInfoBufferLength,
...
FORCEINLINE BOOLEAN ScsiGetFixedSenseKeyAndCodes(IN PVOID SenseInfoBuffer,
						 IN UCHAR SenseInfoBufferLength,
						 OUT OPTIONAL PUCHAR SenseKey,
						 OUT OPTIONAL PUCHAR AdditionalSenseCode,
						 OUT OPTIONAL PUCHAR AdditionalSenseCodeQualifier)
{
    ...
    if (RTL_CONTAINS_FIELD(fixedSenseData, SenseInfoBufferLength,
			   AdditionalSenseLength)) {
	...
	if (AdditionalSenseCode != NULL) {
	    *AdditionalSenseCode = RTL_CONTAINS_FIELD(fixedSenseData, dataLength,
						      AdditionalSenseCode) ?
	...
	if (AdditionalSenseCodeQualifier != NULL) {
	    *AdditionalSenseCodeQualifier =
		RTL_CONTAINS_FIELD(fixedSenseData, dataLength,
				   AdditionalSenseCodeQualifier) ?
...
FORCEINLINE BOOLEAN ScsiGetDescriptorSenseKeyAndCodes(IN PVOID SenseInfoBuffer,
						      IN UCHAR SenseInfoBufferLength,
						      OUT OPTIONAL PUCHAR SenseKey,
						      OUT OPTIONAL PUCHAR AdditionalSenseCode,
						      OUT OPTIONAL PUCHAR AdditionalSenseCodeQualifier)
{
    ...
    if (RTL_CONTAINS_FIELD(descriptorSenseData, SenseInfoBufferLength,
			   AdditionalSenseLength)) {
	if (SenseKey) {
	...
	if (AdditionalSenseCodeQualifier != NULL) {
	    *AdditionalSenseCodeQualifier =
		descriptorSenseData->AdditionalSenseCodeQualifier;
	}
...
FORCEINLINE BOOLEAN ScsiGetSenseKeyAndCodes(IN PVOID SenseInfoBuffer,
					    IN UCHAR SenseInfoBufferLength,
					    ...
					    OUT OPTIONAL PUCHAR AdditionalSenseCodeQualifier)
{
	...
	succeed = ScsiGetDescriptorSenseKeyAndCodes(SenseInfoBuffer,
						    SenseInfoBufferLength, SenseKey,
						    AdditionalSenseCode,
						    AdditionalSenseCodeQualifier);
    } else {
	if ((Options & SCSI_SENSE_OPTIONS_FIXED_FORMAT_IF_UNKNOWN_FORMAT_INDICATED) ||
	    IsFixedSenseDataFormat(SenseInfoBuffer)) {
	    succeed = ScsiGetFixedSenseKeyAndCodes(SenseInfoBuffer, SenseInfoBufferLength,
						   SenseKey, AdditionalSenseCode,
						   AdditionalSenseCodeQualifier);
	}
    }
...
FORCEINLINE BOOLEAN ScsiGetSenseDescriptor(IN PVOID SenseInfoBuffer,
					   IN UCHAR SenseInfoBufferLength,
					   OUT PVOID *DescriptorBuffer,
					   OUT UCHAR *DescriptorBufferLength)
{
    ...
    if (SenseInfoBuffer == NULL || SenseInfoBufferLength == 0 ||
	DescriptorBuffer == NULL || DescriptorBufferLength == NULL) {
	return FALSE;
    }
    ...
    *DescriptorBufferLength = 0;
	...
	if (descriptorSenseData->AdditionalSenseLength <=
	    ...
	    dataLength = descriptorSenseData->AdditionalSenseLength +
			 RTL_SIZEOF_THROUGH_FIELD(DESCRIPTOR_SENSE_DATA,
						  AdditionalSenseLength);
	    ...
	    *DescriptorBufferLength = dataLength -
				      RTL_SIZEOF_THROUGH_FIELD(DESCRIPTOR_SENSE_DATA,
							       AdditionalSenseLength);
...
FORCEINLINE BOOLEAN ScsiValidateInformationSenseDescriptor(IN PVOID DescriptorBuffer,
							   IN UCHAR DescriptorBufferLength)
{
    ...
    additionalLength = sizeof(SCSI_SENSE_DESCRIPTOR_INFORMATION) -
		       RTL_SIZEOF_THROUGH_FIELD(SCSI_SENSE_DESCRIPTOR_INFORMATION,
						Header);
...
FORCEINLINE BOOLEAN ScsiValidateBlockCommandSenseDescriptor(IN PVOID DescriptorBuffer,
							    IN UCHAR DescriptorBufferLength)
{
...
FORCEINLINE BOOLEAN ScsiConvertToFixedSenseFormat(IN PVOID SenseInfoBuffer,
						  IN UCHAR SenseInfoBufferLength,
						  OUT PVOID OutBuffer,
						  IN UCHAR OutBufferLength)
{
    BOOLEAN succeed = FALSE;
	...
	validSense = ScsiGetSenseKeyAndCodes(SenseInfoBuffer, SenseInfoBufferLength,
...
FORCEINLINE BOOLEAN ScsiGetNextSenseDescriptorByType(IN PVOID Buffer,
						     IN UCHAR BufferLength,
						     IN PUCHAR TypeList,
						     IN ULONG TypeListCount,
						     OUT PUCHAR OutType,
						     OUT PVOID *OutBuffer,
						     OUT UCHAR *OutBufferLength)
{
    ...
    *OutType = 0;
	...
	for (i = 0; i < TypeListCount; i++) {
	    ...
	    if (((PSCSI_SENSE_DESCRIPTOR_HEADER)remainingBuffer)->DescriptorType ==
		type) {
