...
NTAPI SCSIPORT_API VOID ScsiPortCompleteRequest(IN PVOID HwDeviceExtension,
						IN UCHAR PathId, IN UCHAR TargetId,
						IN UCHAR Lun, IN UCHAR SrbStatus);

NTAPI SCSIPORT_API ULONG
...
NTAPI SCSIPORT_API SCSI_PHYSICAL_ADDRESS
ScsiPortConvertUlongToPhysicalAddress(IN ULONG_PTR UlongAddress);

NTAPI SCSIPORT_API VOID ScsiPortFlushDma(IN PVOID DeviceExtension);

NTAPI SCSIPORT_API VOID ScsiPortFreeDeviceBase(IN PVOID HwDeviceExtension,
					       IN PVOID MappedAddress);

NTAPI SCSIPORT_API ULONG ScsiPortGetBusData(IN PVOID DeviceExtension,
					    IN ULONG BusDataType,
					    IN ULONG SystemIoBusNumber,
					    IN ULONG SlotNumber, IN PVOID Buffer,
					    IN ULONG Length);

NTAPI SCSIPORT_API PVOID ScsiPortGetDeviceBase(IN PVOID HwDeviceExtension,
...
NTAPI SCSIPORT_API PVOID ScsiPortGetLogicalUnit(IN PVOID HwDeviceExtension,
						IN UCHAR PathId, IN UCHAR TargetId,
						IN UCHAR Lun);

NTAPI SCSIPORT_API SCSI_PHYSICAL_ADDRESS ScsiPortGetPhysicalAddress(IN PVOID HwDeviceExtension,
								    IN PSCSI_REQUEST_BLOCK Srb,
								    IN PVOID VirtualAddress,
								    OUT ULONG *Length);

NTAPI SCSIPORT_API PSCSI_REQUEST_BLOCK ScsiPortGetSrb(IN PVOID DeviceExtension,
						      IN UCHAR PathId, IN UCHAR TargetId,
						      IN UCHAR Lun, IN LONG QueueTag);

NTAPI SCSIPORT_API PVOID ScsiPortGetUncachedExtension(IN PVOID HwDeviceExtension,
						      IN PPORT_CONFIGURATION_INFORMATION Info,
						      IN ULONG NumberOfBytes);

NTAPI SCSIPORT_API PVOID ScsiPortGetVirtualAddress(IN PVOID HwDeviceExtension,
						   IN SCSI_PHYSICAL_ADDRESS PhysicalAddress);

NTAPI SCSIPORT_API ULONG ScsiPortInitialize(IN PVOID Argument1,
					    IN PVOID Argument2,
					    IN struct _HW_INITIALIZATION_DATA *HwInitData,
					    IN PVOID HwContext);

NTAPI SCSIPORT_API VOID ScsiPortIoMapTransfer(IN PVOID HwDeviceExtension,
					      IN PSCSI_REQUEST_BLOCK Srb,
					      IN PVOID LogicalAddress, IN ULONG Length);

NTAPI SCSIPORT_API VOID ScsiPortLogError(IN PVOID HwDeviceExtension,
					 IN OPTIONAL PSCSI_REQUEST_BLOCK Srb,
					 IN UCHAR PathId, IN UCHAR TargetId, IN UCHAR Lun,
					 IN ULONG ErrorCode, IN ULONG UniqueId);

SCSIPORT_API VOID ScsiPortNotification(IN SCSI_NOTIFICATION_TYPE NotificationType,
				       IN PVOID HwDeviceExtension, ...);

NTAPI SCSIPORT_API VOID ScsiPortQuerySystemTime(OUT PLARGE_INTEGER CurrentTime);

NTAPI SCSIPORT_API ULONG ScsiPortSetBusDataByOffset(IN PVOID DeviceExtension,
						    IN ULONG BusDataType,
						    IN ULONG SystemIoBusNumber,
						    IN ULONG SlotNumber, IN PVOID Buffer,
						    IN ULONG Offset, IN ULONG Length);

NTAPI SCSIPORT_API VOID ScsiPortStallExecution(IN ULONG Delay);

NTAPI SCSIPORT_API BOOLEAN ScsiPortValidateRange(IN PVOID HwDeviceExtension,
						 IN INTERFACE_TYPE BusType,
						 IN ULONG SystemIoBusNumber,
						 IN SCSI_PHYSICAL_ADDRESS IoAddress,
						 IN ULONG NumberOfBytes,
...
SCSIPORT_API VOID ScsiDebugPrint(ULONG DebugPrintLevel,
				 PCCHAR DebugMessage, ...);
...
NTAPI SCSIPORT_API UCHAR ScsiPortReadPortUchar(IN PUCHAR Port);

NTAPI SCSIPORT_API ULONG ScsiPortReadPortUlong(IN PULONG Port);

NTAPI SCSIPORT_API USHORT ScsiPortReadPortUshort(IN PUSHORT Port);

NTAPI SCSIPORT_API VOID ScsiPortReadPortBufferUchar(IN PUCHAR Port, IN PUCHAR Buffer,
						    IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortReadPortBufferUlong(IN PULONG Port, IN PULONG Buffer,
						    IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortReadPortBufferUshort(IN PUSHORT Port, IN PUSHORT Buffer,
						     IN ULONG Count);

NTAPI SCSIPORT_API UCHAR ScsiPortReadRegisterUchar(IN PUCHAR Register);

NTAPI SCSIPORT_API ULONG ScsiPortReadRegisterUlong(IN PULONG Register);

NTAPI SCSIPORT_API USHORT ScsiPortReadRegisterUshort(IN PUSHORT Register);

NTAPI SCSIPORT_API VOID ScsiPortReadRegisterBufferUchar(IN PUCHAR Register,
							IN PUCHAR Buffer, IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortReadRegisterBufferUlong(IN PULONG Register,
							IN PULONG Buffer, IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortReadRegisterBufferUshort(IN PUSHORT Register,
							 IN PUSHORT Buffer,
							 IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortWritePortUchar(IN PUCHAR Port, IN UCHAR Value);

NTAPI SCSIPORT_API VOID ScsiPortWritePortUlong(IN PULONG Port, IN ULONG Value);

NTAPI SCSIPORT_API VOID ScsiPortWritePortUshort(IN PUSHORT Port, IN USHORT Value);

NTAPI SCSIPORT_API VOID ScsiPortWritePortBufferUchar(IN PUCHAR Port, IN PUCHAR Buffer,
						     IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortWritePortBufferUlong(IN PULONG Port, IN PULONG Buffer,
						     IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortWritePortBufferUshort(IN PUSHORT Port, IN PUSHORT Buffer,
						      IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortWriteRegisterUchar(IN PUCHAR Register, IN UCHAR Value);

NTAPI SCSIPORT_API VOID ScsiPortWriteRegisterUlong(IN PULONG Register, IN ULONG Value);

NTAPI SCSIPORT_API VOID ScsiPortWriteRegisterUshort(IN PUSHORT Register, IN USHORT Value);

NTAPI SCSIPORT_API VOID ScsiPortWriteRegisterBufferUchar(IN PUCHAR Register,
							 IN PUCHAR Buffer,
							 IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortWriteRegisterBufferUlong(IN PULONG Register,
							 IN PULONG Buffer,
							 IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortWriteRegisterBufferUshort(IN PUSHORT Register,
							  IN PUSHORT Buffer,
							  IN ULONG Count);

NTAPI SCSIPORT_API VOID ScsiPortMoveMemory(IN PVOID WriteBuffer, IN PVOID ReadBuffer,
					   IN ULONG Length);
