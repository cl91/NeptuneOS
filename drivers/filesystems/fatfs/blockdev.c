...
static IO_COMPLETION_ROUTINE FatReadWritePartialCompletion;

static NTAPI NTSTATUS FatReadWritePartialCompletion(IN PDEVICE_OBJECT DeviceObject,
						    IN PIRP Irp,
						    IN PVOID Context)
{
    ...
    DPRINT("FatReadWritePartialCompletion() called\n");
    ...
    DPRINT("FatReadWritePartialCompletion() done\n");
...
again:
    ...
    DPRINT("FatReadDisk(pDeviceObject %p, Offset %I64x, Length %u, Buffer %p)\n",
			    ...
			    IN BOOLEAN Wait)
{
    ...
    DPRINT("FatReadDiskPartial(IrpContext %p, ReadOffset %I64x, ReadLength %u, BufferOffset %u, Wait %u)\n",
...
again:
    ...
    IoSetCompletionRoutine(Irp,
			   FatReadWritePartialCompletion,
			   IrpContext, TRUE, TRUE, TRUE);
 ...
 * This is just a copy paste from FatReadDisk()
 */
...
again:
    ...
    DPRINT("FatWriteDisk(pDeviceObject %p, Offset %I64x, Length %u, Buffer %p)\n",
    ...
    if (!NT_SUCCESS(Status)) {
	DPRINT("IO failed!!! FatWriteDisk : Error code: %x\n", Status);
	DPRINT("(pDeviceObject %p, Offset %I64x, Size %u, Buffer %p\n",
	       pDeviceObject, WriteOffset->QuadPart, WriteLength, Buffer);
	return Status;
    }
			     ...
			     IN ULONG BufferOffset, IN BOOLEAN Wait)
{
    ...
    DPRINT("FatWriteDiskPartial(IrpContext %p, WriteOffset %I64x, WriteLength %u, BufferOffset %x, Wait %u)\n",
