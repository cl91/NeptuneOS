...
NTAPI VOID PciDebugPrintIoResource(IN PIO_RESOURCE_DESCRIPTOR Descriptor)
{
    ...
    DPRINT1("        Option           = 0x%x\n", Descriptor->Option);
    DPRINT1("        Type             = %u (%s)\n", Descriptor->Type,
	    PciDebugCmResourceTypeToText(Descriptor->Type));
...
NTAPI VOID PciDebugPrintPartialResource(IN PCM_PARTIAL_RESOURCE_DESCRIPTOR PartialResource)
{
    ...
    DPRINT1("        Type             = %u (%s)\n", PartialResource->Type,
	    PciDebugCmResourceTypeToText(PartialResource->Type));
...
NTAPI VOID PciDebugPrintCmResList(IN PCM_RESOURCE_LIST PartialList)
{
	...
	for (PartialDescriptor = FullDescriptor->PartialResourceList.PartialDescriptors;
	     Count; PartialDescriptor = CmiGetNextPartialDescriptor(PartialDescriptor)) {
	    /* Print each partial */
