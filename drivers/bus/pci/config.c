...
NTAPI UCHAR PciGetAdjustedInterruptLine(IN PPCI_PDO_EXTENSION PdoExtension)
{
	...
	Length = HalGetBusDataByOffset(
	    ...
	    PdoExtension->Slot.u.AsULONG, &PciInterruptLine,
	    FIELD_OFFSET(PCI_COMMON_HEADER, u.type0.InterruptLine), sizeof(UCHAR));
	if (Length)
...
NTAPI NTSTATUS PciQueryForPciBusInterface(IN PPCI_FDO_EXTENSION FdoExtension)
{
    ...
    PciInterface = ExAllocatePoolWithTag(NonPagedPool, sizeof(PCI_BUS_INTERFACE_STANDARD),
					 PCI_POOL_TAG);
    if (!PciInterface)
	...
	IoStackLocation->Parameters.QueryInterface.Size = sizeof(
