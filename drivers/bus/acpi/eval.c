					   ...
					   _In_ ACPI_OBJECT *Reference)
{
    ...
    OutName.Pointer = (PVOID)&Argument->Data[0];
	...
	DPRINT1("AcpiGetName() failed on %p with status 0x%04x\n",
		Reference->Reference.Handle, AcpiStatus);
 ...
 */
static NTSTATUS EvalGetElementSize(_In_ ACPI_OBJECT *Obj, _In_ ULONG Depth,
				   _Out_opt_ PULONG Count, _Out_ PULONG Size)
{
	...
	for (i = 0; i < Obj->Package.Count; i++) {
	    ...
	    Status = EvalGetElementSize(&Obj->Package.Elements[i], Depth + 1, NULL,
					&ElementSize);
 ...
 */
static NTSTATUS EvalConvertEvaluationResults(_Out_ PACPI_METHOD_ARGUMENT Argument,
					     _In_ ULONG Depth, _In_ ACPI_OBJECT *Obj)
{
    PACPI_METHOD_ARGUMENT Ptr;
    NTSTATUS Status;
	...
	for (i = 0; i < Obj->Package.Count; i++) {
	    Status = EvalConvertEvaluationResults(Ptr, Depth + 1,
 ...
 */
static NTSTATUS EvalConvertParameterObjects(_Out_ ACPI_OBJECT *Arg, _In_ ULONG Depth,
					    ...
					    _In_ ULONG Offset)
{
	...
	Arg->Buffer.Pointer = (PVOID)&Argument->Data[0];
	...
	for (i = 0; i < Arg->Package.Count; i++) {
	    Status = EvalConvertParameterObjects(&Arg->Package.Elements[i], Depth + 1,
						 PackageArgument, IoStack, Offset);
	    if (!NT_SUCCESS(Status)) {
 ...
 */
static NTSTATUS EvalCreateParametersList(_In_ PIRP Irp, _In_ PIO_STACK_LOCATION IoStack,
					 _In_ PACPI_EVAL_INPUT_BUFFER EvalInputBuffer,
					 _Out_ ACPI_OBJECT_LIST *ParamList)
{
    ACPI_OBJECT *Arg;

    if (!AcpiVerifyInBuffer(IoStack, RTL_SIZEOF_THROUGH_FIELD(ACPI_EVAL_INPUT_BUFFER,
							      Signature))) {
	...
	ComplexBuffer = Irp->SystemBuffer;
    ...
    default:
	DPRINT1("Unsupported input buffer signature: 0x%x\n",
				      ...
				      _In_ ACPI_BUFFER *ReturnBuffer)
{
    ...
    return AcpiEvaluateObject(DeviceData->AcpiHandle, MethodName, ParamList,
			      ReturnBuffer);
}
 ...
 */
static NTSTATUS EvalCreateOutputArguments(_In_ PIRP Irp, _In_ PIO_STACK_LOCATION IoStack,
					  _In_ ACPI_BUFFER *ReturnBuffer)
{
    ACPI_OBJECT *Obj;
	...
	DPRINT("Buffer too small (%u/%u)\n",
	       IoStack->Parameters.DeviceIoControl.OutputBufferLength, OutputBufSize);
...
NTSTATUS Bus_PDO_EvalMethod(_In_ PPDO_DEVICE_DATA DeviceData, _Inout_ PIRP Irp)
{
    ...
    EvalInputBuffer = Irp->SystemBuffer;

    Status = EvalCreateParametersList(Irp, IoStack, EvalInputBuffer, &ParamList);
    if (!NT_SUCCESS(Status))
	return Status;

    AcpiStatus = EvalEvaluateObject(DeviceData, EvalInputBuffer, &ParamList,
				    &ReturnBuffer);
	...
	DPRINT("Query method '%.4s' failed on %p with status 0x%04x\n",
	       EvalInputBuffer->MethodName, DeviceData->AcpiHandle, AcpiStatus);
