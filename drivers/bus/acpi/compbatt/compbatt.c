...
NTAPI NTSTATUS CompBattOpenClose(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: ENTERING OpenClose\n");
    ...
    /* Return success */
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: Exiting OpenClose\n");
    return STATUS_SUCCESS;
}

NTAPI NTSTATUS CompBattSystemControl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    PCOMPBATT_DEVICE_EXTENSION DeviceExtension = DeviceObject->DeviceExtension;
    NTSTATUS Status;
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: ENTERING System Control\n");
...
NTAPI VOID CompBattRecalculateTag(IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    PCOMPBATT_BATTERY_DATA BatteryData;
    ULONG Tag;
    PLIST_ENTRY ListHead, NextEntry;
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: ENTERING CompBattRecalculateTag\n");
    ...
    /* We're done */
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: EXITING CompBattRecalculateTag\n");
}

NTAPI NTSTATUS CompBattIoctl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    PCOMPBATT_DEVICE_EXTENSION DeviceExtension = DeviceObject->DeviceExtension;
    NTSTATUS Status;
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: ENTERING Ioctl\n");
    ...
    /* Return status */
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: EXITING Ioctl\n");
    return Status;
}
				...
				OUT PULONG Tag)
{
    NTSTATUS Status;
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: ENTERING QueryTag\n");
	...
	*Tag = 0;
    ...
    /* Return status */
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: EXITING QueryTag\n");
    return Status;
}
...
NTAPI NTSTATUS CompBattDisableStatusNotify(IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    ...
    PLIST_ENTRY ListHead, NextEntry;
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: ENTERING DisableStatusNotify\n");
    ...
    /* Done */
    if (CompBattDebug & 0x100)
	DbgPrint("CompBatt: EXITING DisableStatusNotify\n");
    return STATUS_SUCCESS;
}
...
static NTSTATUS BatteryIoctl(IN ULONG IoControlCode, IN PDEVICE_OBJECT DeviceObject,
			     ...
			     IN BOOLEAN InternalDeviceIoControl)
{
	...
	/* Done */
	if (CompBattDebug & 0x100)
	    DbgPrint("CompBatt: EXITING BatteryIoctl\n");
    } else {
	/* Out of memory */
	if (CompBattDebug & 8)
	    DbgPrint("BatteryIoctl: couldn't create Irp\n");
...
NTAPI NTSTATUS CompBattGetBatteryInformation(OUT PBATTERY_INFORMATION BatteryInfo,
					     IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    ...
    PLIST_ENTRY ListHead, NextEntry;
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: ENTERING GetBatteryInformation\n");
	...
	if (BatteryData->Tag) {
		...
		Status = BatteryIoctl(IOCTL_BATTERY_QUERY_INFORMATION,
				      BatteryData->DeviceObject, &InputBuffer,
				      sizeof(InputBuffer),
				      &BatteryData->BatteryInformation,
				      sizeof(BatteryData->BatteryInformation), 0);
	    ...
	    if (BatteryData->BatteryInformation.FullChargedCapacity !=
		BATTERY_UNKNOWN_CAPACITY) {
	...
	if (CompBattDebug & 2)
	    DbgPrint("CompBattGetBatteryInformation: Returning BATTERY_INFORMATION\n"
		     ...
		     "--------  DefaultAlert2 = %x\n--------  CriticalBias = %x\n"
		     "--------  CycleCount = %x\n",
		     BatteryInfo->Capabilities, BatteryInfo->Technology,
		     BatteryInfo->Chemistry, BatteryInfo->DesignedCapacity,
		     BatteryInfo->FullChargedCapacity, BatteryInfo->DefaultAlert1,
		     BatteryInfo->DefaultAlert2, BatteryInfo->CriticalBias,
		     BatteryInfo->CycleCount);

	/* Copy the data into the device extension */
	RtlCopyMemory(&DeviceExtension->BatteryInformation, BatteryInfo,
    ...
    /* We are done */
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: EXITING GetBatteryInformation\n");
    return Status;
}

NTAPI NTSTATUS CompBattGetBatteryGranularity(OUT PBATTERY_REPORTING_SCALE ReportingScale,
					     IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    NTSTATUS Status = STATUS_SUCCESS;
    BATTERY_QUERY_INFORMATION InputBuffer;
    ...
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: ENTERING GetBatteryGranularity\n");
	...
	if (BatteryData->Tag) {
	    ...
	    Status = BatteryIoctl(IOCTL_BATTERY_QUERY_INFORMATION,
				  BatteryData->DeviceObject, &InputBuffer,
				  sizeof(InputBuffer), &BatteryScale,
				  sizeof(BatteryScale), 0);
	    if (!NT_SUCCESS(Status)) {
		/* Fail if the query had a problem */
		break;
	    }
    ...
    /* All done */
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: EXITING GetBatteryGranularity\n");
    return STATUS_SUCCESS;
}

NTAPI NTSTATUS CompBattGetEstimatedTime(OUT PULONG Time,
					IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    UNIMPLEMENTED;
    return STATUS_NOT_IMPLEMENTED;
...
NTAPI NTSTATUS CompBattQueryInformation(IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension,
					IN ULONG Tag,
					...
					OUT PULONG ReturnedLength)
{
    ...
    NTSTATUS Status = STATUS_SUCCESS;
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: ENTERING QueryInformation\n");
    ...
    *ReturnedLength = QueryLength;
    ...
    if (CompBattDebug & 1)
	DbgPrint("CompBatt: EXITING QueryInformation\n");
    return Status;
}
			   ...
			   IN PUNICODE_STRING RegistryPath)
{
    ...
    DriverObject->AddDevice = CompBattAddDevice;
