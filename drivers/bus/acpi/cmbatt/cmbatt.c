...
/* TODO: Power callback */
#if 0
NTAPI VOID CmBattPowerCallBack(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
			       IN ULONG Action, IN ULONG Value)
{
    BOOLEAN Cancelled;
	...
	if (Value == 1) {
	    if (CmBattDebug & 0x10)
		DbgPrint("CmBattPowerCallBack: Calling CmBattWakeDpc after 10 "
			 "seconds.\n");
	    Cancelled = KeSetTimer(&CmBattWakeDpcTimerObject, CmBattWakeDpcDelay,
				   &CmBattWakeDpcObject);
	    if (CmBattDebug & 0x10)
	    ...
	    for (DeviceObject = DeviceExtension->DeviceObject; DeviceObject;
...
NTAPI VOID CmBattWakeDpc(IN PKDPC Dpc, IN PCMBATT_DEVICE_EXTENSION FdoExtension,
			 IN PVOID SystemArgument1, IN PVOID SystemArgument2)
{
    ...
    ULONG ArFlag;
    if (CmBattDebug & 2)
	DbgPrint("CmBattWakeDpc: Entered.\n");

    /* Loop all device objects */
    for (CurrentObject = FdoExtension->DeviceObject; CurrentObject;
...
VOID CmBattNotifyHandler(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
			 IN ULONG NotifyValue)
{
	...
	if ((DeviceExtension->Tag) &&
	    ...
	    DbgPrint("CmBattNotifyHandler: Received battery #%x insertion, but tag was "
		     "not invalid.\n",
	...
	for (DeviceObject = DeviceExtension->FdoDeviceObject->DriverObject->DeviceObject;
	     DeviceObject; DeviceObject = DeviceObject->NextDevice) {
	    /* Is this a battery? */
...
NTAPI VOID CmBattUnload(IN PDRIVER_OBJECT DriverObject)
{
...
NTAPI NTSTATUS CmBattOpenClose(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
...
NTAPI NTSTATUS CmBattIoctl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	case IOCTL_BATTERY_QUERY_BST:
		...
		Status = CmBattGetBstData(DeviceExtension,
					  Irp->SystemBuffer);
		if (NT_SUCCESS(Status))
		    Irp->IoStatus.Information = sizeof(ACPI_BST_DATA);
	    } else {
		/* Buffer size invalid */
		Status = STATUS_INVALID_BUFFER_SIZE;
	    }
...
NTSTATUS CmBattQueryTag(IN PCMBATT_DEVICE_EXTENSION DeviceExtension, OUT PULONG Tag)
{
	...
	DbgPrint("CmBattQueryTag - Tag (%d), Battery %p, Device %d\n", *Tag,
		 DeviceExtension, DeviceExtension->DeviceId);
	...
	if (StaData & ACPI_STA_BATTERY_PRESENT) {
		...
		if (DeviceExtension->TagData++ == 0xFFFFFFFF)
		    NewTag = 1;
...
NTSTATUS CmBattDisableStatusNotify(IN PCMBATT_DEVICE_EXTENSION DeviceExtension)
{
    NTSTATUS Status;
    if (CmBattDebug & 0xA)
	DbgPrint("CmBattDisableStatusNotify\n");
...
NTSTATUS CmBattSetStatusNotify(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
			       IN ULONG BatteryTag,
			       IN PBATTERY_NOTIFY BatteryNotify)
{
	...
	if ((DesignVoltage != BATTERY_UNKNOWN_VOLTAGE) && (DesignVoltage)) {
		...
		NewTripPoint = (TripPoint + 500) / DesignVoltage +
			       ((TripPoint + 500) % DesignVoltage != 0);
	    } else {
	    ...
	    if (CmBattDebug & CMBATT_ACPI_WARNING)
		DbgPrint("CmBattSetStatusNotify: Can't calculate BTP, DesignVoltage = "
			 "0x%08x\n",
			 DesignVoltage);
	}
    } else if (Charging) {
	...
	if (CmBattDebug & CMBATT_GENERIC_WARNING)
	    DbgPrint("CmBattSetStatusNotify: Trip point already crossed (1): TP = %08x, "
		     "remaining capacity = %08x\n",
    ...
    if (CmBattDebug & CMBATT_ACPI_WARNING)
	DbgPrint("CmBattSetStatusNotify: Set to: [%#08x][%#08x][%#08x] Status %x\n",
		 BatteryNotify->PowerState, BatteryNotify->LowCapacity,
		 BatteryNotify->HighCapacity, Status);
    return Status;
}

NTSTATUS CmBattGetBatteryStatus(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
				IN ULONG Tag)
{
    ULONG PsrData = 0;
    NTSTATUS Status;
    ...
    if (CmBattDebug & CMBATT_GENERIC_INFO)
	DbgPrint("CmBattGetBatteryStatus - CmBatt (%p) Tag (%d)\n", DeviceExtension,
		 Tag);
	...
	DbgPrint("************************ ACPI BIOS BUG ********************\n* "
		 "CmBattGetBatteryStatus: Invalid state: _BST method returned 0x%08x for "
		 "Battery State.\n"
    ...
    if (BstState & ACPI_BATT_STAT_CRITICAL)
	DeviceExtension->State |= BATTERY_CRITICAL;
	...
	if (PresentRate > CM_MAX_VALUE) {
		...
		(CmBattDebug & CMBATT_ACPI_WARNING)) {
	...
	DeviceExtension->RemainingCapacity = BATTERY_UNKNOWN_CAPACITY;
	if (CmBattDebug & CMBATT_ACPI_WARNING) {
	    DbgPrint("CmBattGetBatteryStatus - Can't calculate RemainingCapacity and "
		     "Rate \n");
	...
	if ((BstState & ACPI_BATT_STAT_DISCHARG) &&
	    (CmBattDebug & (CMBATT_ACPI_WARNING | CMBATT_GENERIC_WARNING)))
	    DbgPrint("CmBattGetBatteryStatus: battery rate is unknown when battery is "
		     "not charging!\n");
	...
	if (CmBattDebug & CMBATT_GENERIC_WARNING)
	    DbgPrint("CmBattGetBatteryStatus: battery is not charging or discharging, "
		     "but rate = %x\n",
...
NTSTATUS CmBattQueryInformation(IN PCMBATT_DEVICE_EXTENSION FdoExtension,
				IN ULONG Tag,
				...
				IN ULONG BufferLength, OUT PULONG ReturnedLength)
{
	...
	DbgPrint("CmBattQueryInformation - Tag (%d) Device %d, Informationlevel %d\n",
		 Tag, FdoExtension->DeviceId, InfoLevel);
	...
	if ((KeQueryInterruptTime() - 150000000) > (FdoExtension->InterruptTime)) {
		...
		if (RemainingCapacity > 0x123456) {
		    ...
		    if (CmBattDebug & CMBATT_ACPI_WARNING)
			DbgPrint("CmBattQueryInformation: Data Overflow in calculating "
				 "Remaining Capacity.\n");
		} else {
    ...
    *ReturnedLength = QueryLength;
    if (BufferLength < QueryLength)
	Status = STATUS_BUFFER_TOO_SMALL;

    /* Copy the data if there's enough space and it exists */
    if ((NT_SUCCESS(Status)) && (QueryData))
	RtlCopyMemory(Buffer, QueryData, QueryLength);

    /* Return function result */
    return Status;
}

NTSTATUS CmBattQueryStatus(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
			   IN ULONG Tag, IN PBATTERY_STATUS BatteryStatus)
{
    ...
    /* Return status */
    if (CmBattDebug & (CMBATT_GENERIC_INFO))
	DbgPrint("CmBattQueryStatus: Returning [%#08x][%#08x][%#08x][%#08x]\n",
		 BatteryStatus->PowerState, BatteryStatus->Capacity,
		 BatteryStatus->Voltage, BatteryStatus->Rate);
    return Status;
}
			   ...
			   IN PUNICODE_STRING RegistryPath)
{
    ...
    if (CmBattDebug & CMBATT_GENERIC_INFO)
	DbgPrint("CmBatt DriverEntry - Obj (%p) Path \"%wZ\"\n", DriverObject,
		 RegistryPath);
    ...
    DriverObject->AddDevice = CmBattAddDevice;

    /* TODO: Implement PoCreateCallback */
#if 0
    OBJECT_ATTRIBUTES ObjectAttributes;
    UNICODE_STRING CallbackName;
    /* Create a power callback */
    RtlInitUnicodeString(&CallbackName, L"\\Callback\\PowerState");
    InitializeObjectAttributes(&ObjectAttributes, &CallbackName, OBJ_KERNEL_HANDLE, NULL,
			       NULL);
