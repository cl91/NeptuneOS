...
NTAPI NTSTATUS CmBattPowerDispatch(PDEVICE_OBJECT DeviceObject, PIRP Irp);

NTAPI NTSTATUS CmBattPnpDispatch(PDEVICE_OBJECT DeviceObject, PIRP Irp);

NTAPI NTSTATUS CmBattAddDevice(PDRIVER_OBJECT DriverObject, PDEVICE_OBJECT DeviceObject);

NTAPI NTSTATUS CmBattSystemControl(PDEVICE_OBJECT DeviceObject, PIRP Irp);

NTSTATUS CmBattGetBstData(PCMBATT_DEVICE_EXTENSION DeviceExtension,
			  PACPI_BST_DATA BstData);

NTSTATUS CmBattGetPsrData(PDEVICE_OBJECT DeviceObject, PULONG PsrData);

NTSTATUS CmBattGetStaData(PDEVICE_OBJECT DeviceObject, PULONG StaData);

NTSTATUS CmBattGetBifData(PCMBATT_DEVICE_EXTENSION DeviceExtension,
			  PACPI_BIF_DATA BifData);

NTSTATUS CmBattSetTripPpoint(PCMBATT_DEVICE_EXTENSION DeviceExtension, ULONG AlarmValue);

VOID CmBattNotifyHandler(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
			 IN ULONG NotifyValue);

NTSTATUS CmBattWmiDeRegistration(PCMBATT_DEVICE_EXTENSION DeviceExtension);

NTSTATUS CmBattWmiRegistration(PCMBATT_DEVICE_EXTENSION DeviceExtension);

NTSTATUS CmBattGetUniqueId(PDEVICE_OBJECT DeviceObject, PULONG UniqueId);

NTSTATUS CmBattQueryInformation(IN PCMBATT_DEVICE_EXTENSION FdoExtension, IN ULONG Tag,
				IN BATTERY_QUERY_INFORMATION_LEVEL InfoLevel,
				IN OPTIONAL LONG AtRate, IN PVOID Buffer,
				IN ULONG BufferLength, OUT PULONG ReturnedLength);

NTSTATUS CmBattQueryStatus(IN PCMBATT_DEVICE_EXTENSION DeviceExtension, IN ULONG Tag,
			   IN PBATTERY_STATUS BatteryStatus);

NTSTATUS CmBattSetStatusNotify(IN PCMBATT_DEVICE_EXTENSION DeviceExtension,
			       IN ULONG BatteryTag, IN PBATTERY_NOTIFY BatteryNotify);
