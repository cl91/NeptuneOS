 ...
 */
NTAPI NTSTATUS i8042SynchWritePortKbd(IN PVOID Context,
				      IN UCHAR Value,
			   ...
			   IN PIRP Irp)
{
	...
	i8042StartPacket(PortDeviceExtension,
			 ...
			 PortDeviceExtension->PacketBuffer, 2, Irp);
	break;
    }
    default:
    {
				     ...
				     IN PVOID Context)
{
	...
	if (DeviceExtension->PowerInterfaceName.MaximumLength == 0) {
	    ...
	    Status = IoRegisterDeviceInterface(DeviceExtension->Common.Pdo,
	    ...
	    Status = IoSetDeviceInterfaceState(&DeviceExtension->PowerInterfaceName, FALSE);
	    if (!NT_SUCCESS(Status)) {
	...
	Status = IoSetDeviceInterfaceState(&DeviceExtension->PowerInterfaceName, TRUE);
	if (!NT_SUCCESS(Status)) {
	    /* Ignore the key press... */
	    WARN_(I8042PRT,
...
static BOOLEAN HandlePowerKeys(IN PI8042_KEYBOARD_EXTENSION DeviceExtension)
{
    ...
    IoQueueWorkItem(DeviceExtension->PowerWorkItem, &i8042PowerWorkItem,
		    DelayedWorkQueue, DeviceExtension);
    return TRUE;
}

static NTAPI VOID i8042KbdDpcRoutine(IN PKDPC Dpc,
				     ...
				     IN PIRP Irp)
{
	...
	if (Stack->Parameters.DeviceIoControl.OutputBufferLength != sizeof(ULONG))
	    ...
	    WaitingIrp = InterlockedCompareExchangePointer((PVOID) &
 ...
 */
static BOOLEAN i8042KbdCallIsrHook(IN PI8042_KEYBOARD_EXTENSION DeviceExtension,
				   IN UCHAR Status,
				   IN UCHAR Input,
