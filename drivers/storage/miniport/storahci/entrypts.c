			   ...
			   IN PUNICODE_STRING RegistryPath)
{
    ...
    hwInitializationData.FeatureSupport |= STOR_FEATURE_ATA_PASS_THROUGH; // indicating
									  // this miniport
									  // driver
									  // supports ATA
									  // PASS-THROUGH(16)
	...
	STOR_FEATURE_FULL_PNP_DEVICE_CAPABILITIES; // indicating this miniport driver
						   // supplies values for all PnP Device
						   // Capability fields.
    hwInitializationData.FeatureSupport |= STOR_FEATURE_DUMP_POINTERS; // indicating this
								       // miniport support
								       // the dump
								       // pointers SRBs
    hwInitializationData.FeatureSupport |= STOR_FEATURE_DUMP_RESUME_CAPABLE; // indicating
									     // this
									     // miniport
									     // driver
									     // supports
									     ...
									     // for dump
									     // stack.
    hwInitializationData.FeatureSupport |= STOR_FEATURE_DEVICE_NAME_NO_SUFFIX; // indicating
									       // the
									       // miniport
									       // driver
									       // prefers
									       ...
									       // Device"
    hwInitializationData.FeatureSupport |=
	STOR_FEATURE_DEVICE_DESCRIPTOR_FROM_ATA_INFO_VPD; // indicating that port driver
							  // forms
							  // STORAGE_DEVICE_DESCRIPTOR
							  // from ATA Information VPD page
							  // rather than INQUIRY data
    hwInitializationData.FeatureSupport |= STOR_FEATURE_EXTRA_IO_INFORMATION; // Indicating
									      // that
									      // miniport
									      // driver
									      // wants
									      ...
									      // available
    hwInitializationData.FeatureSupport |=
	STOR_FEATURE_ADAPTER_NOT_REQUIRE_IO_PORT; // Indicating that miniport driver
						  // doesn't require IO Port resource for
						  // its adapter.
    hwInitializationData.FeatureSupport |= STOR_FEATURE_DUMP_INFO; // Indicating that the
								   // miniport driver
								   // supports the dump
								   // info SRBs.
    ...
    // NOTE: Command Table (1st field in AHCI_SRB_EXTENSION structure) must align to 128
    // bytes as physical limitation. StorPort does not have interface allowing miniport
    // requiring this. Adding 128 in SrbExtensionSize so that we can use the part starting
    // from right alignment.
    hwInitializationData.SrbExtensionSize = sizeof(AHCI_SRB_EXTENSION) +
					    128; // SrbExtension contains
						 // AHCI_SRB_EXTENSION
...
/*++
    ...
    1. More than one port share interrupt (line based, single message MSI or there are
       ports share same interrupt message using MSI). StorAHCI looks into IS register to
       understand which port triggers the interrupt. Synchronization will acquire interrupt
       spinlock for the whole adapter.
...
Note:
    AdapterExtension->HighestPort should have been assigned value before calling this
    function.
...
Return Values:
    None
--*/
VOID GetInterruptMode(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    ULONG status;
	...
	ghc.AsUlong = StorPortReadRegisterUlong(
	    AdapterExtension, &AdapterExtension->ABAR_Address->GHC.AsUlong);
...
/*
    ...
    In AHCI, the controller structures are both the command header list and the received
   FIS buffer. The mechanism for requesting all of this memory is
   StorPortGetUncachedExtension. This function returns page-aligned address. NOTE!
   StorPortGetUncachedExtension can only be called while in FindAdapter routine. NOTE! In
   order to perform crashdump/hibernate the UncachedExtensionSize should not be larger
   than 30K.
    ...
    -   The FIS Base Address: must be 256 aligned, and the FIS Receive buffer is sizeof
        (AHCI_RECEIVED_FIS), 256 bytes in length. The Command Table: must be 128 aligned,
	and is sizeof(AHCI_COMMAND_TABLE), 1280 bytes in length thanks to some padding in
	the AHCI_COMMAND_TABLE structure. Command Table is per request, resides in
        AHCI_SRB_EXTENSION for each request.

	Size of nonCachedExtensionSize for every channel should round up to KiloBytes, so
   that the pointer for next channel is starting at 1K boundary.

	The Command Header is variable and must be padded so the Received FIS is on a 256
				    ...
				    _In_range_(1, AHCI_MAX_PORT_COUNT) ULONG PortCount)
{
    ...
    ULONG executionHistorySize = sizeof(EXECUTION_HISTORY) *
				 MAX_EXECUTION_HISTORY_ENTRY_COUNT;
    ...
    // Allocate Identify Data buffer, CommandList, Receive FIS, SRB Extension for Local
    // SRB, INQUIRY buffer paddedNCS is (CAP.NCS+1) rounds up to the next multiple of 8.
    ...
    // size per Port
    nonCachedExtensionSize = sizeof(AHCI_COMMAND_HEADER) * paddedNCS + // align to 1K,
								       // padded to
								       // multiple of 256
								       // bytes.
								       // (nocachedextension
			     ...
			     paddedSrbExtensionSize * 2 + // align to 128 bytes.
							  // Local.SrbExtension and
							  // SenseSrbExtension
			     sizeof(IDENTIFY_DEVICE_DATA) + // 512 bytes
			     ATA_BLOCK_SIZE + // ReadLogExtPageData --- 512 bytes
			     INQUIRYDATABUFFERSIZE; // Inquiry Data

    // round up to KiloBytes if it's not dump mode. this makes sure that
    // NonCachedExtension for next port can align to 1K.
    if (!dumpMode) {
	nonCachedExtensionSize = ((nonCachedExtensionSize - 1) / alignment + 1) *
				 alignment;

	AdapterExtension->NonCachedExtension = StorPortGetUncachedExtension(
	    AdapterExtension, ConfigInfo, nonCachedExtensionSize * PortCount);
    } else {
	// dump mode, address returned from StorPortGetUncachedExtension() is not
	// guaranteed align with 1K. adding 1K into length so that we can start from 1K
	// alignment safely.
	//       NOTE: StorPortAllocatePool is not supported in dump stack, so we allocate
	//       ChannelExtension from UnCachedExtension as work around.
	nonCachedExtensionSize += alignment;

	AdapterExtension->NonCachedExtension = StorPortGetUncachedExtension(
	    AdapterExtension, ConfigInfo,
    ...
    AhciZeroMemory((PCHAR)AdapterExtension->NonCachedExtension,
		   nonCachedExtensionSize * PortCount);

    // 2.2 allocate resources for Ports which need AHCI_CHANNEL_EXTENSION for each of
    // them.
    if (!dumpMode) {
	ULONG status = STOR_STATUS_SUCCESS;
	// allocate pool and zero the content
	status = StorPortAllocatePool(AdapterExtension, channelExtensionSize * PortCount,
				      AHCI_POOL_TAG, (PVOID *)&portsChannelExtension);
	...
	status = StorPortAllocatePool(AdapterExtension,
				      ...
				      AHCI_POOL_TAG, (PVOID *)&portsExecutionHistory);
	...
	portsChannelExtension = (PCHAR)AdapterExtension->NonCachedExtension +
				(nonCachedExtensionSize * PortCount);
    }
	...
	if ((AdapterExtension->PortImplemented & (1 << i)) != 0) {
	    PCHAR portsUncachedExtension = (PCHAR)AdapterExtension->NonCachedExtension +
					   nonCachedExtensionSize * j;

	    // this port is implemented, allocate and initialize extension for the port
	    AdapterExtension->PortExtension[i] =
		(PAHCI_CHANNEL_EXTENSION)((PCHAR)portsChannelExtension +
					  channelExtensionSize * j);
	    ...
	    if (!dumpMode) {
		AdapterExtension->PortExtension[i]->CommandList =
		    (PAHCI_COMMAND_HEADER)(portsUncachedExtension);
		if (portsExecutionHistory != NULL) {
		    AdapterExtension->PortExtension[i]->ExecutionHistory =
			(PEXECUTION_HISTORY)((PCHAR)portsExecutionHistory +
					     executionHistorySize * j);
		}
	    } else {
		...
		if (left > 0) {
		    AdapterExtension->PortExtension[i]->CommandList =
			(PAHCI_COMMAND_HEADER)(portsUncachedExtension + alignment - left);
		} else {
		    AdapterExtension->PortExtension[i]->CommandList =
			(PAHCI_COMMAND_HEADER)(portsUncachedExtension);
		}
...
/*++
    ...
    LEVEL.
    It assumes: Resources for the AHCI controller exist in a single Memory Mapped
    range. No Channels have been initialized or started; no IO is outstanding.
    Called by: Storport

It performs:
    (overview)
    1. Check the working mode and retrieve adapter vendor/model info.
    2. Initialize ChannelExtension with Identifying memory mapped IO resources, Version
and CAP.
    3. Enabling the AHCI interface as per AHCI 1.1 section 10.1.2 (part one)
    4. Initializing the IDE_CONTROLLER_CONFIGURATION structure with data from the AHCI
Interface
    ...
    3.1 Turn on AE, reset the controller if AE is already set
        AHCI 1.1 Section 10.1.2 - 1. "Indicate that system software is AHCI aware by setting
	GHC.AE to '1'."
    3.2 Determine which ports are implemented by the HBA
        AHCI 1.1 Section 10.1.2 - 2. "Determine which ports are implemented by the HBA,
	by reading the PI register. This bitmap value will aid software in determining
	how many ports are available and which port registers need to be initialized."
    ...
    4.1 Turn on IE, pending interrupts will be cleared when port starts. This has to be
        done after 3.2 because we need to know the number of channels before we check
	each PxIS. Verify that none of the PxIS registers are loaded, but take no action
	Note: Due to the multi-tiered nature of the AHCI HBA's interrupt architecture,
	system software must always ensure that the PxIS (clear this first) and IS.IPS
	(clear this second) registers are cleared to '0' before programming the PxIE and
	GHC.IE registers. This will prevent any residual bits set in these registers from
	causing an interrupt to be asserted. However, the interrupt handler hasn't been
	hooked up by StorPort yet, so no interrupts will be handled by software until that
	happens.
    ...
    4.4 Initialize ports and start them. Dump stack will do this when receiving the INQUIRY
        command
...
Affected Variables/Registers:
    ...
    IS and all fields in the HBA's register memory space except PxFB/PxFBU/PxCLB/PxCLBU
    that are not HwInit
...
Note:
    HwStorFindAdapter must set the MaximumTransferLength and NumberOfPhysicalBreaks fields
    in the ConfigInfo structure.

    Other than these fields, the PORT_CONFIGURATION_INFORMATION (Storport) structure will
			...
			_In_ PBOOLEAN Reserved3)
{
	...
	if (dumpContext != NULL) {
	    ...
	    StorPortMarkDumpMemory(AdapterExtension, dumpContext,
				   FIELD_OFFSET(AHCI_DUMP_CONTEXT, Ports) +
				       (dumpContext->PortCount *
					sizeof(AHCI_DUMP_PORT_CONTEXT)),
				   0);
	} else {
	    NT_ASSERT(FALSE);
    ...
    if (!dumpMode) {
	//
	// Enumeration may be via either PCI or ACPI. Bus type is set by
	// storport/miniport. Check if we are enumerating via PCI, and retrieve the
	...
	if (ConfigInfo->AdapterInterfaceType == PCIBus) {
	    ...
	    pcicfgLen = StorPortGetBusData(adapterExtension, PCIConfiguration,
					   ConfigInfo->SystemIoBusNumber,
					   (ULONG)ConfigInfo->SlotNumber,
					   (PVOID)pcicfgBuffer, (ULONG)0x30);
	    if (pcicfgLen == 0x30) {
		...
		// on PCI bus, AHCI Base Address is BAR5. Bits 0-3 defined for other
		// usages, not part of address value.
		adapterExtension->AhciBaseAddress =
		    pciConfigData->Header.Type0.BaseAddresses[5] & (0xFFFFFFF0);
	    } else {
		NT_ASSERT(FALSE);
		return SP_RETURN_ERROR;
	    }
    ...
    // 2.1.1 If abar is still NULL after all of that, malformed resources.  We aren't
    ...
    adapterExtension->Version.AsUlong = StorPortReadRegisterUlong(adapterExtension,
								  &abar->VS.AsUlong);
    adapterExtension->CAP.AsUlong = StorPortReadRegisterUlong(adapterExtension,
							      &abar->CAP.AsUlong);
    adapterExtension->CAP2.AsUlong = StorPortReadRegisterUlong(adapterExtension,
							       &abar->CAP2.AsUlong);
    ...
    } // AE is 0. Either through power up or reset we are now pretty sure the controller
      // is in 5.2.2.1    H:Init
    ghc.AsUlong = 0;
    ghc.AE = 1;
	...
	for (piMask = (ULONG)(1 << (AHCI_MAX_PORT_COUNT - 1)); piMask != 0;
	     piMask = (ULONG)(piMask >> 1)) {
	    numberOfHighestPort--;
    ...
    //
    // AHCI specification requires that implemented port count (number of bit set in PI
    // register) is less than or equal to CAP.NP + 1
    //
    // Currently StorAHCI does not utilize CAP.NP. It remains useful to identify platform
    // BIOSes which may violate the specification.
    //
    ...
    ConfigInfo->NumberOfPhysicalBreaks = 0x21; // Since "NumberOfPhysicalBreaks" has been
					       // used in storage stack as the count of
					       // entries, use value of physical breaks
					       // plus one.
    ...
    ConfigInfo->MaximumNumberOfLogicalUnits = 1; /*AHCI_MAX_LUN*/
          // NOTE: only supports 1 for now. there is a legacy ATAPI device that may have 2
	  // luns.
    // set driver to run in full duplex mode
    ConfigInfo->SynchronizationModel = StorSynchronizeFullDuplex;
    ConfigInfo->BusResetHoldTime = 0; // StorAHCI wait RESET to be completed by itself, no
				      // need for port driver to wait.
	...
	StorPortSetPowerSettingNotificationGuids(AdapterExtension, 2,
						 powerSettingChangeGuids);
    }
	...
	ConfigInfo->SrbExtensionSize += (ULONG)((ConfigInfo->NumberOfPhysicalBreaks + 2) *
						4);
    }

    // 4.3 allocate resources, implemented port information needs to be ready before
    ...
    //
    // 2. This initialization has to be done before trying to getting ports into running
    // state (i.e. by calling AhciAdapterRunAllPorts() later below in this function). The
    // port start process utilizes StartPortTimer (Note: In dump mode the port start process
    // will be using polling).
    //
    // 3. Currently WorkerTimer is used only for PartialToSlumber during interrupt
    // servicing (not applicable in dump mode)
    //
    if (!dumpMode) {
	for (i = 0; i <= adapterExtension->HighestPort; i++) {
	    if (adapterExtension->PortExtension[i] != NULL) {
		if (adapterExtension->PortExtension[i]->StartPortTimer == NULL) {
		    storStatus = StorPortInitializeTimer(
			AdapterExtension,
			&adapterExtension->PortExtension[i]->StartPortTimer);
		...
		if (adapterExtension->PortExtension[i]->WorkerTimer == NULL) {
		    storStatus = StorPortInitializeTimer(
			AdapterExtension,
			&adapterExtension->PortExtension[i]->WorkerTimer);
    ...
    // reset "StoppedState". NOTE: This field should not be referenced anymore in this
    // function after following line.
    ...
    for (i = 0; i <= adapterExtension->HighestPort; i++) {
	if (adapterExtension->PortExtension[i] != NULL) {
	    // in case of PortInitialize fails, ChannelExtension->StateFlags.Initialized
	    // will remain as 'FALSE'. there will be not attempt to start the Port.
    ...
    if (dumpMode) {
	//
	// In dump mode copy registry flags and telemetry configuration from the dump
	// context. If telemetry extends to more than one device the logic should cover
	// migration of per device settings, initially we are limited to a single boot
	// device
	//
	...
	for (i = 0; i < dumpContext->PortCount; i++) {
		...
		adapterExtension->PortExtension[portNumber]->RegistryFlags =
		    dumpContext->Ports[i].PortRegistryFlags;

		//
		// If the table in dump context is empty (not filled) - keep static data
		// intact
		//
		/*if (dumpContext->PublicGPLogTableAddresses[0]) {
		    for(Index=0;
			Index < sizeof(AhciPublicGPLogTableAddresses) /
		sizeof(AhciPublicGPLogTableAddresses[0] ); Index++) {
			AhciPublicGPLogTableAddresses[Index] =
		dumpContext->PublicGPLogTableAddresses[Index];
		    }

		    AhciGPLogPageIntoPrivate = dumpContext->PrivateGPLogPageAddress;
		}*/

		StorPortMoveMemory((PVOID)&adapterExtension->PortExtension[portNumber]
				       ->DeviceExtension->HybridInfo,
...
BOOLEAN AhciHwInitialize(_In_ PVOID AdapterExtension)
{
    ULONG status = STOR_STATUS_SUCCESS;
    ...
    //
    // Get adapter current interrupt mode. Interrupt Messages can be accessed after
    // HwFindAdapter.
    //
	...
	if (adapterExtension->StateFlags.InterruptMessagePerPort == 1) {
		...
		if (adapterExtension->HighestPort > 0) {
		    ...
		    perfConfigData.LastRedirectionMessageNumber =
			adapterExtension->HighestPort;
		} else {
		    perfConfigData.FirstRedirectionMessageNumber = 0;
	...
	//
	// Support concurrent channel so that ports can have StartIo routine running
	// concurrently.
	//
	perfConfigData.Flags |= STOR_PERF_CONCURRENT_CHANNELS;
	perfConfigData.ConcurrentChannels = NumberOfSetBits(
...
BOOLEAN AhciHwPassiveInitialize(_In_ PVOID AdapterExtension)
{
	...
	if (adapterExtension->PortExtension[i] != NULL) {
	    StorPortInitializeDpc(AdapterExtension,
				  &adapterExtension->PortExtension[i]->CompletionDpc,
				  AhciPortSrbCompletionDpcRoutine);
	    StorPortInitializeDpc(AdapterExtension,
				  &adapterExtension->PortExtension[i]->BusChangeDpc,
    ...
    //
    status = StorPortGetD3ColdSupport(AdapterExtension, NULL, &d3ColdSupported);
    if (status == STOR_STATUS_SUCCESS) {
    ...
    // 3. allocate STOR_POFX_DEVICE data structure for adapter, initialize the structure
    // and register for runtime power management.
    status = StorPortAllocatePool(AdapterExtension, bufferSize, AHCI_POOL_TAG,
    ...
    status = StorPortInitializePoFxPower(AdapterExtension, NULL,
					 (PSTOR_POFX_DEVICE)adapterExtension->PoFxDevice,
					 &enableD3Cold);
...
Routine Description:
    ...
    This function do preparation work before StorAHCI can process any device enumeration
commands
...
--*/
VOID AhciAdapterPrepareForBusReScan(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    // 1. If a port/device is powered off, power it on.
    if (AdapterExtension->StateFlags.SupportsAcpiDSM == 1) {
	// ACPI method is implemented, invoke ACPI method to power on all ports and
	// devices
...
--*/
__inline VOID AdapterStop(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    ULONG i;
...
*/
VOID AhciAdapterStop(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    // stop all ports.
    ...
    adapterStopInProcess = InterlockedBitTestAndSet(
	(volatile long *)&AdapterExtension->PoFxPendingWork, 0); // AdapterStop is at bit
								 // 0
	...
	adapterStopPending = InterlockedBitTestAndReset(
	    (volatile long *)&AdapterExtension->PoFxPendingWork,
	    0); // AdapterStop is at bit 0
...
*/
VOID AhciAdapterRemoval(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    ULONG i;
    ...
    // release portsChannelExtension. the whole buffer is sliced for Ports, only need to
    // free the first ChannelExtension.
    for (i = 0; i <= AdapterExtension->HighestPort; i++) {
	if (AdapterExtension->PortExtension[i] != NULL) {
	    ...
	    if (AdapterExtension->PortExtension[i]->StartPortTimer != NULL) {
		StorPortFreeTimer(AdapterExtension,
				  AdapterExtension->PortExtension[i]->StartPortTimer);
		...
		StorPortFreeTimer(AdapterExtension,
				  AdapterExtension->PortExtension[i]->BusChangeTimer);
		AdapterExtension->PortExtension[i]->BusChangeTimer = NULL;
	    }

	    if (AdapterExtension->PortExtension[i]->DeviceInitCommands.CommandTaskFile !=
		NULL) {
		StorPortFreePool(AdapterExtension,
				 (PVOID)AdapterExtension->PortExtension[i]
				     ->DeviceInitCommands.CommandTaskFile);
		AdapterExtension->PortExtension[i]->DeviceInitCommands.CommandTaskFile =
		    NULL;
	    }

	    if (AdapterExtension->PortExtension[i]->PoFxDevice != NULL) {
		StorPortFreePool(AdapterExtension,
				 AdapterExtension->PortExtension[i]->PoFxDevice);
...
AhciHwAdapterControl(_In_ PVOID AdapterExtension,
		     _In_ SCSI_ADAPTER_CONTROL_TYPE ControlType, _In_ PVOID Parameters)
{
    PAHCI_ADAPTER_EXTENSION adapterExtension = (PAHCI_ADAPTER_EXTENSION)AdapterExtension;
    ...
    // StorAHCI supports PnP Srb and ScsiAdapterPower thus does nothing for
    // ScsiStopAdapter. NOTE: ScsiStopAdapter is called after PnP call -StorRemoveDevice
    // or StorSurpriseRemoval, thus resource release should be done here.
    ...
    // StorAHCI supports PnP Srb and ScsiAdapterPower thus does nothing for
    // ScsiRestartAdapter. return value is ScsiAdapterControlSuccess.
    case ScsiRestartAdapter:
	break;

    // ScsiPowerSettingNotification is called when Storport receives notification from
    // Power Manager when power setting is changed.
    case ScsiPowerSettingNotification:
	AhciAdapterPowerSettingNotification(adapterExtension,
					    (PSTOR_POWER_SETTING_INFO)Parameters);
	break;
	...
	if (AdapterPoFxEnabled(adapterExtension)) {
		...
		if (adapterStopPending == 1) {
		    ...
		    StorPortAcquireSpinLock(AdapterExtension, StartIoLock, NULL,
					    &lockhandle);
		    AdapterStop(adapterExtension);
	...
	if (powerHints->Size >= sizeof(STOR_SYSTEM_POWER_HINTS)) {
	    StorPortDebugPrint(3,
			       "StorAHCI - LPM: Adapter SystemIoBusNumber:%d - System "
			       "Power Hint - State: %u - Latency: %u ms\n",
			       adapterExtension->SystemIoBusNumber,
		...
		StorPortEtwEvent2(AdapterExtension, NULL, AhciEtwEventSystemPowerHint,
				  L"Sys Power Hint", STORPORT_ETW_EVENT_KEYWORD_POWER,
				  StorportEtwLevelInformational,
				  StorportEtwEventOpcodeInfo, NULL, L"State",
				  powerHints->SystemPower, L"Latency (ms)",
	...
	//
	// For AHCI, all the cleanup that needs to happen for adapter surprise removals is
	// done in the several ScsiUnitSurpriseRemovals that will be sent down for each
	// LUN. There is no extra work needed to handle adapter surprise removals.
	//
...
BOOLEAN AhciHwResetBus(_In_ PVOID AdapterExtension, _In_ ULONG PathId)
{
	...
	adapterExtension->PortExtension[PathId]
	    ->DeviceExtension[0]
	    .IoRecord.PortDriverResetCount++;

	AhciTelemetryLogResetErrorRecovery(
	    adapterExtension->PortExtension[PathId],
	    (PSTOR_ADDRESS) &
		(adapterExtension->PortExtension[PathId]->DeviceExtension->DeviceAddress),
	    AhciTelemetryEventIdResetBus, "AhciHwResetBus", 0, "PortResetStatus", status);
    }
...
BOOLEAN AhciHwBuildIo(_In_ PVOID AdapterExtension, _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    AhciZeroMemory((PCHAR)(&(srbExtension->AtaFunction)),
		   (sizeof(AHCI_SRB_EXTENSION) -
		    FIELD_OFFSET(AHCI_SRB_EXTENSION, AtaFunction)));
    ...
    //
    // Prepare the Device Command before StartIo routine for SRB_FUNCTION_IO_CONTROL and
    // SRB_FUNCTION_EXECUTE_SCSI requests. This can reduce the lock contention between
    // StartIo and Interrupt locks.
    //
    if ((function == SRB_FUNCTION_EXECUTE_SCSI) ||
	((function == SRB_FUNCTION_IO_CONTROL) &&
	 ((srbFlags & SRB_IOCTL_FLAGS_ADAPTER_REQUEST) == 0))) {
	if (!IsPortValid(adapterExtension, pathId)) {
	    Srb->SrbStatus = SRB_STATUS_NO_DEVICE;
	    StorPortEtwChannelEvent2(adapterExtension, NULL, StorportEtwEventOperational,
				     AhciEtwEventBuildIO, L"No device",
				     STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
				     StorportEtwLevelError, StorportEtwEventOpcodeInfo,
				     Srb, L"function", function, L"srbFlags", srbFlags);
	    goto exit;
	}
...
BOOLEAN AhciHwStartIo(_In_ PVOID AdapterExtension, _In_ PSCSI_REQUEST_BLOCK Srb)
{
	...
	if ((pnpData->SrbPnPFlags & SRB_PNP_FLAGS_ADAPTER_REQUEST) != 0) {
	    ...
	    if ((pnpData->PnPAction == StorRemoveDevice) ||
		(pnpData->PnPAction == StorSurpriseRemoval)) {
		// the adapter is going to be removed, mark the state and release all
		// resources allocated later in AdapterControl - ScsiStopAdapter.
	    ...
	    StorPortEtwChannelEvent2(adapterExtension, NULL,
				     (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
					 StorportEtwEventDiagnostic :
					 StorportEtwEventOperational,
				     AhciEtwEventStartIO, L"Processing PNP",
				     STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
				     (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
					 StorportEtwLevelInformational :
					 StorportEtwLevelError,
				     StorportEtwEventOpcodeInfo, Srb, L"PnPAction",
				     pnpData->PnPAction, L"SrbStatus", Srb->SrbStatus);
	...
	if ((powerData->SrbPowerFlags & SRB_POWER_FLAGS_ADAPTER_REQUEST) != 0) {
	    ...
	    // StorAHCI supports ScsiAdapterPower, thus Storport should not send adapter
	    // power Srb to this miniport. Complete the request as SUCCESS anyway.
	    NT_ASSERT(FALSE);
	    Srb->SrbStatus = SRB_STATUS_SUCCESS;
	    StorPortNotification(RequestComplete, AdapterExtension, Srb);
	}
	...
	StorPortEtwChannelEvent8(adapterExtension, NULL, StorportEtwEventOperational,
				 AhciEtwEventStartIO, L"Port has no device",
				 STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
				 StorportEtwLevelError, StorportEtwEventOpcodeInfo, Srb,
				 L"AdapterNumber", adapterExtension->AdapterNumber,
				 L"SystemIoBusNumber",
    ...
    PSTOR_ADDRESS storAddress = (PSTOR_ADDRESS) &
				(channelExtension->DeviceExtension->DeviceAddress);

    switch (function) {
    case SRB_FUNCTION_RESET_BUS: // this one may come from class driver, not port driver.
				 // same as AhciHwResetBus
    case SRB_FUNCTION_RESET_DEVICE:
    case SRB_FUNCTION_RESET_LOGICAL_UNIT: {
	// these reset requests target to Port
	AhciInterruptSpinlockAcquire(adapterExtension, pathId, &lockhandle);
	Srb->SrbStatus = AhciPortReset(adapterExtension->PortExtension[pathId], TRUE) ?
			     SRB_STATUS_SUCCESS :
			     SRB_STATUS_ERROR;
	StorPortNotification(RequestComplete, AdapterExtension, Srb);
	...
	AhciTelemetryLogResetErrorRecovery(
	    ...
	    (PSTOR_ADDRESS) &
		(adapterExtension->PortExtension[pathId]->DeviceExtension->DeviceAddress),
	    AhciTelemetryEventIdResetDeviceRequest, "AhciHwStartIo RESET Request", 0,
	    "PortResetStatus", Srb->SrbStatus);
	break;
    }
	...
	if (((pnpData->SrbPnPFlags & SRB_PNP_FLAGS_ADAPTER_REQUEST) == 0) &&
	    ...
	    storCapabilities->Removable = (portProperties &
					   PORT_PROPERTIES_EXTERNAL_PORT) ?
					      1 :
					      0;
	...
	StorPortEtwChannelEvent2(adapterExtension, storAddress,
				 (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
				     StorportEtwEventDiagnostic :
				     StorportEtwEventOperational,
				 AhciEtwEventStartIO, L"Completed PnP request",
				 STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
				 (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
				     StorportEtwLevelInformational :
				     StorportEtwLevelError,
				 StorportEtwEventOpcodeInfo, Srb, L"SrbPnPFlags",
				 pnpData->SrbPnPFlags, L"PnPAction", pnpData->PnPAction);
	...
	// The miniport driver must hold on to the shutdown request until no data remains
	// in the HBA's internal cache for the target logical unit and, then, complete the
	// shutdown request.
	PATA_DEVICE_PARAMETERS deviceParameters =
	    &adapterExtension->PortExtension[pathId]->DeviceExtension->DeviceParameters;
	BOOLEAN sendStandby = (IsDumpHiberMode(adapterExtension) ||
			       IsDumpCrashMode(adapterExtension));

	deviceParameters->StateFlags.SystemPoweringDown = TRUE;

	USHORT *eventDescription;
	if (adapterExtension->PortExtension[pathId]
		->DeviceExtension->SupportedCommands.SetDateAndTime == 0x1) {
	    IssueSetDateAndTimeCommand(adapterExtension->PortExtension[pathId], Srb,
				       sendStandby);
	    processIO = TRUE;
	...
	StorPortEtwEvent2(adapterExtension, storAddress, AhciEtwEventStartIO,
			  eventDescription, STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
			  StorportEtwLevelInformational, StorportEtwEventOpcodeInfo, Srb,
			  NULL, 0, NULL, 0);
	...
	PMINIPORT_DUMP_POINTERS dumpPointers = (PMINIPORT_DUMP_POINTERS)SrbGetDataBuffer(
	    Srb);
	...
	if ((dumpPointers == NULL) ||
	    (SrbGetDataTransferLength(Srb) <
	     RTL_SIZEOF_THROUGH_FIELD(MINIPORT_DUMP_POINTERS, MiniportPrivateDumpData))) {
	    Srb->SrbStatus = SRB_STATUS_INVALID_REQUEST;
	    ...
	    // allocate pool and zero the content
	    status = StorPortAllocatePool(AdapterExtension, size, AHCI_POOL_TAG,
					  (PVOID *)&dumpContext);
		...
		dumpContext->Ports[0].PortNumber =
		    adapterExtension->PortExtension[pathId]->PortNumber;
		dumpContext->Ports[0].PortRegistryFlags =
		    adapterExtension->PortExtension[pathId]->RegistryFlags;

		//
		// Preserve Hybrid Disk Information.
		//
		if (IsDeviceHybridInfoSupported(
			adapterExtension->PortExtension[pathId])) {
		    StorPortMoveMemory((PVOID)&dumpContext->Ports[0].HybridInfo,
				       (PVOID)&adapterExtension->PortExtension[pathId]
					   ->DeviceExtension->HybridInfo,
				       sizeof(GP_LOG_HYBRID_INFORMATION_HEADER));
		}
		...
		// If private address is not set we will use T13 defined one . Consider
		// always trying default one and resort to registry if it is not supported
		//
		if (dumpContext->PrivateGPLogPageAddress == 0) {
		    dumpContext->PrivateGPLogPageAddress =
	...
	StorPortEtwChannelEvent2(adapterExtension, storAddress,
				 (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
				     StorportEtwEventDiagnostic :
				     StorportEtwEventOperational,
				 AhciEtwEventStartIO, eventDescription,
				 STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
				 (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
				     StorportEtwLevelInformational :
				     StorportEtwLevelError,
				 StorportEtwEventOpcodeInfo, Srb, L"SrbStatus",
				 Srb->SrbStatus, L"TransferLen",
				 SrbGetDataTransferLength(Srb));
	...
	if ((dumpPointers == NULL) ||
	    ...
	    Srb->SrbStatus = (status == STOR_STATUS_SUCCESS) ? SRB_STATUS_SUCCESS :
							       SRB_STATUS_ERROR;
	}
	...
	StorPortEtwChannelEvent2(adapterExtension, storAddress,
				 (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
				     StorportEtwEventDiagnostic :
				     StorportEtwEventOperational,
				 AhciEtwEventStartIO, L"Completed free dump pointers",
				 STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
				 (Srb->SrbStatus == SRB_STATUS_SUCCESS) ?
				     StorportEtwLevelInformational :
				     StorportEtwLevelError,
	...
	StorPortEtwEvent2(adapterExtension, storAddress, AhciEtwEventStartIO,
			  L"Invalid request", STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
			  StorportEtwLevelInformational, StorportEtwEventOpcodeInfo, Srb,
			  L"Function", function, NULL, 0);
	...
	// Storport already make sure that the Unit in active state before a SCSI command
	// is sent to miniport.
	if ((function != SRB_FUNCTION_EXECUTE_SCSI) &&
...
It performs:
    ...
    1.2 round robin for selecting the port to process in case of multi-ports have
interrupt pending.
    1.3 Initialize Variables
        AHCI 1.1 Section 5.5.3 - 1. "Software determines the cause of the interrupt by
	reading the PxIS register.  It is possible for multiple bits to be set"
    2.1 Understand interrupts on this channel
        AHCI 1.1 Section 5.5.3 - 2. "Software clears appropriate bits in the PxIS register
	corresponding to the cause of the interrupt."
    2.1.1 Handle Fatal Errors Clear the PxIS Fatal Error bits And prep for error handling
    2.1.3 Handle non Native HotPlug Events Clear the PxSERR bits to clear PxIS
    ...
    3.1 Clear channel interrupt
        AHCI 1.1 Section 5.5.3 - 3. "Software clears the interrupt bit in IS.IPS corresponding
	to the port."
    4.1 Complete outstanding commands
    AHCI 1.1 Section 5.5.3 - 4.
    "If executing non-queued commands, software reads the PxCI register, and compares the
    current value to the list of commands previously issued by software that are still
    outstanding.  If executing native queued commands, software reads the PxSACT register and
    compares the current value to the list of commands previously issued by software. Software
...
--*/
VOID __inline AhciInterruptHandler(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
	...
	RecordExecutionHistory(ChannelExtension, 0x00000005); // AhciInterruptHandler
							      // Enter
    }
	...
	serr.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						 &ChannelExtension->Px->SERR.AsUlong);
    }

    // 2.1 Understand interrupts on this channel
    // 2.1.1 Handle Fatal Errors: Interface Fatal Error Status || Host Bus Data Error
    // Status || Host Bus Fatal Error Status || Task File Error Status
	...
	StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
				   &ChannelExtension->Px->IS.AsUlong, pxisMask.AsUlong);

	// call the correct error handling based on current hw queue workload type
	sact = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					 &ChannelExtension->Px->SACT);
	...
	if ((sact == MAXULONG) && IsAdapterRemoved(ChannelExtension->AdapterExtension)) {
		...
		(ChannelExtension != NULL) ?
		    ...
		    (NULL),
		StorportEtwEventOperational, AhciEtwEventHandleInterrupt,
		L"Adapter surprise removed", STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
		StorportEtwLevelWarning, StorportEtwEventOpcodeInfo, NULL, L"pxis",
		pxis.AsUlong, L"ssts", ssts.AsUlong, L"serr", serr.AsUlong, L"cmd",
		cmd.AsUlong, L"sact", sact, NULL, 0, NULL, 0, NULL, 0);
	    return;
	} else if (sact != 0) {
	...
	if ((cmd.AsUlong == MAXULONG) &&
	    IsAdapterRemoved(ChannelExtension->AdapterExtension)) {
	    StorPortEtwChannelEvent8(
		ChannelExtension->AdapterExtension,
		(ChannelExtension != NULL) ?
		    (PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress) :
	...
	if (pxis.PCS) {
	    // When PxSERR.DIAG.X is set to one this bit indicates a COMINIT signal was
	    // received.  This bit is reflected in the PxIS.PCS bit.
	    serrMask.DIAG.X = 1;
	    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
				       &ChannelExtension->Px->SERR.AsUlong,
				       serrMask.AsUlong);
	    // PCS = 1 could be an unsolicited COMINIT on an already detected drive. See
	    // AHCI 6.2.2.3    Recovery of Unsolicited COMINIT
    ...
    if (pxis.PRCS) {
	// Hot plug removals are detected via the PxIS.PRCS bit that directly reflects the
	// PxSERR.DIAG.N bit. Note that PxSERR.DIAG.N is also set to '1' on insertions and
	// during interface power management entry/exit.
	serrMask.DIAG.N = 1;
	StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
				   &ChannelExtension->Px->SERR.AsUlong, serrMask.AsUlong);

	ssts.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						 &ChannelExtension->Px->SSTS.AsUlong);

	if ((ssts.AsUlong == MAXULONG) &&
	    IsAdapterRemoved(ChannelExtension->AdapterExtension)) {
	    // controller has been surprise removed
	    return;
	}
	...
	if (!IgnoreHotPlug(ChannelExtension)) {
		...
		if (ssts.DET == 0) {
			...
			// If that succeeds, complete all outstanding commands.  CI is now
			// cleared. This is precautionary as there shall be no IO when D3
			// occurred, but the miniport may always create its own commands.
			ChannelExtension->SlotManager.CommandsToComplete =
			    GetOccupiedSlots(ChannelExtension);
			...
			NT_ASSERT(FALSE); // Looks like a hardware issue, will recover in
					  // P_Running_StartAttempt() when ZPODD is
					  // powered on again.
		    }
		} else if (ssts.DET == 3) {
    ...
    if (serr.AsUlong > 0) {
	StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
				   &ChannelExtension->Px->SERR.AsUlong, (ULONG)~0);
    }
	...
	//  AHCI 6.1.5 COMRESET is required by software to clean up from this serious
	//  error
	...
	// This async notification could be for media status, device status or device
	// operation events. Notification failure of STOR_STATUS_BUSY is ok as it means
	// that notifications are being coalesced and this is the only async notification.
	// Notification failure of STOR_STATUS_INVALID_DEVICE_REQUEST is ok as Storport
	// may no longer find the unit as it may have been surprise removed.
	...
	storStatus = StorPortAsyncNotificationDetected(
	    ChannelExtension->AdapterExtension,
	...
	ChannelExtension->TaskFileData.AsUlong = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->TFD.AsUlong);

	if ((ChannelExtension->TaskFileData.AsUlong == MAXULONG) &&
	    IsAdapterRemoved(ChannelExtension->AdapterExtension)) {
	    // controller has been surprise removed
	    StorPortEtwChannelEvent8(
		ChannelExtension->AdapterExtension,
		...
		StorportEtwLevelWarning, StorportEtwEventOpcodeInfo, NULL, L"pxis",
		pxis.AsUlong, L"ssts", ssts.AsUlong, L"serr", serr.AsUlong, L"cmd",
		cmd.AsUlong, L"sact", sact, L"StateFlags",
		*(ULONGLONG *)&(ChannelExtension->StateFlags), NULL, 0, NULL, 0);
	    return;
	}
    ...
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
			       ChannelExtension->AdapterExtension->IS, is);

    // 5. Complete outstanding commands
    ci = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
				   &ChannelExtension->Px->CI);
    sact = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
				     &ChannelExtension->Px->SACT);

    if (((ci == MAXULONG) || (sact == MAXULONG)) &&
	IsAdapterRemoved(ChannelExtension->AdapterExtension)) {
	// controller has been surprise removed
	StorPortEtwChannelEvent8(
	    ChannelExtension->AdapterExtension,
		...
		(NULL),
	    StorportEtwEventOperational, AhciEtwEventHandleInterrupt,
	    L"Adapter removed: complete cmd", STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
	    StorportEtwLevelWarning, StorportEtwEventOpcodeInfo, NULL, L"pxis",
	    pxis.AsUlong, L"ssts", ssts.AsUlong, L"serr", serr.AsUlong, L"cmd",
	    cmd.AsUlong, L"is", is, L"ci", ci, L"sact", sact, NULL, 0);
	return;
    }
	...
	RecordInterruptHistory(ChannelExtension, pxis.AsUlong, ssts.AsUlong, serr.AsUlong,
			       ci, sact, 0x20000005); // AhciHwInterrupt complete IO

	AhciCompleteIssuedSRBs(ChannelExtension, SRB_STATUS_SUCCESS, TRUE);
    } else {
	// recording execution history for no SRB to be completed
	RecordInterruptHistory(ChannelExtension, pxis.AsUlong, ssts.AsUlong, serr.AsUlong,
			       ci, sact, 0x20010005); // AhciHwInterrupt No IO completed
    }
...
Return Values:
    ...
    If the adapter (or port) did not generate interrupt, this routine should return FALSE
    as soon as possible.
--*/
BOOLEAN AhciHwInterrupt(_In_ PVOID AdapterExtension)
{
    ULONG is;
    ...
    //
    // In case of multi-ports have interrupt pending, using round robin to choose a port
    // handle its interrupt. After this routine exits, other pending interrupts will
    // trigger interrupt again.
    //
    if (interruptPortCount > 1) {
	i = (adapterExtension->LastInterruptedPort + 1) %
	    (adapterExtension->HighestPort + 1);
    } else {
    ...
    do {
	if (((interruptPorts & (1 << i)) != 0) &&
	    IsPortStartCapable(adapterExtension->PortExtension[i])) {
	    break;
	}
...
--*/
BOOLEAN AhciHwMSIInterrupt(_In_ PVOID AdapterExtension, _In_ ULONG MessageId)
{
    PAHCI_ADAPTER_EXTENSION adapterExtension = (PAHCI_ADAPTER_EXTENSION)AdapterExtension;

    if (adapterExtension->StateFlags.InterruptMessagePerPort == 0) {
	//
	// In case message ID is shared by ports, call legacy ISR to determine if it's a
	// valid AHCI interrupt.
	//
...
VOID AhciHwTracingEnabled(_In_ PVOID AdapterExtension, _In_ BOOLEAN Enabled)
{
    PAHCI_ADAPTER_EXTENSION adapterExtension = (PAHCI_ADAPTER_EXTENSION)AdapterExtension;
...
--*/
SCSI_UNIT_CONTROL_STATUS AhciHwUnitControl(_In_ PVOID AdapterExtension,
					   ...
					   _In_ PVOID Parameters)
{
	...
	// Report ScsiQuerySupportedUnitControlTypes, ScsiUnitStart and ScsiUnitPower are
	// supported.
	if (ScsiQuerySupportedControlTypes < controlTypeList->MaxControlType) {
	...
	// this is the PnP call IRP_MN_START_DEVICE for the LUN. Need to do device
	// initialization work.
	...
	// this is the power call for the LUN. Need to power up or power down the device
	// as necessary.
	if (IsPortValid(adapterExtension, storAddrBtl8->Path)) {
	    PAHCI_CHANNEL_EXTENSION channelExtension =
		adapterExtension->PortExtension[storAddrBtl8->Path];

	    StorPortDebugPrint(3, "StorAHCI - LPM: SystemIoBusNumber:%d Port:%02d - %s\n",
			       adapterExtension->SystemIoBusNumber, storAddrBtl8->Path,
			       unitControlPower->PowerState == StorPowerDeviceD0 ? "D0" :
										   "D3");
	...
	if (IsPortValid(adapterExtension, storAddrBtl8->Path)) {
	    ...
	    ULONG bufferLength = STOR_POFX_DEVICE_V3_SIZE + STOR_POFX_COMPONENT_V2_SIZE +
		...
		// allocate STOR_POFX_DEVICE data structure for Port, initialize the
		// structure and register for runtime power management.
		if (channelExtension->PoFxDevice == NULL) {
		    storStatus = StorPortAllocatePool(
			AdapterExtension, bufferLength, AHCI_POOL_TAG,
			(PVOID *)&channelExtension->PoFxDevice);
		}

		if (storStatus == STOR_STATUS_SUCCESS) {
		    PSTOR_POFX_COMPONENT_V2 component =
			(PSTOR_POFX_COMPONENT_V2) &
			(channelExtension->PoFxDevice->Components[0]);
		    ...
		    // unit out of D3Cold.
		    //
		    channelExtension->PoFxDevice->Flags |=
			...
			channelExtension->PoFxDevice->Flags |=
			    STOR_POFX_DEVICE_FLAG_NO_IDLE_DEBOUNCE;
		    }

		    //
		    // If this is a drive with rotational media then enable adaptive D3
		    // idle timeout, else if this is a SSD on an AOAC system, set the idle
		    // timeout flag and provide a minimum unit idle timeout.
		    //
		    if (DeviceIncursSeekPenalty(channelExtension)) {
			channelExtension->PoFxDevice->Flags |=
			    STOR_POFX_DEVICE_FLAG_ADAPTIVE_D3_IDLE_TIMEOUT;
		    ...
		    storStatus = StorPortInitializePoFxPower(
			AdapterExtension, (PSTOR_ADDRESS)storAddrBtl8,
			(PSTOR_POFX_DEVICE)channelExtension->PoFxDevice, &d3ColdEnabled);
	...
	if (IsPortValid(adapterExtension, storAddrBtl8->Path) &&
	    PortPoFxEnabled(adapterExtension->PortExtension[storAddrBtl8->Path])) {
	    PAHCI_CHANNEL_EXTENSION channelExtension =
		adapterExtension->PortExtension[storAddrBtl8->Path];
		...
		StorPortRequestTimer(channelExtension->AdapterExtension,
				     ...
				     AhciAutoPartialToSlumber, channelExtension, 0, 0);
		StorPortDebugPrint(3,
				   "StorAHCI - LPM: SystemIoBusNumber:%d Port:%02d - "
				   "Transit into Slumber from Partial - Canceled\n",
				   adapterExtension->SystemIoBusNumber,
				   channelExtension->PortNumber);

		busChangePending = InterlockedBitTestAndReset(
		    (volatile long *)&channelExtension->PoFxPendingWork,
		    1); // BusChange is at bit 1
		restorePreservedSettingsPending = InterlockedBitTestAndReset(
		    (volatile long *)&channelExtension->PoFxPendingWork,
		    0); // RestorePreservedSettings is at bit 0
		...
		if (restorePreservedSettingsPending == 1) {
		    ...
		    RecordExecutionHistory(channelExtension,
					   0x10050041); // RestorePreservedSettings
							// continue in Unit Active
		...
		if (PartialToSlumberTransitionIsAllowed(channelExtension, NULL)) {
		    status = StorPortRequestTimer(
			channelExtension->AdapterExtension, channelExtension->WorkerTimer,
			AhciAutoPartialToSlumber, channelExtension,
			channelExtension->AutoPartialToSlumberInterval * 1000, 20000);

		    if (status == STOR_STATUS_SUCCESS) {
			StorPortDebugPrint(3,
					   "StorAHCI - LPM: SystemIoBusNumber:%d "
					   "Port:%02d - Transit into Slumber from "
					   "Partial - Scheduled\n",
					   adapterExtension->SystemIoBusNumber,
					   channelExtension->PortNumber);
		    }
	...
	if (IsPortValid(adapterExtension, storAddrBtl8->Path) &&
	    PortPoFxEnabled(channelExtension)) {
	    StorPortDebugPrint(3,
			       "StorAHCI - LPM: SystemIoBusNumber:%d Port:%02d - "
			       "Transition from F%u to F%u\n",
			       adapterExtension->SystemIoBusNumber,
			       channelExtension->PortNumber, channelExtension->PoFxFState,
    ...
    //
    // ScsiUnitSurpriseRemoval is called when Storport processes the surprise removal IRP
    // for the unit.
    //
	...
	if (IsPortValid(adapterExtension, storAddrBtl8->Path)) {
	    ...
	    AhciInterruptSpinlockRelease(adapterExtension, channelExtension->PortNumber,
					 &lockhandle);
		...
		channelExtension,
		(PSTOR_ADDRESS) & (channelExtension->DeviceExtension->DeviceAddress),
		AhciTelemetryEventIdSurpriseRemove, "ScsiUnitSurpriseRemoval",
		AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING, NULL, 0);
	}
