...
Abstract:
    This file contains the core logic for programming and completing IO commands into an
AHCI controller
...
Return Value:
--*/
ULONG GetSlotToActivate(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			_In_ ULONG TargetSlots)
{
    ...
    NT_ASSERT(ChannelExtension->DeviceExtension[0].DeviceParameters.MaxDeviceQueueDepth <=
	      ChannelExtension->AdapterExtension->CAP.NCS);
...
/*++
    Selects an IO circularly from the programmed Single IO slice starting with the slot
after the most recently programmed slot This ensures the issuing of the SingleIO is FIFO
...
--*/
UCHAR GetSingleIo(PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    UCHAR limit;
    UCHAR i;
...
*/
VOID AddQueue(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
	      _Inout_ PSTORAHCI_QUEUE Queue, _In_ PSTORAGE_REQUEST_BLOCK Srb,
	      _In_ ULONG Signature, _In_ UCHAR Tag)
{
	...
	tempTail = Queue->Tail;
	if (SrbGetNextSrb(tempTail) == NULL) { // Verify SRBs are not about to be severed
					       // from the Queue
...
PSTORAGE_REQUEST_BLOCK RemoveQueue(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
				   ...
				   _In_ ULONG Signature, _In_ UCHAR Tag)
{
    ...
    SrbSetNextSrb(nextSrb,
		  NULL); // This should have been NULL to start with, return it to it's
			 // NULL state
...
It performs:
    ...
    (details)
    1.1  Initialize variables
    1.2 If the programming should not happen now, leave, ActivateQueue will be called
    again when these conditions are changed
    2.1 Choose the Queue with which to program the controller
    Algorithm: 2.1.1 Single IO SRBs (including Request Sense and non data control
                     commands) have highest priority.
	       2.1.2 When there are no Single IO commands, Normal IO get
	             the next highest priority
	       2.1.3 When there are no Single or Normal commands, NCQ commands
	             get the next highest priority
	       2.1.4 In the case that no IO is present in any Slices,
	             program nothing
...
Return Values:
--*/
BOOLEAN ActivateQueue(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		      _In_ BOOLEAN AtDIRQL)
{
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10010022); // ActivateQueue, Channel Not Yet Started
	status = FALSE;
	return status;
    }
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10020022); // ActivateQueue, Channel Queue Paused
	status = FALSE;
	return status;
    }
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10030022); // ActivateQueue, Channel Not Start Capable
	status = FALSE;
	return status;
    }
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10070022); // ActivateQueue, Error Recovery is pending.
	status = FALSE;
	return status;
    }
	...
	if (LogExecuteFullDetail(adapterExtension->LogFlags)) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10040022); // ActivateQueue, No Commands to program
	}
	status = FALSE;
	goto Exit;
    }
	...
	// In case of device initialization is in process but the local command is not
	// ready yet, do not program other commands.
	if (LogExecuteFullDetail(ChannelExtension->AdapterExtension->LogFlags)) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10080022); // ActivateQueue, an init command is in
						// process.
	}
	status = FALSE;
	goto Exit;
    ...
    if (ChannelExtension->SlotManager.SingleIoSlice != 0) {
	if ((ChannelExtension->SlotManager.NCQueueSliceIssued |
		...
		ChannelExtension->StateFlags.QueuePaused = TRUE; // and pause the queue so
								 // no other IO get
								 // programmed

		if (LogExecuteFullDetail(adapterExtension->LogFlags)) {
		    RecordExecutionHistory(ChannelExtension,
					   0x10090022); // ActivateQueue, Single IO pause
							// queue
		}

		// 2.1.1.1 Update the Identify command for enumeration. This needs to be
		// here for PxSIG to be accessible. Accessing PxSIG register here to avoid
		// invalid values from time when the port is being reset or not started,
		// or other scenarios.
		if (slotContent->Srb != NULL) {
			...
			// For enumeration command, it's safe now to check the PxSIG
			// register as starting port has been executed.
			ULONG sig = 0;
			sig =
	...
	// 2.1.2 When there are no Single IO commands, Normal IO get the next highest
	// priority
    } else if (ChannelExtension->SlotManager.NormalQueueSlice != 0) {
	// Normal commands can not be sent when NCQ commands are outstanding.  When the
	// NCQ commands complete ActivateQueue will get called again.
	if (ChannelExtension->SlotManager.NCQueueSliceIssued == 0) {
	    // Grab the High Priority Normal IO before the Low Priority Normal IO
	    slotsToActivate = ChannelExtension->SlotManager.HighPriorityAttribute &
	...
	// 2.1.3 When there are no Single or Normal commands, NCQ commands get the next
	// highest priority
    } else if ((ChannelExtension->SlotManager.NCQueueSlice != 0) &&
	       (ChannelExtension->StateFlags.NcqErrorRecoveryInProcess == 0)) {
	// NCQ commands can not be sent when Normal commands are outstanding.  When the
	// Normal commands complete, Activate Queue will get called again.
	if ((ChannelExtension->SlotManager.SingleIoSliceIssued |
	    ...
	    // And apply any device outstanding IO limits to filter down which IO to
	    // activate
	    if (slotsToActivate > 0) {
		if (ChannelExtension->DeviceExtension[0]
			.DeviceParameters.MaxDeviceQueueDepth <
		    ChannelExtension->MaxPortQueueDepth) {
		    // Get allowed slots if the device queue depth is less than the port
		    // can support.
		    slotsToActivate = GetSlotToActivate(ChannelExtension,
							slotsToActivate);
		}
		if (slotsToActivate > 0) {
	...
	if (adapterExtension->TracingEnabled) {
		...
		// TODO: there is potential race condition that causes slot flags mismatch
		// and Srb is NULL here.
	...
	// Program registers
	if (activateNcq) {
	    StorPortWriteRegisterUlong(adapterExtension, &ChannelExtension->Px->SACT,
				       slotsToActivate);
	}
	StorPortWriteRegisterUlong(adapterExtension, &ChannelExtension->Px->CI,
				   slotsToActivate);
    }

    if (LogExecuteFullDetail(adapterExtension->LogFlags)) {
	RecordInterruptHistory(ChannelExtension, slotsToActivate, 0, 0,
			       (ChannelExtension->SlotManager.SingleIoSliceIssued |
				ChannelExtension->SlotManager.NormalQueueSliceIssued),
...
__inline ULONGLONG CalculateTimeDurationIn100ns(_In_ ULONGLONG TimeDuration,
						_In_ ULONGLONG CounterFrequency)
{
    ...
    // the situation about the counter start over again is considered an error case of
    // inputs.
...
--*/
VOID AhciCompleteIssuedSRBs(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			    _In_ UCHAR SrbStatus, _In_ BOOLEAN AtDIRQL)
{
    ...
    if (adapterExtension->TracingEnabled &&
	(ChannelExtension->SlotManager.CommandsToComplete)) {
	StorPortQueryPerformanceCounter((PVOID)adapterExtension, &perfFrequency,
					&perfCounter);
    }
	...
	if ((ChannelExtension->SlotManager.CommandsToComplete & (1 << i)) > 0) {
		...
		// The completed slot has no SRB so it can not be completed back to
		// Storport. Give back the empty slot
		NT_ASSERT(FALSE);
		...
		ChannelExtension->SlotManager.CommandsToComplete &= ~(1 << i);
		ChannelExtension->SlotManager.HighPriorityAttribute &= ~(1 << i);
		// It is now impossible to determine if a data transfer completed
		// correctly
	    ...
	    if (adapterExtension->TracingEnabled && (srbExtension->StartTime != 0) &&
		(perfCounter.QuadPart != 0) &&
		!IsMiniportInternalSrb(ChannelExtension, slotContent->Srb)) {
		ULONGLONG durationTime = CalculateTimeDurationIn100ns(
		    (perfCounter.QuadPart - srbExtension->StartTime),
		    perfFrequency.QuadPart);
		StorPortNotification(IoTargetRequestServiceTime, (PVOID)adapterExtension,
				     durationTime, slotContent->Srb);
	    }
		...
		(slotContent->CmdHeader->PRDBC !=
		 RequestGetDataTransferLength(slotContent->Srb))) {
		if (slotContent->CmdHeader->PRDBC <
		    RequestGetDataTransferLength(slotContent->Srb)) {
		    // Buffer underrun,
		    RequestSetDataTransferLength(slotContent->Srb,
						 slotContent->CmdHeader->PRDBC);
		...
		// If anything has set a STATUS on this SRB, honor that over the one
		// passed in
	    ...
	    // 2.4 Give the slot back
	    ReleaseSlottedCommand(ChannelExtension, i,
				  AtDIRQL); // Request sense is handled here.
...
Affected Variables/Registers:
    Command Table
--*/
VOID SRBtoATA_CFIS(PAHCI_CHANNEL_EXTENSION ChannelExtension, PSLOT_CONTENT SlotContent)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(SlotContent->Srb);
    PAHCI_COMMAND_TABLE cmdTable = (PAHCI_COMMAND_TABLE)srbExtension;
    ...
    cmdTable->CFIS.ICC = 0;
    cmdTable->CFIS.Control = 0; // Device control consists of the 48bit HighOrderByte,
				// SRST and nIEN.  None apply here.
...
Affected Variables/Registers:
    Command Table
--*/
VOID SRBtoATAPI_CFIS(PAHCI_CHANNEL_EXTENSION ChannelExtension, PSLOT_CONTENT SlotContent)
{
	...
	// If word 62 bit 15 is set to one, then the DMADIR bit in the PACKET command is
	// required by the device for PACKET commands using the DMA data transfer protocol
	// and:
	...
	if ((ChannelExtension->DeviceExtension[0]
		 .IdentifyPacketData->DMADIR.DMADIRBitRequired != 0) &&
	    (ChannelExtension->DeviceExtension[0]
		 .IdentifyPacketData->Capabilities.DmaSupported != 0) &&
	    (ChannelExtension->DeviceExtension[0]
		 .IdentifyPacketData->Capabilities.InterleavedDmaSupported == 0) &&
	    ((ChannelExtension->DeviceExtension[0].IdentifyPacketData->MultiWordDMASupport &
	      0x7) == 0) &&
	    ((ChannelExtension->DeviceExtension[0].IdentifyPacketData->UltraDMASupport &
	      0x7F) == 0)) {
	    // Bit 0 is DMA, bit 2 is DMADIR. (0 = transfer to the device; 4 = transfer to
	    // the host).
...
/*++
    Populates CFIS structure with an ATA command in the given slot from a CFIS data
structure. It assumes: The slot is empty and not being used Called by: AhciHwStartIo
...
Affected Variables/Registers:
    Command Table
--*/
VOID CfistoATA_CFIS(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		    _In_ PSLOT_CONTENT SlotContent)
{
    ...
    //
    StorPortCopyMemory(&cmdTable->CFIS, &srbExtension->Cfis,
		       sizeof(AHCI_H2D_REGISTER_FIS));
	...
	IsNCQWriteCommand(srbExtension) && (srbExtension->RetryCount > 0) &&
...
--*/
ULONG SRBtoPRDT(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		_In_ PSLOT_CONTENT SlotContent)
{
    ULONG i;
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(SlotContent->Srb);
	...
	if ((sgl->List[i].PhysicalAddress.LowPart & 0x1) == 0) {
		...
		if (ChannelExtension->AdapterExtension->CAP.S64A) { // If the controller
								    // supports 64 bits,
								    // write the high part
								    // too
		    cmdTable->PRDT[i].DBAU = sgl->List[i].PhysicalAddress.HighPart;
		}
		// 2.2 Break up a 128K single entry IO into 2 64K IO entries (128K is max
		// transfer so there can be only 1 in any SGL)
		//     although one entry can represent at max 4M length IO, some adapters
		//     cannot handle a DBC >= 128K.
	    } else {
		// Entry 0
		...
		// Entry 1
		cmdTable->PRDT[1].DBA.AsUlong = (sgl->List[0].PhysicalAddress.LowPart +
						 0x10000);
		if (ChannelExtension->AdapterExtension->CAP.S64A) { // If the controller
								    // supports 64 bits,
								    // write the high part
								    // too
		    if ((sgl->List[0].PhysicalAddress.LowPart + 0x10000) <
			sgl->List[0].PhysicalAddress.LowPart) {
			cmdTable->PRDT[1].DBAU = (sgl->List[0].PhysicalAddress.HighPart +
						  1); // add 1 to the highpart if adding
						      // 0x10000 caused a rollover
		    } else {
			cmdTable->PRDT[1].DBAU = sgl->List[0].PhysicalAddress.HighPart;
		    }
		}
	...
	} else if (sgl->List[i].Length <=
		   RequestGetDataTransferLength(SlotContent->Srb)) {
	    // Storport may send down SCSI commands with odd number of data transfer
	    // length, and it builds SGL using that transfer length value. we use the
	    // length -1 to get as much data as we can. If the data length is over (length
	    // - 1), buffer overrun will be reported when the command is completed.
	    RequestSetDataTransferLength(SlotContent->Srb,
					 RequestGetDataTransferLength(SlotContent->Srb) -
					     1);
...
Affected Variables/Registers:
    Command Header
--*/
VOID SRBtoCmdHeader(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		    _In_ PSLOT_CONTENT SlotContent,
		    _In_ ULONG Length, _In_ BOOLEAN Reset)
{
    ...
    cmdHeader->DI.P = 0;
    // f. If a Port Multiplier is attached, the PMP field set to the correct Port
    // Multiplier port.
    cmdHeader->DI.PMP = 0;
...
This routine does following:
    1. Check if no device command associated with Srb, or the port failed to start. Bail
out in these cases.
...
*/
BOOLEAN AhciProcessIo(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		      _In_ PSTORAGE_REQUEST_BLOCK Srb, _In_ BOOLEAN AtDIRQL)
{
    ...
    // 2. central place for command special handling. this part cannot be put in command
    // translation layer, as there might be internal command also needs special handling.
    // 2.1 central place for special handling of Enable/Disable WRITE CACHE, update
    // persistent settings command.
    if ((srbExtension->AtaFunction == ATA_FUNCTION_ATA_COMMAND) &&
	(srbExtension->TaskFile.Current.bCommandReg == IDE_COMMAND_SET_FEATURE)) {
	if (srbExtension->TaskFile.Current.bFeaturesReg ==
	    IDE_FEATURE_DISABLE_WRITE_CACHE) {
	    // IDE_FEATURE_DISABLE_WRITE_CACHE => Modify Persistent Settings
	    UpdateSetFeatureCommands(ChannelExtension, IDE_FEATURE_ENABLE_WRITE_CACHE,
				     IDE_FEATURE_DISABLE_WRITE_CACHE, 0, 0);
	}

	if (srbExtension->TaskFile.Current.bFeaturesReg ==
	    IDE_FEATURE_ENABLE_WRITE_CACHE) {
	    // IDE_FEATURE_ENABLE_WRITE_CACHE => Modify Persistent Settings
	    UpdateSetFeatureCommands(ChannelExtension, IDE_FEATURE_DISABLE_WRITE_CACHE,
				     IDE_FEATURE_ENABLE_WRITE_CACHE, 0, 0);
	}
    }

    // 2.2 update the Identify command for enumeration. This needs to be here waiting for
    // PxSIG to be accessible.
    if ((srbExtension->AtaFunction == ATA_FUNCTION_ATA_IDENTIFY) &&
	(srbExtension->CompletionRoutine == AhciPortIdentifyDevice) &&
	(srbExtension->TaskFile.Current.bCommandReg == IDE_COMMAND_NOT_VALID)) {
	// For enumeration command, it's safe now to check the PxSIG register as starting
	// port has been executed. Note: there is chance that the signature register is
	// FFFFFFFFh when accessing it here because reset is in progress. Handle this
	// special case in the follow checking device type code.
	ULONG sig = 0;
	...
	if (sig == ATA_DEVICE_SIGNATURE_ATA) {
	    ...
	    ULONG ssts = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						   &ChannelExtension->Px->SSTS.AsUlong);
	    RecordExecutionHistory(ChannelExtension,
				   0x10090020); // Unexpected signature register value
		...
		// register becomes 0.
		// Case 2: A value of FFFFFFFFh is set to signature register during reset
		// according to AHCI 1.3.1 spec, while the value maybe also FFFFFFFFh when
		// device is removed, because PCI configuration space that mapped with
		// AHCI registers are FFFFFFFFh. To differentiate whether it is reset or
		// device removed, check Px.SSTS register value(it is 0 during reset).
		//
		// If above situations happen, just set the identify command according to
		// the currently known device type.
		switch (
		    ChannelExtension->DeviceExtension[0].DeviceParameters.AtaDeviceType) {
		case DeviceIsAta:
		    srbExtension->TaskFile.Current.bCommandReg = IDE_COMMAND_IDENTIFY;
		...
		// Other devices types, such as with signature
		// ATA_DEVICE_SIGNATURE_ENCLOSURE, or
	...
	// Wait for 8 IO (random picked number) being completed before re-starting sending
	// IO to miniport for this device. if filled up,
	if (!IsMiniportInternalSrb(ChannelExtension, Srb)) {
	    // Report busy only for IOs from Storport
	    StorPortDeviceBusy(ChannelExtension->AdapterExtension, pathId, targetId, lun,
			       min(8, ChannelExtension->AdapterExtension->CAP.NCS));
	}
	...
	// 3.2 if the tag returned can't be used, something is seriously wrong. Reject the
	// command to be retried later
	if ((((1 << srbExtension->QueueTag) & ci) > 0) ||
	    (((1 << srbExtension->QueueTag) & sact) > 0)) {
	    if (!IsMiniportInternalSrb(ChannelExtension, Srb)) {
		// pause device only for IOs from Storport
		StorPortPauseDevice(ChannelExtension->AdapterExtension, pathId, targetId,
				    lun, 1); // pause for 1 seconds.
	    }
	    ...
	    AhciCompleteRequest(ChannelExtension, Srb, TRUE);
	    RecordExecutionHistory(ChannelExtension,
				   0x10040020); // Tag given for slot in use
	    goto exit;
	}
    }
#endif
...
*/
BOOLEAN AhciFormIo(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		   ...
		   _In_ BOOLEAN AtDIRQL)
{
    ...
    // Command header is allocated in ChannelExtension for all command slots, get the
    // correct one.
    ...
    if (!IsReturnResults(srbExtension->Flags) && (cdb != NULL) &&
	((cdb->CDB10.OperationCode == SCSIOP_WRITE) ||
	 (cdb->CDB10.OperationCode == SCSIOP_WRITE16)) &&
	(cdb->CDB10.ForceUnitAccess == 1) && IsFuaSupported(ChannelExtension)) {
	...
	slotContent->CommandHistoryIndex =
	    ChannelExtension->CommandHistoryNextAvailableIndex;
    ...
    // Already get a slot, after this point, any request completion effort needs to
    // release the slot. just like what's done in: ReleaseSlottedCommand()
	...
	ULONG length;
	// SRB is needed for StorPortGetPhysicalAddress to calculate logical address when
	// DMAR is enabled.
	...
	AhciCompleteJustSlottedRequest(ChannelExtension, Srb, AtDIRQL);
	RecordExecutionHistory(ChannelExtension,
			       0x10080020); // Invalid PhyscialAddress alignment
	return TRUE;
    }
					 ...
					 _In_ PSTORAGE_REQUEST_BLOCK FailingSrb)
{
	...
	// Request Sense is for ATAPI commands which are single IOs. At the same time
	// there should be only one command failed and needs this Srb.
	...
	dataBufferPhysicalAddress.LowPart;
    srbExtension->LocalSgl.List[0].PhysicalAddress.HighPart =
	dataBufferPhysicalAddress.HighPart;
			...
			_In_ UCHAR SrbStatus, _In_ BOOLEAN AtDIRQL)
{
	...
	if (ChannelExtension->Slot[i].Srb != NULL) {
	    ...
	    AhciCompleteJustSlottedRequest(ChannelExtension,
					   ChannelExtension->Slot[i].Srb, AtDIRQL);
	}
    }
				     ...
				     _In_opt_ PVOID SystemArgument2)
{
    ...
    if (LogExecuteFullDetail(channelExtension->AdapterExtension->LogFlags)) {
	RecordExecutionHistory(channelExtension,
			       0x0000001e); // Enter AhciPortSrbCompletionDpcRoutine
    }
	...
	AhciInterruptSpinlockAcquire(channelExtension->AdapterExtension,
				     channelExtension->PortNumber, &lockhandle);
	srb = RemoveQueue(channelExtension, &channelExtension->CompletionQueue,
			  0xDEADC0DE, 0x9F);
	AhciInterruptSpinlockRelease(channelExtension->AdapterExtension,
				     channelExtension->PortNumber, &lockhandle);
	...
	if (srb != NULL) {
		...
		// A request with STATUS_BUS_RESET should be completed after running
		// completion routine.
		if ((srbExtension->AtaFunction != 0) &&
		    (!SrbShouldBeCompleted(srbExtension->Flags)) &&
		    (srb->SrbStatus != SRB_STATUS_BUS_RESET)) {
		    // new command associated needs to be processed, do not complete the
		    // request.
		...
		RecordExecutionHistory(channelExtension,
				       0x1001001e); // AhciPortSrbCompletionDpcRoutine,
						    // ReservedSlotInUse starts to stuck

		AhciTelemetryLog(
		    channelExtension->AdapterExtension,
		    (PSTOR_ADDRESS) & (channelExtension->DeviceExtension->DeviceAddress),
		    AHCI_TELEMETRY_DRIVER_VERSION, AhciTelemetryEventIdReservedSlotStuck,
		    "Reserved Slot Stuck", AHCI_TELEMETRY_EVENT_VERSION, 0, 0, NULL,
		    "CommandsIssued", channelExtension->SlotManager.CommandsIssued,
		    "NCQueueSlice", channelExtension->SlotManager.NCQueueSlice,
		    "NormalQueueSlice", channelExtension->SlotManager.NormalQueueSlice,
		    "SingleIoSlice", channelExtension->SlotManager.SingleIoSlice,
		    "CommandsToComplete",
		    channelExtension->SlotManager.CommandsToComplete, "StateFlags",
		    *(ULONGLONG *)&(channelExtension->StateFlags), "StartState",
    ...
    //
    // If Reserved Slot is freed by one of completion routine, send pending commands to
    // device.
    //
	...
	ActivateQueue(channelExtension,
		      (IsDumpMode(channelExtension->AdapterExtension) ? (TRUE) :
									(FALSE)));
    }
    ...
    if (LogExecuteFullDetail(channelExtension->AdapterExtension->LogFlags)) {
	RecordExecutionHistory(channelExtension,
			       0x1000001e); // Exit AhciPortSrbCompletionDpcRoutine
    }
}
