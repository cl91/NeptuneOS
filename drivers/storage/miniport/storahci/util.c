...
/*++
    Search a set of resources (I/O and memory) for an entry corresponding to an AHCI ABAR
address.
...
Called by:
    AhciHwFindAdapter
It performs:
    1. If any resources are defined, loop over all resource entries, looking for the AHCI
ABAR address. If found, return it.
				      ...
				      _In_ PPORT_CONFIGURATION_INFORMATION ConfigInfo)
{
	...
	for (i = 0; i < ConfigInfo->NumberOfAccessRanges; i++) {
	    ...
	    // Test to see if this is the needed resource entry, depending on the
	    // interface type
	    if (ConfigInfo->AdapterInterfaceType == PCIBus) {
		if ((*(ConfigInfo->AccessRanges))[i].RangeStart.QuadPart ==
		...
		abar = (PAHCI_MEMORY_REGISTERS)StorPortGetDeviceBase(
		    AdapterExtension, ConfigInfo->AdapterInterfaceType,
...
VOID RecordExecutionHistory(PAHCI_CHANNEL_EXTENSION ChannelExtension, ULONG Function)
{
    ...
    if (ChannelExtension->ExecutionHistoryNextAvailableIndex >=
	(MAX_EXECUTION_HISTORY_ENTRY_COUNT - 1)) {
	...
	&(ChannelExtension
	      ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	      .TimeStamp));
	...
	StorPortCopyMemory(
	    &ChannelExtension
		 ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].IS,
	    &ChannelExtension->StateFlags, sizeof(ULONG));
    } else {
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].IS =
	    (ULONG)~0;
	...
	&ChannelExtension
	     ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	     .SlotManager,
	&ChannelExtension->SlotManager, sizeof(SLOT_MANAGER));

    // 2.2 Copy over the AHCI registers
    if (ChannelExtension->Px) {
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[0] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->CLB.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[1] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->CLBU);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[2] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->FB.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[3] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->FBU);

	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[4] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->IS.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[5] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->IE.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[6] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->CMD.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[7] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->DW7_Reserved);

	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[8] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->TFD.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[9] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					       &ChannelExtension->Px->SIG.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[10] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						&ChannelExtension->Px->SSTS.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[11] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						&ChannelExtension->Px->SCTL.AsUlong);

	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[12] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						&ChannelExtension->Px->SERR.AsUlong);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[13] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						&ChannelExtension->Px->SACT);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[14] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						&ChannelExtension->Px->CI);
	ChannelExtension
	    ->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex]
	    .Px[15] = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						&ChannelExtension->Px->SNTF.AsUlong);
...
/*++
    This is just a space saver function that makes the interrupt configuration more
readable
...
VOID Set_PxIE(PAHCI_CHANNEL_EXTENSION ChannelExtension, PAHCI_INTERRUPT_ENABLE IE)
{
    ...
    ie.DHRE = 1; // Device to Host Register FIS Interrupt (DHRS):  A D2H Register FIS has
		 // been received with the I bit set, and has been copied into system
		 // memory.
    ie.PSE = 1; // PIO Setup FIS Interrupt (PSS):  A PIO Setup FIS has been received with
		// the I bit set, it has been copied into system memory, and the data
		// related to that FIS has been transferred.  This bit shall be set even
		// if the data transfer resulted in an error.
    ie.DSE = 1; // DMA Setup FIS Interrupt (DSS):  A DMA Setup FIS has been received with
		// the I bit set and has been copied into system memory.
    ie.SDBE = 1; // Set Device Bits Interrupt (SDBS):  A Set Device Bits FIS has been
		 // received with the I bit set and has been copied into system memory.

    ie.UFE = 0; // Unknown FIS Interrupt (UFS): When set to 1, indicates that an unknown
		// FIS was received and has been copied into system memory.  This bit is
		// cleared to 0 by software clearing the PxSERR.DIAG.F bit to 0.  Note
		// that this bit does not directly reflect the PxSERR.DIAG.F bit.
		// PxSERR.DIAG.F is set immediately when an unknown FIS is detected,
		// whereas this bit is set when that FIS is posted to memory.  Software
		// should wait to act on an unknown FIS until this bit is set to 1 or
		// the two bits may become out of sync.
    ie.DPE = 0; // Descriptor Processed (DPS):  A PRD with the I bit set has transferred
		// all of its data.  Refer to section 5.4.2.
    ie.PCE = 1; // Port Connect Change Status (PCS): 1=Change in Current Connect Status.
		// 0=No change in Current Connect Status.  This bit reflects the state of
		// PxSERR.DIAG.X.  This bit is only cleared when PxSERR.DIAG.X is cleared.
    if (adapterExtension->CAP.SMPS) {
	ie.DMPE = 1; // Device Mechanical Presence Status (DMPS): When set, indicates that
		     // a mechanical presence switch attached to this port has been opened
		     // or closed, which may lead to a change in the connection state of
		     // the device.  This bit is only valid if both CAP.SMPS and
    ...
    // Reserved :14;
    ie.PRCE = 1; // PhyRdy Change Status (PRCS): When set to 1 indicates the internal
		 // PhyRdy signal changed state.  This bit reflects the state of
		 // P0SERR.DIAG.N.  To clear this bit, software must clear P0SERR.DIAG.N
		 // to 0.
    ie.IPME = 0; // Incorrect Port Multiplier Status (IPMS):  Indicates that the HBA
		 // received a FIS from a device whose Port Multiplier field did not match
		 // what was expected.  The IPMS bit may be set during enumeration of
    ...
    ie.OFE = 1; // Overflow Status (OFS):  Indicates that the HBA received more bytes from
		// a device than was specified in the PRD table for the command.
    // Reserved2 :1;
    ie.INFE = 1; // Interface Non-fatal Error Status (INFS):  Indicates that the HBA
		 // encountered an error on the Serial ATA interface but was able to
		 // continue operation.  Refer to section 6.1.2.
    ie.IFE = 1; // Interface Fatal Error Status (IFS):  Indicates that the HBA encountered
		// an error on the Serial ATA interface which caused the transfer to stop.
		// Refer to section 6.1.2.

    ie.HBDE = 1; // Host Bus Data Error Status (HBDS):  Indicates that the HBA encountered
		 // a data error (uncorrectable ECC / parity) when reading from or writing
		 // to system memory.
    ie.HBFE = 1; // Host Bus Fatal Error Status (HBFS):  Indicates that the HBA
		 // encountered a host bus error that it cannot recover from, such as a
		 // bad software pointer.  In PCI, such an indication would be a target or
		 // master abort.
    ie.TFEE = 1; // Task File Error Status (TFES):  This bit is set whenever the status
		 // register is updated by the device and the error bit (bit 0) is set.
    cmd.AsUlong = StorPortReadRegisterUlong(adapterExtension,
					    &ChannelExtension->Px->CMD.AsUlong);
    if (cmd.CPD) { // check for PxCMD.CPD set to 1 before setting CPDE
	ie.CPDE = 1; // Cold Port Detect Status (CPDS): When set, a device status has
		     // changed as detected by the cold presence detect logic.  This bit
		     // can either be set due to a non-connected port receiving a device,
		     // or a connected port having its device removed.  This bit is only
...
--*/
VOID WMultiStringToAscii(_Inout_ PSTR Strings, _In_ ULONG StringBufferLength)
{
	...
	// the converted value in Destination. Ensure that the destination buffer isn't
	// overflowed.
	for (charCount = 0; (charCount < (StringBufferLength / 2 - 1)) &&
			    ((*source != '\0') || (*(source + 1) != '\0'));
	     charCount++) {
	...
	// "StringBufferLehgth" is length in bytes; "charCount" is count of chars from
	// converted ACSII string.
...
Return Value:
    ...
    FALSE - otherwise
*/
BOOLEAN StringToULONG(_In_ PSTR InputString,
		      _Out_ PULONG Value)
{
    BOOLEAN result = TRUE;
...
/*++
Character comparison between DeviceId and TargetId to the min(DeviceIdLength,
TargetIdLength)

Note that TargetId is a multi_sz string. DeviceId will be used to compare with each of
strings in TargetId.

When string matches and 'Value' is asked, put value of remaining TargetId into "Value"
field
...
TargetId -- target string for comparison, in format of multi_sz string.
    TargetId format: xxxxxxxx nnn (first part is the ID to compare, second part is the
    value)

    Special character in TargetId:
	'?' - matches any character in DeviceId
	'*' - indicating end of TargetId, matches any characters left in DeviceId. It
	should be used only at the end of string.

Return Value:
    TRUE - On a match or contain
    FALSE - On a mismatch
--*/
BOOLEAN CompareId(_In_opt_ PSTR DeviceId,
		  ...
		  _Inout_opt_ PULONG Value)
{
	...
	compareLength = min(DeviceIdLength,
			    GetStringLength(localStrings, TargetIdLength));
	if (compareLength == 0) {
	    return FALSE;
	}
	...
	for (i = 0; i < compareLength; i++) {
		...
		matches = FALSE;
		break;
	    } else if ((*localDeviceId == *localTargetId) || (*localTargetId == '?')) {
		localDeviceId++;
		localTargetId++;
	    } else {
		matches = FALSE;
    ...
    if ((Value != NULL) && (matches == TRUE) && (*localTargetId == ' ')) {
...
VOID AhciBusChangeCallback(_In_ PVOID AdapterExtension, _In_opt_ PVOID Context,
			   _In_ SHORT AddressType, _In_ PVOID Address, _In_ ULONG Status)
{
...
VOID PortBusChangeProcess(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    ...
    sctl.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					     &ChannelExtension->Px->SCTL.AsUlong);
    if ((sctl.SPD != 0) && (sctl.SPD != ChannelExtension->AdapterExtension->CAP.ISS)) {
    ...
    // 3 Requests rescan on this port. Using STATE_CHANGE_TARGET to make sure a REPORT
    // LUNS command coming from Storport.
    status = StorPortStateChangeDetected(
	ChannelExtension->AdapterExtension, STATE_CHANGE_TARGET,
	(PSTOR_ADDRESS)&ChannelExtension->DeviceExtension[0].DeviceAddress, 0,
	AhciBusChangeCallback, NULL);

    // If the state change notification fails, set a timer to retry the notification after
    // a delay
    if (status == STOR_STATUS_UNSUCCESSFUL) {
	if (ChannelExtension->BusChangeTimer == NULL) {
	    StorPortInitializeTimer(ChannelExtension->AdapterExtension,
				    &ChannelExtension->BusChangeTimer);
	}

	status = StorPortRequestTimer(ChannelExtension->AdapterExtension,
				      ChannelExtension->BusChangeTimer,
				      AhciBusChangeTimerCallback, ChannelExtension,
	...
	if (ChannelExtension->BusChangeTimer != NULL) {
	    StorPortFreeTimer(ChannelExtension->AdapterExtension,
			      ChannelExtension->BusChangeTimer);
...
Affected Variables/Registers:
    none
--*/
VOID AhciPortBusChangeDpcRoutine(_In_ PSTOR_DPC Dpc, _In_ PVOID AdapterExtension,
				 ...
				 _In_opt_ PVOID SystemArgument2)
{
    ...
    busChangeInProcess = InterlockedBitTestAndSet(
	(volatile long *)&channelExtension->PoFxPendingWork, 1); // BusChange is at bit 1
...
VOID AhciBusChangeTimerCallback(_In_ PVOID AdapterExtension,
				_In_opt_ PVOID ChannelExtension)
{
	...
	// AhciBusChangeTimerCallback, because it is a timer callback function, StartIo
	// spin lock is already held - Storport holds StartIo spin lock before invoking
    ...
    AhciPortBusChangeDpcRoutine(&channelExtension->BusChangeDpc, AdapterExtension,
			   ...
			   _In_ UCHAR SlotNumber, _In_ BOOLEAN AtDIRQL)
{
	...
	slotContent->CommandHistoryIndex %= 64; // should not be 64 or bigger. do it
						// anyway to make sure we are safe

	cmdHistory = &ChannelExtension->CommandHistory[slotContent->CommandHistoryIndex];
	StorPortCopyMemory((PVOID)&cmdHistory->CompletionFIS,
			   (PVOID)&ChannelExtension->ReceivedFIS->D2hRegisterFis,
			   sizeof(AHCI_D2H_REGISTER_FIS));

	cmdHistory->CompletionPx[0] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->CLB.AsUlong);
	cmdHistory->CompletionPx[1] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->CLBU);
	cmdHistory->CompletionPx[2] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->FB.AsUlong);
	cmdHistory->CompletionPx[3] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->FBU);

	cmdHistory->CompletionPx[4] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->IS.AsUlong);
	cmdHistory->CompletionPx[5] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->IE.AsUlong);
	cmdHistory->CompletionPx[6] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->CMD.AsUlong);
	cmdHistory->CompletionPx[7] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->DW7_Reserved);

	cmdHistory->CompletionPx[8] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->TFD.AsUlong);
	cmdHistory->CompletionPx[9] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->SIG.AsUlong);
	cmdHistory->CompletionPx[10] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->SSTS.AsUlong);
	cmdHistory->CompletionPx[11] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->SCTL.AsUlong);

	cmdHistory->CompletionPx[12] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->SERR.AsUlong);
	cmdHistory->CompletionPx[13] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->SACT);
	cmdHistory->CompletionPx[14] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->CI);
	cmdHistory->CompletionPx[15] = StorPortReadRegisterUlong(
	    ChannelExtension->AdapterExtension, &ChannelExtension->Px->SNTF.AsUlong);
	...
	if ((srbExtension->Flags & ATA_FLAGS_SENSEDATA_SET) == 0) {
	    ...
	    // 2.2.2 If this was marked as needing return data, fill in the return Task
	    // File.
    ...
    // 3.3.1 IO is completing, that IO may have paused the queue so unpause the queue
    //       During NCQ Error Recovery process, this flag will be reset in the error
    //       recovery completion process, either through reset or through slot release
    //       again.
    ...
    // 3.3.2 Complete the command
    if ((srbToRelease->SrbStatus != SRB_STATUS_SUCCESS) && !isSenseSrb &&
	(srbExtension->RetryCount == 0)) {
	if ((ChannelExtension->StateFlags.HybridInfoEnabledOnHiberFile == 1) &&
	    IsNCQWriteCommand(srbExtension)) {
	    // NCQ WRITE with Hybrid Information failed, retry once without Hybrid
	    // Information.
    ...
    //
    // There is a bad timing will break the error recovery and cause IO hang eventually,
    // typically happen in SSD but not limited to.
    ...
    //    In ReleaseSlottedCommand, ReadLogExtCommand may got chance to be retried, which
    //    will result in SingleIoSlice set to 1, and SingleIoSlice will skip the
    //    PreservedSettingCommand(the ReservedSlotInUse is already set).
    // 3. Then ReservedSlotInUse never got chance to be cleared because it relies on
    // PreservedSettingCommand execution.
    //    ReservedSlotInUse will block the queue activation, finally the IO will hang and
    //    AhciPortReset will be repeatedly issued by upper layer.
    //
    // Do not retry ReadLogExtCommand when NCQ error recovery in process, which makes
    // sense since ReadLogExtCommand is sense-like command as well.
    //
				    ...
				    _In_ BOOLEAN AtDIRQL)
{
    ...
    // 2. this function can be called from places that don't call ActivateQueue yet. Clear
    // the bit in IO Slices
    if ((ChannelExtension->SlotManager.HighPriorityAttribute &
...
NOTE:
    The caller of this routine should call ActiveQueue to program the command (from Srb
completion routine) to adapter
			 ...
			 _In_ BOOLEAN AtDIRQL)
{
    ...
    // 1. If Srb acquired active reference or has completion routine, put it in completion
    // queue. Otherwise, complete it.
	...
	if (AtDIRQL == FALSE) {
	    ...
	    AhciInterruptSpinlockAcquire(ChannelExtension->AdapterExtension,
					 ChannelExtension->PortNumber, &lockhandle);
	    AddQueue(ChannelExtension, &ChannelExtension->CompletionQueue, Srb,
		     0xDEADBEEF, 0x90);
	    AhciInterruptSpinlockRelease(ChannelExtension->AdapterExtension,
					 ChannelExtension->PortNumber, &lockhandle);
	} else {
	...
	if (LogExecuteFullDetail(ChannelExtension->AdapterExtension->LogFlags)) {
	    RecordExecutionHistory(ChannelExtension,
				   0x30000045); // Exit AhciCompleteRequest, SRB is put in
						// CompletionQueue
	}

	if (!IsDumpMode(ChannelExtension->AdapterExtension)) {
	    StorPortIssueDpc(ChannelExtension->AdapterExtension,
			     &ChannelExtension->CompletionDpc, ChannelExtension, Srb);
	} else {
	    AhciPortSrbCompletionDpcRoutine(&ChannelExtension->CompletionDpc,
	...
	if (IsMiniportInternalSrb(ChannelExtension, Srb)) {
	    ...
	    StorPortNotification(RequestComplete, ChannelExtension->AdapterExtension,
				 Srb);

	    if (LogExecuteFullDetail(ChannelExtension->AdapterExtension->LogFlags)) {
		RecordExecutionHistory(ChannelExtension,
				       0x10000045); // Exit AhciCompleteRequest, SRB from
						    // port driver
	    }
	}
    }
...
/*++
    Returns the next available slot that can be used for an ATA command in the QueueTag
field of the SRB It assumes:
...
getout:
	...
	// If this SRB is for Request Sense, make sure it is given the next chance to run
	// during ActivateQueue by not incrementing CCS.
	return;
    }

    ChannelExtension->CurrentCommandSlot++;
    if (ChannelExtension->CurrentCommandSlot ==
	(ChannelExtension->AdapterExtension->CAP.NCS + 1)) {
				 ...
				 _In_ UCHAR OldSectorCount, _In_ UCHAR NewSectorCount)
{
	...
	//
	if ((ChannelExtension->PersistentSettings.CommandParams[i].Features ==
	     OldFeatures) &&
	    ...
	    break;
	} else if ((ChannelExtension->PersistentSettings.CommandParams[i].Features ==
		    NewFeatures) &&
		   (ChannelExtension->PersistentSettings.CommandParams[i].SectorCount ==
		    NewSectorCount)) {
			      ...
			      _In_ BOOLEAN AtDIRQL)
{
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10010041); // RestorePreservedSettings process already
					    // started
	return;
    }
	...
	restorePreservedSettingsInProcess =
	    InterlockedBitTestAndSet((volatile long *)&ChannelExtension->PoFxPendingWork,
				     0); // RestorePreservedSettings is at bit 0

	if (restorePreservedSettingsInProcess == 1) {
	    // it's already in process, return from here
	    RecordExecutionHistory(ChannelExtension,
				   0x10020041); // RestorePreservedSettings already in
						// process
	    return;
	}
	...
	if (portIdle) {
	    ...
	    RecordExecutionHistory(ChannelExtension,
				   0x10030041); // RestorePreservedSettings will continue
						// in Unit Active
	    return;
	} else {
		...
		RecordExecutionHistory(ChannelExtension,
				       0x10040041); // RestorePreservedSettings already
						    // started in Unit Active
		return;
	    }
	}
    }
...
/*++
    Send each of the commands in the Init Command list and PersistentSettings structure,
one at a time using the local SRB It assumes: The local SRB is not currently in use Called
by: AhciDeviceInitialize
...
VOID AhciPortIssueInitCommands(PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
	...
	// the process has been started.
	RecordExecutionHistory(ChannelExtension,
			       0x10010042); // AhciPortIssueInitCommands process already
					    // started
	return;
    }
...
VOID AhciPortStartTelemetry(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
	...
	if (channelExtension != NULL) {
		...
		((portStartSuccess) ? (AhciTelemetryEventIdPortStartSuccess) :
				      (AhciTelemetryEventIdPortRunningStartFail)),
			      ...
			      _In_ ULONG Flags, _In_ BOOLEAN SuppressFailure)
{
	...
	if ((!SuppressFailure) || ((ChannelExtension->DeviceFailureThrottleFlag &
				    AHCI_BUS_CHANGE_WARNING_THROTTLE_MASK) == 0)) {
	    StorPortMarkDeviceFailed(
		ChannelExtension->AdapterExtension,
		(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
		Flags, L"Too many bus change");
	}

	if (SuppressFailure && ((ChannelExtension->DeviceFailureThrottleFlag &
				 AHCI_BUS_CHANGE_WARNING_THROTTLE_MASK) == 0)) {
	    ChannelExtension->DeviceFailureThrottleFlag |=
		AHCI_BUS_CHANGE_WARNING_THROTTLE_MASK;
	}
	...
	if ((!SuppressFailure) || ((ChannelExtension->DeviceFailureThrottleFlag &
				    AHCI_NCQ_ERROR_WARNING_THROTTLE_MASK) == 0)) {
	    StorPortMarkDeviceFailed(
		ChannelExtension->AdapterExtension,
		(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
		Flags, L"Too many NCQ error");
	}

	if (SuppressFailure && ((ChannelExtension->DeviceFailureThrottleFlag &
				 AHCI_NCQ_ERROR_WARNING_THROTTLE_MASK) == 0)) {
	    ChannelExtension->DeviceFailureThrottleFlag |=
		AHCI_NCQ_ERROR_WARNING_THROTTLE_MASK;
	}
	...
	if ((!SuppressFailure) || ((ChannelExtension->DeviceFailureThrottleFlag &
				    AHCI_NON_QUEUED_ERROR_WARNING_THROTTLE_MASK) == 0)) {
	    StorPortMarkDeviceFailed(
		ChannelExtension->AdapterExtension,
		(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
		Flags, L"Too many NonQueue error");
	}

	if (SuppressFailure && ((ChannelExtension->DeviceFailureThrottleFlag &
				 AHCI_NON_QUEUED_ERROR_WARNING_THROTTLE_MASK) == 0)) {
	    ChannelExtension->DeviceFailureThrottleFlag |=
		AHCI_NON_QUEUED_ERROR_WARNING_THROTTLE_MASK;
	}
	...
	if ((!SuppressFailure) || ((ChannelExtension->DeviceFailureThrottleFlag &
				    AHCI_DEVICE_STUCK_WARNING_THROTTLE_MASK) == 0)) {
	    StorPortMarkDeviceFailed(
		ChannelExtension->AdapterExtension,
		(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress), 0,
		L"Device may stuck");
	}
	...
	if (SuppressFailure && ((ChannelExtension->DeviceFailureThrottleFlag &
				 AHCI_DEVICE_STUCK_WARNING_THROTTLE_MASK) == 0)) {
	    ChannelExtension->DeviceFailureThrottleFlag |=
		AHCI_DEVICE_STUCK_WARNING_THROTTLE_MASK;
	}
