...
__inline VOID AhciZeroMemory(_Out_writes_(BufferSize) PCHAR Buffer, _In_ ULONG BufferSize)
{
    ULONG i;
...
__inline VOID AhciFillMemory(_Out_writes_(BufferSize) PCHAR Buffer, _In_ ULONG BufferSize,
			     _In_ CHAR Fill)
{
    ULONG i;
...
__inline VOID AhciCopyMemory(_Out_writes_(Length) PCHAR Destination,
			     _In_reads_(Length) PCHAR Source, _In_ ULONG Length)
{
    ULONG i;
...
__inline BOOLEAN IsPortValid(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension,
			     _In_ ULONG PathId)
{
    return ((PathId <= AdapterExtension->HighestPort) &&
...
__inline VOID MarkSrbToBeCompleted(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
    SETMASK(srbExtension->Flags, ATA_FLAGS_COMPLETE_SRB);
}

__inline BOOLEAN IsDataTransferNeeded(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
    return ((srbExtension->Flags & (ATA_FLAGS_DATA_IN | ATA_FLAGS_DATA_OUT)) ||
...
*/
__inline BOOLEAN IsDumpMode(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->DumpMode > 0);
}

__inline BOOLEAN IsDumpCrashMode(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->DumpMode == DUMP_MODE_CRASH);
}

__inline BOOLEAN IsDumpHiberMode(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->DumpMode == DUMP_MODE_HIBER);
}

__inline BOOLEAN IsDumpMarkMemoryMode(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->DumpMode == DUMP_MODE_MARK_MEMORY);
}

__inline BOOLEAN IsDumpResumeMode(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->DumpMode == DUMP_MODE_RESUME);
}

__inline BOOLEAN IsMsnEnabled(_In_ PAHCI_DEVICE_EXTENSION DeviceExtension)
{
...
--*/
__inline BOOLEAN IsNCQCommand(_In_ PAHCI_SRB_EXTENSION SrbExtension)
{
...
--*/
__inline BOOLEAN IsNcqReadWriteCommand(_In_ PAHCI_SRB_EXTENSION SrbExtension)
{
    UCHAR command = IDE_COMMAND_NOT_VALID;
...
--*/
__inline BOOLEAN IsNCQWriteCommand(_In_ PAHCI_SRB_EXTENSION SrbExtension)
{
    UCHAR command = IDE_COMMAND_NOT_VALID;
...
--*/
__inline BOOLEAN IsNormalCommand(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
...
--*/
__inline BOOLEAN IsReadWriteCommand(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
    UCHAR command = IDE_COMMAND_NOT_VALID;
    ...
    if ((command == IDE_COMMAND_READ_FPDMA_QUEUED) ||
	(command == IDE_COMMAND_WRITE_FPDMA_QUEUED) || (command == IDE_COMMAND_READ) ||
	(command == IDE_COMMAND_WRITE) || (command == IDE_COMMAND_READ_EXT) ||
	(command == IDE_COMMAND_WRITE_EXT) || (command == IDE_COMMAND_READ_DMA) ||
	(command == IDE_COMMAND_WRITE_DMA) || (command == IDE_COMMAND_READ_DMA_EXT) ||
...
__inline BOOLEAN NeedRequestSense(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
    ...
    return (IsAtapiCommand(srbExtension->AtaFunction) &&
	    ...
	    (Srb->SrbStatus == SRB_STATUS_ERROR) && (srbSenseBuffer != NULL) &&
	    (srbSenseBufferLength > 0));
}

__inline VOID SetSenseData(_In_ PSENSE_DATA SenseBuffer, _In_ UCHAR SenseKey,
			   _In_ UCHAR ASC, _In_ UCHAR ASCQ)
{
    SenseBuffer->ErrorCode = SCSI_SENSE_ERRORCODE_FIXED_CURRENT;
    SenseBuffer->Valid = 1;
    SenseBuffer->AdditionalSenseLength = sizeof(SENSE_DATA) -
...
BOOLEAN __inline IsDeviceSupportsTrim(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    // word 169: bit 0 -- Trim function is supported.
    return (ChannelExtension->DeviceExtension[0]
		.IdentifyDeviceData->DataSetManagementFeature.SupportsTrim == 1);
}

__inline BOOLEAN IsDeviceSupportsAN(_In_ PIDENTIFY_PACKET_DATA IdentifyPacketData)
{
    // This bit is actually from IDENTIFY_PACKET_DATA structure for ATAPI devices.
    return (IdentifyPacketData->SerialAtaFeaturesSupported.AsynchronousNotification ==
	    TRUE);
}

__inline BOOLEAN IsDeviceEnabledAN(_In_ PIDENTIFY_PACKET_DATA IdentifyPacketData)
{
    // This bit is actually from IDENTIFY_PACKET_DATA structure for ATAPI devices.
    return (IdentifyPacketData->SerialAtaFeaturesEnabled.AsynchronousNotification ==
	    TRUE);
}

__inline BOOLEAN IsExternalPort(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    if ((ChannelExtension->Px->CMD.HPCP) ||
	((ChannelExtension->AdapterExtension->CAP.SXS) &&
	 (ChannelExtension->Px->CMD.ESP)) ||
	((ChannelExtension->AdapterExtension->CAP.SMPS) &&
	 (ChannelExtension->Px->CMD.MPSP))) {
	// 1. Px->CMD.HPCP indicates that the port is hot-pluggable. (both signal and
	// power cable)
	// 2. CAP.SXS && Px->CMD.ESP indicates that it's an ESATA port. (only signal
	// cable)
	// 3. CAP.SMPS && Px->CMD.MPSP indicates that Mechanical Switch is implemented on
	// the port.
	return TRUE;
    }
...
__inline BOOLEAN IsLPMCapablePort(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    // if a port is marked eSATA port but not having mechanical switch nor supporting Cold
    // Presence Detection, don't turn on LPM on it.
    if ((ChannelExtension->Px->CMD.HPCP) ||
	((ChannelExtension->AdapterExtension->CAP.SXS) &&
	 (ChannelExtension->Px->CMD.ESP))) {
	if (((ChannelExtension->AdapterExtension->CAP.SMPS == 0) ||
	     (ChannelExtension->Px->CMD.MPSP == 0)) &&
...
__inline BOOLEAN IsDeviceHybridInfoSupported(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    if (IsAtaDevice(&ChannelExtension->DeviceExtension[0].DeviceParameters) &&
	(ChannelExtension->DeviceExtension[0]
	     .IdentifyDeviceData->SerialAtaFeaturesSupported.HybridInformation == 1)) {
	return TRUE;
    }
...
__inline BOOLEAN IsDeviceHybridInfoEnabled(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
...
__inline BOOLEAN
IsDeviceGeneralPurposeLoggingSupported(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
	...
	(ChannelExtension->DeviceExtension->IdentifyDeviceData->CommandSetSupport
	     .GpLogging == 1) &&
	(ChannelExtension->DeviceExtension->IdentifyDeviceData->CommandSetSupport.BigLba ==
	 1)) {
	return TRUE;
    }
...
__inline BOOLEAN DeviceIncursSeekPenalty(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    if (IsAtaDevice(&ChannelExtension->DeviceExtension->DeviceParameters)) {
	if (ChannelExtension->DeviceExtension->IdentifyDeviceData
		    ->NominalMediaRotationRate >= 0x0401 &&
	    ChannelExtension->DeviceExtension->IdentifyDeviceData
		    ->NominalMediaRotationRate <= 0xFFFE) {
	    //
	    // NominalMediaRotationRate = 0 means no rate is reported.
	    // NominalMediaRotationRate = 1 means there is no seek penalty (e.g. pure
	    // SSD). NominalMediaRotationRate = 0x0401-0xFFFE is the rotation rate (i.e.
	    // incurs seek penalty). All other values are reserved.
	    //
	    return TRUE;
	}
    }
...
__inline BOOLEAN DeviceIncursNoSeekPenalty(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    if (IsAtaDevice(&ChannelExtension->DeviceExtension->DeviceParameters)) {
	if (ChannelExtension->DeviceExtension->IdentifyDeviceData
		->NominalMediaRotationRate == 1) {
	    //
	    // NominalMediaRotationRate = 0 means no rate is reported.
	    // NominalMediaRotationRate = 1 means there is no seek penalty (e.g. pure
	    // SSD). NominalMediaRotationRate = 0x0401-0xFFFE is the rotation rate (i.e.
...
__inline BOOLEAN IsD3ColdAllowed(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
...
__inline BOOLEAN IsPortD3ColdEnabled(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
...
__inline BOOLEAN IsReceivingSystemPowerHints(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->SystemPowerHintState != RaidSystemPowerUnknown);
}

__inline BOOLEAN AdapterPoFxEnabled(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (AdapterExtension->StateFlags.PoFxEnabled == 1);
}

__inline BOOLEAN PortPoFxEnabled(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    return (ChannelExtension->StateFlags.PoFxEnabled == 1);
}

__inline BOOLEAN AdapterAcquireActiveReference(
    _In_ PAHCI_ADAPTER_EXTENSION AdapterExtension, _Inout_opt_ PBOOLEAN InIdle)
{
    BOOLEAN idle = FALSE;
	...
	ULONG status;
	status = StorPortPoFxActivateComponent(AdapterExtension, NULL, NULL, 0, 0);
...
__inline VOID AdapterReleaseActiveReference(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    if (AdapterPoFxEnabled(AdapterExtension)) {
	StorPortPoFxIdleComponent(AdapterExtension, NULL, NULL, 0, 0);
    }
}

__inline BOOLEAN PortAcquireActiveReference(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
...
__inline VOID PortReleaseActiveReference(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
					 _In_opt_ PSTORAGE_REQUEST_BLOCK Srb)
{
    if (PortPoFxEnabled(ChannelExtension)) {
	if (Srb != NULL) {
...
__inline BOOLEAN
DeviceIdentificationCompletedSuccess(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    ULONG i;

    //
    // Note: There is possibility that 1st inquiry/identify will fail, then identify
    // device
    //       data is invalid, but AtaDeviceType is initialized correctly during IO
    //       process. So it is also needed to check IdentifyDeviceSuccess flag to see
    //       whether identify command success.
    //

    for (i = 0; i <= AdapterExtension->HighestPort; i++) {
	if ((AdapterExtension->PortExtension[i] != NULL) &&
	    ((AdapterExtension->PortExtension[i]
		  ->DeviceExtension->DeviceParameters.AtaDeviceType == DeviceUnknown) ||
	     (AdapterExtension->PortExtension[i]->StateFlags.IdentifyDeviceSuccess ==
	      0))) {
	    return FALSE;
	}
    }
...
__inline ULONG RequestGetDataTransferLength(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
...
__inline VOID RequestSetDataTransferLength(_In_ PSTORAGE_REQUEST_BLOCK Srb,
					   _In_ ULONG Length)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
...
--*/
__inline UCHAR NumberOfSetBits(_In_ ULONG Value)
{
    //
...
/*++
    ...
    It grabs the adapter interrupt spinlock if it's not in InterruptMessagePerPort mode,
    or the caller asks for it by using 0xFFFFFFFF as MessageId value.

Return Value:
    None

--*/
__inline VOID AhciInterruptSpinlockAcquire(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension,
...
--*/
__inline VOID AhciInterruptSpinlockRelease(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension,
					   _In_ ULONG MessageId,
					   _In_ PSTOR_LOCK_HANDLE LockHandle)
{
    if ((AdapterExtension->StateFlags.InterruptMessagePerPort == 0) ||
...
VOID RecordExecutionHistory(PAHCI_CHANNEL_EXTENSION ChannelExtension, ULONG Function);

VOID RecordInterruptHistory(PAHCI_CHANNEL_EXTENSION ChannelExtension, ULONG PxIS,
			    ULONG PxSSTS, ULONG PxSERR, ULONG PxCI, ULONG PxSACT,
			    ULONG Function);

VOID Set_PxIE(PAHCI_CHANNEL_EXTENSION ChannelExtension, PAHCI_INTERRUPT_ENABLE IE);

__inline BOOLEAN IsFuaSupported(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    return (ChannelExtension->DeviceExtension->DeviceParameters.StateFlags.FuaSupported ==
	    1);
}

__inline BOOLEAN IsDeviceSupportsHIPM(_In_ PIDENTIFY_DEVICE_DATA IdentifyDeviceData)
{
    return (IdentifyDeviceData->SerialAtaCapabilities.HIPM == TRUE);
}

__inline BOOLEAN IsDeviceSupportsDIPM(_In_ PIDENTIFY_DEVICE_DATA IdentifyDeviceData)
{
    return (IdentifyDeviceData->SerialAtaFeaturesSupported.DIPM == TRUE);
}

__inline BOOLEAN NoLpmSupport(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    ...
    return (
	(ChannelExtension->AdapterExtension->CAP.SALP == 0) &&
	(!IsDeviceSupportsDIPM(ChannelExtension->DeviceExtension[0].IdentifyDeviceData)));
}

__inline BOOLEAN NeedToSetTransferMode(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    UNREFERENCED_PARAMETER(ChannelExtension); // make WDK sample build clean
    return FALSE;
}

__inline BOOLEAN IsSingleIoDevice(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    UNREFERENCED_PARAMETER(AdapterExtension); // make WDK sample build clean
    return FALSE;
}

__inline BOOLEAN AdapterResetInInit(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    UNREFERENCED_PARAMETER(AdapterExtension); // make WDK sample build clean
    return FALSE;
}

__inline BOOLEAN IgnoreHotPlug(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    return (ChannelExtension->StateFlags.IgnoreHotplugInterrupt == 1);
}

__inline BOOLEAN
AdapterNoNonQueuedErrorRecovery(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    UNREFERENCED_PARAMETER(AdapterExtension); // make WDK sample build clean
    return FALSE;
}

__inline BOOLEAN CloResetEnabled(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    UNREFERENCED_PARAMETER(AdapterExtension); // make WDK sample build clean
    return (AdapterExtension->CAP.SCLO == 1);
}

__inline BOOLEAN IsNCQSupported(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    if (IsAtaDevice(&ChannelExtension->DeviceExtension->DeviceParameters) &&
	...
	// Queue Depth is a 0 based value (i.e. 0x0 == 1). StorAHCI reserves slot 0 for
	// internal use.
	return TRUE;
    }
    return FALSE;
...
__inline BOOLEAN IsFirmwareUpdateSupported(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    BOOLEAN support;
...
__inline UCHAR GetFirmwareUpdateCommand(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    ...
    } else if (ChannelExtension->DeviceExtension->IdentifyDeviceData->CommandSetSupport
		   .DownloadMicrocode == 1) {
...
__inline ULONG GetOccupiedSlots(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
...
__inline BOOLEAN ErrorRecoveryIsPending(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
...
__inline BOOLEAN IsMiniportInternalSrb(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
...
__inline VOID PortClearPendingInterrupt(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
			       &ChannelExtension->Px->SERR.AsUlong, (ULONG)~0);
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
			       &ChannelExtension->Px->IS.AsUlong, (ULONG)~0);
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
			       ChannelExtension->AdapterExtension->IS,
			       (1 << ChannelExtension->PortNumber));
}

__inline BOOLEAN PartialToSlumberTransitionIsAllowed(
    _In_ PAHCI_CHANNEL_EXTENSION ChannelExtension, _In_opt_ PAHCI_COMMAND CMD)
{
    PAHCI_COMMAND cmdRegister = NULL;
    AHCI_COMMAND cmd = { 0 };

    if ((ChannelExtension->LastUserLpmPowerSetting & 0x3) == 0) {
	// Neither HIPM nor DIPM is allowed. e.g. LastUserLpmPowerSetting --- bit 0: HIPM;
	// bit 1: DIPM
	return FALSE;
    }
	...
	(ChannelExtension->DeviceExtension->IdentifyDeviceData->SerialAtaCapabilities
	     .DeviceAutoPS == 1) &&
	(ChannelExtension->DeviceExtension->IdentifyDeviceData->SerialAtaFeaturesEnabled
	     .DeviceAutoPS == 1)) {
    ...
    if (((ChannelExtension->AdapterExtension->CAP.SALP == 0) ||
	 (cmdRegister->ALPE == 0)) &&
	(!IsDeviceSupportsDIPM(ChannelExtension->DeviceExtension[0].IdentifyDeviceData))) {
    ...
    if ((ChannelExtension->AdapterExtension->CAP.SALP == 1) && (cmdRegister->ALPE != 0) &&
	((cmdRegister->ASP == 1) ||
	 ((ChannelExtension->AdapterExtension->CAP2.APST != 0) &&
	  (cmdRegister->APSTE != 0)))) {
	// HIPM is enabled. AND
...
__inline ULONG
GetHybridMaxLbaRangeCountForChangeLba(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    //
    // If CacheBehavior is supported, disk doesn't move data from primary medium to
    // caching medium when executes HybridChangeByLba command. Use 64 as default
    // MaxLbaRangeCountForChangeLba value. Otherwise, use 8 as default value.
    //
    if (ChannelExtension->DeviceExtension->HybridInfo.SupportedOptions
	    .SupportCacheBehavior == 1) {
	return 64;
    } else {
	return 8;
    }
...
__inline BOOLEAN
SupportsEnhancedNcqErrorRecovery(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    if ((ChannelExtension->DeviceExtension->SupportedGPLPages.SinglePage.NcqCommandError ==
	 1) &&
...
__inline BOOLEAN IsAtaPioDataInCmd(_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    PAHCI_SRB_EXTENSION srbExtension = GetSrbExtension(Srb);
    UCHAR command = IDE_COMMAND_NOT_VALID;
...
__inline VOID SetReturnRegisterValues(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
				      _In_ PSTORAGE_REQUEST_BLOCK Srb,
				      _In_opt_ PGP_LOG_NCQ_COMMAND_ERROR NcqErrorLog)
{
    ...
    if ((IsReturnResults(srbExtension->Flags) == FALSE) ||
	(srbExtension->ResultBuffer == NULL)) {
	return;
    }

    if ((cdb != NULL) && (cdb->CDB10.OperationCode == SCSIOP_ATA_PASSTHROUGH16)) {
	// for ATA PASS THROUGH 16 command, return Descriptor Format Sense Data, including
	// ATA Status Return info.
	if (srbExtension->ResultBufferLength >= sizeof(DESCRIPTOR_SENSE_DATA)) {
	    ...
	    // For PIO Data In command, if it is successful, then the device will not send
	    // RD2H FIS. Only update the ATA status/error without getting the stale copy
	    // of information in D2H FIS.
		...
		// - Device will report the ATA Status/Error register contents in the
		// E_Status/Error field of the PIO Setup FIS. For ATA status return
		// descriptor, refer to ATA Pass-Through specification 13.2.4.
		...
		ataStatus->LbaMid7_0 =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.CylLow;
		ataStatus->LbaHigh7_0 =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.CylHigh;
	...
	if (NcqErrorLog == NULL) {
	    returnTaskFile->Current.bCommandReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.Status;
	    returnTaskFile->Current.bFeaturesReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.Error;
	    returnTaskFile->Current.bCylHighReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.CylHigh;
	    returnTaskFile->Current.bCylLowReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.CylLow;
	    returnTaskFile->Current.bDriveHeadReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.Dev_Head;
	    returnTaskFile->Current.bSectorCountReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.SectorCount;
	    returnTaskFile->Current.bSectorNumberReg =
		ChannelExtension->ReceivedFIS->D2hRegisterFis.SectorNumber;
	    // if 48bit, get all of it
	    if (Is48BitCommand(srbExtension->Flags) &&
		(srbExtension->ResultBufferLength >= sizeof(ATA_TASK_FILE))) {
		returnTaskFile->Previous.bCommandReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.Status;
		returnTaskFile->Previous.bFeaturesReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.Error;
		returnTaskFile->Previous.bCylHighReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.CylHigh_Exp;
		returnTaskFile->Previous.bCylLowReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.CylLow_Exp;
		returnTaskFile->Previous.bDriveHeadReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.Dev_Head;
		returnTaskFile->Previous.bSectorCountReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.SectorCount_Exp;
		returnTaskFile->Previous.bSectorNumberReg =
		    ChannelExtension->ReceivedFIS->D2hRegisterFis.SectorNum_Exp;
	    }
	} else {
...
Exit:
    // set flag SRB_STATUS_AUTOSENSE_VALID so that Storport will copy it back to original
    // Sense Buffer
...
__inline BOOLEAN NeedsPuisSpinUpOnPowerUp(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    //
...
__inline STORAGE_DEVICE_FORM_FACTOR AtaFormFactorToStorageFormFactor(_In_ USHORT AtaFormFactor)
{
...
__inline BOOLEAN IsAdapterRemovable(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    return (BOOLEAN)AdapterExtension->StateFlags.Removable;
}

__inline BOOLEAN IsAdapterRemoved(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    //
    ...
    // the AHCI spec.
    //
    ULONG version = StorPortReadRegisterUlong(
	AdapterExtension, &AdapterExtension->ABAR_Address->VS.AsUlong);
...
PAHCI_MEMORY_REGISTERS GetABARAddress(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension,
...
VOID AhciCompleteRequest(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			 _In_ PSTORAGE_REQUEST_BLOCK Srb, _In_ BOOLEAN AtDIRQL);

BOOLEAN UpdateSetFeatureCommands(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
				 _In_ UCHAR OldFeatures, _In_ UCHAR NewFeatures,
				 _In_ UCHAR OldSectorCount, _In_ UCHAR NewSectorCount);

VOID GetAvailableSlot(PAHCI_CHANNEL_EXTENSION ChannelExtension,
		      PSTORAGE_REQUEST_BLOCK Srb);

VOID ReleaseSlottedCommand(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			   _In_ UCHAR SlotNumber, _In_ BOOLEAN AtDIRQL);

VOID RestorePreservedSettings(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			      _In_ BOOLEAN AtDIRQL);

VOID AhciPortIssueInitCommands(PAHCI_CHANNEL_EXTENSION ChannelExtension);

VOID AhciPortStartTelemetry(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension);

VOID AhciPortMarkDeviceFailed(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			      _In_ AHCI_DEVICE_FAILURE_REASON FailureReason,
			      _In_ ULONG Flags, _In_ BOOLEAN SuppressFailure);

BOOLEAN CompareId(_In_opt_ PCHAR DeviceId,
		  _In_ ULONG DeviceIdLength,
		  _In_opt_ PCHAR TargetId,
		  _In_ ULONG TargetIdLength,
		  _Inout_opt_ PULONG Value);

ULONG GetStringLength(_In_ PSTR String, _In_ ULONG MaxLength);

__inline VOID AhciUlongIncrement(_Inout_ PULONG Value)
{
    if (Value != NULL && *Value != MAXULONG) {
	(*Value)++;
    }
}

VOID FillAtaCommandErrorStruct(_Out_ PATA_COMMAND_ERROR DataBuffer,
			       _In_ PSTORAGE_REQUEST_BLOCK Srb,
			       _In_ PAHCI_CHANNEL_EXTENSION ChannelExtension);

FORCEINLINE VOID AhciTelemetryLog(_In_ PVOID HwDeviceExtension,
				  ...
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param0Name,
				  _In_ ULONGLONG Param0Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param1Name,
				  _In_ ULONGLONG Param1Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param2Name,
				  _In_ ULONGLONG Param2Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param3Name,
				  _In_ ULONGLONG Param3Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param4Name,
				  _In_ ULONGLONG Param4Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param5Name,
				  _In_ ULONGLONG Param5Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param6Name,
				  _In_ ULONGLONG Param6Value,
				  _In_reads_or_z_opt_(EVENT_MAX_PARAM_NAME_LEN) PSTR Param7Name,
				  _In_ ULONGLONG Param7Value)
{
    ...
    if (Param0Name != NULL) {
	StorPortCopyMemory(event.ParameterName0, Param0Name,
			   GetStringLength(Param0Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue0 = Param0Value;
    }

    if (Param1Name != NULL) {
	StorPortCopyMemory(event.ParameterName1, Param1Name,
			   GetStringLength(Param1Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue1 = Param1Value;
    }

    if (Param2Name != NULL) {
	StorPortCopyMemory(event.ParameterName2, Param2Name,
			   GetStringLength(Param2Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue2 = Param2Value;
    }

    if (Param3Name != NULL) {
	StorPortCopyMemory(event.ParameterName3, Param3Name,
			   GetStringLength(Param3Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue3 = Param3Value;
    }

    if (Param4Name != NULL) {
	StorPortCopyMemory(event.ParameterName4, Param4Name,
			   GetStringLength(Param4Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue4 = Param4Value;
    }

    if (Param5Name != NULL) {
	StorPortCopyMemory(event.ParameterName5, Param5Name,
			   GetStringLength(Param5Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue5 = Param5Value;
    }

    if (Param6Name != NULL) {
	StorPortCopyMemory(event.ParameterName6, Param6Name,
			   GetStringLength(Param6Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue6 = Param6Value;
    }

    if (Param7Name != NULL) {
	StorPortCopyMemory(event.ParameterName7, Param7Name,
			   GetStringLength(Param7Name, EVENT_MAX_PARAM_NAME_LEN));
	event.ParameterValue7 = Param7Value;
    }

    StorPortLogTelemetry(HwDeviceExtension, Address, &event);
}

FORCEINLINE VOID AhciTelemetryLogResetErrorRecovery(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
						    _In_opt_ PSTOR_ADDRESS Address,
						    _In_ ULONG EventId,
						    _In_reads_or_z_(EVENT_NAME_MAX_LENGTH) PSTR EventName,
						    _In_ ULONG Flags,
						    ...
						    _In_ ULONGLONG AdditionalParameterValue)
{
	...
	if (((Flags & AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING) != 0) ||
	    ...
	    AhciTelemetryLog(
		ChannelExtension->AdapterExtension, Address,
		AHCI_TELEMETRY_DRIVER_VERSION, EventId, EventName,
		AHCI_TELEMETRY_EVENT_VERSION, Flags, 0, NULL,
		...
		"StateFlags", *(ULONGLONG *)&(ChannelExtension->StateFlags), "StartState",
		*(ULONGLONG *)&(ChannelExtension->StartState), "TotalCountSurpriseRemove",
		ChannelExtension->TotalCountSurpriseRemove, AdditionalParameterName,
		AdditionalParameterValue);
	}
    }
}

FORCEINLINE VOID AhciTelemetryLogPowerSettingChange(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
						    _In_opt_ PSTOR_ADDRESS Address,
						    _In_ ULONG EventId,
						    _In_reads_or_z_(EVENT_NAME_MAX_LENGTH) PSTR EventName,
						    _In_ ULONG Flags,
...
{
    if (ChannelExtension != NULL) {
	AhciTelemetryLog(ChannelExtension->AdapterExtension, Address,
			 AHCI_TELEMETRY_DRIVER_VERSION, EventId, EventName,
			 AHCI_TELEMETRY_EVENT_VERSION, Flags, 0, NULL,
			 "PowerSettingChangeCount",
			 ChannelExtension->TotalCountPowerSettingNotification,
			 "StateFlags", *(ULONGLONG *)&(ChannelExtension->StateFlags),
			 "LastUserLpmPowerSetting",
			 ChannelExtension->LastUserLpmPowerSetting, "CAP",
			 ChannelExtension->AdapterExtension->CAP.AsUlong, "CAP2",
			 ChannelExtension->AdapterExtension->CAP2.AsUlong, "PxCMD",
			 StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
						   &ChannelExtension->Px->CMD.AsUlong),
			 "IPM", Ipm, "LpmMode|SlumberInterval", LpmMode);
    }
}

FORCEINLINE VOID AhciTelemetryLogPortStart(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
					   _In_opt_ PSTOR_ADDRESS Address,
					   _In_ ULONG EventId,
					   _In_reads_or_z_(EVENT_NAME_MAX_LENGTH) PSTR EventName,
					   _In_ ULONG Flags,
					   ...
					   _In_ ULONGLONG AdditionalParameterValue)
{
	...
	if (((Flags && AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING) != 0) ||
		...
		ChannelExtension->DeviceExtension[0].IoRecord.PortDriverResetCount,
		"StateFlags", *(ULONGLONG *)&(ChannelExtension->StateFlags), "StartState",
		*(ULONGLONG *)&(ChannelExtension->StartState), "PortStartTime(ms)",
		ChannelExtension->TotalPortStartTime, AdditionalParameterName,
		AdditionalParameterValue);
	}
    }
}
