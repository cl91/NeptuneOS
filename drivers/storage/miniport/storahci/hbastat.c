...
__inline VOID ForceGenxSpeed(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			     _In_ ULONG GenNumber)
{
    ...
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension,
			       &ChannelExtension->Px->SERR.AsUlong, serr.AsUlong);

    return;
}

/*
This function brings the HBA and all ports to the H:Idle and P:Idle states by way of HBA
Reset and P:Reset
...
*/
BOOLEAN AhciAdapterReset(_In_ PAHCI_ADAPTER_EXTENSION AdapterExtension)
{
    ...
    // 10.4.3    HBA Reset
    // If the HBA becomes unusable for multiple ports, and a software reset or port reset
    // does not correct the problem, software may reset the entire HBA by setting GHC.HR
    // to ‘1’.  When software sets the GHC.HR bit to ‘1’, the HBA shall perform an
    // internal reset action.
    ghc.HR = 1;
    StorPortWriteRegisterUlong(AdapterExtension, &abar->GHC.AsUlong, ghc.AsUlong);

    // The bit shall be cleared to ‘0’ by the HBA when the reset is complete.  A software
    // write of ‘0’ to GHC.HR shall have no effect.  To perform the HBA reset, software
    // sets GHC.HR to ‘1’ and may poll until this bit is read to be ‘0’, at which point
    // software knows that the HBA reset has completed.
    ...
    // If the HBA has not cleared GHC.HR to ‘0’ within 1 second of software setting GHC.HR
    // to ‘1’, the HBA is in a hung or locked state.
    if (i == 50) {
	AdapterExtension->ErrorFlags = (1 << 29);
	return FALSE;
    }

    // When GHC.HR is set to ‘1’, GHC.AE, GHC.IE, the IS register, and all port register
    // fields (except PxFB/PxFBU/PxCLB/PxCLBU) that are not HwInit in the HBA’s register
    // memory space are reset.  The HBA’s configuration space and all other global
    // registers/bits are not affected by setting GHC.HR to ‘1’.  Any HwInit bits in the
    // port specific registers are not affected by setting GHC.HR to ‘1’.  The port
    // specific registers PxFB, PxFBU, PxCLB, and PxCLBU are not affected by setting
    // GHC.HR to ‘1’.  If the HBA supports staggered spin-up, the PxCMD.SUD bit will be
    // reset to ‘0’; software is responsible for setting the PxCMD.SUD and PxSCTL.DET
    // fields appropriately such that communication can be established on the Serial ATA
    // link.  If the HBA does not support staggered spin-up, the HBA reset shall cause a
    // COMRESET to be sent on the port
...
PHY Reset:COMRESET
    ...
    interface being reset and communications reinitialized.  While this field is 1h, COMRESET
    is transmitted on the interface. Software should leave the DET field set to 1h for a
    minimum of 1 millisecond to ensure that a COMRESET is sent on the interface. since we are
    in 5.3.2.3    P:NotRunning and PxCMD.SUD = ‘0’ does this still take us to P:StartComm?
Called By:
...
It performs:
    ...
    (details)
    1.1 make sure ST is 0.  DET cannot be altered while ST == 1 as per AHCI 1.1
        section 5.3.2.3.
    1.2 Don't allow a comm init to trigger a hotplug
    1.3 Ignore Hotplug events until the channel is started again
    2.1 Perform COMRESET 2.2 Wait for DET to be set
    3.1 Clear SERR
...
Return Values:
    none
*/
VOID AhciCOMRESET(PAHCI_CHANNEL_EXTENSION ChannelExtension, PAHCI_PORT Px)
{
    AHCI_SERIAL_ATA_CONTROL sctl;
    AHCI_COMMAND cmd;
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10fa0010); // AhciCOMRESET, PxCMD.ST is 1. Abort
	return;
    }

    // 1.2 Don't allow a COMINIT to trigger a hotplug
    ieTemp.AsUlong = ieOrig.AsUlong =
	StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &Px->IE.AsUlong);
    ieTemp.PRCE = 0; // "PhyRdy Change Interrupt Enable", no generate interrupt with
		     // PxIS.PRCS
    ieTemp.PCE = 0; // "Port Change Interrupt Enable", no generate interrupt with PxIS.PCS
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &Px->IE.AsUlong,
			       ieTemp.AsUlong);
    ...
    sctl.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					     &Px->SCTL.AsUlong);
    sctl.DET = 1;
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &Px->SCTL.AsUlong,
			       sctl.AsUlong);
    // DET=1 Performs interface communication initialization sequence to establish
    // communication. This is functionally equivalent to a hard reset and results in the
    // interface being reset and communications reinitialized.  While this field is 1h,
    // COMRESET is transmitted on the interface. Software should leave the DET field set
    // to 1h for a minimum of 1 millisecond to ensure that a COMRESET is sent on the
    // interface.
    ...
    //  communication to be re-established as indicated by bit 0 of PxSSTS.DET being set
    //  to ‘1’. typically, it will be done in 10ms. max wait 30ms to be safe
    ...
    // 3.1 Clear SERR
    //  AHCI 10.4.2 software should write all 1s to the PxSERR register to clear any bits
    //  that were set as part of the port reset.
    StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &Px->SERR.AsUlong,
			       (ULONG)~0);
...
*/
BOOLEAN P_NotRunning(PAHCI_CHANNEL_EXTENSION ChannelExtension, PAHCI_PORT Px)
{
    ...
    supportsCLO = CloResetEnabled(adapterExtension); // 3.1.1 make sure HBA supports
						     // Command List Override.

    if (ChannelExtension->StartState.ChannelNextStartState == Stopped) {
	RecordExecutionHistory(ChannelExtension, 0x00030011); // P_NotRunning, already
    ...
    if (supportsCLO) {
	RecordExecutionHistory(ChannelExtension,
			       0x10810011); // P_NotRunning, CLO is enabled.
    } else {
	RecordExecutionHistory(ChannelExtension,
			       0x00000011); // P_NotRunning, CLO isn't enabled.
    }
    ...
    // for PxCMD.CR to return ‘0’ when read.
    cmd.ST = 0;
	...
	if (i == 101) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10820011); // P_NotRunning, After 500ms of writing 0
						// to PxCMD.ST, it's still 1.
	}

	tfd.AsUlong = StorPortReadRegisterUlong(adapterExtension,
						&ChannelExtension->Px->TFD.AsUlong);
	if (tfd.STS.BSY) {
	    // AHCI 3.3.7 Command List Override (CLO):  Setting this bit to ‘1’ causes
	    // PxTFD.STS.BSY and PxTFD.STS.DRQ to be cleared to ‘0’. This allows a
	    // software reset to be transmitted to the device regardless of whether the
	    // BSY and DRQ bits are still set in the PxTFD.STS register.
    ...
    // AHCI 10.4.2 If PxCMD.CR or PxCMD.FR do not clear to ‘0’ correctly, then software
    // may attempt a port reset or a full HBA reset to recover.
    if (i == 101) {
	RecordExecutionHistory(ChannelExtension,
			       0x10350011); // P_NotRunning, PxCMD.CR or PxCMD.FR do not
					    // clear to ‘0’ correctly
	return FALSE;
    }
    ...
    if (i == 101) {
	RecordExecutionHistory(ChannelExtension,
			       0x10360011); // P_NotRunning, CI does not clear to ‘0’
	return FALSE;
    }
    ...
    cmd.AsUlong = StorPortReadRegisterUlong(adapterExtension, &Px->CMD.AsUlong);
    if ((cmd.FRE | cmd.FR) != 0) {
	// If PxCMD.FRE is set to ‘1’, software should clear it to ‘0’ and wait at least
	// 500 milliseconds for PxCMD.FR to return ‘0’ when read. AHCI 10.3.2 Software
	// shall not clear this bit while PxCMD.ST or PxCMD.CR is set to ‘1’.
	cmd.FRE = 0;
	StorPortWriteRegisterUlong(adapterExtension, &Px->CMD.AsUlong, cmd.AsUlong);
	...
	if (supportsCLO) {
		...
		cmd.CLO = 1;
		StorPortWriteRegisterUlong(adapterExtension, &Px->CMD.AsUlong,
					   cmd.AsUlong);
	    }
	}
	...
	if (i == 101) { // If PxCMD.CR or PxCMD.FR do not clear to ‘0’ correctly, then
			// software may attempt a port reset or a full HBA reset to
			// recover.
	    if (supportsCLO) {
		RecordExecutionHistory(ChannelExtension,
				       0x10380011); // P_NotRunning, PxCMD.CR or PxCMD.FR
						    // do not clear to ‘0’, CLO enabled
	    } else {
		RecordExecutionHistory(ChannelExtension,
				       0x10370011); // P_NotRunning, PxCMD.CR or PxCMD.FR
						    // do not clear to ‘0’, CLO not
						    // enabled
	    }
	    return FALSE;
	}
    ...
    //  AHCI 3.3.7 Software must wait for CLO to be cleared to ‘0’ before setting PxCMD.ST
    //  to ‘1’. register bit CLO might be set in this function, and it should have been
	...
	if (i == 101) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10390011); // P_NotRunning, PxCMD.CLO not clear to
						// ‘0’, CLO enabled
	    return FALSE;
	}
    }
    ...
    RecordExecutionHistory(ChannelExtension,
			   0x10000011); // Exit P_NotRunning, port stopped
...
VOID RunNextPort(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension, _In_ BOOLEAN AtDIRQL)
{
	...
	if ((nextPortExtension->StartState.ChannelNextStartState == 0) ||
	    ...
	    // RunNextPort() is only used in AhciAdapterRunAllPorts() which is called by
	    // AhciHwFindAdapter. In other cases, P_Running_StartAttempt() will be called
	...
	//
	// Note: If we are at DIRQL when get here, it implies previous port start finishes
	// quickly since no
	//       timer scheduled during port start. Then there is no need to log the port
	//       start time.
	//
	if (!AtDIRQL) {
...
Called By:
    ...
    Through StorPortRequestTimer which is set up by each of the Start Channel state
    functions: P_Running_WaitOnDET P_Running_WaitWhileDET1 P_Running_WaitOnDET3
	    P_Running_WaitOnFRE
...
It performs:
    ...
    1.4 Check to make sure that FR and CR are both 0.  Attempt to bring the controller
    ...
    2.1 Dispatch to the current state (states are responsible for selecting the next
        state)
...
*/
BOOLEAN P_Running(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		  _In_ BOOLEAN TimerCallbackProcess)
{
	...
	(ChannelExtension->StartState.DirectStartInProcess == 1)) {
	RecordExecutionHistory(ChannelExtension,
			       0x10130015); // This is timer callback and a direct port
					    // start process has been started separately,
					    // bail out this one.
	RunNextPort(ChannelExtension,
		    (!TimerCallbackProcess && ChannelExtension->StartState.AtDIRQL));
	return FALSE;
    ...
    // 1.4 Check to make sure that FR and CR are both 0.  If not then ST and/or FRE are 0
    // which a bad scenario.
	...
	if (!P_NotRunning(ChannelExtension, ChannelExtension->Px)) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10880015); // CR or FR 1 when ST or FRE 0
	    AhciCOMRESET(ChannelExtension,
			 ChannelExtension->Px); // Issue COMRESET to recover
		...
		RecordExecutionHistory(ChannelExtension,
				       0x10150014); // cmd.FR or cmd.CR cannot be stopped.
		return FALSE;
	    } else {
    ...
    // 1.5 CMD.ST has to be set, when that happens check to see that PxSSTS.DET is not
    // ‘4h’
	...
	break;
    default:
	// Above cases cover all intermediate states. If a port start process has been
	// completed, just exit.
	break;
    }
...
Affected Variables/Registers:
    none
*/
VOID P_Running_WaitOnDET(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			 _In_ BOOLEAN TimerCallbackProcess)
{
    AHCI_SERIAL_ATA_STATUS ssts;
	...
	// if D3 Cold is enabled, a device was there; Add additional wait time to make
	// sure device is able to get ready after a possible power on.
...
WaitOnDET_Start:
    ...
    // 1 Look for signs of life while DET == 0
    ssts.AsUlong = StorPortReadRegisterUlong(adapterExtension,
					     &ChannelExtension->Px->SSTS.AsUlong);

    if (ssts.DET == 0) { // When a COMRESET is sent to the device the PxSSTS.DET field
			 // shall be cleared to 0h.

	// 1.1 When looking for device presence, seeing link level power management shall
	...
	if ((ssts.IPM == 0x2) || (ssts.IPM == 0x6) || (ssts.IPM == 0x8)) {
	    ...
	    RecordExecutionHistory(ChannelExtension,
				   0x10800016); // P_Running_WaitOnDET is 0 w/ LPM
						// activity, goto FRE
	    P_Running_WaitOnFRE(ChannelExtension, TimerCallbackProcess);
	    return;
	    // 1.3 After 30ms give up (spec requires 10ms, use 30ms for safety). There is
	    // no device connected. from SATA-IO spec 3.0 - 15.2.2.2: If a device is
	    // present, the Phy shall detect device presence within 10 ms of a power-on
	    // reset (i.e. COMINIT shall be returned within 10 ms of an issued COMRESET).
	} else if (ChannelExtension->StartState.ChannelStateDETCount > waitMaxCount) {
	    P_Running_StartFailed(ChannelExtension, TimerCallbackProcess);
	    RecordExecutionHistory(ChannelExtension,
				   0x10ff0016); // P_Running_WaitOnDET Timed out, No
						// Device
	    return;
	    ...
	    RecordExecutionHistory(ChannelExtension,
				   0x10810016); // P_Running_WaitOnDET is 0, still waiting
		...
		ULONG status = StorPortRequestTimer(adapterExtension,
						    ...
						    P_Running_Callback, ChannelExtension,
	...
	// 2 Look for signs of life while DET == 1
	if (ssts.DET == 1) {
	    // 2.1 When looking for device presence, seeing link level power management
	    // shall count as device present as per 10.3.1
	    if ((ssts.IPM == 0x2) || (ssts.IPM == 0x6) || (ssts.IPM == 0x8)) {
		ChannelExtension->StartState.ChannelNextStartState = WaitOnFRE;
		RecordExecutionHistory(ChannelExtension,
				       0x10a00016); // P_Running_WaitOnDET is 1 w/ LPM
						    // activity, goto FRE
		P_Running_WaitOnFRE(ChannelExtension, TimerCallbackProcess);
		return;
		// 2.2 Otherwise continue on to WaitOnDET1
		...
		RecordExecutionHistory(ChannelExtension,
				       0x10a10016); // P_Running_WaitOnDET is 1
		P_Running_WaitWhileDET1(ChannelExtension, TimerCallbackProcess);
		return;
	    }
	    // 3.1 If DET==3 we are ready for WaitOnFRE already.
	    ...
	    ChannelExtension->StartState.ChannelNextStartState = WaitOnFRE;
	    RecordExecutionHistory(ChannelExtension,
				   0x10a20016); // P_Running_WaitOnDET is 3
	    P_Running_WaitOnFRE(ChannelExtension, TimerCallbackProcess);
	    return;
	    ...
	    P_Running_StartFailed(ChannelExtension, TimerCallbackProcess);
	    RecordExecutionHistory(ChannelExtension,
				   0x10a30016); // P_Running_WaitOnDET is Bogus, aborting
	    return;
	}
    }
}
...
It performs:
    ...
    2.2 After 1 second of waiting, force the controller to 150MB/s speeds and go to
        WaitOnDET3 Affected Variables/Registers: SCTL, SERR
*/
VOID P_Running_WaitWhileDET1(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			     _In_ BOOLEAN TimerCallbackProcess)
{
    AHCI_SERIAL_ATA_STATUS ssts;
...
WaitWhileDET1_Start:
	...
	ChannelExtension->StartState.ChannelNextStartState = WaitOnFRE;
	RecordExecutionHistory(ChannelExtension,
			       0x10a20017); // P_Running_WaitWhileDET1 done
	P_Running_WaitOnFRE(ChannelExtension, TimerCallbackProcess);
	return;
	...
	if (ChannelExtension->StartState.ChannelStateDET1Count > 100) {
	    ...
	    RecordExecutionHistory(ChannelExtension,
				   0x100a0017); // P_Running_WaitWhileDET1 timed out,
						// speed stepping down
	    if (IsDumpMode(adapterExtension)) {
		StorPortStallExecution(10000); // 10 milliseconds
		P_Running_WaitOnDET3(ChannelExtension, TimerCallbackProcess);
		...
		ULONG status;
		status = StorPortRequestTimer(adapterExtension,
					      ChannelExtension->StartPortTimer,
					      P_Running_Callback, ChannelExtension, 10000,
					      0); // 10 milliseconds
		if ((status != STOR_STATUS_SUCCESS) && (status != STOR_STATUS_BUSY)) {
		    StorPortStallExecution(10000); // 10 milliseconds
		    P_Running_WaitOnDET3(ChannelExtension, TimerCallbackProcess);
	    ...
	    ChannelExtension->StartState.ChannelStateDET1Count++;
	    RecordExecutionHistory(ChannelExtension,
				   0x10600017); // P_Running_WaitWhileDET1 still waiting
	    if (IsDumpMode(adapterExtension)) {
		StorPortStallExecution(10000); // 10 milliseconds
...
Affected Variables/Registers:
    none
*/
VOID P_Running_WaitOnDET3(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			  _In_ BOOLEAN TimerCallbackProcess)
{
    AHCI_SERIAL_ATA_STATUS ssts;
    PAHCI_ADAPTER_EXTENSION adapterExtension = ChannelExtension->AdapterExtension;
...
WaitOnDET3_Start:
	...
	if (ChannelExtension->StartState.ChannelStateDET3Count > 100) {
	    P_Running_StartFailed(ChannelExtension, TimerCallbackProcess);
	    RecordExecutionHistory(ChannelExtension,
				   0x10ff0018); // P_Running_WaitOnDET3 timed out
	    return;
	    // 1.1 Wait for 1 second until DET becomes 3
	} else {
	    ChannelExtension->StartState.ChannelStateDET3Count++;
	    RecordExecutionHistory(ChannelExtension,
				   0x10080018); // P_Running_WaitOnDET3 still waiting
	    if (IsDumpMode(adapterExtension)) {
		StorPortStallExecution(10000); // 10 milliseconds
	...
	ChannelExtension->StartState.ChannelNextStartState = WaitOnFRE;
	RecordExecutionHistory(ChannelExtension,
			       0x100a0018); // P_Running_WaitOnDET3 Success
	P_Running_WaitOnFRE(ChannelExtension, TimerCallbackProcess);
	return;
    }
}
...
Affected Variables/Registers:
    CMD
*/
VOID P_Running_WaitOnFRE(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			 _In_ BOOLEAN TimerCallbackProcess)
{
    AHCI_COMMAND cmd;
...
WaitOnFRE_Start:
    ...
    // There is a device connected when we get here. Trace how long the port need to set
    // DET from 0 to 1.
    ...
    cmd.FRE = 1;
    StorPortWriteRegisterUlong(adapterExtension, &ChannelExtension->Px->CMD.AsUlong,
			       cmd.AsUlong);
	...
	RecordExecutionHistory(ChannelExtension,
			       0x000a0019); // P_Running_WaitOnFRE Success
	P_Running_WaitOnBSYDRQ(ChannelExtension, TimerCallbackProcess);
	return;
	...
	if (ChannelExtension->StartState.ChannelStateFRECount > 5) {
	    ChannelExtension->StartState.ChannelNextStartState = WaitOnBSYDRQ;
	    RecordExecutionHistory(ChannelExtension,
				   0x00ff0019); // P_Running_WaitOnFRE timed out
	    P_Running_WaitOnBSYDRQ(ChannelExtension, TimerCallbackProcess);
	    return;
	    // 2.1 Wait for 50ms for FR to reflect running status
	} else {
	    ChannelExtension->StartState.ChannelStateFRECount++;
	    RecordExecutionHistory(ChannelExtension,
				   0x00080019); // P_Running_WaitOnFRE still waiting
	    if (IsDumpMode(adapterExtension)) {
		StorPortStallExecution(10000); // 10 milliseconds
...
*/
VOID P_Running_WaitOnBSYDRQ(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			    _In_ BOOLEAN TimerCallbackProcess)
{
    AHCI_COMMAND cmd;
    AHCI_TASK_FILE_DATA tfd;
...
WaitOnBSYDRQ_Start:
	...
	// When [serr.DIAG.X is] set to one this bit indicates a COMINIT signal was
	// received.  This bit is reflected in the P0IS.PCS bit. to allow the TFD to be
	// updated serr.DIAG.X must be cleared.
	serr.AsUlong = 0;
	serr.DIAG.X = 1;
	StorPortWriteRegisterUlong(adapterExtension, &ChannelExtension->Px->SERR.AsUlong,
				   serr.AsUlong);
    }
    // 3.1 Set ST to 1
	...
	if (TimerCallbackProcess &&
	    (ChannelExtension->StartState.DirectStartInProcess == 1)) {
	    // This is timer callback and a direct port start process has been started
	    // separately, bail out this one.
	    RunNextPort(ChannelExtension,
			(!TimerCallbackProcess && ChannelExtension->StartState.AtDIRQL));
	    return;
	}

	needSpinLock = TimerCallbackProcess ||
		       (ChannelExtension->StartState.AtDIRQL == 0);
	...
	//
	// Note: If ChannelStateBSYDRQCount is larger than 50, which implies the other
	// counts are all cleared to 0 according to port start state machine
	// implementation. But typically WaitOnBSYDRQ is always the most significant time
	// consuming phase of port start, so the total port start time here is still able
	// to reflect the rough port start status.
	//
	ChannelExtension->TotalPortStartTime =
	    (ULONG)(ChannelExtension->StartState.ChannelStateDETCount +
		    ChannelExtension->StartState.ChannelStateDET1Count +
		    ChannelExtension->StartState.ChannelStateDET3Count +
		    ChannelExtension->StartState.ChannelStateFRECount +
	...
	RecordExecutionHistory(ChannelExtension,
			       0x100a001a); // Exit P_Running_WaitOnBSYDRQ Succeeded
	...
	if (ChannelExtension->StateFlags.NeedQDR) {
	    StorPortIssueDpc(ChannelExtension->AdapterExtension,
			     &(ChannelExtension->BusChangeDpc), ChannelExtension, NULL);
	...
	// 2.3 After waiting for the remainder of the 60 second maximum Channel Start time
	// for BSY and DRQ to clear, give up Some big HDDs takes close to 20 second to
	// spin up
	...
	if (totalstarttime > portStartTimeoutIn10MS) {
	    P_Running_StartFailed(ChannelExtension, TimerCallbackProcess);
	    RecordExecutionHistory(ChannelExtension,
				   0x00ff001a); // P_Running_WaitOnBSYDRQ timed out
	    return;
		...
		// This is taking way too long and the controller is not responding
		// properly. Abort the start.
		P_Running_StartFailed(ChannelExtension, TimerCallbackProcess);
		RecordExecutionHistory(ChannelExtension,
				       0x00fc001a); // P_Running_WaitOnBSYDRQ Stop Failed
						    // on COMRESET
		return;
	    }
	    ...
	    // Set the timers for time remaining.  This is a best case scenario and the
	    // best that can be offered.
	    ChannelExtension->StartState.ChannelStateDETCount = 0;
	    ChannelExtension->StartState.ChannelStateDET1Count = 0;
	    ChannelExtension->StartState.ChannelStateDET3Count = 0; // won't exceed 100 *
								    // 10ms for 1 second
	    ChannelExtension->StartState.ChannelStateFRECount = 0; // won't exceed 5 *
								   // 10ms for 50 ms
	    ChannelExtension->StartState.ChannelStateBSYDRQCount = 51; // won't exceed
								       // 3000 * 20ms for
								       // 60 seconds minus
								       // what DET3 and
								       // FRE use.
	    ...
	    ChannelExtension->StartState.ChannelNextStartState = WaitOnDET3;
	    RecordExecutionHistory(ChannelExtension,
				   0x00fd001a); // P_Running_WaitOnBSYDRQ crossing 1
						// second.  COMRESET done, going to
						// WaitOnDET3
	    P_Running_WaitOnDET3(ChannelExtension, TimerCallbackProcess);
	    return;
	    ...
	    ChannelExtension->StartState.ChannelStateBSYDRQCount++;
	    RecordExecutionHistory(ChannelExtension,
				   0x0008001a); // P_Running_WaitOnBSYDRQ still waiting
	    if (IsDumpMode(adapterExtension)) {
		StorPortStallExecution(20000); // 20 milliseconds
		...
		status = StorPortRequestTimer(adapterExtension,
					      ChannelExtension->StartPortTimer,
					      P_Running_Callback, ChannelExtension, 20000,
					      0); // 10 milliseconds
		if ((status != STOR_STATUS_SUCCESS) && (status != STOR_STATUS_BUSY)) {
		    StorPortStallExecution(20000); // 20 milliseconds
		    goto WaitOnBSYDRQ_Start;
			   ...
			   _In_ BOOLEAN TimerCallbackProcess)
{
    ...
    //
    // Note: If ChannelStateBSYDRQCount is larger than 50, which implies the other counts
    // are all cleared to 0 according to port start state machine implementation. But
    // typically WaitOnBSYDRQ is always the most significant time consuming phase of port
    // start, so the total port start time here is still able to reflect the rough port
    // start status.
    //
    ChannelExtension->TotalPortStartTime =
	(ULONG)(ChannelExtension->StartState.ChannelStateDETCount +
    ...
    ChannelExtension->StateFlags.IgnoreHotplugInterrupt = FALSE;
    ChannelExtension->StateFlags.D3ColdEnabled = FALSE; // this flag may be set to "TRUE"
							// for last connected device,
							// clear it when no device
							// connected.
	...
	AhciZeroMemory((PCHAR)&ChannelExtension->PersistentSettings,
		       sizeof(PERSISTENT_SETTINGS));
    }
    ...
    // 2.1 Call AhciPortFailAllIos to complete all outstanding commands now that
    // ChannelNextStartState is StartFailed
...
/*
  ...
  "The flow for system software to recover from an error when non-queued commands are issued
  is as follows: Reads PxCI to see which commands are still outstanding Reads PxCMD.CCS to
  determine the slot that the HBA was processing when the error occurred Clears PxCMD.ST to
  ‘0’ to reset the PxCI register, waits for PxCMD.CR to clear to ‘0’ Clears any error bits
  in PxSERR to enable capturing new errors. Clears status bits in PxIS as appropriate If
  PxTFD.STS.BSY or PxTFD.STS.DRQ is set to ‘1’, issue a COMRESET to the device to put it in
  an idle state Sets PxCMD.ST to ‘1’ to enable issuing new commands
...
It performs:
    ...
    1.3 Reads PxCMD.CCS to determine the slot that the HBA was processing when the error
        occurred
    1.4.1 Clears PxCMD.ST to ‘0’ to reset the PxCI register, waits for PxCMD.CR to
        clear to ‘0’
     1.5 Clears any error bits in PxSERR to enable capturing new errors.
     1.6 Clears status bits in PxIS as appropriate
    1.4.2 If PxTFD.STS.BSY or PxTFD.STS.DRQ is set to ‘1’, issue a COMRESET to the device
...
Affected Variables/Registers:
    ...
    Channel Extension
*/
VOID AhciNonQueuedErrorRecovery(PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
	...
	AhciTelemetryEventIdNonqueuedErrorRecovery, "AhciNonQueuedErrorRecovery", 0, NULL,
	0);
    ...
    // 1.3 Reads PxCMD.CCS to determine the slot that the HBA was processing when the
    // error occurred
    cmd.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					    &ChannelExtension->Px->CMD.AsUlong);

    // 1.4 Clears PxCMD.ST to ‘0’ to reset the PxCI register, waits for PxCMD.CR to clear
    // to ‘0’
    if (!P_NotRunning(ChannelExtension, ChannelExtension->Px)) { // This clears PxCI
	RecordExecutionHistory(ChannelExtension,
			       0x10160013); // AhciNonQueuedErrorRecovery, Port Stop
					    // Failed
	...
	// if there is only 1 command, failing commands will be 1 greater than that slot
	...
	if (ChannelExtension->Slot[failingCommand].Srb == NULL) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10170013); // AhciNonQueuedErrorRecovery, didn't find
						// the expected command from slot.
	    AhciPortReset(ChannelExtension, FALSE);
	    return;
	}
	// If NonQueuedErrorRecovery should not be done for more than 1 command stop here.
    } else if ((cmd.ST == 1) &&
	       AdapterNoNonQueuedErrorRecovery(ChannelExtension->AdapterExtension)) {
	RecordExecutionHistory(ChannelExtension,
			       0x10180013); // AhciNonQueuedErrorRecovery,
					    // NCQ_NeverNonQueuedErrorRecovery
	AhciPortReset(ChannelExtension, FALSE);
	return;
    } else {
	// otherwise the failing command is in CCS, as long as there is an SRB in that
	// command slot.
	if (((ChannelExtension->SlotManager.CommandsIssued & (1 << cmd.CCS)) > 0) &&
	    ...
	    RecordExecutionHistory(ChannelExtension,
				   0x10190013); // AhciNonQueuedErrorRecovery, command to
						// be failed is not indicated by PxCMD.CCS
	    AhciPortReset(ChannelExtension, FALSE);
	    return;
	}
    }

    // Keep in mind although Px.CMD.ST was just shut off, we are working with the previous
    // snapshot of ST.
    if (cmd.ST == 1) {
	...
	// Check whether the failed command is paging IO, if so, return busy status to
	...
	if (NeedRequestSense(ChannelExtension->Slot[failingCommand].Srb)) {
	    ...
	    senseSrb = BuildRequestSenseSrb(
		ChannelExtension,
		ChannelExtension->Slot[failingCommand].Srb); // it will be programmed to
							     // slot later in this
							     // routine.
	    if (senseSrb != NULL) {
		// Make the slot available again now, its slot content will not be cleaned
		// up later.
	...
	if (senseSrb == NULL) {
	    // if not need Request Sense or cannot get Request Sense Srb, Complete the
	    // command manually
	    ReleaseSlottedCommand(ChannelExtension, (UCHAR)failingCommand,
				  TRUE); //&ChannelExtension->Slot[cmd.CCS] );
	    RecordExecutionHistory(ChannelExtension,
				   0x10630013); // NonNCQ Error Completed failed command
	}
    }
    ...
    // 1.4.2 If PxTFD.STS.BSY or PxTFD.STS.DRQ is set to ‘1’, issue a COMRESET to the
    // device to put it in an idle state
    tfd.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension,
					    &ChannelExtension->Px->TFD.AsUlong);
    ...
    // 2.2 Restore the unsent programmed commands for careful reprocessing
    ChannelExtension->SlotManager.NormalQueueSlice |= ci; // put the commands that didn't
							  // get a chance to finish back
							  // into the normal queue
    ChannelExtension->SlotManager.CommandsIssued &= ~ci; // Remove the unfinished commands
							 // from the 'issued' list
	...
	AhciCompleteIssuedSRBs(ChannelExtension, SRB_STATUS_SUCCESS,
			       TRUE); // complete the successful commands to start the RS
				      // SRB.
    }
    ...
    RecordExecutionHistory(ChannelExtension,
			   0x10000013); // Exit AhciNonQueuedErrorRecovery
}

/*
    This completion routine continue handling NCQ Error Recovery after NCQ Error Log is
    read.
...
*/
VOID NcqErrorRecoveryCompletion(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
				_In_ PSTORAGE_REQUEST_BLOCK Srb)
{
    ...
    RecordExecutionHistory(ChannelExtension,
			   0x0000001b); // Enter NcqErrorRecoveryCompletion

    ++(ChannelExtension->TotalCountNCQErrorRecoveryComplete);
    AhciTelemetryLogResetErrorRecovery(
	ChannelExtension,
	(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
	AhciTelemetryEventIdNCQErrorRecoveryComplete, "NcqErrorRecoveryCompletion", 0,
	NULL, 0);

    //
    // In case of reset happened when Read Log command was executing, all requests have
    // been returned back to port driver. There is no more recovery work this routine
    // needs to do.
    //
    if (Srb->SrbStatus == SRB_STATUS_BUS_RESET) {
	StorPortDebugPrint(3,
			   "StorAHCI - Port %02d - NCQ Error Recovery: NCQ Error Log "
			   "read - reset\n",
			   ChannelExtension->PortNumber);
	...
	RecordExecutionHistory(ChannelExtension,
			       0x1001001b); // NcqErrorRecoveryCompletion RESET happened
					    // on read NCQ Error Log command.
	...
	// Other commands will be put back into NCQ slices and will be issued again to
	// adapter.
	//
	...
	if (((1 << failingCommand) & issuedCommands) != 0) {
		...
		if ((ChannelExtension->DeviceExtension->IdentifyDeviceData
		    ...
		    StorPortDebugPrint(3,
				       "StorAHCI - Port %02d - NCQ Error Recovery: NCQ "
				       "Error Log read successfully. Sense Data "
				       "returned: %02X/%02X/%02X\n",
				       ChannelExtension->PortNumber,
				       ncqErrorLog->SenseKey, ncqErrorLog->ASC,
				       ncqErrorLog->ASCQ);

		    AhciSetSenseData(failingSrb, SRB_STATUS_ERROR, ncqErrorLog->SenseKey,
				     ncqErrorLog->ASC, ncqErrorLog->ASCQ);
		} else {
		    //
		    // Set SrbStatus indicates it's an error condition.
		    // Sense data will be translated from Error register in
		    // ReleaseSlottedCommand()
		    //
		    StorPortDebugPrint(3,
				       "StorAHCI - Port %02d - NCQ Error Recovery: NCQ "
				       "Error Log read successfully. No Sense Data "
				       "returned\n",
				       ChannelExtension->PortNumber);

		    failingSrb->SrbStatus = SRB_STATUS_ERROR;
		    AtaMapError(ChannelExtension, failingSrb,
				ChannelExtension->Slot[failingCommand].StateFlags.FUA);
		}
		...
		//
		// Clear the NCQ Error Recovery state flag, so that queue will be resumed
		// in ReleaseSlottedCommand().
		//
		ChannelExtension->StateFlags.NcqErrorRecoveryInProcess = 0;
		...
		StorPortDebugPrint(3,
				   "StorAHCI - Port %02d - NCQ Error Recovery: NCQ Error "
				   "Log read successfully, but the failing srb is "
				   "NULL!\n",
				   ChannelExtension->PortNumber);

		//
		// Capture the scenarios that failing srb is NULL, need investigate cases
		// other than surprise remove.
		//
	    ...
	    StorPortDebugPrint(3,
			       "StorAHCI - Port %02d - NCQ Error Recovery: NCQ Tag "
			       "doesn't belong to issued commands.\n",
			       ChannelExtension->PortNumber);
	    NT_ASSERT(((1 << failingCommand) & issuedCommands) != 0);
	    fallBacktoReset = TRUE;

	    RecordExecutionHistory(ChannelExtension,
				   0x1002001b); // NcqErrorRecoveryCompletion, NCQ Tag
						// doesn't belong to issued commands.
	}

    } else if ((Srb->SrbStatus == SRB_STATUS_BUSY) && (srbExtension->QueueTag == 0xFF)) {
	//
	...
	RecordExecutionHistory(ChannelExtension,
			       0x1004001b); // NcqErrorRecoveryCompletion, Read NCQ Error
					    // Log cannot be issued, as all slots are
					    // occupied..
	...
	RecordExecutionHistory(ChannelExtension,
			       0x1003001b); // NcqErrorRecoveryCompletion, read NCQ Error
					    // Log failed.
    }
...
AHCI 1.1 Section 6.2.2.2 Native Command Queuing Error Recovery
    The flow for system software to recover from an error when native command queuing
commands are issued is as follows:
    ...
    To be able to finish this process, StorAHCI needs to hold all other requests until
    READ LOG EXT command is completed. This is achieved by utilizing
    "ChannelExtension->StateFlags.QueuePaused" flag.
	1. READ LOG EXT command will be issued using miniport reserved slot 0, so it will
	   be the next command being executed (by StorAHCI slot management design).
	2. This command is a SingleSlice command. StorAHCI will set "QueuePaused" flag to
	   be TRUE.
	3. "QueuePaused" flag will be set to FALSE in NcqErrorRecoveryCompletion after
...
*/
VOID AhciNcqErrorRecovery(PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
	...
	//
	// If PxTFD.STS.BSY or PxTFD.STS.DRQ is set to ‘1’, issue a COMRESET to the device
	// to put it in an idle state ChannelExtension->TaskFileData has been read from
	// register in ISR.
	//
	if ((ChannelExtension->TaskFileData.STS.BSY != 0) ||
	    (ChannelExtension->TaskFileData.STS.DRQ != 0)) {
	    needCOMRESET = TRUE;
	}
    }
	...
	//
	RecordExecutionHistory(ChannelExtension,
			       0x10160014); // AhciNcqErrorRecovery, RESET is being
					    // issued.
	AhciPortReset(ChannelExtension, FALSE);
	return;
    }
	...
	//
	RecordExecutionHistory(ChannelExtension,
			       0x10170014); // AhciNcqErrorRecovery, RESET is being issued
					    // because slot 0 is in use.
	AhciPortReset(ChannelExtension, FALSE);
	return;
    }

    StorPortDebugPrint(3,
		       "StorAHCI - Port %02d - NCQ Error Recovery: Send READ LOG EXT to "
		       "read NCQ Error Log. Issued Requests:: %08X\n",
		       ChannelExtension->PortNumber,
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10170015); // AhciNcqErrorRecovery, RESET is being issued
					    // because NCQ error recovery is already in
					    // progress.
	AhciPortReset(ChannelExtension, FALSE);
	return;
    }
    ...
    ChannelExtension->Sense.SrbExtension->CompletionContext =
	(PVOID)(ULONG_PTR)ChannelExtension->SlotManager.CommandsIssued;
	...
	ChannelExtension,
	(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
	AhciTelemetryEventIdNCQErrorRecovery, "AhciNcqErrorRecovery Exit", 0, NULL, 0);
...
Return Values:
    None
*/
VOID AhciPortErrorRecovery(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension)
{
    ++(ChannelExtension->TotalCountPortErrorRecovery);
    AhciTelemetryLogResetErrorRecovery(
	ChannelExtension,
	(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
	AhciTelemetryEventIdPortErrorRecovery, "AhciPortErrorRecovery", 0, NULL, 0);
	...
	L"AhciPortErrorRecovery", STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE,
	StorportEtwLevelError, StorportEtwEventOpcodeInfo, NULL, L"StateFlags",
	*(ULONGLONG *)&(ChannelExtension->StateFlags), L"PortErrRecovery",
	ChannelExtension->TotalCountPortErrorRecovery, L"StartState",
	*(ULONGLONG *)&(ChannelExtension->StartState), L"PortReset",
	ChannelExtension->TotalCountPortReset, L"RunningStrtFail",
	ChannelExtension->TotalCountRunningStartFailed, L"NonQ'dErrRecov",
	ChannelExtension->TotalCountNonQueuedErrorRecovery, L"NCQError|NCQErrorComplete",
	(((ULONGLONG)ChannelExtension->TotalCountNCQError << 32) |
	 ((ULONGLONG)ChannelExtension->TotalCountNCQErrorRecoveryComplete)),
	L"BusChange", ChannelExtension->TotalCountBusChange);

    if (ChannelExtension->StateFlags.CallAhciReportBusChange == 1) {
	// Handle AHCI 6.2.2.3 Recovery of Unsolicited COMINIT and hot plug
	// AhciPortBusChangeDpcRoutine() will issue RESET, ignore other error recovery
	// marks.
	...
	if (ChannelExtension->TotalCountBusChange >=
	    AHCI_BUS_CHANGE_COUNT_WARNING_THRESHOLD) {
	    AhciPortMarkDeviceFailed(ChannelExtension, AhciDeviceFailureTooManyBusChange,
				     0, TRUE);
	}
	...
	if (!IsDumpMode(ChannelExtension->AdapterExtension)) {
	    ...
	    AhciPortBusChangeDpcRoutine(&ChannelExtension->BusChangeDpc,
					ChannelExtension->AdapterExtension,
	...
	if (ChannelExtension->TotalCountNCQError >=
	    AHCI_NCQ_ERROR_COUNT_WARNING_THRESHOLD) {
	    AhciPortMarkDeviceFailed(ChannelExtension, AhciDeviceFailureTooManyNCQError,
				     0, TRUE);
	}
	...
	if (SupportsEnhancedNcqErrorRecovery(ChannelExtension)) {
	    ...
	    //
	    // In case Enhanced NCQ Error Recovery is not supported, use the legacy method
	    // to reset device.
	    //
	...
	// Handle AHCI 6.2.2.2 Native Command Queuing Error Recovery and other events
	// require RESET.
	ChannelExtension->StateFlags.CallAhciNonQueuedErrorRecovery = 0;
	ChannelExtension->StateFlags.CallAhciReset = 0;
	...
	if (ChannelExtension->TotalCountNonQueuedErrorRecovery >=
	    AHCI_NON_QUEUED_ERROR_COUNT_WARNING_THRESHOLD) {
	    AhciPortMarkDeviceFailed(ChannelExtension,
				     AhciDeviceFailureTooManyNonQueuedError, 0, TRUE);
	}
...
/*++
    AhciPortReset can be called even if the miniport driver is not ready for another
    request. The miniport driver should complete all pending requests and must reset the given
    channel.
...
It performs:
    COMRESET along with completing commands to be retried and restoring settings that may
    be be persistent across a COMRESET
...
--*/
BOOLEAN AhciPortReset(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		      _In_ BOOLEAN CompleteAllRequests)
{
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10040050); // AhciPortReset, Port Start still in
					    // progress, device probably hang
	...
	(isPortStartInProgress ? AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING : 0), NULL, 0);

    if (ChannelExtension->CompletionQueue.LastTimeStampAddQueue.QuadPart >
	ChannelExtension->LastTimeStampDpcStart.QuadPart) {
	isCompletionDPCQueued = TRUE;
    }

    if (ChannelExtension->LastTimeStampDpcStart.QuadPart >
	ChannelExtension->LastTimeStampDpcCompletion.QuadPart) {
	...
	if (isCompletionDPCQueued && isCompletionDPCInterrupted) {
	    if (ChannelExtension->LastTimeStampDpcStart.QuadPart <
		ChannelExtension->CompletionQueue.LastTimeStampRemoveQueue.QuadPart) {
		duration = (ULONGLONG)(currentTimeStamp.QuadPart -
				       ChannelExtension->CompletionQueue
					   .LastTimeStampRemoveQueue.QuadPart);
	    } else {
		duration = (ULONGLONG)(currentTimeStamp.QuadPart -
				       ChannelExtension->LastTimeStampDpcStart.QuadPart);
	    }
	    ...
	    duration = (ULONGLONG)(currentTimeStamp.QuadPart -
				   ChannelExtension->CompletionQueue.LastTimeStampAddQueue
				       .QuadPart);
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10050050); // AhciPortReset, DPC queued or interrupted

	AhciTelemetryLogResetErrorRecovery(
	    ChannelExtension,
	    (PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
	    AhciTelemetryEventIdPortReset, "DPC queued or interrupted",
	    AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING, "Duration(in 100ns)", duration);
    }

    StorPortEtwChannelEvent8(
	ChannelExtension->AdapterExtension,
	(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
	StorportEtwEventOperational, AhciEtwEventPortReset, L"Port reset",
	STORPORT_ETW_EVENT_KEYWORD_COMMAND_TRACE, StorportEtwLevelInformational,
	StorportEtwEventOpcodeInfo, NULL, L"PortReset",
	ChannelExtension->TotalCountPortReset, L"RunningStrtFail",
	ChannelExtension->TotalCountRunningStartFailed, L"PortErrRecovery",
	ChannelExtension->TotalCountPortErrorRecovery, L"NonQ'dErrRecov",
	ChannelExtension->TotalCountNonQueuedErrorRecovery, L"NCQErrRecovery",
	ChannelExtension->TotalCountNCQError, L"NCQErrRecovCmpl",
	ChannelExtension->TotalCountNCQErrorRecoveryComplete, L"SurpriseRemove",
	ChannelExtension->TotalCountSurpriseRemove, L"StateFlags",
	*(ULONGLONG *)&(ChannelExtension->StateFlags));
    ...
    //  AHCI 10.1.2 - 3: If PxCMD.CR or PxCMD.FR do not clear to '0' correctly, then
    ...
    if ((CompleteAllRequests) && (ChannelExtension->StateFlags.QueuePaused == 1) &&
	(ChannelExtension->SlotManager.CommandsIssued == 0)) {
	//
	...
	RecordExecutionHistory(ChannelExtension,
			       0x10010050); // AhciPortReset, Unpausing queue
	AhciTelemetryLogResetErrorRecovery(
	    ChannelExtension,
	    (PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
	    AhciTelemetryEventIdPortReset, "Unpausing queue",
	    AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING, NULL, 0);
    }

    // 2.3 If either Init Command or PreservedSettings Command is being processed, reset
    // the command index to start from the first one again.
    ...
    if ((CompleteAllRequests) && (ChannelExtension->StateFlags.ReservedSlotInUse == 1) &&
	(ChannelExtension->SlotManager.SingleIoSliceIssued == 0) &&
	((ChannelExtension->SlotManager.SingleIoSlice & 0x1) == 0)) {
	NT_ASSERT(FALSE);

	//
	// This could happen if init/preserved setting command completed by device, but
	// the completion DPC is queued(or interrupted) for long time. If there is other
	// scenario, it is needed to further triage.
	//
	// Note: Force clear ReservedSlotInUse flag only if corresponding registry key is
	// set.
	//       Otherwise, there maybe add queue issue. E.g. Buggy device repeatedly
	//       generate bus change interrupt, which could result in add queue bugcheck
	//       eventually.
	//

	if (!(isCompletionDPCQueued || isCompletionDPCInterrupted)) {
	    RecordExecutionHistory(ChannelExtension,
				   0x10020050); // AhciPortReset, ReservedSlotInUse flag
						// maybe stuck
	    //
	    // There maybe possibility that init/restore preserved settings command stuck
	    // in device and never complete.
	    //
	    AhciPortMarkDeviceFailed(ChannelExtension, AhciDeviceFailureDeviceStuck, 0,
				     TRUE);
	} else {
	    RecordExecutionHistory(ChannelExtension,
				   0x10060050); // AhciPortReset, ReservedSlotInUse flag
						// outstanding
	}
    }
	...
	if ((ChannelExtension->Local.SrbExtension != NULL) &&
	    ...
	    RecordExecutionHistory(ChannelExtension,
				   0x10030050); // AhciPortReset, Init/Preserved Setting
						// command timeout
	    AhciTelemetryLogResetErrorRecovery(
		ChannelExtension,
		(PSTOR_ADDRESS) & (ChannelExtension->DeviceExtension->DeviceAddress),
		AhciTelemetryEventIdPortReset, "Slot 0 command timeout",
		AHCI_TELEMETRY_FLAG_NOT_SUPPRESS_LOGGING,
		(isInitCommand ? "Init Command" : "Preserved Setting Command"), 0);
	}
    }
    ...
    commandsToCompleteCount = NumberOfSetBits(
	ChannelExtension->SlotManager.CommandsToComplete);
	...
	if (commandsToCompleteCount == 1) {
		...
		if (((ChannelExtension->SlotManager.CommandsToComplete & (1 << i)) !=
		    ...
		    (!IsMiniportInternalSrb(ChannelExtension,
					    ChannelExtension->Slot[i].Srb))) {
		    //
		    // we know the error is for this command as it's the only one
		    // programmed to adapter, set status to be SRB_STATUS_ERROR for
		    // external Srb to make sure the function - AtaMapError() assigns the
		    // real error to Srb It's preferred to return SRB_STATUS_BUS_RESET to
		    // internal request to make it aware of reset happened.
		    //
	...
	AhciCompleteIssuedSRBs(ChannelExtension, completeStatus,
			       TRUE); // AhciPortReset is under Interrupt spinlock
    }

    // 3.2 Complete all other commands miniport own for this device, when it's necessary
    // or when the Reset is requested by Storport
    if (CompleteAllRequests) {
