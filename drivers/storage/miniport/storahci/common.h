...
// per ATA spec, the SectorCount value range is 1 - 0xffff. 0 means this entry is not
// valid.
...
#define ATA_FLAGS_RETURN_RESULTS \
    (1 << 6) // task file should be copied back into SenseInfoBuffer
#define ATA_FLAGS_HIGH_PRIORITY \
    (1 << 7) // the request should be processed as a high priority request
#define ATA_FLAGS_NEW_CDB \
    (1 << 8) // new CDB in SrbExtension should be issued to device rather than CDB in Srb
#define ATA_FLAGS_COMPLETE_SRB \
    (1 << 9) // indicates the Srb should be completed, AhciCompleteRequest will not send
	     // command from SrbExtension.
#define ATA_FLAGS_ACTIVE_REFERENCE \
    (1 << 10) // indicates Active Reference needs to be acquired before processing the Srb
	      // and released after processing the Srb
...
#define SetDeviceReg(reg, val) \
    (reg->bDriveHeadReg |= (0xA0 | val)) // only have Device0 for SATA
...
// addressing mode
//
typedef enum { UnknownMode = 0, LbaMode, Lba48BitMode } ATA_ADDRESS_TRANSLATION;

typedef struct _ATA_DEVICE_PARAMETERS {
    ATA_DEVICE_TYPE AtaDeviceType; // device type (ata, atapi etc)
    UCHAR ScsiDeviceType; // Scsi device type from inquiry data; Ata device:
...
typedef struct _ATA_TRIM_CONTEXT {
    ...
    //    max Block Descriptors carried by one Unmap command: 0xFFFE; max ATA Lba Ranges
    //    need for a Unmap Block Descriptor: 0xFFFFFFFF/0xFFFF ULONG is big enough to hold
    //    the value.
...
__inline BOOLEAN IsUnknownDevice(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
    return (DeviceParameters->AtaDeviceType == DeviceUnknown);
}

__inline BOOLEAN IsAtapiDevice(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
    return (DeviceParameters->AtaDeviceType == DeviceIsAtapi);
}

__inline BOOLEAN IsAtaDevice(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
    return (DeviceParameters->AtaDeviceType == DeviceIsAta);
}

__inline BOOLEAN IsRemovableMedia(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
    return (DeviceParameters->StateFlags.RemovableMedia == 1);
}

__inline BOOLEAN Support48Bit(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
    return (DeviceParameters->AddressTranslation == Lba48BitMode);
}

__inline ULONG BytesPerLogicalSector(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
    return DeviceParameters->BytesPerLogicalSector;
}

__inline ULONG64 MaxUserAddressableLba(_In_ PATA_DEVICE_PARAMETERS DeviceParameters)
{
...
--*/
__inline PCDB RequestGetSrbScsiData(_In_ PSTORAGE_REQUEST_BLOCK Srb,
				    _In_opt_ PULONG CdbLength, _In_opt_ PUCHAR ScsiStatus,
...
__inline ULONG GetSectorCountFromCdb(PCDB Cdb, ULONG CdbLength)
{
    ULONG sectorCount = 0;
...
--*/
VOID __inline ByteSwap(_Inout_updates_bytes_(Length) PUCHAR Buffer, _In_ ULONG Length)
{
    UCHAR temp;
    ULONG i;
...
--*/
ULONG __inline RemoveTrailingBlanks(_Inout_updates_z_(Length) PUCHAR Buffer, _In_ ULONG Length)
{
    LONG i;
...
ULONG __inline AhciAllocateDmaBuffer(_In_ PVOID AdapterExtension, _In_ ULONG BufferLength,
				     ...
				     _Out_ PSTOR_PHYSICAL_ADDRESS PhysicalAddress)
{
    ...
    status = StorPortAllocateDmaMemory(AdapterExtension, BufferLength, minPhysicalAddress,
				       maxPhysicalAddress, boundaryPhysicalAddress,
				       MmCached, MM_ANY_NODE_OK, Buffer, PhysicalAddress);

    return status;
}

ULONG __inline AhciFreeDmaBuffer(_In_ PVOID AdapterExtension, _In_ ULONG_PTR BufferLength,
				 _In_reads_bytes_(BufferLength) _Post_invalid_ PVOID Buffer,
				 _In_opt_ STOR_PHYSICAL_ADDRESS PhysicalAddress)
{
    ULONG status;

    status = StorPortFreeDmaMemory(AdapterExtension, Buffer, BufferLength, MmCached,
				   PhysicalAddress);

    return status;
}

__inline BOOLEAN DmaSafeAtapiCommand(_In_ UCHAR CdbCommand)
{
    ULONG i;
    BOOLEAN useDma;
    static const UCHAR dmaCommandList[] = { SCSIOP_READ6,
					    SCSIOP_READ,
					    SCSIOP_READ_CD,
					    SCSIOP_READ_CD_MSF,
					    SCSIOP_READ12,
...
__inline BOOLEAN IsSupportedReadCdb(_In_ PCDB Cdb)
{
    return ((Cdb->CDB10.OperationCode == SCSIOP_READ) ||
	    (Cdb->CDB10.OperationCode == SCSIOP_READ16));
}

__inline BOOLEAN IsSupportedWriteCdb(_In_ PCDB Cdb)
{
    return ((Cdb->CDB10.OperationCode == SCSIOP_WRITE) ||
	    (Cdb->CDB10.OperationCode == SCSIOP_WRITE16));
}

struct AHCI_CHANNEL_EXTENSION;
typedef struct _AHCI_CHANNEL_EXTENSION AHCI_CHANNEL_EXTENSION, *PAHCI_CHANNEL_EXTENSION;
...
ULONG AtaReadWriteRequest(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
...
ULONG AtaReportLunsCommand(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension, _In_ PVOID Context);
...
UCHAR AtaMapError(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
		  _In_ PSTORAGE_REQUEST_BLOCK Srb, _In_ BOOLEAN FUAcommand);
...
BOOLEAN FillClippedSGL(_In_ PSTOR_SCATTER_GATHER_LIST SourceSgl,
		       _Inout_ PSTOR_SCATTER_GATHER_LIST LocalSgl, _In_ ULONG BytesLeft,
...
ULONG DatasetManagementIoctl(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
			     _In_ PSTORAGE_REQUEST_BLOCK Srb);
