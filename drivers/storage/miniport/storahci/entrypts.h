...
Abstract:
    ...
    This header contains the declaration of functions that will be called by Storport
driver.
...
// some big HDDs (e.g. 2TB) takes ~ 20 seconds to spin up.  We hit timeout during S3
// resume for now, we reset after 1/3 * AHCI_PORT_START_TIMEOUT_IN_SECONDS Busy, and give
// up after AHCI_PORT_START_TIMEOUT_IN_SECONDS seconds
...
typedef struct _CHANNEL_STATE_FLAGS {
    ...
    ULONG NoMoreIO : 1;
    ULONG QueuePaused : 1;
    ULONG ReservedSlotInUse : 1; // NOTE: this field is accessed in
				 // InterlockedBitTestAndReset, bit position (currently:
				 // 3) is used there.

    ULONG NCQ_Activated : 1;
    ULONG NCQ_Succeeded : 1;
    ULONG RestorePreservedSettingsActiveReferenced : 1; // this bit will be set if active
							// reference is acquired for
							// RestorePreservedSettings
							// process
    ULONG HybridInfoEnabledOnHiberFile : 1; // this bit indicates if priority level should
					    // be put for NCQ Write by StorAHCI when
					    // writes a hiber file.
    ...
    ULONG PowerDown : 1; // NOTE: this field is accessed in InterlockedBitTestAndReset,
			 // bit position (currently: 12) is used there.
    ULONG PoFxEnabled : 1;
    ULONG PoFxActive : 1;
    ...
    ULONG NeedQDR : 1;
    ULONG PowerUpInitializationInProgress : 1; // Note: this field indicates that
					       // init/preserved settings commands(invoked
					       // by power up) are not completed yet.
    ULONG Reserved0 : 9;
...
// If a Log Address contains more than one different Log Pages, there normally is Log Page
// 0x00 to get supported log pages.
//
...
typedef struct _AHCI_DEVICE_EXTENSION {
    ...
    ATA_IO_RECORD
    IoRecord; // IO counts completed by device, the count only change if an IO reaches
	      // hardware
...
typedef struct _AHCI_DEVICE_INIT_COMMANDS {
    UCHAR CommandCount; // count of total commands, mainly used to allocate memory for
			// CommandTaskFile
...
typedef VOID (*PSRB_COMPLETION_ROUTINE)(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
					_In_opt_ PSTORAGE_REQUEST_BLOCK Srb);

typedef struct _LOCAL_SCATTER_GATHER_LIST {
    ULONG NumberOfElements;
    ULONG_PTR Reserved;
    _Field_size_(NumberOfElements) STOR_SCATTER_GATHER_ELEMENT List[257];
...
typedef struct _AHCI_SRB_EXTENSION {
    ...
    STOR_PHYSICAL_ADDRESS ResultBufferPhysicalAddress; // Physical address of
						       // ResultBuffer.
...
typedef BOOLEAN (*PAHCI_TIMER_CALLBACK)(_In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
					_In_ PVOID Context);
...
//
// Note: When adding new members to AHCI channel extension, make sure the uncached
...
typedef struct _AHCI_CHANNEL_EXTENSION {
    ...
    UCHAR CurrentCommandSlot; // miniport driver use to remember which slot to put command
			      // in.
    ...
    //
    struct {
	ULONG InterfaceNotReady; // Number of times the interface was no ready (CMD.ICC !=
				 // 0)
    ...
    struct {
	ULONG RestorePreservedSettings : 1; // NOTE: this field is accessed in
					    // InterlockedBitTestAndReset, bit position
					    // (currently: 0) is used there.
	ULONG BusChange : 1; // NOTE: this field is accessed in
			     // InterlockedBitTestAndReset, bit position (currently: 1) is
			     // used there.
    ...
    PEXECUTION_HISTORY
    ExecutionHistory; // For non-dump mode, contains MAX_EXECUTION_HISTORY_ENTRY_COUNT
		      // elements.
...
typedef struct _ADAPTER_STATE_FLAGS {
    ...
    ULONG SupportsAcpiDSM : 1; // indicates if the system has _DSM method implemented to
			       // control port/device power. when the value is 1, the _DSM
			       // method at least supports powering on all connected
			       // devices.
    ULONG Removed : 1;
...
typedef struct _AHCI_ADAPTER_EXTENSION {
    ...
    ULONG SlotNumber;
    ULONG AhciBaseAddress; // AHCI Base Address from bus. for example: BAR5 value on PCI
			   // bus
    ...
    struct {
	ULONG AdapterStop : 1; // NOTE: this field is accessed in
			       // InterlockedBitTestAndReset, bit position (currently: 0)
			       // is used there.
    ...
    ABAR_Address; // mapped AHCI Base Address. StorAHCI uses this field to control the
		  // adapter and ports.
