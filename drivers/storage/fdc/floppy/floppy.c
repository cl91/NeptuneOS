...
static VOID NTAPI
MotorStopDpcFunc(PKDPC UnusedDpc, PVOID DeferredContext,
 ...
 */
{
	...
	KeWaitForSingleObject(&DriveInfo->ControllerInfo->
			      MotorStoppedEvent, Executive, KernelMode,
			      FALSE, NULL);
 ...
 */
{
    ...
    Status =
	KeWaitForSingleObject(&ControllerInfo->SynchEvent, Executive,
 ...
 */
{
    ...
    KeWaitForSingleObject(QueueThreadObject, Executive, KernelMode, FALSE,
			  0);
	...
	for (j = 0; j < gControllerInfo[i].NumberOfDrives; j++) {
		...
		IoDeleteDevice(gControllerInfo[i].DriveInfo[j].
			       DeviceObject);
	    }
	}
 ...
 */
{
    ...
    for (i = 0;
	 i < ControllerResourceDescriptor->PartialResourceList.Count;
	 i++) {
	KeInitializeEvent(&gControllerInfo[gNumberOfControllers].
			  SynchEvent, NotificationEvent, FALSE);

	PartialDescriptor =
	    &ControllerResourceDescriptor->PartialResourceList.
	    PartialDescriptors[i];

	if (PartialDescriptor->Type == CmResourceTypeInterrupt) {
	    gControllerInfo[gNumberOfControllers].Level =
		PartialDescriptor->u.Interrupt.Level;
	    gControllerInfo[gNumberOfControllers].Vector =
		PartialDescriptor->u.Interrupt.Vector;

	    if (PartialDescriptor->Flags & CM_RESOURCE_INTERRUPT_LATCHED)
		gControllerInfo[gNumberOfControllers].InterruptMode =
		    Latched;
	    else
		gControllerInfo[gNumberOfControllers].InterruptMode =
		    LevelSensitive;
		...
		(BusType, BusNumber, PartialDescriptor->u.Port.Start,
	    ...
	    if (AddressSpace == 0)
		gControllerInfo[gNumberOfControllers].BaseAddress =
		    MmMapIoSpace(TranslatedAddress, FDC_PORT_BYTES,
				 MmNonCached);
	    else
		gControllerInfo[gNumberOfControllers].BaseAddress =
		    (PUCHAR) (ULONG_PTR) TranslatedAddress.QuadPart;
	}

	else if (PartialDescriptor->Type == CmResourceTypeDma)
	    gControllerInfo[gNumberOfControllers].Dma =
    ...
    for (i = 0;
	 i < PeripheralResourceDescriptor->PartialResourceList.Count;
	 i++) {
	PDRIVE_INFO DriveInfo =
	    &gControllerInfo[gNumberOfControllers].DriveInfo[i];

	PartialDescriptor =
	    &PeripheralResourceDescriptor->PartialResourceList.
	    PartialDescriptors[i];
	...
	FloppyDeviceData =
	    (PCM_FLOPPY_DEVICE_DATA) (PartialDescriptor + 1);

	DriveInfo->ControllerInfo = &gControllerInfo[gNumberOfControllers];
	DriveInfo->UnitNumber = i;

	DriveInfo->FloppyDeviceData.MaxDensity =
	    FloppyDeviceData->MaxDensity;
	DriveInfo->FloppyDeviceData.MountDensity =
	    FloppyDeviceData->MountDensity;
	DriveInfo->FloppyDeviceData.StepRateHeadUnloadTime =
	    FloppyDeviceData->StepRateHeadUnloadTime;
	DriveInfo->FloppyDeviceData.HeadLoadTime =
	    FloppyDeviceData->HeadLoadTime;
	DriveInfo->FloppyDeviceData.MotorOffTime =
	    FloppyDeviceData->MotorOffTime;
...
VOID NTAPI
DpcForIsr(PKDPC UnusedDpc, PVOID Context, PVOID SystemArgument1,
 ...
 */
{
    ...
    if (HwConfigure(ControllerInfo, FALSE, TRUE, TRUE, 0, 0) !=
	STATUS_SUCCESS) {
	...
	if (HwConfigure(ControllerInfo, TRUE, TRUE, TRUE, 0, 0) !=
	    STATUS_SUCCESS) {
	    WARN_(FLOPPY,
    ...
    if (HwSpecify
	(ControllerInfo, HeadLoadTime, HeadUnloadTime, StepRateTime,
    ...
    KeInitializeEvent(&ControllerInfo->MotorStoppedEvent,
 ...
 */
{
    ...
    Status =
	IoGetDeviceObjectPointer(&MountMgrDevice, FILE_READ_ATTRIBUTES,
    ...
    DeviceLen =
	wcslen(&gControllerInfo[ControlerId].DriveInfo[DriveId].
	       DeviceNameBuffer[0]) * sizeof(WCHAR);
    ...
    RtlCopyMemory(MountTarget->DeviceName,
		  gControllerInfo[ControlerId].DriveInfo[DriveId].
		  DeviceNameBuffer, DeviceLen);
	...
	(IOCTL_MOUNTMGR_VOLUME_ARRIVAL_NOTIFICATION, DeviceObject,
	 MountTarget, sizeof(MOUNTMGR_TARGET_NAME) + DeviceLen, NULL, 0,
 ...
 */
{
    ...
    for (i = 0;
	 i < gNumberOfControllers && gControllerInfo[i].NumberOfDrives > 0;
	 i++) {
	...
	gControllerInfo[i].MappedVector =
	    HalGetInterruptVector(gControllerInfo[i].InterfaceType,
				  gControllerInfo[i].BusNumber,
				  gControllerInfo[i].Level,
				  gControllerInfo[i].Vector,
	...
	if (IoConnectInterrupt
	    (&gControllerInfo[i].InterruptObject, Isr, &gControllerInfo[i],
	     0, gControllerInfo[i].MappedVector,
	     gControllerInfo[i].MappedLevel,
	     gControllerInfo[i].MappedLevel,
	     gControllerInfo[i].InterruptMode, FALSE, Affinity,
	     0) != STATUS_SUCCESS) {
	...
	DeviceDescription.DmaWidth =
	    gControllerInfo[i].Dma > 3 ? Width16Bits : Width8Bits;

	gControllerInfo[i].AdapterObject =
	    HalGetAdapter(&DeviceDescription,
			  &gControllerInfo[i].MapRegisters);
	...
	for (j = 0; j < gControllerInfo[i].NumberOfDrives; j++) {
	    ...
	    swprintf(gControllerInfo[i].DriveInfo[j].ArcPathBuffer,
		     L"\\ArcName\\multi(%d)disk(%d)fdisk(%d)",
		     gControllerInfo[i].BusNumber, i, DriveNumber);

	    RtlInitUnicodeString(&ArcPath,
				 gControllerInfo[i].DriveInfo[j].
				 ArcPathBuffer);
	    ...
	    /* 3e: Set up the DPC */
	    IoInitializeDpcRequest(gControllerInfo[i].DriveInfo[j].
				   DeviceObject,
	    ...
	    gControllerInfo[i].DriveInfo[j].DiskGeometry.MediaType =
		Unknown;
 ...
 */
{
    ...
    for (;;) {
	KeWaitForMultipleObjects(2, Objects, WaitAny, Executive,
 ...
 */
{
	...
	(PDRIVER_DISPATCH) ReadWrite;
    DriverObject->MajorFunction[IRP_MJ_WRITE] =
	(PDRIVER_DISPATCH) ReadWrite;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] =
	(PDRIVER_DISPATCH) DeviceIoctl;
	...
	(&ThreadHandle, THREAD_ALL_ACCESS, 0, 0, 0, QueueThread,
	 0) != STATUS_SUCCESS) {
	...
	(ThreadHandle, STANDARD_RIGHTS_ALL, *PsThreadType, KernelMode,
