 ...
 */
{
	...
	if (HwSpecify
	    (DriveInfo->ControllerInfo, HeadLoadTime, HeadUnloadTime,
	...
	if (Status == STATUS_TIMEOUT
	    || HwReadIdResult(DriveInfo->ControllerInfo, NULL,
			      NULL) != STATUS_SUCCESS) {
	    WARN_(FLOPPY,
...
RWComputeCHS(PDRIVE_INFO IN DriveInfo,
	     ...
	     PUCHAR OUT Cylinder, PUCHAR OUT Head, PUCHAR OUT Sector)
/*
 ...
 */
{
    ...
    *Head =
	AbsoluteSector % SectorsPerCylinder <
	DriveInfo->DiskGeometry.SectorsPerTrack ? 0 : 1;
     ...
     */
    *Sector =
	((UCHAR) (AbsoluteSector % SectorsPerCylinder) + 1) -
 ...
 */
{
    ...
    TRACE_(FLOPPY,
	   ...
	   (Stack->MajorFunction ==
	    IRP_MJ_READ ? Stack->Parameters.Read.Length : Stack->
	    Parameters.Write.Length),
	   (Stack->MajorFunction ==
	    IRP_MJ_READ ? Stack->Parameters.Read.ByteOffset.u.
	    LowPart : Stack->Parameters.Write.ByteOffset.u.LowPart));
    ...
    Status =
	IoAllocateAdapterChannel(DriveInfo->ControllerInfo->AdapterObject,
				 DeviceObject,
	 ...
	 */
	if (RWComputeCHS
	    (DriveInfo, DiskByteOffset + TransferByteOffset, &Cylinder,
	...
	if (!DriveInfo->ControllerInfo->ImpliedSeeks) {
		...
		RWFreeAdapterChannel(DriveInfo->ControllerInfo->
				     AdapterObject);
		IoCompleteRequest(Irp, IO_NO_INCREMENT);
		StopMotor(DriveInfo->ControllerInfo);
		return;
	 ...
	 */
	INFO_(FLOPPY,
	      "ReadWritePassive(): computing number of sectors to transfer (StartSector 0x%x): ",
	      StartSector);

	/* 1-based sector number */
	if ((((DriveInfo->DiskGeometry.TracksPerCylinder -
	       Head) * DriveInfo->DiskGeometry.SectorsPerTrack -
	      StartSector) + 1) <
	    (Length -
	     TransferByteOffset) /
	    DriveInfo->DiskGeometry.BytesPerSector) {
	...
	if (BYTES_TO_PAGES(CurrentTransferBytes) >
	    DriveInfo->ControllerInfo->MapRegisters) {
	    CurrentTransferSectors =
		(UCHAR) ((DriveInfo->ControllerInfo->MapRegisters *
			  PAGE_SIZE) /
			 DriveInfo->DiskGeometry.BytesPerSector);
	...
	if (HwReadWriteData
	    (DriveInfo->ControllerInfo, !WriteToDevice,
	     DriveInfo->UnitNumber, Cylinder, Head, StartSector,
	     DriveInfo->BytesPerSectorCode,
	     DriveInfo->DiskGeometry.SectorsPerTrack, Gap,
	     0xff) != STATUS_SUCCESS) {
	    WARN_(FLOPPY,
