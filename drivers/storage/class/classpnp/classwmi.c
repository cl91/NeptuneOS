...
//
GUIDREGINFO wmiClassGuids[] = { { MSStorageDriver_ClassErrorLogGuid, 1, 0 } };
...
--*/
NTAPI NTSTATUS ClassSystemControl(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	((minorFunction != IRP_MN_REGINFO) && (commonExtension->GuidCount == 0))) {
	//
	...
	if (commonExtension->GuidRegInfo != NULL &&
	    ClassFindGuid(commonExtension->GuidRegInfo, commonExtension->GuidCount,
			  (LPGUID)irpStack->Parameters.WMI.DataPath, &guidIndex)) {
	...
	TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_WMI,
		    "WMI Find Guid = %x, isInternalGuid = %x", status, isInternalGuid));
	if (NT_SUCCESS(status) && ((minorFunction == IRP_MN_QUERY_SINGLE_INSTANCE) ||
		...
		status = STATUS_WMI_INSTANCE_NOT_FOUND;
		TracePrint(
		    (TRACE_LEVEL_WARNING, TRACE_FLAG_WMI, "WMI Instance not found!"));
	    }
	}
	...
	if ((classWmiInfo->GuidRegInfo != NULL) &&
	    ...
	    status = classWmiInfo->ClassQueryWmiRegInfo(DeviceObject, &nameFlags, &name);

	    RtlInitUnicodeString(&mofName, MOFRESOURCENAME);

	} else if ((classWmiInfo->GuidRegInfo != NULL) &&
		   (ClassQueryWmiRegInfoEx != NULL)) {
	    RtlInitUnicodeString(&mofName, L"");

	    status = (*ClassQueryWmiRegInfoEx)(DeviceObject, &nameFlags, &name, &mofName);
	} else {
	    RtlInitUnicodeString(&mofName, L"");
	...
	if (NT_SUCCESS(status) &&
	    (!(nameFlags & WMIREG_FLAG_INSTANCE_PDO) && (name.Buffer == NULL))) {
	    //
	...
	if (NT_SUCCESS(status)) {
	    ...
	    guidCount = (classWmiInfo->GuidRegInfo == NULL ? 0 :
							     classWmiInfo->GuidCount) +
			NUM_CLASS_WMI_GUIDS;

	    nameOffset = sizeof(WMIREGINFO) + guidCount * sizeof(WMIREGGUIDW);

	    if (nameFlags & WMIREG_FLAG_INSTANCE_PDO) {
		nameSize = 0;
		nameInfo = commonExtension->IsFdo ?
			       (ULONG_PTR)((PFUNCTIONAL_DEVICE_EXTENSION)commonExtension)
				   ->LowerPdo :
	    ...
	    registryPathOffset = mofResourceOffset + mofName.Length + sizeof(USHORT);
		...
		if (bufferSize >= bufferNeeded) {
		    ...
		    *stringPtr++ = regPath->Length;
		    ...
		    NT_ASSERT(bufferSize >= bufferNeeded);
		    TracePrint(
			(TRACE_LEVEL_WARNING, TRACE_FLAG_WMI, "Invalid Buffer Size!"));
		    status = STATUS_INVALID_BUFFER_SIZE;
		}
	...
	if (isInternalGuid) {
	    status = ClassQueryInternalDataBlock(DeviceObject, Irp, guidIndex,
						 bufferAvail,
						 buffer + sizeof(WNODE_ALL_DATA));
	} else {
	    status = classWmiInfo->ClassQueryWmiDataBlock(DeviceObject, Irp, guidIndex,
							  bufferAvail,
							  buffer +
	...
	_Analysis_assume_(isInternalGuid);
	if (isInternalGuid) {
	    status = ClassWmiCompleteRequest(DeviceObject, Irp, STATUS_WMI_GUID_NOT_FOUND,
					     0, IO_NO_INCREMENT);
	} else {
	    NT_ASSERT(guidIndex != (ULONG)-1);

	    status = classWmiInfo->ClassSetWmiDataBlock(DeviceObject, Irp, guidIndex,
							wnode->SizeDataBlock,
							(PUCHAR)wnode +
	...
	if (isInternalGuid) {
	    ...
	    status = classWmiInfo->ClassSetWmiDataItem(DeviceObject, Irp, guidIndex,
						       wnode->ItemId, wnode->SizeDataItem,
						       (PUCHAR)wnode +
							   wnode->DataBlockOffset);
	}
	...
	if (isInternalGuid) {
		...
		DeviceObject, Irp, guidIndex, wnode->MethodId, wnode->SizeDataBlock,
		bufferSize - wnode->DataBlockOffset, buffer + wnode->DataBlockOffset);
	}
	...
	if (isInternalGuid) {
	    ...
	    status = classWmiInfo->ClassWmiFunctionControl(DeviceObject, Irp, guidIndex,
			     ...
			     OUT PWCHAR String)
{
    ...
    RtlStringCbPrintfW(outDateTime, sizeof(outDateTime),
		       L"%04d%02d%02d%02d%02d%02d.%03d***+000", timeFields.Year,
		       timeFields.Month, timeFields.Day, timeFields.Hour,
