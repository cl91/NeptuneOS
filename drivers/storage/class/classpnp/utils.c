				   ...
				   IN OUT PULONG ParameterValue) // also default value
{
    ...
    status = IoOpenDeviceRegistryKey(FdoExtension->LowerPdo, PLUGPLAY_REGKEY_DEVICE,
				     KEY_READ, &deviceParameterHandle);
	...
	queryTable->DefaultType = (REG_DWORD << RTL_QUERY_REGISTRY_TYPECHECK_SHIFT) |
				  REG_NONE;
	queryTable->DefaultData = NULL;
	queryTable->DefaultLength = 0;

	status = RtlQueryRegistryValues(RTL_REGISTRY_HANDLE,
					(PWSTR)(SubkeyName ? deviceSubkeyHandle :
	...
	status = IoOpenDeviceRegistryKey(FdoExtension->LowerPdo, PLUGPLAY_REGKEY_DRIVER,
					 KEY_READ, &deviceParameterHandle);
	...
	if (NT_SUCCESS(status)) {
		...
		ClassSetDeviceParameter(FdoExtension, SubkeyName, ParameterName,
					*ParameterValue);
				       ...
				       IN ULONG ParameterValue)
{
	...
	status = RtlWriteRegistryValue(RTL_REGISTRY_HANDLE,
				       (PWSTR)(SubkeyName ? deviceSubkeyHandle :
							    deviceParameterHandle),
				       ParameterName, REG_DWORD, &ParameterValue,
				       sizeof(ULONG));
			       ...
			       IN PCLASS_SCAN_FOR_SPECIAL_HANDLER Function)
{
    ...
    for (; (DeviceList->VendorId != NULL || DeviceList->ProductId != NULL ||
	    DeviceList->ProductRevision != NULL);
	 DeviceList++) {
	if (ClasspMyStringMatches(DeviceList->VendorId, (PCHAR)vendorId) &&
	    ...
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_INIT,
			"ClasspScanForSpecialByInquiry: Found matching "
...
VOID ClasspPerfIncrementSuccessfulIo(IN PFUNCTIONAL_DEVICE_EXTENSION FdoExtension)
{
	...
	if (errors < CLASS_ERROR_LEVEL_2) {
		...
		CLEAR_FLAG(FdoExtension->SrbFlags, SRB_FLAGS_DISABLE_DISCONNECT);
	    }
	}
	...
	if (errors < CLASS_ERROR_LEVEL_1) {
		...
		CLEAR_FLAG(FdoExtension->SrbFlags, SRB_FLAGS_DISABLE_SYNCH_TRANSFER);
	    }
	    if (TEST_FLAG(fdoData->Perf.OriginalSrbFlags,
			  SRB_FLAGS_QUEUE_ACTION_ENABLE)) {
		SET_FLAG(FdoExtension->SrbFlags, SRB_FLAGS_QUEUE_ACTION_ENABLE);
	    }
	    if (TEST_FLAG(fdoData->Perf.OriginalSrbFlags, SRB_FLAGS_NO_QUEUE_FREEZE)) {
...
NTSTATUS ClasspDuidGetDriveLayout(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	RtlCopyMemory(&driveLayoutSignature->DeviceSpecific.MbrSignature,
		      &layoutEx->Mbr.Signature, sizeof(layoutEx->Mbr.Signature));

    } else {
	driveLayoutSignature->Mbr = FALSE;

	RtlCopyMemory(&driveLayoutSignature->DeviceSpecific.GptDiskId,
		      &layoutEx->Gpt.DiskId, sizeof(layoutEx->Gpt.DiskId));
    }
...
NTSTATUS ClasspDuidQueryProperty(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    status = ClasspDuidGetDeviceIdProperty(DeviceObject, Irp);
    ...
    status = ClasspDuidGetDeviceProperty(DeviceObject, Irp);
    ...
    status = ClasspDuidGetDriveLayout(DeviceObject, Irp);
				  ...
				  IN OUT PSCSI_REQUEST_BLOCK Srb)
{
    ...
    writeCache->UserDefinedPowerProtection = TEST_FLAG(fdoExtension->DeviceFlags,
						       DEV_POWER_PROTECTED);
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_IOCTL,
		    "ClasspWriteCacheProperty: Synchronize cache failed with status "
		    "0x%X\n",
		    status));
    ...
    length = ClassModeSense(DeviceObject, (PCHAR)modeData, MODE_PAGE_DATA_SIZE,
			    MODE_PAGE_CACHING);
    ...
    pageData = ClassFindModePage((PCHAR)modeData, length, MODE_PAGE_CACHING, TRUE);
	...
	writeCache->WriteCacheEnabled = pageData->WriteCacheEnable ? WriteCacheEnabled :
								     WriteCacheDisabled;
    ...
    length = ClasspModeSense(DeviceObject, (PCHAR)modeData, MODE_PAGE_DATA_SIZE,
			     MODE_PAGE_CACHING, MODE_SENSE_CHANGEABLE_VALUES);
				       ...
				       IN ULONG BytesPerBlockInBigEndian)
{
	...
	if (logicalSectorSize & (logicalSectorSize - 1)) {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_INIT,
			"FDO %ph has non-standard sector size 0x%x.", Fdo,
			logicalSectorSize));
	    do {
				     ...
				     IN PREAD_CAPACITY16_DATA ReadCapacity16Data)
{
    ...
    PCLASS_READ_CAPACITY16_DATA cachedData = &(
	FdoExtension->FunctionSupportInfo->ReadCapacity16Data);
	...
	ClasspCalculateLogicalSectorSize(FdoExtension->DeviceObject,
					 ReadCapacity16Data->BytesPerBlock);

    // FdoExtension->DiskGeometry.BytesPerSector might be 0 for class drivers that don't get READ CAPACITY info yet.
    NT_ASSERT(
	(FdoExtension->DiskGeometry.BytesPerSector == 0) ||
	(FdoExtension->DiskGeometry.BytesPerSector ==
	 FdoExtension->FunctionSupportInfo->ReadCapacity16Data.BytesPerLogicalSector));

    logicalBlocksPerPhysicalBlock = 1 << ReadCapacity16Data->LogicalPerPhysicalExponent;
    lowestAlignedBlock = (ReadCapacity16Data->LowestAlignedBlock_MSB << 8) |
			 ReadCapacity16Data->LowestAlignedBlock_LSB;
	...
	lowestAlignedBlock = (logicalBlocksPerPhysicalBlock - lowestAlignedBlock) %
			     logicalBlocksPerPhysicalBlock;
    }

    if (NT_SUCCESS(status)) {
	// fill output buffer
	cachedData->BytesPerPhysicalSector = cachedData->BytesPerLogicalSector *
					     logicalBlocksPerPhysicalBlock;
	cachedData->BytesOffsetForSectorAlignment = cachedData->BytesPerLogicalSector *
						    lowestAlignedBlock;
			     ...
			     IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	FdoExtension->FunctionSupportInfo->ReadCapacity16Data.CommandStatus =
				       ...
				       IN OUT PSCSI_REQUEST_BLOCK Srb)
{
    ...
    PCOMMON_DEVICE_EXTENSION commonExtension = (PCOMMON_DEVICE_EXTENSION)
	...
	if (ClasspLowerLayerNotSupport(status)) {
		...
		accessAlignment->BytesPerPhysicalSector =
		    fdoExtension->FunctionSupportInfo->ReadCapacity16Data
			.BytesPerPhysicalSector;
		accessAlignment->BytesOffsetForSectorAlignment =
		    fdoExtension->FunctionSupportInfo->ReadCapacity16Data
				       ...
				       IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceMediaTypeProperty (%p): Unsupported query type %x for "
		    "media type property.\n",
    ...
    //
    if (fdoExtension->FunctionSupportInfo->ValidInquiryPages.BlockDeviceCharacteristics ==
	TRUE) {
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceGetBlockDeviceCharacteristicsVPDPage (%p): VPD "
							  ...
							  IN PSCSI_REQUEST_BLOCK Srb)
{
    NTSTATUS status = STATUS_UNSUCCESSFUL;
    PCDB cdb;
    UCHAR bufferLength = sizeof(
	VPD_BLOCK_DEVICE_CHARACTERISTICS_PAGE); // data is 64 bytes
    ...
    status = ClassSendSrbSynchronous(fdoExtension->CommonExtension.DeviceObject, Srb,
				     dataBuffer, allocationBufferLength, FALSE);
    if (NT_SUCCESS(status)) {
	if (SrbGetDataTransferLength(Srb) < 0x8) {
	    // the device should return at least 8 bytes of data for use.
	    status = STATUS_UNSUCCESSFUL;
	} else if ((dataBuffer->PageLength != 0x3C) ||
		   (dataBuffer->PageCode != VPD_BLOCK_DEVICE_CHARACTERISTICS)) {
	    // 'PageLength' shall be 0x3C; and 'PageCode' shall match.
	    status = STATUS_UNSUCCESSFUL;
	} else {
	    // cache data into fdoExtension
	    fdoExtension->FunctionSupportInfo->DeviceCharacteristicsData
		.MediumRotationRate = (dataBuffer->MediumRotationRateMsb << 8) |
				      dataBuffer->MediumRotationRateLsb;
	    fdoExtension->FunctionSupportInfo->DeviceCharacteristicsData
		.MediumProductType = dataBuffer->MediumProductType;
	    fdoExtension->FunctionSupportInfo->DeviceCharacteristicsData
		.NominalFormFactor = dataBuffer->NominalFormFactor;
	}
    } else {
					 ...
					 IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	if (ClasspLowerLayerNotSupport(status)) {
		...
		status = ClasspDeviceGetBlockDeviceCharacteristicsVPDPage(fdoExtension,
									  Srb);
	    } else {
	    ...
	    if (NT_SUCCESS(status)) {
		status =
		    IncursSeekPenalty(fdoExtension->FunctionSupportInfo
					  ->DeviceCharacteristicsData.MediumRotationRate,
				      &incursSeekPenalty);
	    }
	...
	if (NT_SUCCESS(status)) {
	    status = IncursSeekPenalty(fdoExtension->FunctionSupportInfo
					   ->DeviceCharacteristicsData.MediumRotationRate,
				       &incursSeekPenalty);
	}
					      ...
					      IN OUT OPTIONAL PSCSI_REQUEST_BLOCK Srb)
{
    ...
    ULONG allocationBufferLength =
	bufferLength; // Since the CDB size may differ from the actual buffer allocation
    PCDB cdb;
    ULONG dataTransferLength = 0;
    ...
    //
    fdoExtension->FunctionSupportInfo->LBProvisioningData.ProvisioningType =
	...
	if (NT_SUCCESS(status) &&
	    ((dataTransferLength < 0x08) ||
	     (pageLength <
	      (FIELD_OFFSET(VPD_LOGICAL_BLOCK_PROVISIONING_PAGE, Reserved2) -
	       FIELD_OFFSET(VPD_LOGICAL_BLOCK_PROVISIONING_PAGE, ThresholdExponent))) ||
	...
	if (NT_SUCCESS(status)) {
		...
		lbProvisioning->LBPU;
	    fdoExtension->FunctionSupportInfo->LBProvisioningData.ANC_SUP =
		lbProvisioning->ANC_SUP;
	    fdoExtension->FunctionSupportInfo->LBProvisioningData.ThresholdExponent =
		...
		(TRACE_LEVEL_INFORMATION, TRACE_FLAG_PNP,
		 ...
		 lbProvisioning->ProvisioningType, lbProvisioning->LBPRZ,
		 lbProvisioning->LBPU, lbProvisioning->ANC_SUP,
		 lbProvisioning->ThresholdExponent));
	}
    } else {
	status = STATUS_INVALID_DEVICE_REQUEST;
					   ...
					   OUT PCLASS_VPD_B0_DATA BlockLimitsData)
{
	...
	if (NT_SUCCESS(status)) {
		...
		REVERSE_BYTES_SHORT(&BlockLimitsData->OptimalTransferLengthGranularity,
				    &blockLimits->OptimalTransferLengthGranularity);
		// (8:11) MAXIMUM TRANSFER LENGTH
		REVERSE_BYTES(&BlockLimitsData->MaximumTransferLength,
			      &blockLimits->MaximumTransferLength);
		// (12:15) OPTIMAL TRANSFER LENGTH
		REVERSE_BYTES(&BlockLimitsData->OptimalTransferLength,
			      &blockLimits->OptimalTransferLength);
	    }

	    if ((dataTransferLength < 0x24) ||
		(pageLength < (FIELD_OFFSET(VPD_BLOCK_LIMITS_PAGE, Reserved1) -
			       FIELD_OFFSET(VPD_BLOCK_LIMITS_PAGE, Reserved0))) ||
	...
	if (NT_SUCCESS(status)) {
	    ...
	    REVERSE_BYTES(&BlockLimitsData->MaxUnmapLbaCount,
			  &blockLimits->MaximumUnmapLBACount);
	    // (24:27) MAXIMUM UNMAP BLOCK DESCRIPTOR COUNT
	    REVERSE_BYTES(&BlockLimitsData->MaxUnmapBlockDescrCount,
			  &blockLimits->MaximumUnmapBlockDescriptorCount);
	    // (28:31) OPTIMAL UNMAP GRANULARITY
	    REVERSE_BYTES(&BlockLimitsData->OptimalUnmapGranularity,
			  &blockLimits->OptimalUnmapGranularity);
		...
		REVERSE_BYTES(&BlockLimitsData->UnmapGranularityAlignment,
			      &blockLimits->UnmapGranularityAlignment);
		BlockLimitsData->UnmapGranularityAlignment &=
		    0x7FFFFFFF; // remove value of UGAVALID bit.
	    } else {
		BlockLimitsData->UnmapGranularityAlignment = 0;
	    }
		...
		BlockLimitsData->OptimalTransferLength, BlockLimitsData->MaxUnmapLbaCount,
		BlockLimitsData->MaxUnmapBlockDescrCount,
		BlockLimitsData->OptimalUnmapGranularity, BlockLimitsData->UGAVALID,
		BlockLimitsData->UnmapGranularityAlignment));
	}
    } else {
	status = STATUS_INVALID_DEVICE_REQUEST;
				  ...
				  IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	if ((status == STATUS_NOT_SUPPORTED) || (status == STATUS_NOT_IMPLEMENTED) ||
		...
		trimDescr->Version = sizeof(DEVICE_TRIM_DESCRIPTOR);
		trimDescr->Size = sizeof(DEVICE_TRIM_DESCRIPTOR);
		trimDescr->TrimEnabled = ClasspSupportsUnmap(
					    ...
					    IN OUT PSCSI_REQUEST_BLOCK Srb)
{
    ...
    // we can return.
    //
    blockLimitsStatus = ClasspBlockLimitsDataSnapshot(fdoExtension, TRUE,
						      &blockLimitsData, &generationCount);
    ...
    lbpDescr->ThinProvisioningEnabled = ClasspIsThinProvisioned(
	fdoExtension->FunctionSupportInfo);
    ...
    lbpDescr->ThinProvisioningReadZeros =
	fdoExtension->FunctionSupportInfo->LBProvisioningData.LBPRZ;
    lbpDescr->AnchorSupported =
	fdoExtension->FunctionSupportInfo->LBProvisioningData.ANC_SUP;
	...
	// to the caller.
	//
	lbpDescr->OptimalUnmapGranularity = (ULONGLONG)
					  ...
					  IN OUT PDEVICE_DATA_SET_RANGE DataSetRange)
{
	...
	//
	startingSector = (ULONGLONG)(DataSetRange->StartingOffset /
				     FdoExtension->DiskGeometry.BytesPerSector);
	sectorCount = (DataSetRange->LengthInBytes /
		       FdoExtension->DiskGeometry.BytesPerSector);
	...
	REVERSE_BYTES_QUAD(BlockDescr[*CurrentBlockDescrIndex].StartingLba,
			   &startingSector);
	REVERSE_BYTES(BlockDescr[*CurrentBlockDescrIndex].LbaCount, (PULONG)&sectorCount);

	DataSetRange->StartingOffset += sectorCount *
					FdoExtension->DiskGeometry.BytesPerSector;
	DataSetRange->LengthInBytes -= sectorCount *
				       FdoExtension->DiskGeometry.BytesPerSector;
				     ...
				     IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	lbaCount = DataSetRanges[i].LengthInBytes /
		   FdoExtension->DiskGeometry.BytesPerSector;
	...
	FdoExtension->FunctionSupportInfo->BlockLimitsData.MaxUnmapBlockDescrCount !=
	    MAXULONG) {
	maxParameterListLength =
	    (ULONGLONG)(FdoExtension->FunctionSupportInfo->BlockLimitsData
			    .MaxUnmapBlockDescrCount *
    ...
    //
    bufferLength = min(FdoExtension->PrivateFdoData->HwMaxXferLen,
		       (ULONG)maxParameterListLength);
    bufferLength = min(bufferLength,
		       (neededBlockDescrCount * sizeof(UNMAP_BLOCK_DESCRIPTOR) +
			sizeof(UNMAP_LIST_HEADER)));

    maxBlockDescrCount = (bufferLength - sizeof(UNMAP_LIST_HEADER)) /
			 sizeof(UNMAP_BLOCK_DESCRIPTOR);
	...
	//
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_IOCTL,
		    "DeviceProcessDsmTrimRequest (%p): Max Block Descriptor count is "
		    "Zero\n",
		    FdoExtension->DeviceObject));
    ...
    //
    maxLbaCount = (ULONGLONG)
		      FdoExtension->FunctionSupportInfo->BlockLimitsData.MaxUnmapLbaCount;
    if (maxLbaCount == 0) {
	//
	// This shouldn't happen since we've already done validation.
	//
	...
	ConvertDataSetRangeToUnmapBlockDescr(FdoExtension, blockDescrPointer,
					     &blockDescrIndex, maxBlockDescrCount,
					     &lbaCount, maxLbaCount, &tempDataSetRange);
	...
	allDataSetRangeFullyConverted = tempDataSetRangeFullyConverted &&
					(dataSetRangeIndex == DataSetRangesCount);
	...
	if ((blockDescrIndex ==
	    ...
	    //
	    transferSize = (USHORT)(blockDescrIndex * sizeof(UNMAP_BLOCK_DESCRIPTOR) +
				    sizeof(UNMAP_LIST_HEADER));
	    ...
	    tempSize = transferSize -
		       (USHORT)FIELD_OFFSET(UNMAP_LIST_HEADER, BlockDescrDataLength);
	    REVERSE_BYTES_SHORT(buffer->DataLength, &tempSize);
	    tempSize = transferSize -
		       (USHORT)FIELD_OFFSET(UNMAP_LIST_HEADER, Descriptors[0]);
	    ...
	    status = ClassSendSrbSynchronous(FdoExtension->DeviceObject, Srb, buffer,
					     transferSize, TRUE);

	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_IOCTL,
			"DeviceProcessDsmTrimRequest (%p): UNMAP command issued. "
				 ...
				 IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		    "ClasspDeviceTrimProcess (%p): Lower layer supports Trim DSM IOCTL, "
		    "forwarding IOCTL.\n",
		    DeviceObject));
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		    "ClasspDeviceTrimProcess (%p): Trim DSM IOCTL support unknown.  "
		    "Forwarded IOCTL and received NTSTATUS %!STATUS!.\n",
		    DeviceObject, status));
	...
	if (ClasspLowerLayerNotSupport(status)) {
	    ...
	    if (fdoExtension->FunctionSupportInfo->ValidInquiryPages.BlockLimits ==
		TRUE) {
		NT_ASSERT(
	...
	if (ClasspSupportsUnmap(fdoExtension->FunctionSupportInfo)) {
	    ...
	    //
	    ClasspBlockLimitsDataSnapshot(fdoExtension, FALSE, &blockLimitsData,
					  &generationCount);
		...
		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			    "ClasspDeviceTrimProcess (%p): Optimal Unmap Granularity not "
			    "provided, defaulted to 1.\n",
			    DeviceObject));
	    }

	    if (blockLimitsData.UGAVALID == TRUE) {
		granularityAlignmentInBytes =
		    (ULONGLONG)blockLimitsData.UnmapGranularityAlignment *
		    fdoExtension->DiskGeometry.BytesPerSector;
	    } else {
		...
		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			    "ClasspDeviceTrimProcess (%p): Unmap Granularity Alignment "
			    "not provided, defaulted to 0.\n",
			    DeviceObject));
	    }
		...
		entireDataSetRange.LengthInBytes =
		    (ULONGLONG)fdoExtension->CommonExtension.PartitionLength.QuadPart -
		    (ULONGLONG)entireDataSetRange.StartingOffset;

		dataSetRanges = &entireDataSetRange;
		dataSetRangesCount = 1;
	    } else {
		dataSetRanges =
		    (PDEVICE_DATA_SET_RANGE)((PUCHAR)dsmAttributes +
		...
		//
		for (i = 0; i < dataSetRangesCount; i++) {
		    if ((dataSetRanges[i].StartingOffset %
			     fdoExtension->DiskGeometry.BytesPerSector !=
			 0) ||
			...
			(dataSetRanges[i].StartingOffset <
			 (LONGLONG)granularityAlignmentInBytes) ||
			(dataSetRanges[i].LengthInBytes == 0) ||
			((ULONGLONG)dataSetRanges[i].StartingOffset +
			     dataSetRanges[i].LengthInBytes >
			 (ULONGLONG)
	    ...
	    if (!TEST_FLAG(dsmAttributes->Flags, DEVICE_DSM_FLAG_TRIM_NOT_FS_ALLOCATED)) {
		{
			...
			!fdoExtension->FunctionSupportInfo->LBProvisioningData.LBPRZ) {
			TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
				    "ClasspDeviceTrimProcess (%p): Device does not "
				    "support file level TRIM.\n",
				    DeviceObject));
	    ...
	    // process DSM IOCTL
	    status = DeviceProcessDsmTrimRequest(fdoExtension, dataSetRanges,
						 dataSetRangesCount, granularityInBlocks,
						 srbFlags, Irp, ActivityId, Srb);
	} else {
		      ...
		      IN BOOLEAN ConsolidateableBlocksOnly)
{
    ...
    status = ClassSendSrbSynchronous(FdoExtension->DeviceObject, Srb, LBAStatusHeader,
				     LBAStatusSize, FALSE);
    ...
    // buffer was larger than necessary.
    //
    if (status == STATUS_DATA_OVERRUN && SrbGetDataTransferLength(Srb) < LBAStatusSize) {
	status = STATUS_SUCCESS;
    }
				  ...
				  IN OUT PSCSI_REQUEST_BLOCK Srb)
{
    ...
    // Note that InputBufferLength and IsFdo have been validated beforing entering this routine.
    //
    if (dsmOutput == NULL || dsmAttributes == NULL) {
	...
	if (fdoExtension->FunctionSupportInfo->BlockLimitsData.UGAVALID) {
	    unmapGranularityAlignment = fdoExtension->FunctionSupportInfo->BlockLimitsData
					    .UnmapGranularityAlignment;
	}
	startingOffset = unmapGranularityAlignment;
	lengthInBytes = (ULONGLONG)fdoExtension->CommonExtension.PartitionLength.QuadPart -
	...
	//
	dataSetRanges = (PDEVICE_DATA_SET_RANGE)((PUCHAR)dsmAttributes +
						 dsmAttributes->DataSetRangesOffset);
    ...
    //
    finalStatus = ClasspBlockLimitsDataSnapshot(fdoExtension, TRUE, &blockLimitsData,
						&generationCount1);
	...
	getLBAWorkerStatus = ClasspDeviceGetLBAStatusWorker(
	    DeviceObject, &blockLimitsData, startingOffset, lengthInBytes, dsmOutput,
	    &dsmOutputLength, Srb, consolidateableBlocksOnly, outputVersion,
					...
					OUT PBOOLEAN BlockLimitsDataMayHaveChanged)
{
    ...
    ULONGLONG
    slabsPerCommand; // Number of slabs we can ask for in one Get LBA Status command.
    BOOLEAN doneProcessing =
	FALSE; // Indicates we should break out of the Get LBA Status loop.
    ...
    *BlockLimitsDataMayHaveChanged = FALSE;
	...
	alignmentInBytes = (ULONGLONG)alignmentInBlocks *
			   (ULONGLONG)fdoExtension->DiskGeometry.BytesPerSector;
    }
    ...
    // region.
    //
    if (LengthInBytes == 0 || StartingOffset < alignmentInBytes ||
    ...
    bytesPerSlab = (ULONGLONG)blocksPerSlab *
		   (ULONGLONG)fdoExtension->DiskGeometry.BytesPerSector;
	...
	startingLBA = (((StartingOffset - alignmentInBytes) / bytesPerSlab) + 1) *
			  (ULONGLONG)blocksPerSlab +
		      alignmentInBlocks;
	startingOffsetDelta = (startingLBA * fdoExtension->DiskGeometry.BytesPerSector) -
			      StartingOffset;
    } else {
	startingLBA = ((StartingOffset - alignmentInBytes) / bytesPerSlab) *
			  (ULONGLONG)blocksPerSlab +
		      alignmentInBlocks;
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceGetLBAStatusWorker (%p): Given output buffer is %u "
		    "bytes, needs to be %u bytes\n",
		    DeviceObject, outputLength, requiredOutputLength));
    ...
    maxSlabs = (maxBufferSize - sizeof(LBA_STATUS_LIST_HEADER)) /
	       sizeof(LBA_STATUS_DESCRIPTOR);
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceGetLBAStatusWorker (%p): Failed to allocate %u bytes "
		    "for descriptors\n",
		    DeviceObject, lbaStatusSize));
    ...
    *DsmOutputLength = DsmOutput->Size;
    ...
    RtlZeroMemory(lbpState, requiredOutputLength -
	...
	//
	status = GetLBAStatus(fdoExtension, Srb, startingLBA, lbaStatusListHeader,
			      lbaStatusSize, ConsolidateableBlocksOnly);
	...
	if (NT_SUCCESS(status)) {
	    ...
	    descrSizeOverhead = FIELD_OFFSET(LBA_STATUS_LIST_HEADER, Descriptors[0]) -
				RTL_SIZEOF_THROUGH_FIELD(LBA_STATUS_LIST_HEADER,
							 ParameterLength);
		...
		if (descrCount > 0) {
		    ...
		    //
		    for (descrIndex = 0;
			 descrIndex < descrCount &&
			 totalProcessedSlabs < requestedSlabs && !doneProcessing;
			 descrIndex++) {
			PLBA_STATUS_DESCRIPTOR lbaStatusDescr = &(
			    lbaStatusListHeader->Descriptors[descrIndex]);
			ULONGLONG returnedStartingLBA;
			ULONG mapped = (lbaStatusDescr->ProvisioningStatus !=
					LBA_STATUS_MAPPED) ?
					   0x0 :
					   0x1;
			ULONG lbaCount = 0;

			REVERSE_BYTES_QUAD(&returnedStartingLBA,
					   &(lbaStatusDescr->StartingLBA));
			...
			if (returnedStartingLBA != expectedStartingLBA) {
			    ...
			    *BlockLimitsDataMayHaveChanged = TRUE;
			    ...
			    TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_IOCTL,
					"ClasspDeviceGetLBAStatusWorker (%p): Device "
					"returned an LBA count (%u) that is not a "
					"multiple of the slab size (%u)\n",
					DeviceObject, lbaCount, blocksPerSlab));
			...
			if (processCurrentDescriptor) {
			    ULONG descrSlabs =
				lbaCount /
				blocksPerSlab; // Number of slabs in this descriptor.

			    for (; 0 < descrSlabs && totalProcessedSlabs < requestedSlabs;
				 descrSlabs--, commandProcessedSlabs++,
				 totalProcessedSlabs++) {
				ULONG bitMapIndex = totalProcessedSlabs /
			...
			//
			startingLBA += ((ULONGLONG)commandProcessedSlabs *
					(ULONGLONG)blocksPerSlab);
			...
			//
			TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
				    "ClasspDeviceGetLBAStatusWorker (%p): The slab "
				    "allocation bitmap has zero length.\n",
				    DeviceObject));
		    ...
		    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
				"ClasspDeviceGetLBAStatusWorker (%p): Device returned no "
		...
		TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
			    "ClasspDeviceGetLBAStatusWorker (%p): not enough bytes "
			    "returned\n",
			    DeviceObject));
	...
	//
    } while (NT_SUCCESS(status) && !doneProcessing &&
	...
	*DsmOutputLength = DsmOutput->Size + DsmOutput->OutputBlockLength;
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_IOCTL,
		...
		DeviceObject, totalProcessedSlabs));
Exit:
				       ...
				       IN OUT PLOG_PAGE_LOGICAL_BLOCK_PROVISIONING LogPage)
{
    ...
    //
    if (LogPage == NULL || LogPageSize < sizeof(LOG_PAGE_LOGICAL_BLOCK_PROVISIONING) ||
	LogPageSize > MAXUSHORT) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassGetLBProvisioningLogPage: DO (%p), Invalid parameter, LogPage "
		    "= %p, LogPageSize = %u.\n",
		    DeviceObject, LogPage, LogPageSize));
    ...
    status = ClassSendSrbSynchronous(fdoExtension->DeviceObject, Srb, LogPage,
				     LogPageSize, FALSE);
    ...
    // buffer was larger than necessary.
    //
    if (status == STATUS_DATA_OVERRUN && SrbGetDataTransferLength(Srb) < LogPageSize) {
	status = STATUS_SUCCESS;
    }
    ...
    //
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_IOCTL,
		"ClassGetLBProvisioningLogPage: DO (%p), LogSense command issued for LBP "
		"log page. NT Status: %!STATUS!.\n",
		DeviceObject, status));
					     ...
					     OUT PSTORAGE_LB_PROVISIONING_MAP_RESOURCES Resources)
{
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassInterpretLBProvisioningLogPage: DO (%p), Threshold Exponent "
		    "(%u) is invalid.\n",
		    DeviceObject, thresholdExponent));
	...
	LogPage == NULL || LogPageSize < sizeof(LOG_PAGE_LOGICAL_BLOCK_PROVISIONING)) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassInterpretLBProvisioningLogPage: DO (%p), Invalid parameter, "
		    "Resources = %p, ResourcesSize = %u, LogPage = %p, LogPageSize = "
		    "%u.\n",
		    DeviceObject, Resources, ResourcesSize, LogPage, LogPageSize));
    ...
    //
    pageLength = (USHORT)min(
	pageLength,
	(LogPageSize - FIELD_OFFSET(LOG_PAGE_LOGICAL_BLOCK_PROVISIONING, Parameters)));

    parameter = (PLOG_PARAMETER_HEADER)((PUCHAR)LogPage +
					FIELD_OFFSET(LOG_PAGE_LOGICAL_BLOCK_PROVISIONING,
						     Parameters));
	...
	if (parameter->ParameterLength > 0) {
		...
		//
		if (parameter->ParameterLength >
		    FIELD_OFFSET(LOG_PARAMETER_THRESHOLD_RESOURCE_COUNT,
				 ResourceCount[3])) {
		    Resources->AvailableMappingResourcesScope =
			((PLOG_PARAMETER_THRESHOLD_RESOURCE_COUNT)parameter)->Scope;
		}
		...
		if (parameter->ParameterLength >
		    FIELD_OFFSET(LOG_PARAMETER_THRESHOLD_RESOURCE_COUNT,
				 ResourceCount[3])) {
		    Resources->UsedMappingResourcesScope =
			((PLOG_PARAMETER_THRESHOLD_RESOURCE_COUNT)parameter)->Scope;
		}
	...
	//
	parameter = (PLOG_PARAMETER_HEADER)((PUCHAR)parameter +
					    sizeof(LOG_PARAMETER_HEADER) +
					 ...
					 IN OUT PSTORAGE_LB_PROVISIONING_MAP_RESOURCES Resources)
{
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassGetLBProvisioningResources: DO (%p), Device does not support "
	...
	ResourcesSize < sizeof(STORAGE_LB_PROVISIONING_MAP_RESOURCES)) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassGetLBProvisioningResources: DO (%p), Invalid parameter, "
		    "Resources = %p, ResourcesSize = %u.\n",
		    DeviceObject, Resources, ResourcesSize));
	...
	//
	status = ClassGetLBProvisioningLogPage(DeviceObject, Srb, logPageSize, logPage);

	if (NT_SUCCESS(status)) {
	    //
	    // Interpret the log page and fill in the output buffer.
	    //
	    status = ClassInterpretLBProvisioningLogPage(DeviceObject, logPageSize,
							 logPage, ResourcesSize,
							 Resources);
	}
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassGetLBProvisioningResources: DO (%p), Failed to allocate memory "
		    "for LBP log page.\n",
		    DeviceObject));
					   ...
					   IN OPTIONAL PVOID Context)
{
    ...
    if (NT_SUCCESS(
	    ClasspLogSystemEventWithDeviceNumber(DeviceObject,
						 IO_ERROR_DISK_RESOURCES_EXHAUSTED))) {
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		    "ClassLogResourceExhaustionEvent: DO (%p), Permanent resource "
		    "exhaustion logged.\n",
		    DeviceObject));
    } else {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassLogResourceExhaustionEvent: DO (%p), Failed to allocate memory "
		    "for error log entry.\n",
		    DeviceObject));
    }
...
VOID ClassQueueThresholdEventWorker(IN PDEVICE_OBJECT DeviceObject)
{
	...
	InterlockedCompareExchange(
	    (PLONG) & (fdoExtension->FunctionSupportInfo->LBProvisioningData
			   .SoftThresholdEventPending),
	    1, 0) == 0) {
	...
	if (workItem) {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			"ClassQueueThresholdEventWorker: DO (%p), Queueing soft "
			"threshold notification work item.\n",
			DeviceObject));
	    ...
	    InterlockedExchange((PLONG) &
				    (fdoExtension->FunctionSupportInfo->LBProvisioningData
					 .SoftThresholdEventPending),
				0);

	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
			"ClassQueueThresholdEventWorker: DO (%p), Failed to allocate "
			"memory for the work item.\n",
			DeviceObject));
	}
    }
}
...
VOID ClassQueueResourceExhaustionEventWorker(IN PDEVICE_OBJECT DeviceObject)
{
	...
	if (workItem) {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			"ClassQueueResourceExhaustionEventWorker: DO (%p), Queueing "
			"permanent resource exhaustion event work item.\n",
			DeviceObject));
	    ...
	    // system event log.
	    //
	    IoQueueWorkItem(workItem, ClassLogResourceExhaustionEvent, DelayedWorkQueue,
			    workItem);
	} else {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
			"ClassQueueResourceExhaustionEventWorker: DO (%p), Failed to "
					  ...
					  IN OPTIONAL PVOID Context)
{
    ...
    status = ClasspLogSystemEventWithDeviceNumber(DeviceObject,
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassLogCapacityChangedEvent: DO (%p), Failed to allocate memory "
		    "for error log entry.\n",
		    DeviceObject));
    }
    ...
    if (!NT_SUCCESS(status)) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassLogCapacityChangedEvent: DO (%p), ClassReadDriveCapacity "
		    "returned %!STATUS!.\n",
		    DeviceObject, status));
    }
...
VOID ClassQueueCapacityChangedEventWorker(IN PDEVICE_OBJECT DeviceObject)
{
	...
	if (workItem) {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			"ClassQueueCapacityChangedEventWorker: DO (%p), Queueing "
			"capacity changed event work item.\n",
			DeviceObject));
	    ...
	    // system event log.
	    //
	    IoQueueWorkItem(workItem, ClassLogCapacityChangedProcess, DelayedWorkQueue,
			    workItem);
	} else {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
			"ClassQueueCapacityChangedEventWorker: DO (%p), Failed to "
			"allocate memory for the work item.\n",
			DeviceObject));
	}
    }
						...
						PVOID Context)
{
    ...
    if (NT_SUCCESS(ClasspLogSystemEventWithDeviceNumber(
	    DeviceObject, IO_WARNING_DISK_PROVISIONING_TYPE_CHANGED))) {
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		    "ClassLogProvisioningTypeChangedEvent: DO (%p), LB Provisioning Type "
		    "changed logged.\n",
		    DeviceObject));
    } else {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
		    "ClassLogProvisioningTypeChangedEvent: DO (%p), Failed to allocate "
		    "memory for error log entry.\n",
		    DeviceObject));
    }
...
VOID ClassQueueProvisioningTypeChangedEventWorker(IN PDEVICE_OBJECT DeviceObject)
{
	...
	if (workItem) {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			"ClassQueueProvisioningTypeChangedEventWorker: DO (%p), Queueing "
			"LB provisioning type changed event work item.\n",
			DeviceObject));
	    ...
	    // system event log.
	    //
	    IoQueueWorkItem(workItem, ClassLogProvisioningTypeChangedEvent,
			    DelayedWorkQueue, workItem);
	} else {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
			"ClassQueueProvisioningTypeChangedEventWorker: DO (%p), Failed "
			"to allocate memory for the work item.\n",
			DeviceObject));
	}
    }
}
				       ...
				       IN OPTIONAL PVOID Context)
{
    ...
    PFUNCTIONAL_DEVICE_EXTENSION fdoExtension = (PFUNCTIONAL_DEVICE_EXTENSION)
						    DeviceObject->DeviceExtension;
    POPCODE_SENSE_DATA_IO_LOG_MESSAGE_CONTEXT_HEADER ioLogMessageContextHeader =
	(POPCODE_SENSE_DATA_IO_LOG_MESSAGE_CONTEXT_HEADER)Context;
	...
	status = IoGetDeviceProperty(fdoExtension->LowerPdo,
				     DevicePropertyPhysicalDeviceObjectName, 0, NULL,
				     &pdoNameLength);
    ...
    //
    senseBufferSize = ALIGN_UP_BY(ioLogMessageContextHeader->SenseDataSize,
				  sizeof(ULONG));

    logEntrySize = FIELD_OFFSET(IO_ERROR_LOG_PACKET, DumpData) + sizeof(ULONG) +
		   senseBufferSize;
	...
	errorLogEntry->StringOffset = (USHORT)(FIELD_OFFSET(IO_ERROR_LOG_PACKET,
							    DumpData) +
	...
	if (senseBufferSize > ioLogMessageContextHeader->SenseDataSize) {
	    RtlCopyMemory(&errorLogEntry->DumpData[1],
			  ioLogMessageContextHeader->SenseData,
			  ioLogMessageContextHeader->SenseDataSize);
	    RtlFillMemory((PCHAR)&errorLogEntry->DumpData[1] +
			      ioLogMessageContextHeader->SenseDataSize,
			  (senseBufferSize - ioLogMessageContextHeader->SenseDataSize),
			  '-');
	} else {
	    RtlCopyMemory(&errorLogEntry->DumpData[1],
			  ioLogMessageContextHeader->SenseData, senseBufferSize);
	}

	stringIndex = (PWCHAR)((PCHAR)errorLogEntry->DumpData +
			       errorLogEntry->DumpDataSize);
	...
	case IO_ERROR_IO_HARDWARE_ERROR:
	case IO_WARNING_IO_OPERATION_RETRIED: {
	    PIO_RETRIED_LOG_MESSAGE_CONTEXT ioLogMessageContext =
		(PIO_RETRIED_LOG_MESSAGE_CONTEXT)Context;
		...
		if (stringSize > 0) {
		    status = RtlStringCbPrintfW(stringIndex, stringSize, L"%d",
						ioLogMessageContext->DeviceNumber);
			...
			if (stringSize >= (LONG)pdoNameLength && pdoNameLength > 0) {
				...
				DevicePropertyPhysicalDeviceObjectName, pdoNameLength,
				stringIndex, &resultLength);
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		    "ClasspLogIORetriedEvent: DO (%p), Soft threshold notification "
		    "logged.\n",
		    DeviceObject));
    }
					    ...
					    IN OPTIONAL PTRANSFER_PACKET Pkt)
{
	...
	ioLogMessageContextHeader = (POPCODE_SENSE_DATA_IO_LOG_MESSAGE_CONTEXT_HEADER)
	    ioLogMessageContext;
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		"ClasspQueueLogIOEventWithContextWorker: DO (%p), Pkt (%p), Queueing IO "
		"retried event log message work item.\n",
		DeviceObject, Pkt));
    ...
    //
    IoQueueWorkItem(workItem, ClasspLogIOEventWithContext, DelayedWorkQueue,
		    ioLogMessageContextHeader);
				 ...
				 IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	cdb->PERSISTENT_RESERVE_OUT.ParameterListLength[1] = (UCHAR)sizeof(
	    PRO_PARAMETER_LIST);
	...
	RtlMoveMemory(prCommand, prCommand->PR_OUT.ParameterList,
    ...
    status = ClassSendSrbAsynchronous(DeviceObject, Srb, Irp, prCommand, dataBufLen,
				      writeToDevice);
...
NTSTATUS ClasspPriorityHint(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    priSupport->SupportFlags |= (1 << IoPriorityVeryLow) | (1 << IoPriorityLow) |
				(1 << IoPriorityNormal);

    TracePrint((TRACE_LEVEL_FATAL, TRACE_FLAG_IOCTL,
		"ClasspPriorityHint: I/O priorities supported: %X\n",
		priSupport->SupportFlags));
					     ...
					     OUT PULONG MaximumListIdentifier)
{
    ...
    //
    status = RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, RegistryPath, queryTable, NULL,
				    NULL);
	...
	*MaximumListIdentifier = 0;
    ...
    TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_PNP,
		"ClasspGetMaximumTokenListIdentifier (%p): Exiting function with status "
		"%x (maxListId %u).\n",
		DeviceObject, status, *MaximumListIdentifier));
					 ...
					 OUT PULONG MaxDuration)
{
    ...
    //
    if (NT_SUCCESS(status) && value > DEFAULT_MAX_TARGET_DURATION) {
	*MaxDuration = value;
    } else {
	*MaxDuration = DEFAULT_MAX_TARGET_DURATION;
    ...
    TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_PNP,
		"ClasspGetCopyOffloadMaxDuration (%p): Exiting function with status %x "
		"(Max Duration %u seconds).\n",
		DeviceObject, status, *MaxDuration));
					 ...
					 IN OUT PSCSI_REQUEST_BLOCK Srb)
{
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceCopyOffloadProperty (%p): Unsupported query type %x for "
		    "Copy Offload property.\n",
		    DeviceObject, query->QueryType));
	...
	if (length >= sizeof(STORAGE_DESCRIPTOR_HEADER)) {
	    TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_IOCTL,
			"ClasspDeviceCopyOffloadProperty (%p): Length %u specified for "
			"Copy Offload property enough only for header.\n",
			DeviceObject, length));
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceCopyOffloadProperty (%p): Incorrect length %u specified "
		    "for Copy Offload property.\n",
		    DeviceObject, length));
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceCopyOffloadProperty (%p): Command not supported on this "
		    "device.\n",
		    DeviceObject));
    ...
    if (!NT_SUCCESS(
	    fdoExtension->FunctionSupportInfo->BlockDeviceRODLimitsData.CommandStatus)) {
	status = fdoExtension->FunctionSupportInfo->BlockDeviceRODLimitsData.CommandStatus;

	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspDeviceCopyOffloadProperty (%p): VPD retrieval had failed with "
		    "%x.\n",
		    DeviceObject, status));
...
__ClasspDeviceCopyOffloadProperty_Exit:
    ...
    TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_IOCTL,
		"ClasspDeviceCopyOffloadProperty (%p): Exiting function with status "
		"%x.\n",
		DeviceObject, status));
					...
					IN PIRP Irp)
{
	...
	if (!NT_SUCCESS(
		fdoExt->FunctionSupportInfo->BlockDeviceRODLimitsData.CommandStatus)) {
	    status = fdoExt->FunctionSupportInfo->BlockDeviceRODLimitsData.CommandStatus;

	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
			"ClasspValidateOffloadSupported (%p): VPD retrieval failed with "
			"%x.\n",
			DeviceObject, status));
	...
	TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_IOCTL,
		    "ClasspValidateOffloadSupported (%p): Suported only on Disk "
		    "devices.\n",
		    DeviceObject));
					      ...
					      IN PIRP Irp)
{
    ...
    if (!dsmAttributes) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspValidateOffloadInputParameters (%p): NULL DsmAttributes "
		    "passed in.\n",
		    DeviceObject));
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspValidateOffloadInputParameters (%p): Input buffer size (%u) "
		    "too small.\n",
		    DeviceObject,
		    irpStack->Parameters.DeviceIoControl.InputBufferLength));
	...
	(dsmAttributes->DataSetRangesLength == 0)) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspValidateOffloadInputParameters (%p): Incorrect DataSetRanges "
		    "[offset %u, length %u].\n",
		    DeviceObject, dsmAttributes->DataSetRangesOffset,
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspValidateOffloadInputParameters (%p): DataSetRanges specifies "
		    "no extents.\n",
		    DeviceObject));
	...
	if ((!NT_SUCCESS(status)) || (fdoExtension->DiskGeometry.BytesPerSector == 0)) {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
			"ClasspValidateOffloadInputParameters (%p): Couldn't retrieve "
			"disk geometry, status: %x, bytes/sector: %u.\n",
	...
	if ((dataSetRanges[i].StartingOffset %
		 fdoExtension->DiskGeometry.BytesPerSector !=
	     0) ||
	    (dataSetRanges[i].LengthInBytes % fdoExtension->DiskGeometry.BytesPerSector !=
	     0) ||
	    (dataSetRanges[i].LengthInBytes == 0)) {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
						    ...
						    OUT OPTIONAL PULONG RecvTokenInfoBufLen)
{
	...
	tokenOperationHeaderSize = FIELD_OFFSET(POPULATE_TOKEN_HEADER,
						BlockDeviceRangeDescriptor);
	responseSize = FIELD_OFFSET(RECEIVE_TOKEN_INFORMATION_RESPONSE_HEADER,
				    TokenDescriptor) +
		       sizeof(BLOCK_DEVICE_TOKEN_DESCRIPTOR);
    } else {
	tokenOperationHeaderSize = FIELD_OFFSET(WRITE_USING_TOKEN_HEADER,
						BlockDeviceRangeDescriptor);
	responseSize = 0;
    }
	...
	MIN(FIELD_OFFSET(RECEIVE_TOKEN_INFORMATION_HEADER, SenseData) +
		MAX_SENSE_BUFFER_SIZE + responseSize,
	...
	*RecvTokenInfoBufLen = receiveTokenInformationBufferLength;
    ...
    TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_IOCTL,
		"ClasspGetTokenOperationCommandBufferLengths (%p): Exiting function with "
		"bufferLength %u (tokenOpBufLen %u, recTokenInfoBufLen %u).\n",
		Fdo, bufferLength, tokenOperationBufferLength,
		receiveTokenInformationBufferLength));
						 ...
						 OUT PULONGLONG MaxBlockDescriptorsLength)
{
    ...
    *MaxBlockDescriptorsCount = (MaxParameterBufferLength - tokenOperationHeaderSize) /
				sizeof(BLOCK_DEVICE_RANGE_DESCRIPTOR);
					   ...
					   IN OUT PULONGLONG TotalSectorsProcessed)
{
	...
	//
	sectorCount = MIN(DataSetRange->LengthInBytes /
			      fdoExtension->DiskGeometry.BytesPerSector,
	...
	REVERSE_BYTES_QUAD(blockDescr[*CurrentBlockDescrIndex].LogicalBlockAddress,
			   &startingSector);
			   ...
			   IN OUT PULONG UpdateLength)
{
	...
	*UpdateLength = 0;
    ...
    if (!buffer) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspBinaryToAscii (HexBuff %p): Failed to allocate buffer for "
		    "ASCII equivalent.\n",
		    HexBuffer));
...
__ClasspBinaryToAscii_Exit:
    ...
    TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_IOCTL,
		...
		HexBuffer, (buffer == NULL) ? "" : (const char *)buffer));
					...
					IN PIRP Irp)
{
    ...
    if (!storageEvents) {
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspStorageEventNotification (%p): NULL storage events passed "
		    "in.\n",
		    DeviceObject));
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspStorageEventNotification (%p): Input buffer size (%u) too "
		    "small.\n",
		    DeviceObject,
		    irpStack->Parameters.DeviceIoControl.InputBufferLength));
	...
	TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_IOCTL,
		    "ClasspStorageEventNotification (%p): Invalid version/size [version "
		    "%u, size %u].\n",
...
VOID ClasspZeroQERR(IN PDEVICE_OBJECT DeviceObject)
{
    ...
    if (modeData == NULL) {
	TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_SCSI,
    ...
    size = ClassModeSense(DeviceObject, (PCHAR)modeData, MODE_PAGE_DATA_SIZE,
			  MODE_PAGE_CONTROL);
    ...
    pageData = ClassFindModePage((PCHAR)modeData, size, MODE_PAGE_CONTROL, TRUE);

    if (pageData) {
	TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_SCSI,
		    "ClasspZeroQERR (%p): Current settings: QERR = %u, TST = %u, TAS = "
		    "%u.\n",
		    DeviceObject, pageData->QERR, pageData->TST, pageData->TAS));
	...
	if (pageData->QERR != 0) {
	    ...
	    status = ClasspModeSelect(DeviceObject, (PCHAR)modeData, size, pageSavable);

	    if (!NT_SUCCESS(status)) {
		TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_SCSI,
			    "ClasspZeroQERR (%p): Failed to set QERR = 0 with status "
			    "%x\n",
			    DeviceObject, status));
	    }
	}
...
NTSTATUS ClasspPowerActivateDevice(IN PDEVICE_OBJECT DeviceObject)
{
    ...
    irp = IoBuildDeviceIoControlRequest(IOCTL_STORAGE_POWER_ACTIVE,
					fdoExtension->LowerPdo, NULL, 0, NULL, 0, FALSE,
					&event, &ioStatus);
...
NTSTATUS ClasspPowerIdleDevice(IN PDEVICE_OBJECT DeviceObject)
{
    ...
    irp = IoBuildDeviceIoControlRequest(IOCTL_STORAGE_POWER_IDLE, fdoExtension->LowerPdo,
...
BOOLEAN ClasspIsThinProvisioningError(IN PSCSI_REQUEST_BLOCK Srb)
{
	...
	if (senseBuffer) {
	    ...
	    BOOLEAN validSense = ScsiGetSenseKeyAndCodes(senseBuffer,
							 ...
							 &senseKey, &addlSenseCode,
							 &addlSenseCodeQual);

	    return (validSense && (senseKey == SCSI_SENSE_UNIT_ATTENTION) &&
