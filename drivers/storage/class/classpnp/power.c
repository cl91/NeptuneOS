				       ...
				       IN PVOID Context)
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		"ClasspPowerUpCompletion: Device Object %p, Irp %p, "
		"Context %p\n",
	...
	if (NT_SUCCESS(Irp->IoStatus.Status)) {
		...
		if (secondsRemaining >=
		    (TIME_LEFT_FOR_UPPER_DRIVERS + startUnitTimeout)) {
		    ...
		    timeoutValue =
			MINIMUM_START_UNIT_TIMEOUT_VALUE; // use the minimum value for this corner case.
		}
	    ...
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
			"(%p)\tSending start unit to device\n", Irp));
	    ...
	    IoSetCompletionRoutine(fdoExtension->PrivateFdoData->PowerProcessIrp,
				   ClasspPowerUpCompletion, PowerContext, TRUE, TRUE,
				   TRUE);
	...
	if (SRB_STATUS(srb->SrbStatus) != SRB_STATUS_SUCCESS) {
	    ...
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_POWER,
		...
		fdoExtension->DeviceObject, Irp, srb,
		...
		SrbSetTimeOutValue(srb, startUnitTimeout);

		RetryPowerRequest(commonExtension->DeviceObject, Irp, PowerContext);
	...
	if (PowerContext->QueueLocked) {
	    ...
	    status = InitializeStorageRequestBlock(srb, STORAGE_ADDRESS_TYPE_BTL8,
						   CLASS_SRBEX_NO_SRBEX_DATA_BUFFER_SIZE, 0);
	    if (NT_SUCCESS(status)) {
		srb->SrbFunction = SRB_FUNCTION_UNLOCK_QUEUE;
	    } else {
		// Should not occur.
		NT_ASSERT(FALSE);
		return status;
	    }
	    SrbAssignSrbFlags(srb, SRB_FLAGS_BYPASS_LOCKED_QUEUE);
	    SrbSetOriginalRequest(srb,
				  fdoExtension->PrivateFdoData->PowerProcessIrp);
	...
	if (PowerContext->QueueLocked) {
	    ...
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
			"(%p)\tFall-through (queue not locked)\n", OriginalIrp));
	}

	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tFreeing srb and completing\n", OriginalIrp));
	...
	if ((commonExtension->DriverExtension->InitData.ClassTick != NULL) ||
	    ...
	    if ((FailurePredictionEnabled) &&
		(ClasspFailurePredictionPeriodMissed(fdoExtension))) {
	...
	PoSetPowerState(PowerContext->DeviceObject, currentStack->Parameters.Power.Type,
			currentStack->Parameters.Power.State);

	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tStarting next power irp\n", OriginalIrp));
	...
	if ((fdoExtension->PrivateFdoData) &&
	    ...
	    ClassCompleteRequest(commonExtension->DeviceObject, OriginalIrp,
				 IO_NO_INCREMENT);
					 ...
					 IN PVOID Context)
{
    PCLASS_POWER_CONTEXT PowerContext = (PCLASS_POWER_CONTEXT)Context;
    PFUNCTIONAL_DEVICE_EXTENSION fdoExtension =
	PowerContext->DeviceObject->DeviceExtension;
    ...
    PIO_STACK_LOCATION nextStack = IoGetNextIrpStackLocation(
	fdoExtension->PrivateFdoData->PowerProcessIrp);
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		"ClasspPowerDownCompletion: Device Object %p, "
		"Irp %p, Context %p\n",
		PowerContext->DeviceObject, Irp, Context));
	...
	if (PowerContext->Options.LockQueue && !NT_SUCCESS(Irp->IoStatus.Status)) {
	    ...
	    srb = &fdoExtension->PrivateFdoData->PowerSrb.SrbEx;
	    ...
	    status = InitializeStorageRequestBlock(srb, STORAGE_ADDRESS_TYPE_BTL8,
						   CLASS_SRBEX_NO_SRBEX_DATA_BUFFER_SIZE, 0);
	    if (NT_SUCCESS(status)) {
		srb->SrbFunction = SRB_FUNCTION_QUIESCE_DEVICE;
	    } else {
		//
		// Should not happen.
		//
	    ...
	    SrbAssignSrbFlags(srb,
			      (SRB_FLAGS_NO_DATA_TRANSFER | SRB_FLAGS_DISABLE_AUTOSENSE |
			       SRB_FLAGS_DISABLE_SYNCH_TRANSFER |
			       SRB_FLAGS_NO_QUEUE_FREEZE | SRB_FLAGS_BYPASS_LOCKED_QUEUE |
			       SRB_FLAGS_D3_PROCESSING));

	    IoSetCompletionRoutine(fdoExtension->PrivateFdoData->PowerProcessIrp,
				   ClasspPowerDownCompletion, PowerContext, TRUE, TRUE,
				   TRUE);
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tPreviously sent device quiesce\n", Irp));
	...
	if (!TEST_FLAG(fdoExtension->PrivateFdoData->HackFlags, FDO_HACK_NO_SYNC_CACHE)) {
	    //
	    ...
	    TracePrint((TRACE_LEVEL_WARNING, TRACE_FLAG_POWER,
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tPreviously send SCSIOP_SYNCHRONIZE_CACHE\n", Irp));
	...
	if (SRB_STATUS(srb->SrbStatus) != SRB_STATUS_SUCCESS) {
	    ...
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
			"(%p)\tSYNCHRONIZE_CACHE not retried\n", Irp));
	    fdoExtension->PrivateFdoData->MaxPowerOperationRetryCount = MAXIMUM_RETRIES;
	    PowerContext->RetryCount = MAXIMUM_RETRIES;
	...
	if ((currentStack->Parameters.Power.State.DeviceState == PowerDeviceD3) &&
	    ...
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
			"(%p)\tPower Down: not sending SPIN DOWN due to hibernation "
			"path\n",
			PowerContext->DeviceObject));
	...
	if (SRB_STATUS(srb->SrbStatus) != SRB_STATUS_SUCCESS) {
		...
		SrbSetTimeOutValue(srb, DEFAULT_IO_TIMEOUT_VALUE);
	...
	if (!NT_SUCCESS(status)) {
		...
		validSense = ScsiGetSenseKeyAndCodes(
		    senseBuffer, SrbGetSenseInfoBufferLength(srb),
	...
	if (PowerContext->QueueLocked) {
	    ...
	    SrbSetOriginalRequest(srb,
				  fdoExtension->PrivateFdoData->PowerProcessIrp);
	    SrbAssignSrbFlags(srb,
			      (SRB_FLAGS_BYPASS_LOCKED_QUEUE | SRB_FLAGS_D3_PROCESSING));
				   ...
				   IN CLASS_POWER_OPTIONS Options)
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		"ClasspPowerHandler: Power irp %p to %s %p\n", Irp,
		(commonExtension->IsFdo ? "fdo" : "pdo"), DeviceObject));
	...
	TracePrint(
	    (TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER, "(%p)\tIRP_MN_SET_POWER\n", Irp));

	TracePrint(
	    (TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER, "(%p)\tSetting %s state to %d\n",
	     Irp,
	     (irpStack->Parameters.Power.Type == SystemPowerState ? "System" : "Device"),
    ...
    if (irpStack->Parameters.Power.State.DeviceState == fdoExtension->DevicePowerState) {
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tAlready in device state %x\n", Irp,
		    fdoExtension->DevicePowerState));
	goto ClasspPowerHandlerCleanup;
    }
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tNot handling spinup to state %x\n", Irp,
		    fdoExtension->DevicePowerState));
	fdoExtension->DevicePowerState = irpStack->Parameters.Power.State.DeviceState;
	goto ClasspPowerHandlerCleanup;
    }
	...
	(irpStack->Parameters.Power.State.DeviceState != PowerDeviceD0)) {
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		    "(%p)\tNot handling spindown to state %x\n", Irp,
		    fdoExtension->DevicePowerState));
	fdoExtension->DevicePowerState = irpStack->Parameters.Power.State.DeviceState;
	goto ClasspPowerHandlerCleanup;
    }
    ...
    status = InitializeStorageRequestBlock(srb,
					   ...
					   CLASS_SRBEX_NO_SRBEX_DATA_BUFFER_SIZE, 0);
    if (NT_SUCCESS(status)) {
	srb->SrbFunction = SRB_FUNCTION_LOCK_QUEUE;
    } else {
	//
	// Should not happen.
	//
    ...
    SrbSetOriginalRequest(srb, fdoExtension->PrivateFdoData->PowerProcessIrp);
    SrbSetSrbFlags(srb,
		   (SRB_FLAGS_BYPASS_LOCKED_QUEUE | SRB_FLAGS_NO_QUEUE_FREEZE));
	...
	PoSetPowerState(DeviceObject, irpStack->Parameters.Power.Type,
	...
	IoSetCompletionRoutine(fdoExtension->PrivateFdoData->PowerProcessIrp,
			       context->CompletionRoutine, context, TRUE, TRUE, TRUE);
	...
	context->CompletionRoutine(DeviceObject,
				   fdoExtension->PrivateFdoData->PowerProcessIrp,
				   context);
    }
					 ...
					 _In_ PIRP Irp)
{
    ...
    if (!TEST_FLAG(fdoExtension->ScanForSpecialFlags, CLASS_SPECIAL_DISABLE_SPIN_DOWN)) {
	options.HandleSpinDown = TRUE;
    }

    if (!TEST_FLAG(fdoExtension->ScanForSpecialFlags, CLASS_SPECIAL_DISABLE_SPIN_UP)) {
	options.HandleSpinUp = TRUE;
    }

    if (!TEST_FLAG(fdoExtension->ScanForSpecialFlags, CLASS_SPECIAL_NO_QUEUE_LOCK)) {
	options.LockQueue = TRUE;
    }

    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		"ClasspPowerHandler: Devobj %p\n"
		"\t%shandling spin down\n"
		"\t%shandling spin up\n"
		"\t%slocking queue\n",
		DeviceObject, (options.HandleSpinDown ? "" : "not "),
		(options.HandleSpinUp ? "" : "not "), (options.LockQueue ? "" : "not ")));
			      ...
			      PCLASS_POWER_CONTEXT Context)
{
    ...
    IoSetCompletionRoutine(Irp, Context->CompletionRoutine, Context, TRUE, TRUE, TRUE);
...
NTSTATUS ClasspSendEnableIdlePowerIoctl(_In_ PDEVICE_OBJECT DeviceObject)
{
	...
	fdoExtension->FunctionSupportInfo->IdlePower.DeviceWakeable;
    ...
    ClassSendDeviceIoControlSynchronous(IOCTL_STORAGE_ENABLE_IDLE_POWER,
					commonExtension->LowerDeviceObject, &idlePower,
					sizeof(STORAGE_IDLE_POWER), 0, FALSE, &ioStatus);

    status = ioStatus.Status;

    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_POWER,
		"ClasspSendEnableIdlePowerIoctl: Port driver returned status (%x) for "
		"FDO (%p)\n"
		"\tWakeCapableHint: %u\n"
		"\tD3ColdSupported: %u\n"
		"\tD3IdleTimeout: %u (ms)",
		status, DeviceObject, idlePower.WakeCapableHint,
		idlePower.D3ColdSupported, idlePower.D3IdleTimeout));
					  ...
					  _Inout_opt_ PVOID Context)
{
	...
	while ((PLIST_ENTRY)fdoEntry != &IdlePowerFDOList) {
	    ...
	    if (!isRemoved) {
		PFUNCTIONAL_DEVICE_EXTENSION fdoExtension =
		    (PFUNCTIONAL_DEVICE_EXTENSION)fdoEntry->Fdo->DeviceExtension;
		...
		if (!fdoExtension->FunctionSupportInfo->IdlePower.D3IdleTimeoutOverridden) {
		    fdoExtension->FunctionSupportInfo->IdlePower.D3IdleTimeout =
			DiskIdleTimeoutInMS;
		...
		if (!isRemoved) {
			...
			if ((fdoExtension->DevicePowerState != PowerDeviceD0) &&
			    ...
			    (fdoExtension->FunctionSupportInfo
				 ->AsynchronousNotificationSupported == FALSE)) {
			...
			//
			if ((fdoExtension->FailurePredictionInfo != NULL) &&
			    (fdoExtension->FailurePredictionInfo->Method !=
...
NTSTATUS ClasspEnableIdlePower(_In_ PDEVICE_OBJECT DeviceObject)
{
    ...
    ClassGetDeviceParameter(fdoExtension, CLASSP_REG_SUBKEY_NAME,
			    CLASSP_REG_DISABLE_D3COLD, &d3ColdDisabledByUser);
	...
	fdoExtension->FunctionSupportInfo->IdlePower.D3IdleTimeout =
	    (idleTimeoutOverrideInSeconds * 1000);
	...
	if (fdoEntry) {
		...
		PoRegisterPowerSettingCallback(DeviceObject, &GUID_DISK_IDLE_TIMEOUT,
