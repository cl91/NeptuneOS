...
NTSTATUS InitializeTransferPackets(PDEVICE_OBJECT Fdo)
{
    ...
    status = ClassGetDescriptor(fdoExt->CommonExtension.LowerDeviceObject, &propertyId,
	...
	if (workingSet->XferPacketsWorkingSetMinimum != 0) {
		...
		workingSet->XferPacketsWorkingSetMinimum;
	    // adjust maximum upwards if needed
	    if (fdoData->LocalMaxWorkingSetTransferPackets <
		fdoData->LocalMinWorkingSetTransferPackets) {
	...
	if (workingSet->XferPacketsWorkingSetMaximum != 0) {
	    fdoData->LocalMaxWorkingSetTransferPackets =
		workingSet->XferPacketsWorkingSetMaximum;
	    // adjust minimum downwards if needed
	    if (fdoData->LocalMinWorkingSetTransferPackets >
		fdoData->LocalMaxWorkingSetTransferPackets) {
		fdoData->LocalMinWorkingSetTransferPackets =
		    fdoData->LocalMaxWorkingSetTransferPackets;
	    }
    ...
    for (index = 0; index < arraySize; index++) {
	while (fdoData->FreeTransferPacketsLists[index].NumFreeTransferPackets <
	       MIN_INITIAL_TRANSFER_PACKETS) {
	    PTRANSFER_PACKET pkt = NewTransferPacket(Fdo);
	    if (pkt) {
		InterlockedIncrement((volatile LONG *)&(
		    fdoData->FreeTransferPacketsLists[index].NumTotalTransferPackets));
	...
	fdoData->FreeTransferPacketsLists[index].DbgPeakNumTransferPackets =
	    fdoData->FreeTransferPacketsLists[index].NumTotalTransferPackets;
    }
...
VOID EnqueueFreeTransferPacket(PDEVICE_OBJECT Fdo, PTRANSFER_PACKET Pkt)
{
	...
	if (fdoData->FreeTransferPacketsLists[allocateNode].NumTotalTransferPackets >
		...
		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
			    "EnqueueFreeTransferPacket: Device (%p), queuing work item "
			    "to clean up free transfer packets.\n",
			    Fdo));
		...
		TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_GENERAL,
			    "EnqueueFreeTransferPacket: Device (%p), Failed to allocate "
	...
	if (fdoData->FreeTransferPacketsLists[allocateNode].NumTotalTransferPackets >
		...
		if (pktToDelete) {
			...
			(volatile LONG *)&(fdoData->FreeTransferPacketsLists[allocateNode]
					       .NumTotalTransferPackets));
		} else {
		    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW,
				"Extremely unlikely condition (non-fatal): %d packets "
				"dequeued at once for Fdo %p. NumTotalTransferPackets=%d "
				"(2). Node=%d",
				fdoData->LocalMinWorkingSetTransferPackets, Fdo,
				fdoData->FreeTransferPacketsLists[allocateNode]
				    .NumTotalTransferPackets,
				allocateNode));
		}
	    }
					     ...
					     IN ULONG Node)
{
	...
	InterlockedDecrement((volatile LONG *)&(
	    fdoData->FreeTransferPacketsLists[Node].NumFreeTransferPackets));
	...
	if (AllocIfNeeded) {
		...
		InterlockedIncrement(
		    (volatile LONG *)&fdoData->FreeTransferPacketsLists[Node]
			.NumTotalTransferPackets);
				  ...
				  PIRP OriginalIrp)
{
	...
	if (TEST_FLAG(fdoExt->DeviceFlags, DEV_USE_16BYTE_CDB)) {
	    ...
	    pCdb->CDB16.OperationCode = (majorFunc == IRP_MJ_READ) ? SCSIOP_READ16 :
								     SCSIOP_WRITE16;
    ...
    SrbSetSrbFlags(Pkt->Srb,
		   (majorFunc == IRP_MJ_READ) ? SRB_FLAGS_DATA_IN : SRB_FLAGS_DATA_OUT);
				   ...
				   IN PVOID Context)
{
	...
	if (numPacketsRemaining > 0) {
		...
		if (fdoExt->CommonExtension.DriverExtension->InitData.ClassStartIo) {
		    if (TEST_FLAG(SrbGetSrbFlags(pkt->Srb),
					   ...
					   IN ULONG Node)
{
    ...
    ULONG requiredNumPktToDelete =
	fdoData->FreeTransferPacketsLists[Node].NumTotalTransferPackets -
	fdoData->LocalMaxWorkingSetTransferPackets;

    if (LimitNumPktToDelete) {
	requiredNumPktToDelete = MIN(requiredNumPktToDelete,
				     MAX_CLEANUP_TRANSFER_PACKETS_AT_ONCE);
    }

    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW,
		"CleanupTransferPacketToWorkingSetSize (%p): Exiting stress, block "
		"freeing %d packets.",
	...
	if (pktToDelete) {
	    SimplePushSlist(&pktList, (PSINGLE_LIST_ENTRY)&pktToDelete->SlistEntry);
	    InterlockedDecrement((volatile LONG *)&fdoData->FreeTransferPacketsLists[Node]
				     .NumTotalTransferPackets);
	} else {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_RW,
			"Extremely unlikely condition (non-fatal): %d packets dequeued "
			"at once for Fdo %p. NumTotalTransferPackets=%d (1). Node=%d",
			fdoData->LocalMaxWorkingSetTransferPackets, Fdo,
			fdoData->FreeTransferPacketsLists[Node].NumTotalTransferPackets,
			Node));
	    break;
	}
					    ...
					    IN ULONG ListIdentifier)
{
    ...
    TracePrint((TRACE_LEVEL_VERBOSE, TRACE_FLAG_IOCTL,
		"ClasspSetupPopulateTokenTransferPacket (%p): Exiting function with Irp "
		"%p\n",
		Pkt->Fdo, OriginalIrp));
