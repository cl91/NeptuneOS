...
#define DiskReadSmartData(FdoExtension, SrbControl, BufferSize)                   \
    DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_READ_SMART_ATTRIBS, \
			    SMART_CMD, READ_ATTRIBUTES, 0, 0, (SrbControl),       \
			    (BufferSize))
...
#define DiskReadSmartThresholds(FdoExtension, SrbControl, BufferSize)                \
    DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_READ_SMART_THRESHOLDS, \
			    SMART_CMD, READ_THRESHOLDS, 0, 0, (SrbControl),          \
			    (BufferSize))
...
#define DiskReadSmartStatus(FdoExtension, SrbControl, BufferSize)                       \
    DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_RETURN_STATUS, SMART_CMD, \
			    RETURN_SMART_STATUS, 0, 0, (SrbControl), (BufferSize))
...
#define DiskGetIdentifyData(FdoExtension, SrbControl, BufferSize)                        \
    DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_IDENTIFY, ID_CMD, 0, 0, 0, \
			    (SrbControl), (BufferSize))
...
static NTSTATUS DiskEnableSmart(PFUNCTIONAL_DEVICE_EXTENSION FdoExtension)
{
    ...
    return DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_ENABLE_SMART,
				   SMART_CMD, ENABLE_SMART, 0, 0,
...
static NTSTATUS DiskDisableSmart(PFUNCTIONAL_DEVICE_EXTENSION FdoExtension)
{
    ...
    return DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_DISABLE_SMART,
				   SMART_CMD, DISABLE_SMART, 0, 0,
				   (PSRB_IO_CONTROL)srbControl, &bufferSize);
}
					    ...
					    UCHAR Subcommand)
{
    ...
    return DiskPerformSmartCommand(FdoExtension,
				   IOCTL_SCSI_MINIPORT_EXECUTE_OFFLINE_DIAGS, SMART_CMD,
				   EXECUTE_OFFLINE_DIAGS, 0, Subcommand,
				   (PSRB_IO_CONTROL)srbControl, &bufferSize);
}
			  ...
			  IN UCHAR SectorCount, IN UCHAR LogAddress, OUT PUCHAR Buffer)
{
	...
	status = DiskPerformSmartCommand(FdoExtension, IOCTL_SCSI_MINIPORT_READ_SMART_LOG,
					 SMART_CMD, SMART_READ_LOG, SectorCount,
					 LogAddress, srbControl, &bufferSize);

	if (NT_SUCCESS(status)) {
	    sendCmdOutParams = (PSENDCMDOUTPARAMS)((PUCHAR)srbControl +
						   sizeof(SRB_IO_CONTROL));
	    RtlCopyMemory(Buffer, &sendCmdOutParams->bBuffer[0], logSize);
	}
			   ...
			   IN UCHAR SectorCount, IN UCHAR LogAddress, IN PUCHAR Buffer)
{
	...
	RtlCopyMemory(&sendCmdInParams->bBuffer[0], Buffer, logSize);
	status = DiskPerformSmartCommand(FdoExtension,
					 IOCTL_SCSI_MINIPORT_WRITE_SMART_LOG, SMART_CMD,
					 SMART_WRITE_LOG, SectorCount, LogAddress,
					 srbControl, &bufferSize);
				 ...
				 OUT PULONG BufferSize)
{
	...
	if (Command == SMART_CMD) {
		...
		lengthNeeded = max(lengthNeeded, sizeof(SENDCMDOUTPARAMS) - 1 +
    ...
    irp = IoBuildSynchronousFsdRequest(IRP_MJ_SCSI, commonExtension->LowerDeviceObject,
				       SrbControl, length, &startingOffset, &event,
				       &ioStatus);
    ...
    storAddrBtl8->Lun = diskData->ScsiAddress.Lun;
			     ...
			     PBOOLEAN SupportSmart)
{
	...
	PUSHORT identifyData =
	    (PUSHORT) &
	    (outBuffer[sizeof(SRB_IO_CONTROL) + sizeof(SENDCMDOUTPARAMS) - 1]);
	USHORT commandSetSupported = identifyData[82];

	*SupportSmart = ((commandSetSupported != 0xffff) && (commandSetSupported != 0) &&
	...
	*SupportSmart = FALSE;
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_IOCTL,
		...
		*SupportSmart ? "is" : "is not", FdoExtension->DeviceObject, status));
				     ...
				     PSTORAGE_PREDICT_FAILURE checkFailure)
{
    ...
    irp = IoBuildDeviceIoControlRequest(IOCTL_STORAGE_PREDICT_FAILURE, deviceObject, NULL,
					0, checkFailure, sizeof(STORAGE_PREDICT_FAILURE),
			 ...
			 OUT PVOID *PageData)
{
	...
	*ModeDataSize < sizeof(MODE_PARAMETER_HEADER) || PageData == NULL) {
    ...
    size = ClassModeSenseEx(Fdo, (PCHAR)ModeData, *ModeDataSize, PageMode, PageControl);

    if (size < sizeof(MODE_PARAMETER_HEADER)) {
	//
	// Retry the request in case of a check condition.
	//
	size = ClassModeSenseEx(Fdo, (PCHAR)ModeData, *ModeDataSize, PageMode,
				PageControl);

	if (size < sizeof(MODE_PARAMETER_HEADER)) {
	    TracePrint((TRACE_LEVEL_ERROR, TRACE_FLAG_WMI,
			"DiskGetModePage: Mode Sense for Page Mode %d with Page Control "
			"%d failed\n",
    ...
    *ModeDataSize = size;
    ...
    // Find the mode page
    //
    pageData = ClassFindModePage((PCHAR)ModeData, size, PageMode, TRUE);
				  ...
				  IN BOOLEAN Enable)
{
    ...
    //
    status = DiskGetModePage(FdoExtension->DeviceObject, MODE_PAGE_FAULT_REPORTING,
			     1, // Page Control = 1 indicates we want changeable values.
			     modeData, &modeDataSize, (PVOID *)&pageData);
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		    "DiskEnableInfoExceptions: does NOT support DEXCPT bit for device "
		    "%p\n",
		    FdoExtension->DeviceObject));
	FREE_POOL(modeData);
	return STATUS_NOT_SUPPORTED;
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		    "DiskEnableInfoExceptions: MRIE field is 0 and is not changeable for "
		    "device %p\n",
		    FdoExtension->DeviceObject));
	FREE_POOL(modeData);
	return STATUS_NOT_SUPPORTED;
    ...
    status = ClassModeSelect(FdoExtension->DeviceObject, (PCHAR)modeData, modeDataSize,
			     pageData->PSBit);
				      ...
				      PSTORAGE_FAILURE_PREDICT_STATUS DiskSmartStatus)
{
	...
	UCHAR outBuffer[sizeof(SRB_IO_CONTROL) +
			max(sizeof(SENDCMDINPARAMS),
			    sizeof(SENDCMDOUTPARAMS) - 1 + sizeof(IDEREGS))] = { 0 };
	ULONG outBufferSize = sizeof(outBuffer);
	PSENDCMDOUTPARAMS cmdOutParameters;

	status = DiskReadSmartStatus(FdoExtension, (PSRB_IO_CONTROL)outBuffer,
				     &outBufferSize);

	if (NT_SUCCESS(status)) {
	    cmdOutParameters = (PSENDCMDOUTPARAMS)(outBuffer + sizeof(SRB_IO_CONTROL));
				    ...
				    PSTORAGE_FAILURE_PREDICT_DATA DiskSmartData)
{
	...
	if (outBuffer != NULL) {
		...
		RtlCopyMemory(DiskSmartData->VendorSpecific, cmdOutParameters->bBuffer,
...
NTAPI VOID DiskReregWorker(IN PDEVICE_OBJECT DevObject, IN PVOID Context)
{
	...
	if (reregRequest != NULL) {
	    ...
	    status = IoWMIRegistrationControl(deviceObject, WMIREG_ACTION_UPDATE_GUIDS);
...
NTSTATUS DiskPostReregisterRequest(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if (InterlockedIncrement(&DiskReregWorkItems) == 1) {
	    ...
	    IoQueueWorkItem(workItem, DiskReregWorker, DelayedWorkQueue, workItem);
	} else {
	    //
					 ...
					 PVOID Context)
{
    ...
    srbStatus = SRB_STATUS(srbEx->SrbStatus);
	...
	(void)GetSrbScsiData(srbEx, &cdbLength8, &cdbLength32, NULL, &senseBuffer,
			     &senseBufferLength);
    }
	...
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_GENERAL,
		    "DiskInfoExceptionComplete: IRP %p, SRB %p\n", Irp, srbEx));

	if (TEST_FLAG(srbEx->SrbStatus, SRB_STATUS_QUEUE_FROZEN)) {
	    ClassReleaseQueue(DeviceObject);
	}

	retry = ClassInterpretSenseInfo(
	    DeviceObject, srbEx, irpStack->MajorFunction, 0,
	...
	if (retry) {
	    ...
	    if ((srbEx->SrbFunction == SRB_FUNCTION_EXECUTE_SCSI) &&
		(srbEx->NumSrbExData > 0)) {
		SetSrbScsiData(srbEx, cdbLength8, cdbLength32, 0, senseBuffer,
			       senseBufferLength);
	    }
	    ...
	    nextIrpStack->Parameters.Scsi.Srb = srbEx;
	...
	pageData = ClassFindModePage((PCHAR)modeData, modeDataLength,
				     MODE_PAGE_FAULT_REPORTING, TRUE);
	if (pageData != NULL) {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			"DiskInfoExceptionComplete: %p supports SMART\n", DeviceObject));
	    ...
	    //
	    if (pageData->Dexcpt == 0 && pageData->ReportMethod >= 2 &&
				     ...
				     BOOLEAN ScsiAddressAvailable)
{
    ...
    *FailurePredictCapability = FailurePredictionNone;
    ...
    //
    status = DiskSendFailurePredictIoctl(FdoExtension, &checkFailure);

    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		"Disk: Device %p %s IOCTL_STORAGE_FAILURE_PREDICT\n",
		FdoExtension->DeviceObject, NT_SUCCESS(status) ? "does" : "does not"));
	...
	*FailurePredictCapability = FailurePredictionIoctl;
	...
	if (checkFailure.PredictFailure) {
		...
		(BOOLEAN)(FdoExtension->FailurePredicted == FALSE), 0x11,
		diskData->ScsiAddress.PathId, diskData->ScsiAddress.TargetId,
				      ...
				      IN BOOLEAN Enable)
{
	...
	if ((GuidIndex == SmartStatusGuid) || (GuidIndex == SmartDataGuid) ||
	    (GuidIndex == SmartThresholdsGuid) || (GuidIndex == SmartPerformFunction)) {
	    status = DiskEnableDisableFailurePrediction(fdoExtension, TRUE);
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			"Disk: DeviceObject %p, Irp %p Enable -> %lx\n", DeviceObject,
			Irp, status));

	} else {
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			"Disk: DeviceObject %p, Irp %p, GuidIndex %d %s for Collection\n",
			DeviceObject, Irp, GuidIndex, Enable ? "Enabled" : "Disabled"));
	}
    } else if (Function == EventGeneration) {
	TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		    "Disk: DeviceObject %p, Irp %p, GuidIndex %d %s for Event "
		    "Generation\n",
		    DeviceObject, Irp, GuidIndex, Enable ? "Enabled" : "Disabled"));

	if ((GuidIndex == SmartEventGuid) && Enable) {
	    status = DiskEnableDisableFailurePredictPolling(fdoExtension, Enable, 0);
	    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			"Disk: DeviceObject %p, Irp %p %s -> %lx\n", DeviceObject, Irp,
    ...
    status = ClassWmiCompleteRequest(DeviceObject, Irp, status, 0, IO_NO_INCREMENT);
    return status;
}
				      ...
				      OUT PUNICODE_STRING InstanceName)
{
    ...
    switch (diskData->FailurePredictionCapability) {
    case FailurePredictionSmart: {
	CLEAR_FLAG(DiskWmiFdoGuidList[SmartThresholdsGuid].Flags,
	...
	CLEAR_FLAG(DiskWmiFdoGuidList[SmartPerformFunction].Flags,
		   WMIREG_FLAG_REMOVE_GUID);
	...
	CLEAR_FLAG(DiskWmiFdoGuidList[SmartPerformFunction].Flags,
		   WMIREG_FLAG_REMOVE_GUID);
	CLEAR_FLAG(DiskWmiFdoGuidList[ScsiInfoExceptionsGuid].Flags,
		   WMIREG_FLAG_REMOVE_GUID);
					...
					OUT PUNICODE_STRING MofName)
{
    ...
    status = DiskFdoQueryWmiRegInfo(DeviceObject, RegFlags, InstanceName);
					...
					OUT PUCHAR Buffer)
{
	...
	if (BufferAvail >= sizeNeeded) {
	    ...
	    RtlMoveMemory(Buffer, &(fdoExtension->DiskGeometry), sizeof(DISK_GEOMETRY));
	...
	NT_ASSERT((diskData->FailurePredictionCapability == FailurePredictionSmart) ||
	...
	if (BufferAvail >= sizeNeeded) {
	    ...
	    status = DiskReadFailurePredictThresholds(fdoExtension, diskSmartThresholds);
	} else {
	    status = STATUS_BUFFER_TOO_SMALL;
	}
	...
	NT_ASSERT((diskData->FailurePredictionCapability == FailurePredictionSense));

	sizeNeeded = sizeof(STORAGE_SCSI_INFO_EXCEPTIONS);
	if (BufferAvail >= sizeNeeded) {
	    infoExceptions = (PSTORAGE_SCSI_INFO_EXCEPTIONS)Buffer;
	    status = DiskGetInfoExceptionInformation(fdoExtension, &modeInfo);
	    if (NT_SUCCESS(status)) {
		...
		REVERSE_BYTES(&infoExceptions->IntervalTimer, &modeInfo.IntervalTimer);
		REVERSE_BYTES(&infoExceptions->ReportCount, &modeInfo.ReportCount)
	    }
	} else {
	    status = STATUS_BUFFER_TOO_SMALL;
	}
				      ...
				      IN PUCHAR Buffer)
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		...
		DeviceObject, Irp, GuidIndex, BufferSize, Buffer));
	...
	if (BufferSize >= sizeof(STORAGE_SCSI_INFO_EXCEPTIONS)) {
	    ...
	    REVERSE_BYTES(&modeInfo.IntervalTimer[0], &infoExceptions->IntervalTimer);

	    REVERSE_BYTES(&modeInfo.ReportCount[0], &infoExceptions->ReportCount);
	    ...
	    status = DiskSetInfoExceptionInformation(fdoExtension, &modeInfo);
	} else {
				     ...
				     IN ULONG BufferSize, IN PUCHAR Buffer)
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		...
		"      BufferSize %#x Buffer %p\n",
		DeviceObject, Irp, GuidIndex, DataItemId, BufferSize, Buffer));
				       ...
				       IN PUCHAR Buffer)
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
		...
		DeviceObject, Irp, GuidIndex, MethodId, InBufferSize, OutBufferSize,
		Buffer));
	...
	case AllowDisallowPerformanceHit: {
		...
		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			    "DiskFdoWmiExecuteMethod: AllowPerformanceHit %x for device "
			    "%p --> %lx\n",
			    allowPerfHit, fdoExtension->DeviceObject, status));
	    } else {
		status = STATUS_INVALID_PARAMETER;
	    }
	    break;
	...
	case EnableDisableHardwareFailurePrediction: {
		...
		if (!enable) {
		    ...
		    // polling
		    //
		    DiskEnableDisableFailurePredictPolling(fdoExtension, enable, 0);
		}

		status = DiskEnableDisableFailurePrediction(fdoExtension, enable);

		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			    "DiskFdoWmiExecuteMethod: "
			    "EnableDisableHardwareFailurePrediction: %x for device %p "
			    "--> %lx\n",
			    enable, fdoExtension->DeviceObject, status));
	    } else {
		status = STATUS_INVALID_PARAMETER;
	    }
	    break;
	...
	case EnableDisableFailurePredictionPolling: {
		...
		status = DiskEnableDisableFailurePredictPolling(fdoExtension, enable,
								period);

		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			    "DiskFdoWmiExecuteMethod: "
			    "EnableDisableFailurePredictionPolling: %x %x for device %p "
			    "--> %lx\n",
			    enable, period, fdoExtension->DeviceObject, status));
	    } else {
		status = STATUS_INVALID_PARAMETER;
	    }
	    break;
	...
	case GetFailurePredictionCapability: {
		...
		*((PFAILURE_PREDICTION_METHOD)Buffer) =
		    diskData->FailurePredictionCapability;
		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			    "DiskFdoWmiExecuteMethod: GetFailurePredictionCapability: %x "
			    "for device %p --> %lx\n",
			    *((PFAILURE_PREDICTION_METHOD)Buffer),
	...
	case EnableOfflineDiags: {
		...
		if (diskData->FailurePredictionCapability == FailurePredictionSmart) {
		    ...
		    // of disk checking.
		    //
		    status = DiskExecuteSmartDiagnostics(fdoExtension, 0);
		...
		*((PBOOLEAN)Buffer) = NT_SUCCESS(status);
		...
		TracePrint((TRACE_LEVEL_INFORMATION, TRACE_FLAG_WMI,
			    "DiskFdoWmiExecuteMethod: EnableOfflineDiags for device %p "
			    "--> %lx\n",
			    fdoExtension->DeviceObject, status));
	    } else {
		status = STATUS_BUFFER_TOO_SMALL;
	...
	case ReadLogSectors: {
		...
		if (InBufferSize >= sizeof(READ_LOG_SECTORS_IN)) {
		    ...
		    sizeNeeded = FIELD_OFFSET(READ_LOG_SECTORS_OUT, LogSectors) +
				 readSize;

		    if (OutBufferSize >= sizeNeeded) {
			outParams = (PREAD_LOG_SECTORS_OUT)Buffer;
			status = DiskReadSmartLog(fdoExtension, inParams->SectorCount,
						  inParams->LogAddress,
	...
	case WriteLogSectors: {
	    if (diskData->FailurePredictionCapability == FailurePredictionSmart) {
		if ((LONG)InBufferSize >=
		    FIELD_OFFSET(WRITE_LOG_SECTORS_IN, LogSectors)) {
		    ...
		    if (InBufferSize >=
			(FIELD_OFFSET(WRITE_LOG_SECTORS_IN, LogSectors) + writeSize)) {
