			       ...
			       OUT PDEVICE_OBJECT *Pdo)
{
    ...
    partitionDevice->StackSize = Fdo->StackSize;
    ...
    partExt->OnDiskNumber = PartitionEntry->PartitionNumber; // the "physical" partition
							     // number
...
static NTSTATUS PartitionHandleStartDevice(IN PPARTITION_EXTENSION PartExt,
					   IN PIRP Irp)
{
    PAGED_CODE();
    ...
    swprintf(nameBuf, PartitionSymLinkFormat, fdoExtension->DiskData.DeviceNumber,
	     PartExt->DetectedNumber);
    ...
    status = IoRegisterDeviceInterface(PartExt->DeviceObject,
				       &GUID_DEVINTERFACE_PARTITION, NULL,
				       &interfaceName);
    if (!NT_SUCCESS(status)) {
	return status;
    }
    ...
    status = IoRegisterDeviceInterface(PartExt->DeviceObject, &GUID_DEVINTERFACE_VOLUME,
				       NULL, &interfaceName);
    if (!NT_SUCCESS(status)) {
	return status;
    }
 ...
 **/
static NTSTATUS VolumeDeleteMountPoints(IN PPARTITION_EXTENSION PartExt)
{
    NTSTATUS Status;
	...
	Status = IssueSyncIoControlRequest(IOCTL_MOUNTMGR_DELETE_POINTS, DeviceObject,
					   InputBuffer, InputSize, OutputBuffer,
					   OutputSize, FALSE);
			       ...
			       IN BOOLEAN FinalRemove)
{
	...
	if (!NT_SUCCESS(status)) {
	    ERR("VolumeDeleteMountPoints(%wZ) failed with status 0x%08x\n",
...
static NTSTATUS PartitionHandleDeviceRelations(IN PPARTITION_EXTENSION PartExt,
					       IN PIRP Irp)
{
    PAGED_CODE();
...
static NTSTATUS PartitionHandleQueryId(IN PPARTITION_EXTENSION PartExt, IN PIRP Irp)
{
    PIO_STACK_LOCATION ioStack = IoGetCurrentIrpStackLocation(Irp);
	...
	if (fdoExtension->DiskData.PartitionStyle == PARTITION_STYLE_MBR) {
	    swprintf(string, L"S%08lx_O%I64x_L%I64x",
		     fdoExtension->DiskData.Mbr.Signature, PartExt->StartingOffset,
	    ...
	    swprintf(string,
		     ...
		     fdoExtension->DiskData.Gpt.DiskId.Data4[7], PartExt->StartingOffset,
		     PartExt->PartitionLength);
	}
...
static NTSTATUS PartitionHandleQueryCapabilities(IN PPARTITION_EXTENSION PartExt,
						 IN PIRP Irp)
{
    PIO_STACK_LOCATION ioStack = IoGetCurrentIrpStackLocation(Irp);
...
NTSTATUS PartitionHandlePnp(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
				      ...
				      IN PIRP Irp)
{
	...
	*partInfoEx = (PARTITION_INFORMATION_EX){
	...
	if (fdoExtension->DiskData.PartitionStyle == PARTITION_STYLE_MBR) {
	    ...
	    RtlCopyMemory(partInfoEx->Gpt.Name, partExt->Gpt.Name,
	...
	PSET_PARTITION_INFORMATION inputBuffer = Irp->SystemBuffer;
	...
	PSET_PARTITION_INFORMATION_EX inputBuffer = Irp->SystemBuffer;
	if (!VerifyIrpInBufferSize(Irp, sizeof(*inputBuffer))) {
	    status = STATUS_INFO_LENGTH_MISMATCH;
	    break;
	}
	...
	status = IoSetPartitionInformationEx(fdoExtension->LowerDevice,
					     partExt->OnDiskNumber, inputBuffer);
	...
	if (NT_SUCCESS(status)) {
		...
		RtlMoveMemory(partExt->Gpt.Name, inputBuffer->Gpt.Name,
			      sizeof(partExt->Gpt.Name));
	    }
	}
	...
	PGET_LENGTH_INFORMATION lengthInfo = Irp->SystemBuffer;
	...
	PVOLUME_DISK_EXTENTS volExts = Irp->SystemBuffer;
	...
	*volExts = (VOLUME_DISK_EXTENTS){
	    ...
	    .Extents = { { .DiskNumber = fdoExtension->DiskData.DeviceNumber,
			   .StartingOffset.QuadPart = partExt->StartingOffset,
	...
	PVOLUME_NUMBER volNum = Irp->SystemBuffer;
	...
	PVOLUME_GET_GPT_ATTRIBUTES_INFORMATION gptAttrs = Irp->SystemBuffer;
	...
	PMOUNTDEV_NAME name = Irp->SystemBuffer;
	...
	PMOUNTDEV_UNIQUE_ID uniqueId = Irp->SystemBuffer;
	...
	//
	// Format:
	// - Basic volume on MBR disk: disk Mbr.Signature + partition StartingOffset
	// (length: 0x0C)
	...
	if ((fdoExtension->DiskData.PartitionStyle == PARTITION_STYLE_MBR) &&
	    ...
	    RtlCopyMemory(uniqueId->UniqueId, InterfaceName->Buffer,
	...
	WARN("Ignored MountMgr notification: 0x%X\n",
