 ...
 * partmgr.sys does. Here is acts like both partition and volume manager, because
 * volmgr.sys does not (yet) exist in ReactOS. Thus handles some IOCTL_VOLUME_*, and
 * IOCTL_MOUNTMGR_* IOCTLs.
 */
...
static PDRIVE_LAYOUT_INFORMATION
PartMgrConvertExtendedToLayout(_In_ CONST PDRIVE_LAYOUT_INFORMATION_EX LayoutEx)
{
...
static PDRIVE_LAYOUT_INFORMATION_EX
PartMgrConvertLayoutToExtended(_In_ CONST PDRIVE_LAYOUT_INFORMATION Layout)
{
...
static BOOLEAN PartMgrIsDiskSuperFloppy(_In_ PFDO_EXTENSION FdoExtension)
{
    ...
    ASSERT(FdoExtension->DiskData.PartitionStyle ==
	   FdoExtension->LayoutCache->PartitionStyle);
    ...
    ASSERT(FdoExtension->DiskData.Mbr.Signature ==
	   FdoExtension->LayoutCache->Mbr.Signature);
    if (FdoExtension->DiskData.Mbr.Signature != 1) {
	WARN("Super-Floppy disk %u signature %08x != 1!\n",
	...
	WARN("Super-Floppy disk %u does not return default settings!\n"
					  ...
					  _Inout_ PDRIVE_LAYOUT_INFORMATION_EX NewLayout)
{
	...
	status = PartitionCreateDevice(FdoExtension->DeviceObject, partEntry, pdoNumber,
				       NewLayout->PartitionStyle, &partitionDevice);
	...
	while (curEntry != NULL) {
	    ...
	    } else { // we found where to put the partition
		break;
	    }
	}
					       ...
					       _In_ PIRP Irp)
{
    ...
    PDISK_GEOMETRY_EX_INTERNAL geometryEx = Irp->SystemBuffer;
    ...
    status = IssueSyncIoControlRequest(IOCTL_DISK_GET_DRIVE_GEOMETRY_EX,
				       FdoExtension->LowerDevice, NULL, 0, geometryEx,
				       outBufferLength, FALSE);
...
static NTSTATUS FdoIoctlDiskGetPartitionInfo(_In_ PFDO_EXTENSION FdoExtension,
					     _In_ PIRP Irp)
{
    PPARTITION_INFORMATION partInfo = Irp->SystemBuffer;
...
static NTSTATUS FdoIoctlDiskGetPartitionInfoEx(_In_ PFDO_EXTENSION FdoExtension,
					       _In_ PIRP Irp)
{
    PPARTITION_INFORMATION_EX partInfoEx = Irp->SystemBuffer;
					   ...
					   _In_ PIRP Irp)
{
    ...
    RtlMoveMemory(Irp->SystemBuffer, partitionList, size);
					     ...
					     _In_ PIRP Irp)
{
    ...
    RtlCopyMemory(Irp->SystemBuffer, layoutEx, size);
...
static NTSTATUS FdoIoctlDiskSetDriveLayout(_In_ PFDO_EXTENSION FdoExtension,
					   _In_ PIRP Irp)
{
    PDRIVE_LAYOUT_INFORMATION layoutInfo = Irp->SystemBuffer;
    ...
    IoReportTargetDeviceChangeAsynchronous(FdoExtension->PhysicalDiskDO, &notification,
					   NULL, NULL);
...
static NTSTATUS FdoIoctlDiskSetDriveLayoutEx(_In_ PFDO_EXTENSION FdoExtension,
					     _In_ PIRP Irp)
{
    PDRIVE_LAYOUT_INFORMATION_EX layoutEx, layoutUser = Irp->SystemBuffer;
    NTSTATUS status;
...
static NTSTATUS FdoIoctlDiskCreateDisk(_In_ PFDO_EXTENSION FdoExtension, _In_ PIRP Irp)
{
    ...
    PCREATE_DISK createDisk = Irp->SystemBuffer;
...
static NTSTATUS FdoHandleStartDevice(_In_ PFDO_EXTENSION FdoExtension, _In_ PIRP Irp)
{
    ...
    NTSTATUS status = IssueSyncIoControlRequest(IOCTL_STORAGE_GET_DEVICE_NUMBER,
						FdoExtension->LowerDevice, NULL, 0,
    ...
    status = IoRegisterDeviceInterface(FdoExtension->PhysicalDiskDO,
				       &GUID_DEVINTERFACE_DISK, NULL, &interfaceName);
    if (!NT_SUCCESS(status)) {
	ERR("Failed to register GUID_DEVINTERFACE_DISK, status %x\n", status);
	/* Failure to register device interface is not a fatal error, so we continue. */
	return STATUS_SUCCESS;
    }
...
static NTSTATUS FdoHandleSurpriseRemoval(_In_ PFDO_EXTENSION FdoExtension, _In_ PIRP Irp)
{
    ...
    for (PSINGLE_LIST_ENTRY curEntry = FdoExtension->PartitionList.Next; curEntry != NULL;
	 curEntry = curEntry->Next) {
	PPARTITION_EXTENSION partExt = CONTAINING_RECORD(curEntry, PARTITION_EXTENSION,
							 ListEntry);
				       ...
				       _In_ PDEVICE_OBJECT PhysicalDeviceObject)
{
     ...
     */
    NTSTATUS status = IoCreateDevice(DriverObject, sizeof(FDO_EXTENSION), NULL,
					   ...
					   _In_ PIRP Irp)
{
    ...
    // IOCTLs at an IRQL higher than PASSIVE_LEVEL

    INFO("IRP_MJ_DEVICE_CONTROL %p Irp %p IOCTL %x isFdo: %u\n", DeviceObject, Irp,
	 ioStack->Parameters.DeviceIoControl.IoControlCode, fdoExtension->IsFDO);
...
static NTSTATUS NTAPI PartMgrPnp(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp)
{
    ...
    INFO("IRP_MJ_PNP %p Irp %p %s isFDO: %u\n", DeviceObject, Irp,
	 GetIRPMinorFunctionString(ioStack->MinorFunction), fdoExtension->IsFDO);
...
static DRIVER_DISPATCH PartMgrPower;
static NTAPI NTSTATUS PartMgrPower(_In_ PDEVICE_OBJECT DeviceObject,
				   _In_ PIRP Irp)
{
    PPARTITION_EXTENSION partExt = DeviceObject->DeviceExtension;
    PIO_STACK_LOCATION ioStack = IoGetCurrentIrpStackLocation(Irp);
...
static DRIVER_DISPATCH PartMgrShutdownFlush;
static NTAPI NTSTATUS PartMgrShutdownFlush(_In_ PDEVICE_OBJECT DeviceObject,
					   _In_ PIRP Irp)
{
    PPARTITION_EXTENSION partExt = DeviceObject->DeviceExtension;
...
static NTAPI VOID PartMgrUnload(_In_ PDRIVER_OBJECT DriverObject)
{
}
