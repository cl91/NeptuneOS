				   ...
				   IN PUNICODE_STRING DeviceName)
{
    ...
    /* Get device name */
    Status = QueryDeviceInformation(DeviceName, &TargetDeviceName, NULL, NULL, NULL, NULL,
				    NULL, NULL);
	...
	Status = RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath,
				       SymLink.Buffer, REG_BINARY, UniqueId->UniqueId,
    ...
    Status = RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, SymLink.Buffer,
    ...
    SymlinkInformation->Name.Buffer = AllocatePool(
	SymlinkInformation->Name.MaximumLength);
    ...
    RtlCopyMemory(SymlinkInformation->Name.Buffer, SymLink.Buffer,
		  SymlinkInformation->Name.Length);
    SymlinkInformation->Name.Buffer[SymlinkInformation->Name.Length / sizeof(WCHAR)] =
	UNICODE_NULL;
			       ...
			       IN OPTIONAL PUNICODE_STRING SymbolicName)
{
	...
	TotalSize += DeviceInformation->DeviceName.Length +
    ...
    TotalSize = sizeof(MOUNTMGR_MOUNT_POINTS) +
					 ...
					 IN PIRP Irp)
{
	...
	MountPoints->MountPoints[0].UniqueIdOffset = sizeof(MOUNTMGR_MOUNT_POINTS) +
						     SymlinkInformation->Name.Length;
	MountPoints->MountPoints[0].UniqueIdLength =
	    DeviceInformation->UniqueId->UniqueIdLength;
    } else {
    ...
    MountPoints->MountPoints[0].DeviceNameOffset =
	sizeof(MOUNTMGR_MOUNT_POINTS) + SymlinkInformation->Name.Length +
	DeviceInformation->UniqueId->UniqueIdLength;
    MountPoints->MountPoints[0].DeviceNameLength = DeviceInformation->DeviceName.Length;

    RtlCopyMemory((PWSTR)((ULONG_PTR)MountPoints +
			  MountPoints->MountPoints[0].SymbolicLinkNameOffset),
		  SymlinkInformation->Name.Buffer, SymlinkInformation->Name.Length);

    if (SymlinkInformation->Online) {
	RtlCopyMemory((PWSTR)((ULONG_PTR)MountPoints +
			      MountPoints->MountPoints[0].UniqueIdOffset),
		      DeviceInformation->UniqueId->UniqueId,
		      DeviceInformation->UniqueId->UniqueIdLength);
    }
    ...
    RtlCopyMemory((PWSTR)((ULONG_PTR)MountPoints +
			  MountPoints->MountPoints[0].DeviceNameOffset),
		  DeviceInformation->DeviceName.Buffer,
		  DeviceInformation->DeviceName.Length);
