					   ...
					   IN LPGUID Guid)
{
    ...
    *SymLinkCount = 0;
    ...
    Status = RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, DatabasePath, QueryTable,
				    DeviceInformation->UniqueId, NULL);
    if (!NT_SUCCESS(Status)) {
	*SymLinkCount = 0;
    }
    ...
    if (WriteNew) {
	/* Write link */
	RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath,
			      SuggestedLinkName->Buffer, REG_BINARY,
    ...
    *SymLinks = AllocatePool(*SymLinkCount * sizeof(UNICODE_STRING));
    ...
    /* Now, query */
    RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, DatabasePath, QueryTable,
			   DeviceInformation->UniqueId, NULL);
				...
				OUT PBOOLEAN UseOnlyIfThereAreNoOtherLinks)
{
    ...
    SuggestedLinkName->Buffer = AllocatePool(IoCtlSuggested->NameLength +
					     sizeof(UNICODE_NULL));
    if (!SuggestedLinkName->Buffer) {
	Status = STATUS_INSUFFICIENT_RESOURCES;
    } else {
	RtlCopyMemory(SuggestedLinkName->Buffer, IoCtlSuggested->Name,
		      IoCtlSuggested->NameLength);
	SuggestedLinkName->Buffer[SuggestedLinkName->Length / sizeof(WCHAR)] =
	    UNICODE_NULL;
    }
				   ...
				   IN OUT PUNICODE_STRING LinkTarget)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, SymbolicName,
