				     ...
				     IN PVOID EntryContext)
{
    ...
    if ((ValueType != REG_BINARY) || (OldUniqueId->UniqueIdLength != ValueLength)) {
	return STATUS_SUCCESS;
    }

    if (RtlCompareMemory(OldUniqueId->UniqueId, ValueData, ValueLength) == ValueLength) {
	/* Write new data */
	RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, ValueName, REG_BINARY,
			      NewUniqueId, NewUniqueId->UniqueIdLength);
    }
				   ...
				   IN PMOUNTDEV_UNIQUE_ID NewUniqueId)
{
    ...
    /* Write new data */
    RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, DatabasePath, QueryTable, OldUniqueId,
			   NULL);
	...
	if (DeviceInformation->UniqueId->UniqueIdLength == OldUniqueId->UniqueIdLength &&
	    RtlCompareMemory(OldUniqueId->UniqueId, DeviceInformation->UniqueId->UniqueId,
	...
	while (ReplicatedHead != NextReplicated) {
		...
		if (RtlCompareMemory(DuplicateId->UniqueId->UniqueId,
		    ...
		    NewDuplicateId = AllocatePool(NewUniqueId->UniqueIdLength +
			...
			DuplicateId->UniqueId->UniqueIdLength =
			    NewUniqueId->UniqueIdLength;
			RtlCopyMemory(NewDuplicateId->UniqueId, NewUniqueId->UniqueId,
				      NewUniqueId->UniqueIdLength);
		    }
		}
	    }
...
VOID CreateNoDriveLetterEntry(IN PMOUNTDEV_UNIQUE_ID UniqueId)
{
    ...
    /* Write the entry */
    RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, String, REG_BINARY,
			       ...
			       IN PDATABASE_ENTRY DatabaseEntry)
{
	...
	if (ReplicatedUniqueId->UniqueId->UniqueIdLength !=
	    DatabaseEntry->UniqueIdLength) {
	    continue;
	}
	...
	if (RtlCompareMemory(ReplicatedUniqueId->UniqueId->UniqueId,
			     (PVOID)((ULONG_PTR)DatabaseEntry +
				     DatabaseEntry->UniqueIdOffset),
			     ReplicatedUniqueId->UniqueId->UniqueIdLength) ==
	    ReplicatedUniqueId->UniqueId->UniqueIdLength) {
	    break;
	}
    }
    ...
    NewEntry->UniqueId = AllocatePool(DatabaseEntry->UniqueIdLength +
				      sizeof(MOUNTDEV_UNIQUE_ID));
