					...
					IN PVOID Parameter)
{
	...
	NewWorkItem->Link.Flink = NULL;
	IoQueueWorkItem(&NewWorkItem->WorkItem, SendOnlineNotificationWorker,
			DelayedWorkQueue, NewWorkItem);
    } else {
	/* Mark it's over */
			    ...
			    IN PUNICODE_STRING SymbolicName)
{
    ...
    IoInitializeWorkItem(DeviceExtension->DeviceObject, &WorkItem->WorkItem);
	...
	IoQueueWorkItem(&WorkItem->WorkItem, SendOnlineNotificationWorker,
...
VOID NTAPI UniqueIdChangeNotifyWorker(IN PDEVICE_OBJECT DeviceObject, IN PVOID Context)
{
    ...
    NewUniqueId = AllocatePool(UniqueIdChange->NewUniqueIdLength +
			       sizeof(MOUNTDEV_UNIQUE_ID));
    if (!NewUniqueId) {
			       ...
			       IN PVOID Context)
{
    ...
    IoQueueWorkItem(WorkItem->WorkItem, UniqueIdChangeNotifyWorker, DelayedWorkQueue,
		    WorkItem);
