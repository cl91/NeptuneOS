...
LONG GetRemoteDatabaseSize(IN HANDLE Database)
{
    ...
    /* Just query the size */
    Status = NtQueryInformationFile(Database, &IoStatusBlock, &StandardInfo,
...
NTSTATUS AddRemoteDatabaseEntry(IN HANDLE Database, IN PDATABASE_ENTRY Entry)
{
    ...
    return NtWriteFile(Database, NULL, NULL, NULL, &IoStatusBlock, Entry,
...
NTSTATUS TruncateRemoteDatabase(IN HANDLE Database, IN LONG NewSize)
{
    ...
    /* First set EOF */
    Status = NtSetInformationFile(Database, &IoStatusBlock, &EndOfFile,
				  sizeof(FILE_END_OF_FILE_INFORMATION),
				  FileEndOfFileInformation);
    if (NT_SUCCESS(Status)) {
	/* And then, properly set allocation information */
	Status = NtSetInformationFile(Database, &IoStatusBlock, &Allocation,
...
PDATABASE_ENTRY GetRemoteDatabaseEntry(IN HANDLE Database, IN LONG StartingOffset)
{
    ...
    Status = NtReadFile(Database, NULL, NULL, NULL, &IoStatusBlock, &EntrySize,
			sizeof(EntrySize), &ByteOffset, NULL);
    if (!NT_SUCCESS(Status)) {
	return NULL;
    }
    ...
    Status = NtReadFile(Database, NULL, NULL, NULL, &IoStatusBlock, Entry, EntrySize,
			&ByteOffset, NULL);
    /* If it fails or returns inconsistent data, drop it (= truncate) */
    if (!NT_SUCCESS(Status) || (IoStatusBlock.Information != EntrySize) ||
				  ...
				  IN PDATABASE_ENTRY Entry)
{
    ...
    Status = NtWriteFile(Database, NULL, NULL, NULL, &IoStatusBlock, Entry,
			 Entry->EntrySize, &ByteOffset, NULL);
    if (NT_SUCCESS(Status)) {
...
NTSTATUS DeleteRemoteDatabaseEntry(IN HANDLE Database, IN LONG StartingOffset)
{
    ...
    Status = NtReadFile(Database, NULL, NULL, NULL, &IoStatusBlock, TmpBuffer, EndSize,
			&EndEntriesOffset, NULL);
    if (!NT_SUCCESS(Status)) {
	FreePool(TmpBuffer);
	return Status;
    }
    ...
    Status = NtWriteFile(Database, NULL, NULL, NULL, &IoStatusBlock, TmpBuffer, EndSize,
			 &EndEntriesOffset, NULL);
			     ...
			     IN PMOUNTDEV_UNIQUE_ID UniqueId)
{
    ...
    RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, DatabasePath, QueryTable, UniqueId,
			   NULL);
}
...
{
    KeSetEvent(&DeviceExtension->RemoteDatabaseLock);
}
					...
					OUT PMOUNTDEV_UNIQUE_ID *UniqueId)
{
    ...
    *UniqueId = AllocatePool(DeviceInformation->UniqueId->UniqueIdLength +
    ...
    RtlCopyMemory(&((*UniqueId)->UniqueId), &(DeviceInformation->UniqueId->UniqueId),
			       ...
			       IN PDATABASE_ENTRY DatabaseEntry)
{
    ...
    Status = RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, SymbolicName,
				   REG_BINARY,
    ...
    SymbolicString.Buffer = (PVOID)((ULONG_PTR)DatabaseEntry +
	...
	if (DeviceInformation->UniqueId->UniqueIdLength !=
	    DatabaseEntry->UniqueIdLength) {
	    continue;
	}
	...
	MountMgrCreatePointWorker(DeviceExtension, &SymbolicString,
...
NTAPI VOID ReconcileThisDatabaseWithMasterWorker(IN PVOID Parameter)
{
	...
	((PRECONCILE_WORK_ITEM_CONTEXT)Parameter)->DeviceExtension;
    PDEVICE_INFORMATION DeviceInformation =
	((PRECONCILE_WORK_ITEM_CONTEXT)Parameter)->DeviceInformation;
	...
	while (EntryInfo != &ListDeviceInfo->AssociatedDevicesHead) {
	    AssociatedDevice = CONTAINING_RECORD(EntryInfo, ASSOCIATED_DEVICE_ENTRY,
    ...
    ReparseFile.MaximumLength = DeviceInformation->DeviceName.Length +
				ReparseIndex.Length + sizeof(UNICODE_NULL);
    ...
    InitializeObjectAttributes(&ObjectAttributes, &ReparseFile,
			       OBJ_KERNEL_HANDLE | OBJ_CASE_INSENSITIVE, NULL, NULL);

    /* Open reparse point directory */
    Status = NtOpenFile(&Handle, FILE_GENERIC_READ, &ObjectAttributes, &IoStatusBlock,
    ...
    Status = NtQueryDirectoryFile(Handle, NULL, NULL, NULL, &IoStatusBlock,
				  &ReparsePointInformation,
				  sizeof(FILE_REPARSE_POINT_INFORMATION),
				  FileReparsePointInformation, TRUE, &FileName, FALSE);
    if (!NT_SUCCESS(Status)) {
	NtClose(Handle);
	if (DatabaseHandle != 0) {
	    TruncateRemoteDatabase(DatabaseHandle, 0);
    ...
    for (;;) {
	RtlCopyMemory(&SavedReparsePointInformation, &ReparsePointInformation,
		      sizeof(FILE_REPARSE_POINT_INFORMATION));
	Status = NtQueryDirectoryFile(Handle, NULL, NULL, NULL, &IoStatusBlock,
				      &ReparsePointInformation,
				      sizeof(FILE_REPARSE_POINT_INFORMATION),
				      FileReparsePointInformation, TRUE,
				      Restart ? &FileName : NULL, Restart);
	/* Restart only once */
	if (Restart) {
	    Restart = FALSE;
	} else {
	...
	if (!NT_SUCCESS(Status) || Unloading ||
	    ReparsePointInformation.Tag != IO_REPARSE_TAG_MOUNT_POINT) {
	    break;
	}

	/* Get the volume name associated to the mount point */
	Status = QueryVolumeName(Handle, &ReparsePointInformation, 0, &SymbolicName,
				 &VolumeName);
	if (!NT_SUCCESS(Status)) {
	    continue;
	}
	...
	if (DatabaseEntry != NULL) {
		...
		KeWaitForSingleObject(&DeviceExtension->DeviceLock, Executive, KernelMode,
				      FALSE, NULL);
		Status = QueryUniqueIdFromMaster(DeviceExtension, &SymbolicName,
						 &UniqueId);
		if (!NT_SUCCESS(Status)) {
		    /* If we failed doing so, reuse the old Unique ID and push it to
		     * master */
		...
		/* Would, by chance, the Unique ID be present elsewhere? */
		    ...
		    /* Configure it */
		    DatabaseEntry->EntrySize = UniqueId->UniqueIdLength +
					       SymbolicName.Length +
					       sizeof(DATABASE_ENTRY);
		    ...
		    DatabaseEntry->UniqueIdOffset = SymbolicName.Length +
						    sizeof(DATABASE_ENTRY);
		    DatabaseEntry->UniqueIdLength = UniqueId->UniqueIdLength;
		    RtlCopyMemory((PVOID)((ULONG_PTR)DatabaseEntry +
					  DatabaseEntry->SymbolicNameOffset),
				  SymbolicName.Buffer, DatabaseEntry->SymbolicNameLength);
		    RtlCopyMemory((PVOID)((ULONG_PTR)DatabaseEntry +
					  DatabaseEntry->UniqueIdOffset),
	...
	if (!NT_SUCCESS(Status)) {
		...
		InsertTailList(&ListDeviceInfo->AssociatedDevicesHead,
			       &AssociatedDevice->AssociatedDevicesEntry);
	    }
...
static NTAPI VOID WorkerThread(IN PDEVICE_OBJECT DeviceObject, IN PVOID Context)
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, &SafeVolumes,
    ...
    do {
	Status = NtWaitForSingleObject(SafeEvent, FALSE, &Timeout);
			 ...
			 OUT PUNICODE_STRING VolumeName)
{
	...
	Reference.Length = Reference.MaximumLength = sizeof(
	    ReparsePointInformation->FileReference);
	Reference.Buffer = (PWSTR) & (ReparsePointInformation->FileReference);
	InitializeObjectAttributes(&ObjectAttributes, &Reference, OBJ_KERNEL_HANDLE,
				   RootDirectory, NULL);
    } else {
    ...
    Status = NtFsControlFile(Handle, 0, NULL, NULL, &IoStatusBlock,
			     FSCTL_GET_REPARSE_POINT, NULL, 0, ReparseDataBuffer,
    ...
    if (ReparseDataBuffer->MountPointReparseBuffer.SubstituteNameLength +
	    sizeof(UNICODE_NULL) >
	SymbolicName->MaximumLength) {
	FreePool(ReparseDataBuffer);
	NtClose(Handle);
    ...
    RtlCopyMemory(VolumeName->Buffer, FileNameInfo->FileName,
			  ...
			  IN PDEVICE_INFORMATION DeviceInformation)
{
	...
	Status = NtQueryDirectoryFile(Handle, NULL, NULL, NULL, &IoStatusBlock,
				      ...
				      sizeof(FILE_REPARSE_POINT_INFORMATION),
				      FileReparsePointInformation, TRUE,
				      (RestartScan) ? &FileName : NULL, RestartScan);
	if (!RestartScan) {
	    if (ReparsePointInformation.FileReference ==
		    SavedReparsePointInformation.FileReference &&
		ReparsePointInformation.Tag == SavedReparsePointInformation.Tag) {
	...
	/* Get the volume name associated to the mount point */
	Status = QueryVolumeName(Handle, &ReparsePointInformation, NULL, &SymbolicName,
				 &VolumeName);
	if (!NT_SUCCESS(Status)) {
	    continue;
	}
				     ...
				     IN PDEVICE_INFORMATION DeviceInformation)
{
    ...
    WorkItem->ReconcileWorkItemContext.DeviceExtension = DeviceExtension;
    WorkItem->ReconcileWorkItemContext.DeviceInformation = DeviceInformation;
    QueueWorkItem(DeviceExtension, WorkItem, &WorkItem->ReconcileWorkItemContext);
	...
	DeviceExtension->AutomaticDriveLetter && DeviceExtension->NoAutoMount) {
					     ...
					     IN PVOID Context)
{
    ...
    DatabaseName.MaximumLength = DeviceInformation->DeviceName.Length +
				 RemoteDatabase.Length + sizeof(UNICODE_NULL);
    ...
    /* Create database */
    InitializeObjectAttributes(&ObjectAttributes, &DatabaseName,
			       OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);

    Status = NtCreateFile(&Database,
			      ...
			      IN OUT PHANDLE Database)
{
	...
	*Database = 0;
    ...
    WorkItem->WorkItem = IoAllocateWorkItem(
	DeviceInformation->DeviceExtension->DeviceObject);
    if (!WorkItem->WorkItem) {
	FreePool(WorkItem);
	*Database = 0;
	return STATUS_INSUFFICIENT_RESOURCES;
    ...
    /* And queue it */
    IoQueueWorkItem(WorkItem->WorkItem, CreateRemoteDatabaseWorker, DelayedWorkQueue,
		    WorkItem);
			  ...
			  IN BOOLEAN MigrateDatabase)
{
    ...
    DeviceRemoteDatabase.Buffer[DeviceRemoteDatabase.Length / sizeof(WCHAR)] =
	UNICODE_NULL;

    /* Open database */
    InitializeObjectAttributes(&ObjectAttributes, &DeviceRemoteDatabase,
			       OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
				  ...
				  IN PMOUNTDEV_UNIQUE_ID NewUniqueId)
{
	...
	NewEntry = AllocatePool(Entry->EntrySize + NewUniqueId->UniqueIdLength -
	...
	NewEntry->EntrySize = Entry->EntrySize + NewUniqueId->UniqueIdLength -
			      OldUniqueId->UniqueIdLength;
