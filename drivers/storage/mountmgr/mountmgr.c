...
#define DRIVE_C_UNC_PATH L"\\??\\C:"
#define DRIVE_C_UNC_PATH_LETTER_POSITION 4
...
BOOLEAN IsOffline(PUNICODE_STRING SymbolicName)
{
    ...
    /* Query status */
    Status = RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, OfflinePath, QueryTable, NULL,
				    NULL);
...
BOOLEAN HasDriveLetter(IN PDEVICE_INFORMATION DeviceInformation)
{
	...
	SymlinkInfo = CONTAINING_RECORD(NextEntry, SYMLINK_INFORMATION,
			...
			OUT PDEVICE_INFORMATION *DeviceInformation)
{
	...
	/* Otherwise, query it */
	Status = QueryDeviceInformation(SymbolicName, &DeviceName, NULL, NULL, NULL, NULL,
					NULL, NULL);
	if (!NT_SUCCESS(Status)) {
	    return Status;
	}
...
BOOLEAN MountmgrReadNoAutoMount(IN PUNICODE_STRING RegistryPath)
{
    ...
    Status = RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, RegistryPath->Buffer,
				      ...
				      IN BOOLEAN ManuallyRegistered)
{
    ...
    WCHAR CSymLinkBuffer[RTL_NUMBER_OF(DRIVE_C_UNC_PATH)], LinkTargetBuffer[MAX_PATH];
    UNICODE_STRING TargetDeviceName, SuggestedLinkName, DeviceName, VolumeName,
	DriveLetter, LinkTarget, CSymLink;
	...
	.Buffer[DeviceInformation->SymbolicName.Length / sizeof(WCHAR)] = UNICODE_NULL;
    ...
    if (DeviceExtension->DriveLetterData &&
	UniqueId->UniqueIdLength == DeviceExtension->DriveLetterData->UniqueIdLength) {
	...
	RtlCopyMemory(CSymLinkBuffer, DRIVE_C_UNC_PATH, sizeof(DRIVE_C_UNC_PATH));
	...
	for (Letter = L'D'; Letter <= L'Z'; Letter++) {
	    CSymLink.Buffer[DRIVE_C_UNC_PATH_LETTER_POSITION] = Letter;
	    ...
	    if (!RtlEqualUnicodeString(&LinkTarget, &DeviceInformation->DeviceName,
				       FALSE)) {
		continue;
	    }
	...
	if (!NT_SUCCESS(Status)) {
	    ...
	    if ((SavedLinkInformation &&
		 !RedirectSavedLink(SavedLinkInformation, &(SymLinks[i]),
				    &TargetDeviceName)) ||
		!SavedLinkInformation) {
		Status = QueryDeviceInformation(&(SymLinks[i]), &DeviceName, NULL, NULL,
						NULL, NULL, NULL, NULL);
		if (NT_SUCCESS(Status)) {
		    LinkError = RtlEqualUnicodeString(&TargetDeviceName, &DeviceName,
						      TRUE);
		    FreePool(DeviceName.Buffer);
		}
	...
	if (NT_SUCCESS(Status)) {
	    ...
	    RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, VolumeName.Buffer,
	...
	Status = CreateNewDriveLetterName(&DriveLetter, &TargetDeviceName,
					  DeviceInformation->SuggestedDriveLetter, NULL);
	if (!NT_SUCCESS(Status)) {
	    CreateNoDriveLetterEntry(UniqueId);
	} else {
	    /* Save it to global database */
	    RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, DriveLetter.Buffer,
				  REG_BINARY, UniqueId->UniqueId,
				  UniqueId->UniqueIdLength);
	...
	RtlCopyMemory(NewUniqueId->UniqueId, UniqueId->UniqueId,
		      UniqueId->UniqueIdLength);
    }
 ...
 * @implemented
 */
static NTAPI NTSTATUS MountMgrMountedDeviceNotification(IN PVOID NotificationStructure,
							IN PVOID Context)
{
    PDEVICE_EXTENSION DeviceExtension;
 ...
 * @implemented
 */
static NTAPI NTSTATUS MountMgrCreateClose(IN PDEVICE_OBJECT DeviceObject,
					  IN PIRP Irp)
{
 ...
 * @implemented
 */
static NTAPI NTSTATUS MountMgrCleanup(IN PDEVICE_OBJECT DeviceObject,
				      IN PIRP Irp)
{
    PIRP ListIrp;
    PLIST_ENTRY NextEntry;
 ...
 * @implemented
 */
static NTAPI NTSTATUS MountMgrShutdown(IN PDEVICE_OBJECT DeviceObject,
				       IN PIRP Irp)
{
    PDEVICE_EXTENSION DeviceExtension;
			   ...
			   IN PUNICODE_STRING RegistryPath)
{
    ...
    DeviceExtension->RegistryPath.Buffer = AllocatePool(
