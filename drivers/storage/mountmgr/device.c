...
MountmgrWriteNoAutoMount(IN PDEVICE_EXTENSION DeviceExtension)
{
    ...
    return RtlWriteRegistryValue(RTL_REGISTRY_ABSOLUTE,
				 DeviceExtension->RegistryPath.Buffer, L"NoAutoMount",
		     ...
		     IN ULONG ValueLength, IN PVOID Context, IN PVOID EntryContext)
{
	...
	for (NextEntry = DeviceExtension->DeviceListHead.Flink;
	    ...
	    DeviceInfo = CONTAINING_RECORD(NextEntry, DEVICE_INFORMATION,
					   DeviceListEntry);
	    ...
	    if (RtlCompareMemory(DeviceInfo->UniqueId->UniqueId, ValueData,
    ...
    Status = RtlDeleteRegistryValue(RTL_REGISTRY_ABSOLUTE, DatabasePath, ValueName);
...
MountMgrScrubRegistry(IN PDEVICE_EXTENSION DeviceExtension)
{
	...
	Status = RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, DatabasePath, QueryTable,
...
MountMgrCheckUnprocessedVolumes(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp)
{
	...
	ArrivalStatus = MountMgrMountedDeviceArrival(
	    DeviceExtension, &(DeviceInformation->SymbolicName),
...
VOID ProcessSuggestedDriveLetters(IN PDEVICE_EXTENSION DeviceExtension)
{
	...
	/* Suggested letter & no entry */
	    ...
	    SymbolicName.Length = SymbolicName.MaximumLength = DRIVE_LETTER_LENGTH;

	    MountMgrCreatePointWorker(DeviceExtension, &SymbolicName,
			      ...
			      OUT PMOUNTMGR_DRIVE_LETTER_INFORMATION DriveLetterInfo)
{
    ...
    Status = QueryDeviceInformation(DeviceName, &TargetDeviceName, NULL, &Removable,
				    &GptDriveLetter, NULL, NULL, NULL);
    if (!NT_SUCCESS(Status)) {
	return Status;
    }
	...
	if (RtlCompareUnicodeString(&(DeviceInformation->DeviceName), &TargetDeviceName,
				    TRUE) == 0) {
	    break;
	}
...
MountMgrNextDriveLetter(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp)
{
    ...
    DeviceName.Length = DeviceName.MaximumLength = DriveLetterTarget->DeviceNameLength;
...
MountMgrQuerySystemVolumeName(OUT PUNICODE_STRING SystemVolumeName)
{
    ...
    RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, L"\\Registry\\Machine\\System\\Setup",
...
VOID MountMgrAssignDriveLetters(IN PDEVICE_EXTENSION DeviceExtension)
{
	...
	if (NT_SUCCESS(Status) &&
	    RtlEqualUnicodeString(&SystemVolumeName, &(DeviceInformation->DeviceName),
				  TRUE)) {
	    /* Keep track of it */
	    DeviceExtension->DriveLetterData = AllocatePool(
...
MountMgrQueryDosVolumePath(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp)
{
    ...
    SymbolicName.Length = SymbolicName.MaximumLength = Target->DeviceNameLength;
	...
	for (SymlinksEntry = DeviceInformation->SymbolicLinksListHead.Flink;
	    ...
	    if (MOUNTMGR_IS_DRIVE_LETTER(&SymlinkInformation->Name) &&
	...
	AssociatedDevice = CONTAINING_RECORD(&(DeviceInformation->AssociatedDevicesHead),
	...
	RtlCopyMemory(DeviceString, AssociatedDevice->String.Buffer,
		      AssociatedDevice->String.Length);
	if (OldBuffer) {
	    RtlCopyMemory(&DeviceString[AssociatedDevice->String.Length / sizeof(WCHAR)],
			  OldBuffer, OldLength);
	    FreePool(OldBuffer);
	}
			    ...
			    OUT PBOOLEAN Invalid)
{
    ...
    RtlAppendUnicodeStringToString(&FullName,
				   &AssociatedDeviceEntry->DeviceInformation->DeviceName);
    ...
    Status = NtOpenFile(&Handle, SYNCHRONIZE | FILE_READ_ATTRIBUTES, &ObjectAttributes,
			&IoStatusBlock,
	...
	*Invalid = TRUE;
    ...
    /* Query reparse data */
    Status = NtFsControlFile(Handle, NULL, NULL, NULL, &IoStatusBlock,
			     FSCTL_GET_REPARSE_POINT, NULL, 0, ReparseData,
	...
	*Invalid = TRUE;
	...
	(PWSTR)((ULONG_PTR)ReparseData->SymbolicLinkReparseBuffer.PathBuffer +
	...
	if (SubstituteName.Length == 98 && SubstituteName.Buffer[1] == L'?') {
		...
		if (RtlEqualUnicodeString(&SubstituteName, &SymlinkInformation->Name,
					  TRUE)) {
		    FreePool(ReparseData);
		    return STATUS_SUCCESS;
		}
			 ...
			 OUT PDEVICE_INFORMATION *FailedDevice)
{
    ...
    for (Entry = DeviceInformation->SymbolicLinksListHead.Flink;
	 Entry != &(DeviceInformation->SymbolicLinksListHead); Entry = Entry->Flink) {
	SymlinkInformation = CONTAINING_RECORD(Entry, SYMLINK_INFORMATION,
					       SymbolicLinksListEntry);
	...
	DeviceInfoEntry = CONTAINING_RECORD(Entry, DEVICE_INFORMATION_ENTRY,
	...
	for (Entry = DeviceInformation->AssociatedDevicesHead.Flink;
	     Entry != &DeviceInformation->AssociatedDevicesHead; Entry = Entry->Flink) {
	    ++NumberOfPaths;
	}
    ...
    /* Start the hot loop to gather all the paths and be able to compute total output
     * length! */
    ReturnedPaths = 0;
    CurrentPath = Paths;
    for (Entry = DeviceInformation->AssociatedDevicesHead.Flink;
	...
	Status = MountMgrQueryVolumePaths(DeviceExtension,
					  ...
					  DeviceInfoList, CurrentPath, FailedDevice);
	if (!NT_SUCCESS(Status)) {
	    ULONG i;
	...
	if ((*CurrentPath)->MultiSzLength != sizeof(UNICODE_NULL)) {
	    ...
	    for (i = 0; i < (*CurrentPath)->MultiSzLength / sizeof(WCHAR);
		 ++i, ++MultiSz) {
	...
	++CurrentPath;
	/* Multiply String.Length by the number of found paths, we always add it after a
	 * path */
	OutputPathLength += (*CurrentPath)->MultiSzLength +
			    InnerStrings * AssociatedDeviceEntry->String.Length -
			    sizeof(UNICODE_NULL);
    }
    ...
    *VolumePaths = AllocatePool(sizeof(ULONG) + OutputPathLength);
	...
	if ((*CurrentPath)->MultiSzLength != sizeof(UNICODE_NULL)) {
	    ...
	    /* This offset is used to "jump" into MultiSz, so, start with the string begin
	     * (ie, skip MultiSzLength) */
	    Offset = sizeof(ULONG);
...
MountMgrQueryDosVolumePaths(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp)
{
	...
	/* Query paths */
	Status = MountMgrQueryVolumePaths(DeviceExtension, DeviceInformation, &Devices,
					  &Paths, &FailedDevice);
	if (NT_SUCCESS(Status)) {
	    break;
	}
	...
	for (Entry = DeviceExtension->DeviceListHead.Flink;
	     Entry != &DeviceExtension->DeviceListHead; Entry = Entry->Flink) {
	    ListDeviceInfo = CONTAINING_RECORD(Entry, DEVICE_INFORMATION,
					       DeviceListEntry);
	    /* It's online, it's OK! */
...
MountMgrQueryPoints(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp)
{
    ...
    if ((MountPoint->UniqueIdOffset & 1) || (MountPoint->UniqueIdLength & 1)) {
	return STATUS_INVALID_PARAMETER;
    }

    if ((MountPoint->DeviceNameOffset & 1) || (MountPoint->DeviceNameLength & 1)) {
	return STATUS_INVALID_PARAMETER;
    }
...
MountMgrDeletePoints(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp)
{
	...
	RtlCopyMemory(SymbolicName.Buffer,
		      (PWSTR)((ULONG_PTR)MountPoints +
			      MountPoints->MountPoints[Link].SymbolicLinkNameOffset),
		      SymbolicName.Length);
	SymbolicName.Buffer[SymbolicName.Length / sizeof(WCHAR)] = UNICODE_NULL;
	...
	if (CreateNoDrive && IsDriveLetter(&SymbolicName)) {
		...
		RtlCopyMemory(
		    UniqueId->UniqueId,
		    (PMOUNTDEV_UNIQUE_ID)((ULONG_PTR)MountPoints +
					  MountPoints->MountPoints[Link].UniqueIdOffset),
	...
	DeviceName.Buffer = (PWSTR)((ULONG_PTR)MountPoints +
				    MountPoints->MountPoints[Link].DeviceNameOffset);
 ...
 */
NTSTATUS
MountMgrVolumeMountPointChanged(IN PDEVICE_EXTENSION DeviceExtension, IN PIRP Irp,
				...
				OUT PUNICODE_STRING TargetVolumeName)
{
    ...
    SourceVolumeName.Length = SourceVolumeName.MaximumLength =
	VolumeMountPoint->SourceVolumeNameLength;
    SourceVolumeName.Buffer = (PWSTR)((ULONG_PTR)VolumeMountPoint +
				      VolumeMountPoint->SourceVolumeNameOffset);

    InitializeObjectAttributes(&ObjectAttributes, &SourceVolumeName,
    ...
    Status = NtQueryVolumeInformationFile(Handle, &IoStatusBlock, &FsDeviceInfo,
					  sizeof(FsDeviceInfo), FileFsDeviceInformation);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }

    /* Mount points can be stored only on storage disks */
    if (FsDeviceInfo.DeviceType != FILE_DEVICE_DISK &&
	FsDeviceInfo.DeviceType != FILE_DEVICE_VIRTUAL_DISK) {
    ...
    /* Reference it */
    Status = ObReferenceObjectByHandle(Handle, 0, *IoFileObjectType, KernelMode,
    ...
    Status = NtQueryInformationFile(Handle, &IoStatusBlock, FileNameInfo,
    ...
    Status = ObQueryNameString(FileObject->DeviceObject, ObjectNameInfoPtr,
    ...
    Status = QueryDeviceInformation(&ObjectNameInfoPtr->Name, SourceDeviceName, NULL,
	...
	VolumeMountPoint->TargetVolumeNameLength;
    TargetVolumeName->Buffer = (PWSTR)((ULONG_PTR)VolumeMountPoint +
				       VolumeMountPoint->TargetVolumeNameOffset);

    /* Query its device name */
    Status = QueryDeviceInformation(TargetVolumeName, &TargetDeviceName, NULL, NULL, NULL,
				    NULL, NULL, NULL);
    if (!NT_SUCCESS(Status)) {
	goto Cleanup;
    }
    ...
    SourceSymbolicName->Length = SourceSymbolicName->MaximumLength =
	(USHORT)FileNameInfo->FileNameLength;
				...
				IN NTSTATUS LockStatus)
{
    ...
    UNICODE_STRING LinkTarget, SourceDeviceName, SourceSymbolicName, TargetVolumeName,
	VolumeName, DbName;
     ...
     */
    Status = MountMgrVolumeMountPointChanged(DeviceExtension, Irp, LockStatus,
					     &SourceDeviceName, &SourceSymbolicName,
					     &TargetVolumeName);
	...
	RtlCopyMemory((PVOID)((ULONG_PTR)DatabaseEntry + sizeof(DATABASE_ENTRY)),
		      TargetVolumeName.Buffer, DatabaseEntry->SymbolicNameLength);
	RtlCopyMemory((PVOID)((ULONG_PTR)DatabaseEntry + DatabaseEntry->UniqueIdOffset),
    ...
    AssociatedEntry->String.MaximumLength = AssociatedEntry->String.Length +
					    sizeof(UNICODE_NULL);
    ...
    RtlCopyMemory(AssociatedEntry->String.Buffer, SourceSymbolicName.Buffer,
		  SourceSymbolicName.Length);
    AssociatedEntry->String.Buffer[SourceSymbolicName.Length / sizeof(WCHAR)] =
	UNICODE_NULL;
				...
				IN NTSTATUS LockStatus)
{
	...
	for (Entry = DeviceInformation->ReplicatedUniqueIdsListHead.Flink;
	     ...
	     Entry = Entry->Flink) {
	    UniqueIdReplicate = CONTAINING_RECORD(Entry, UNIQUE_ID_REPLICATE,
						  ReplicatedUniqueIdsListEntry);

	    if (UniqueIdReplicate->UniqueId->UniqueIdLength ==
    ...
    for (Entry = TargetDeviceInformation->AssociatedDevicesHead.Flink;
	 Entry != &TargetDeviceInformation->AssociatedDevicesHead; Entry = Entry->Flink) {
	AssociatedEntry = CONTAINING_RECORD(Entry, ASSOCIATED_DEVICE_ENTRY,
					    AssociatedDevicesEntry);
