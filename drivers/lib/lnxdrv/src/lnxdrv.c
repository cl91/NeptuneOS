...
static PVOID LnxCreateMutex(VOID)
{
    return IoCreateMutex();
}

static VOID LnxAcquireMutex(IN PVOID Mutex)
{
    IoAcquireMutex(Mutex);
}

static VOID LnxReleaseMutex(IN PVOID Mutex)
{
    IoReleaseMutex(Mutex);
}
...
static NTSTATUS LnxRegisterFramebuffer(IN PULONG_PTR PfnDb,
				       IN ULONG PfnCount,
				       IN ULONG Offset,
				       IN ULONG Width,
				       IN ULONG Height,
...
{
    return HalRegisterFrameBuffer(PfnDb, PfnCount, Offset, Width, Height, Pitch,
				  BitsPerPixel, BlueIndex, GreenIndex, RedIndex, NeedFlush);
}

static NTSTATUS LnxUnregisterFramebuffer(IN ULONG_PTR PhysicalBase)
{
    return HalUnregisterFrameBuffer(PhysicalBase);
}
...
static LNX_DRV_IMPORT_TABLE LnxDrvImportTable = {
    ...
    .AcquireMutex = LnxAcquireMutex,
    .ReleaseMutex = LnxReleaseMutex,
					  ...
					  IN PVOID Context)
{
    ...
    KEVENT Event;
    KeInitializeEvent(&Event, SynchronizationEvent, FALSE);
    LnxDrvExportTable.HandleFrameBufferDamage(&Event, Info->PhyBase,
...
static NTAPI VOID LnxDrvHandleFrameBufferDamage(IN ULONG_PTR PhyBase,
						IN ULONG StartWidth,
