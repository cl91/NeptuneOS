...
typedef struct _DELAYED_MODULE_LOADING_WORK {
    IO_WORKITEM WorkItem;
    KEVENT LoadingDone;
    LIST_ENTRY Entry;
    PLNX_DRV_LOADED_MODULE Module;
} DELAYED_MODULE_LOADING_WORK, *PDELAYED_MODULE_LOADING_WORK;
...
static LIST_ENTRY LnxDrvDelayedModuleLoadingWorkList;
...
static NTAPI VOID LnxAddModuleWorkerRoutine(IN PDEVICE_OBJECT Unused,
					    IN PVOID Context)
{
    PDELAYED_MODULE_LOADING_WORK Work = Context;
    PLNX_DRV_LOADED_MODULE Module = Work->Module;
    assert(Module);
    ...
    KeClearEvent(&Event);
    KeSetEvent(&Work->LoadingDone);
    if (!NT_SUCCESS(Status)) {
	if (Module->ModulePath) {
	    ExFreePool(Module->ModulePath);
	}
	if (Module->Args) {
	    ExFreePool(Module->Args);
	}
	ExFreePool(Module);
	Work->Module = NULL;
...
static PDELAYED_MODULE_LOADING_WORK LnxDrvAllocateDelayedModuleLoadingWork()
{
    PDELAYED_MODULE_LOADING_WORK Work = ExAllocatePool(NonPagedPool,
						       sizeof(DELAYED_MODULE_LOADING_WORK));
    if (!Work) {
	return NULL;
    }
    IoInitializeWorkItem(NULL, &Work->WorkItem);
    KeInitializeEvent(&Work->LoadingDone, SynchronizationEvent, FALSE);
    InsertTailList(&LnxDrvDelayedModuleLoadingWorkList, &Work->Entry);
    return Work;
}
				 ...
				 IN OPTIONAL PKEVENT Event)
{
    ...
    /* TODO: Get module arguments from the registry */
    PCSTR Args = "";
    Module->Args = ExAllocatePool(NonPagedPool, strlen(Args) + 1);
    if (!Module->Args) {
	ExFreePool(Module->ModulePath);
	ExFreePool(Module);
	return STATUS_INSUFFICIENT_RESOURCES;
    }
    RtlCopyMemory((PVOID)Module->Args, Args, strlen(Args) + 1);
	...
	PDELAYED_MODULE_LOADING_WORK DelayedWork = LnxDrvAllocateDelayedModuleLoadingWork();
	if (!DelayedWork) {
	    Status = STATUS_INSUFFICIENT_RESOURCES;
	    goto out;
	}
	DelayedWork->Module = Module;
	IoQueueWorkItem(&DelayedWork->WorkItem,
			LnxAddModuleWorkerRoutine, DelayedWorkQueue, DelayedWork);
...
static VOID LnxDrvWaitForDelayedModuleLoading()
{
    LoopOverList(Work, &LnxDrvDelayedModuleLoadingWorkList,
		 DELAYED_MODULE_LOADING_WORK, Entry) {
	DPRINT("Waiting for delayed module %ws loading done\n",
	       Work->Module->ModulePath);
	KeWaitForSingleObject(&Work->LoadingDone, Executive, KernelMode, FALSE, NULL);
	RemoveEntryList(&Work->Entry);
	ExFreePool(Work);
    }
}
...
{
    LnxDrvWaitForDelayedModuleLoading();
			   ...
			   IN PUNICODE_STRING RegistryPath)
{
    ...
    InitializeListHead(&LnxDrvDelayedModuleLoadingWorkList);
